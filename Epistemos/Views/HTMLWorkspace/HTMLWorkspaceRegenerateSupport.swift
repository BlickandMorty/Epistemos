...
enum HTMLWorkspaceRegeneratePromptBuilder {
    ...
    Include a compact in-surface context picker/filter over available data.json records when workspace context is part of the request; picking a record should visibly add/pin that real record into the workspace, and absent records must show an honest empty or stale state.
    Do not create drag/drop-only context controls. Context-backed sections must select/pin records already present in data.json, and unavailable context must show an honest unavailable status instead of silently fabricating or accepting arbitrary text.
...
nonisolated struct HTMLWorkspaceRegenerateContextItem: Identifiable, Sendable, Equatable {
    ...
    private static func bounded(_ value: String, limit: Int) -> String {
        ...
        // interpolated line-by-line into the regenerate prompt context, so an
