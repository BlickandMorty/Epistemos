...
struct ThemedGlassToolbarModifier: ViewModifier {
        ...
        content
            .background {
                WindowAccessor { window in
                    ...
                    let config = GlassToolbarConfiguration(theme: uiState.theme)
                    
                    // Only apply if not already applied
                    let alreadyApplied = window.titlebarAccessoryViewControllers
                        .contains { $0.identifier?.rawValue == "GlassToolbar" }
                    
                    if !alreadyApplied {
                        window.applyThemedGlassToolbar(configuration: config)
                    }
                }
            }
...
extension View {
    func themedGlassToolbar() -> some View {
        modifier(ThemedGlassToolbarModifier())
    }
}
