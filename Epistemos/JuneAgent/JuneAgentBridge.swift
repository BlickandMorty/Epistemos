...
final class JuneAgentBridge: NSObject, WKScriptMessageHandler {
    ...
    private func handleSpeak(action: String, text: String) {
        ...
        _ = EpistemosReadAloud.speak(text, synthesizer: synth, surface: .juneLatestAssistantReply)
    }
    ...
    private func handleInvoke(cmd: String, args: [String: Any]) -> Any? {
        ...
        case "os_accounts_status":
            // June is hosted locally in the app, so no June account is required. The
            ...
            // surface reachable only by the accidental absence of a `credits`
            ...
            // and provider consent live in June Settings, so the
    ...
    private static func bootstrapPayload() -> [String: Any] {
        let providerConfigured = JuneCloudProvider.allCases.contains {
            JuneCloudConfigurationStore.shared.isConfigured($0)
        }
        return [
    ...
    private static func skillDocumentPayload(name: String) -> [String: Any] {
        guard
            ...
            let content = JuneSkillDocumentReader.readSkillMarkdown(vaultPath: vaultPath, skillName: name)
        else {
