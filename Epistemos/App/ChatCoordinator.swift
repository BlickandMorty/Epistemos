...
final class ChatCoordinator {
  ...
  struct ReferenceSearchResults: Sendable {
    ...
    let indexedNoteProvenanceByPageID: [String: VaultRecallCandidate]

    init(
      notes: [NoteMentionChoice],
      chats: [ChatReferenceResult],
      vaultTitle: String?,
      vaultNoteCount: Int,
      isInventoryComplete: Bool,
      query: String,
      indexedMatchedNoteIDs: Set<String>,
      indexedNoteSnippetsByPageID: [String: String],
      indexedNoteProvenanceByPageID: [String: VaultRecallCandidate] = [:]
    ) {
      self.notes = notes
      self.chats = chats
      self.vaultTitle = vaultTitle
      self.vaultNoteCount = vaultNoteCount
      self.isInventoryComplete = isInventoryComplete
      self.query = query
      self.indexedMatchedNoteIDs = indexedMatchedNoteIDs
      self.indexedNoteSnippetsByPageID = indexedNoteSnippetsByPageID
      self.indexedNoteProvenanceByPageID = indexedNoteProvenanceByPageID
    }
  }
  ...
  func handleCommandCenterSubmission(
    ...
    let vaultPathClosure: @MainActor () -> String = { [weak self] in
      self?.managedToolRuntimeVaultPath() ?? ""
    }
        ...
        if effectiveOperatingMode == .agent {
          ...
          for try await event in stream {
            ...
            case .thinkingDelta(let thought):
              agentChat.appendStreamingThinking(thought, explicit: true)
  ...
  private func runCommandCenterRustAgentPath(
    ...
    let stream = AsyncStream<AgentStreamEvent>(bufferingPolicy: .bufferingNewest(256)) {
      continuation in
    ...
    for await event in stream {
      ...
      case .thinkingDelta(let text):
        ...
        agentChat.appendStreamingThinking(text, explicit: true)
  ...
  private func commandCenterExecutionPlan(
    ...
    let toolPermissions: [OverseerToolPermission] = compiled.resolvedToolPermissions.compactMap {
      permission in
  ...
  private func commandCenterPlanDocumentSeed(
    ...
    AgentPlanDocumentSeed(
      ...
      budgets: [
        ...
        AgentPlanDocumentBudget(
          label: "Tool calls", value: compiled.resolvedExecutionPolicy.maxToolCalls),
        AgentPlanDocumentBudget(
          label: "Reasoning steps", value: compiled.resolvedExecutionPolicy.maxReasoningSteps),
        AgentPlanDocumentBudget(
          label: "Output tokens", value: compiled.resolvedExecutionPolicy.maxOutputTokens),
      ]
    )
  }
  ...
  private func buildMainChatBrainSnapshot(
    ...
    return ChatBrainSnapshot(
      ...
      routeSummary: snapshotRouteContext.summary,
      providerLabel: Self.mainChatProviderLabel(for: selection),
      modelLabel: selection.displayName,
      allowedToolNames: snapshotRouteContext.allowedToolNames,
  ...
  private static func buildMainChatBrainSections(
    ...
    if let graphContextSection {
      sections.append(ChatBrainSection(title: "Graph Context", body: graphContextSection))
    }
  ...
  private static func graphContextSection(from request: GraphChatRequest?) -> String? {
    guard let request else { return nil }

    let label = request.nodeLabel.trimmingCharacters(in: .whitespacesAndNewlines)
    let effectiveLabel = label.isEmpty ? request.nodeType : label
    let sourceId = request.sourceId?.trimmingCharacters(in: .whitespacesAndNewlines)

    var lines: [String] = [
      "This turn was launched from the graph workspace.",
      "Selected node: \(effectiveLabel)",
      "Node type: \(request.nodeType)",
      "Graph node id: \(request.graphNodeId)",
      "Workspace route: \(graphRouteLabel(request.route))",
    ]
    if let sourceId, !sourceId.isEmpty {
      lines.append("Backing source id: \(sourceId)")
    }

    return wrapSupplementalContextSection(
      title: "Graph Context",
      instruction:
        "Use this only as situational context from the graph workspace. Do not imply access to extra graph state beyond what is listed here.",
  ...
  private static func augmentedExecutionPlan(
    ...
    preferredCommand: ACCSlashCommand?
  ) -> OverseerComplexityRouter.ExecutionPlan? {
    guard let executionPlan, let command = preferredCommand else {
      return executionPlan
    }

    let mergedExpertAllowlist = Array(
      NSOrderedSet(
        array: executionPlan.plan.maskPlan.expertAllowlist + command.expertAllowlist
      )
    ).compactMap { $0 as? String }

    var mergedToolPermissions = executionPlan.plan.toolPermissions
    let existingTools = Set(mergedToolPermissions.map(\.toolName))
    for toolName in command.preferredToolNames.sorted() where !existingTools.contains(toolName) {
      mergedToolPermissions.append(
        OverseerToolPermission(toolName: toolName, mode: .ask)
      )
    }
    ...
    let adjustedPlan = OverseerPlanV1(
      ...
      maskPlan: OverseerMaskPlan(
        expertAllowlist: mergedExpertAllowlist,
        rationale: executionPlan.plan.maskPlan.rationale
      ),
    ...
    return OverseerComplexityRouter.ExecutionPlan(
      ...
      summary: executionPlan.summary + " • /\(command.rawValue)"
    )
  }
  ...
  private static func mainChatBrainSnapshotRouteContext(
    inferenceState: InferenceState,
    executionPlan: OverseerComplexityRouter.ExecutionPlan?,
    inferenceMode: InferenceMode,
    operatingMode: EpistemosOperatingMode,
    ...
    let directToolNames = inferenceState.capabilityToolNames(
      for: operatingMode,
      executionPlan: nil
    )
    ...
    if inferenceMode == .api {
      if executionPlan.route == .managedAgentSession {
        return (
          route: executionPlan.route,
          summary: executionPlan.summary,
          allowedToolNames: inferenceState.capabilityToolNames(
            for: operatingMode,
            executionPlan: executionPlan
          )
        )
      ...
      if operatingMode == .pro, isCloudSelectedSurface {
        return (
          route: .managedAgentSession,
          summary: "Use the managed tools runtime for long-running external orchestration.",
          allowedToolNames: inferenceState.capabilityToolNames(
            for: operatingMode,
            executionPlan: executionPlan
          )
    ...
    case .localOnly:
      return (
        route: executionPlan.route,
        summary: executionPlan.summary,
        allowedToolNames: directToolNames
      )
    case .overseerLocalExecution:
      return (
        route: executionPlan.route,
  ...
  func handleQuery(
    ...
    if operatingMode.capturesReasoningTrace {
      llmService.reasoningSink = { [weak chatState] delta in
        ...
        Task { @MainActor in
          chatState.appendStreamingThinking(delta, explicit: true)
        }
      }
    } else {
    ...
    bootstrap.queryTask = Task {
      ...
      do {
        ...
        if hasVault, hasExplicitContext {
          ...
          chatState.pendingVaultRecallTrace = nil
        }
        ...
        if shouldInjectWorkspaceContext {
          ...
          workspaceContextSection = Self.wrapSupplementalContextSection(
            ...
            instruction:
              "Treat this as optional background. Use it only when it helps answer the request, and let explicit user attachments take priority.",
        ...
        if !isVaultBriefing {
          ...
          conversationHistory = chatState.serializedConversationHistory(
            maxCharacters: historyBudgetChars,
            maxMessages: maxMessagesForModel
          )
        } else {
        ...
        if let executionPlan, mode == .api, executionPlan.route == .managedAgentSession {
          ...
          do {
            try await self.runRustAgentPath(
              ...
              notesContext: effectiveNotesContextWithWorkspace,
              conversationHistory: conversationHistory,
              surfaceOperatingMode: operatingMode,
              chatState: chatState,
        ...
        if !usedRustAgent {
          @MainActor func persistCompletedMainChatTurn() {
            chatState.recalculateContextEstimate()
          ...
          @MainActor func completeWithIndexedVaultFallbackIfPossible() async -> Bool {
            ...
            guard
              let fallback = await Self.buildIndexedVaultLookupFallbackAnswer(
                ...
                traceVaultRecall: { [weak self] traceQuery in
                  self?.recordVaultRecallTrace(query: traceQuery, sessionId: sessionID)
                }
              )
            else { return false }
            ...
            chatState.pendingVaultRecallTrace = fallback.vaultRecallTrace
          ...
          let stream = pipeline.run(
            ...
            toolEventHandler: { event in
              ...
              case .started(let id, let name, let inputJson):
                ...
                if let traceQuery = Self.vaultRecallTraceQueryFromToolCall(
                  toolName: name,
                  inputJson: inputJson,
                  fallbackQuery: query
                ) {
                  chatState.pendingVaultRecallTrace = self.recordVaultRecallTrace(
                    query: traceQuery,
                    sessionId: sessionID
                  )
                }
            ...
            modelInputCaptureHandler: { captured in
              chatState.captureModelInput(captured)
            }
          )
          ...
          for try await event in stream {
            switch event {
            case .thinkingDelta(let thought):
              chatState.appendStreamingThinking(thought, explicit: true)
        ...
        if chatState.completeCancelledProcessing(
          ...
          if chatState.chatTitle == nil {
            self.generateChatTitle(query: query, chatId: chatState.activeChatId, chatState: chatState)
          }
        }
      } catch {
        if chatState.completeCancelledProcessing(
  ...
  private func runRustAgentPath(
    ...
    notesContext: String?,
    conversationHistory: String?,
    surfaceOperatingMode: EpistemosOperatingMode,
    chatState: ChatState,
    chatId: String?,
    originalQuery: String,
    hasVault: Bool,
    ...
    let selectedSurface = inferenceState.effectiveChatSurfaceSelection(for: surfaceOperatingMode)
    ...
    let providerName = resolveRustProviderName(for: selectedSurface)
    ...
    let allowedToolDefinitions = ToolTierBridge(
      vaultPath: vaultPath,
      tier: runtimeToolTier,
      allowedToolNames: allowedTools
    ).loadTools()
    chatState.captureModelInput(
      CapturedModelInput(
    ...
    for await event in stream {
      ...
      case .toolStarted(let id, let name, let inputJson):
        ...
        if let traceQuery = Self.vaultRecallTraceQueryFromToolCall(
          ...
          chatState.pendingVaultRecallTrace = recordVaultRecallTrace(
            query: traceQuery,
      ...
      case .toolCompleted(let id, let result, let isError):
        ...
        {
          attemptedRequiredVaultRead = true
          if isError {
            lastRequiredVaultReadFailure = Self.summarizedToolFailure(
              toolName: toolInput.name,
              rawResult: result
            )
          } else {
            successfulRequiredVaultRead = true
      ...
      case .complete(_, let inputTokens, let outputTokens, let answerPacketId, _):
        ...
        if requiresVerifiedVaultRead, !successfulRequiredVaultRead {
          chatState.overrideStreamingAnswerForCompletion(
            Self.enforcedVaultReadFailureMessage(
              query: query,
              attemptedRead: attemptedRequiredVaultRead,
              lastFailure: lastRequiredVaultReadFailure
            )
          )
        }
        chatState.completeProcessing(
  ...
  private func rustAgentEffort(for operatingMode: EpistemosOperatingMode) -> String {
    switch inferenceState.sanitizedReasoningTier(
      inferenceState.chatReasoningTier,
      for: operatingMode
    ) {
    case .off, .low:
      return "low"
  ...
  private func resolveRustProviderName(for selection: ChatModelSelection) -> String {
    switch selection {
    case .appleIntelligence:
      return resolveRustProviderName()
    case .localMLX:
    ...
    Task {
      ...
      do {
        ...
        chatState.updateBrainSnapshotSection(
          ChatBrainSection(
            title: sectionTitle,
            body: Self.sessionWakeUpPreviewBody(
              forErrorDescription: error.localizedDescription
            )
          ),
          matchingCapturedAt: capturedAt
        )
  ...
  nonisolated static func sessionWakeUpPreviewBody(forErrorDescription errorDescription: String)
    -> String
  {
    let normalized =
      errorDescription
    ...
    if normalized.contains("failed to open vault")
    ...
    if normalized.contains("failed to open vault") {
      return
        "Temporarily unavailable while the vault preview reloads. This preview will refresh automatically."
    }

    return "Temporarily unavailable right now. This preview will refresh automatically."
  }
  ...
  private func managedToolRuntimeVaultPath() -> String {
    FoundationSafety.managedToolRuntimeVaultDirectory(
      preferredVaultPath: bootstrap.vaultSync.vaultURL?.path
    ).path
  }
  ...
  private enum ToolApprovalPromptChoice {
    case allowOnce
    case alwaysAllow
  ...
  private func storedAuthorityDecision(for request: AgentPermissionRequest) -> AuthorityDecision {
    let authorityCategory = request.authorityCategory(vaultPath: vaultSync.vaultURL?.path)
    return bootstrap.agentAuthorityStore.snapshot.decision(for: authorityCategory)
  }
  ...
  private func promptForToolApproval(_ request: AgentPermissionRequest) async -> Bool {
    ...
    let choice = await promptUserForToolApproval(
      request,
        ...
        if let chatId {
          ...
          if let sdChat = fetchFirst(
            ...
            do {
              ...
              Log.pipeline.error(
                "Failed to save chat title: \(error.localizedDescription, privacy: .public)")
            }
          }
        }
      } catch {
        Log.pipeline.debug(
          "Chat title generation failed: \(error.localizedDescription, privacy: .public)")
      }
    }
  }
  ...
  static func resolveNotesContext(
    ...
    traceVaultRecall: ((String) -> VaultRecallTrace?)? = nil
    ...
    func recordTraceIfNeeded(_ traceQuery: String) {
      guard vaultRecallTrace == nil else { return }
      let trimmed = traceQuery.trimmingCharacters(in: .whitespacesAndNewlines)
      guard !trimmed.isEmpty else { return }
      vaultRecallTrace = traceVaultRecall?(trimmed)
    }
    ...
    do {
      ...
      for match in matches.reversed() {
        ...
        if let fullRange = Range(match.range, in: cleanedQuery) {
          let replacement =
            title.caseInsensitiveCompare(Self.allNotesMentionToken) == .orderedSame ? "" : title
    ...
    {
      recordTraceIfNeeded(referencedTitle)
    ...
    {
      recordTraceIfNeeded(cleanedQuery)
  ...
  static func buildIndexedVaultLookupFallbackAnswer(
    ...
    let vaultRecallTrace = traceVaultRecall?(phrases.first ?? query)
  ...
  static func searchReferenceResults(
    ...
    indexedNoteProvenance: [String: VaultRecallCandidate] = [:]
    ...
    let recentChats =
      ...
      .map { chat in
        let preview = chat.sortedMessages.last?.content.trimmingCharacters(
    ...
    let filteredChats = (recentChats + transientThreads).filter { item in
      ...
      let haystack = [
        item.result.attachment.title, item.result.attachment.subtitle, item.result.preview,
      ]
      ...
      return lhs.result.attachment.title.localizedCaseInsensitiveCompare(
        rhs.result.attachment.title)
        == .orderedAscending
    }
    ...
    return ReferenceSearchResults(
      ...
      indexedNoteProvenanceByPageID: indexedNoteProvenance
    )
  }
  ...
  nonisolated(unsafe) private static var _cachedSearchPreparedEntries:
    [PreparedManifestSearchEntry]?
  nonisolated(unsafe) private static var _cachedEmptyManifestResults:
    CachedEmptyManifestSearchResults?
  ...
  func buildContextAttachments(
    ...
    chatState.pendingVaultRecallTrace = resolution.vaultRecallTrace
    return (resolution.context, resolution.cleanedQuery)
  }

  nonisolated private func recordVaultRecallTrace(
    query: String,
    sessionId: String
  ) -> VaultRecallTrace? {
    let trimmed = query.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmed.isEmpty else { return nil }
    guard let trace = VaultRecallBridge.trace(query: trimmed) else { return nil }
    EventStore.shared?.appendVaultRecallTrace(sessionId: sessionId, trace: trace)
    return trace
  }

  nonisolated static func vaultRecallTraceQueryFromToolCall(
    toolName: String,
    inputJson: String,
    fallbackQuery: String
  ) -> String? {
    let lowercasedTool = toolName.lowercased()
    guard lowercasedTool.contains("vault"),
          lowercasedTool.contains("search")
            || lowercasedTool.contains("read")
            || lowercasedTool.contains("find")
    else { return nil }

    if let data = inputJson.data(using: .utf8),
       let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
      for key in ["query", "q", "title", "path", "page_id", "pageId"] {
    ...
    let fallback = fallbackQuery.trimmingCharacters(in: .whitespacesAndNewlines)
  ...
  static func resolveAttachedContext(
    ...
    let requestedNoteContext =
      queryContainsExplicitNoteContext(query)
    ...
    let noteResolution = await resolveNotesContext(
      ...
      fetchNoteBodies: fetchNoteBodies,
      searchNoteIDs: searchNoteIDs,
      traceVaultRecall: traceVaultRecall
    )
    ...
    if let context = noteResolution.context, !context.isEmpty {
      let wrappedContext =
        ...
        : wrapSupplementalContextSection(
          ...
          instruction:
            "This note material was auto-matched from the vault to help answer the query. Use it only when it is relevant; explicit attachments above take priority.",
          body: context
        )
    ...
    return AttachedContextResolution(
      ...
      vaultRecallTrace: noteResolution.vaultRecallTrace
    )
  }
  ...
  private nonisolated static func explicitNoteReferenceDisplayTitle(in query: String) -> String? {
    extractedExplicitNoteReferenceTitle(in: query, lowercased: false)
  }

  private nonisolated static func extractedExplicitNoteReferenceTitle(
    in query: String,
    lowercased: Bool
  ) -> String? {
    let patterns = [
      #/(?i)\b(?:go\s+to|open|find|use|read|show|look\s+for|check)\s+(?:the\s+|my\s+)?note\s+(?:titled|called)\s+["“]?(.+?)["”]?(?=\s+(?:in\s+(?:my\s+)?(?:notes|vault)|and|then|please|summarize|rewrite|analyze|compare|review|explain|tell|show|use)\b|[?.!,]|$)/#,
      ...
      #/(?i)\b(?:go\s+to|open|find|use|read|show|look\s+for|check)\s+(?:the\s+|my\s+)?note\s+(.+?)(?=\s+(?:in\s+(?:my\s+)?(?:notes|vault)|and|then|please|summarize|rewrite|analyze|compare|review|explain|tell|show|use)\b|[?.!,]|$)/#,
      #/(?i)\b(?:my\s+)?note\s+(.+?)(?=\s+(?:in\s+(?:my\s+)?(?:notes|vault)|and|then|please|summarize|rewrite|analyze|compare|review|explain|tell|show|use)\b|[?.!,]|$)/#,
    ]

    for pattern in patterns {
      if let match = query.firstMatch(of: pattern) {
        var title = String(match.output.1)
          .trimmingCharacters(in: .whitespacesAndNewlines)
          .trimmingCharacters(in: CharacterSet(charactersIn: "\"'“”‘’"))
        if lowercased {
  ...
  static func buildChatContextPack(
    ...
    return wrapRequiredContextSection(
      ...
      instruction:
        "These chats were explicitly attached by the user for this request. Use them whenever they are relevant. Use these chats before recall/search tools. Only broaden beyond them if the user asks or the attached chats are clearly insufficient.",
      body: sections.joined(separator: "\n\n")
    )
  }
  ...
  static func queryRequiresVerifiedVaultRead(_ query: String) -> Bool {
    ...
    let noteNouns = ["note", "notes", "essay", "essays", "draft", "drafts", "vault"]
    guard noteNouns.contains(where: { normalized.contains($0) }) else { return false }
    ...
    let readSignals = [
      ...
      "compare different essays",
      "what's in my note",
      "what is in my note",
    ]
    return readSignals.contains { normalized.contains($0) }
  }

  static func queryContainsExplicitContext(_ query: String, attachments: [ContextAttachment])
    -> Bool
  {
  ...
  static func buildWorkspaceAwarenessContext(bootstrap: AppBootstrap, deepContext: Bool = false)
    -> String
  {
    var parts: [String] = []
    ...
    if !recentEvents.isEmpty {
      ...
      if chatMsgCount > 0 && !deepContext {
        activityLines.append(
          "- \(chatMsgCount) chat message\(chatMsgCount == 1 ? "" : "s") this hour")
      }
    ...
    if deepContext {
      ...
      for node in store.nodes.values where node.type == .idea && node.metadata.originChatId != nil {
        ...
        if anchorHeap.count > 5 {
          if let minIdx = anchorHeap.indices.min(by: {
            anchorHeap[$0].createdAt < anchorHeap[$1].createdAt
          }) {
      ...
      if !recentAnchors.isEmpty {
        ...
        if !themes.isEmpty {
          parts.append(
            "[Proactive Hint] The user has been exploring these themes recently: \(themes.joined(separator: ", ")). Look for connections between their current question and these themes. Adapt your communication style to be concise and direct — the user works intensively and prefers actionable insights over lengthy explanations."
          )
        }
      }
    }
  ...
  nonisolated static func buildFileAttachmentContext(
    ...
    let sections = attachments.compactMap {
      fileAttachmentSection(for: $0, supportsVision: supportsVision)
    }
    return wrapRequiredContextSection(
  ...
  private nonisolated static func buildRequiredAttachmentContractSection() -> String? {
    wrapRequiredContextSection(
      ...
      instruction:
        "The user intentionally attached or referenced files, notes, or chats for this request. Use that material directly, and do not ask them to provide it again unless something is missing or unreadable.",
      body: """
  ...
  private nonisolated static func buildRequestedVaultLookupContractSection() -> String? {
    wrapRequiredContextSection(
      title: "Requested Vault Lookup Contract",
      instruction:
        "The user asked you to find or read notes from the vault for this request. If note content is resolved below, treat it as a successful vault lookup rather than an attachment.",
      body: """
        If note content appears below, answer from it directly.
        Do not open with a provenance sentence like "I found it in your notes" unless the user explicitly asks whether the lookup succeeded.
  ...
  private nonisolated static func isVaultReadVerificationTool(
    toolName: String,
    inputJson: String,
    vaultPath: String
  ) -> Bool {
    ...
    case "file.read":
      ...
      return isLikelyVaultPath(rawPath, vaultPath: vaultPath)
    default:
      return false
    }
  }

  private nonisolated static func isLikelyVaultPath(_ rawPath: String, vaultPath: String) -> Bool {
    ...
    return standardizedPath == standardizedVaultPath
      || standardizedPath.hasPrefix(standardizedVaultPath + "/")
  }

  private nonisolated static func summarizedToolFailure(
    toolName: String,
    rawResult: String
  ) -> String {
    let cleaned =
    ...
    return String(cleaned.prefix(220)).trimmingCharacters(in: .whitespacesAndNewlines) + "…"
  }

  private nonisolated static func enforcedVaultReadFailureMessage(
    query: String,
    attemptedRead: Bool,
    lastFailure: String?
  ) -> String {
    let referencedTitle = explicitNoteReferenceDisplayTitle(in: query)
    if let referencedTitle, !referencedTitle.isEmpty {
      if attemptedRead || (lastFailure?.isEmpty == false) {
        return "I couldn't read \"\(referencedTitle)\" from your notes on this turn."
      }
      return "I couldn't find a note titled \"\(referencedTitle)\" in your notes."
    }
    if attemptedRead || (lastFailure?.isEmpty == false) {
      return "I couldn't read that note from your notes on this turn."
    }
    return "I couldn't find that note in your notes."
  }
  ...
  private nonisolated static func readTextAttachment(at url: URL) -> String? {
    ...
    do {
      ...
      Log.pipeline.error(
        "Failed to open file handle for attachment at \(url.path): \(error.localizedDescription)")
      return nil
    }
    defer {
      do {
        ...
        Log.pipeline.warning(
          "Failed to close file handle for \(url.path): \(error.localizedDescription)")
      }
    }
    ...
    do {
      ...
      Log.pipeline.error(
        "Failed to read attachment data at \(url.path): \(error.localizedDescription)")
      return nil
    }
    ...
    guard let decoded = FoundationSafety.decodedText(from: data) else {
      Log.pipeline.warning(
        "Failed to decode text from attachment at \(url.path) (\(data.count) bytes)")
      return nil
    }
  ...
  static func sanitizeVaultActionMarkers(in response: String) -> (
    cleaned: String, blockedActions: [String]
  ) {
    ...
    while let range = cleaned.range(of: #"\[ACTION:TAG\s+(.+?)\]"#, options: .regularExpression) {
      ...
      if !tags.isEmpty {
        blockedActions.append(
          "Approval required before adding tags [\(tags.joined(separator: ", "))].")
      }
