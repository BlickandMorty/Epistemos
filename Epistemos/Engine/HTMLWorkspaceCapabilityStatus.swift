...
enum HTMLWorkspaceCapabilityStatus {
    ...
    static let capabilities: [Capability] = [
        ...
        Capability(name: "Full-surface regenerate", isLive: false, note: "replaceDocument/regenerate patch plumbing exists, with one-click stream-to-preview/apply, visible revert, pixel-captioned presets, advanced manual fallback kept recovery-only, workspace-context grounding, source-labelled bounded context payloads, explicit context metadata preservation, required-source availability metadata, structured prompt context records, selected-preview prompt targeting, source-kind compact feed status, preview-side context rail, in-preview context picker, selected-preview drop targets, generated dashboard section nav, generated data-backed context picker, scoped context-tab visible counts, and clickable vault result detail panes; awaits current in-app proof"),
