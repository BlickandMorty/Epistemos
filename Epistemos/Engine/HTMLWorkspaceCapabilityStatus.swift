...
enum HTMLWorkspaceCapabilityStatus {
    ...
    static let capabilities: [Capability] = [
        ...
        Capability(name: "Full-surface regenerate", isLive: false, note: "replaceDocument/regenerate patch plumbing exists, with one-click stream-to-preview/apply, primary stream context-query auto-attach, hash-matched preview apply gating, visible revert, revert snapshot preserved through snapshot churn, pixel-captioned presets, advanced manual fallback kept recovery-only, workspace-context grounding, source-labelled bounded context payloads, explicit recent-capture context feed, explicit note context feed, explicit PDF-note context feed, explicit folder-note context feed, explicit meeting-note context feed, explicit web-clip context feed, explicit recent-chat context feed, explicit provenance-claim context feed, explicit graph-related-note context feed, normalized explicit context metadata/source labels/status fields, required-source availability metadata that survives data-feed refresh, manifest-hash-stamped context feeds, structured prompt context records, stable drag context IDs, validated read-only context drops, feed-envelope source/query/limit matching, selected-preview prompt targeting, target-aware stream loading preview, source-kind compact feed status with required-source availability, preview-side context rail, source-visible in-preview context picker, source-visible preview context sidebar, sheet-and-preview context shortcut fetching, selected-preview drop targets, native payload-aware generated section drop zones with honest unmatched-drop status, generated dashboard section nav, generated data-backed context picker with pick-to-pin, in-memory generated pinned-context tray, generated drag-to-pin context tray, generated data-backed sorting, scoped context-tab visible counts, and clickable tabbed vault result detail panes; awaits current in-app proof"),
        Capability(name: "App message-bridge", isLive: false, note: "Sandbox-gated safe API parses bounded ping/status/event.record messages with structured event attributes, records diagnostics, dispatches required command-matched correlated response events with ok/error detail that reject unsupported requests, exposes a promise request helper, and has manual probe + insertable demo scaffold paths; awaits current in-app proof"),
        Capability(name: "JS console / error capture", isLive: false, note: "Console log/info/warn/error + window-error/unhandled-rejection capture script, idempotent runtime install guard, typed severity/source pipeline, clearable panel, and manual probe path are wired; awaits current in-app proof"),
        Capability(name: "DOM picker / style inspector", isLive: false, note: "Click-pick inspector bridge, hover-highlighted picker, element-normalized event targets, escaped selector copy, nth-of-type focused selectors, style payload parser, focused style edits with unsafe CSS value rejection, and open-panel probe path are wired; awaits current in-app proof"),
        Capability(name: "Python (Pyodide / WASM)", isLive: false, note: "Build-vendored Pyodide assets, sandbox-gated runtime path, bounded and serialized run input, local/blob worker CSP, insertable demo scaffold, and runtime probe are wired; awaits current in-app proof"),
    ]
    ...
    static var summary: String {
        "\(liveCount) live, \(deferredCount) awaiting current in-app proof — renders, edits, agent-patches, serves package-local assets/data/routes in preview, exports HTML/PDF with inlined package assets, exports site folders with routes and route-relative asset mirrors, shows a live DOM outline, and can opt into Vault-backed data.json refresh. Full regenerate stream-to-preview/apply/revert UX with primary stream context-query auto-attach, hash-matched preview apply gating, revert snapshot preserved through snapshot churn, pixel-captioned presets, advanced manual fallback kept recovery-only, workspace-context grounding, source-labelled bounded context payloads, explicit recent-capture context feed, explicit note context feed, explicit PDF-note context feed, explicit folder-note context feed, explicit meeting-note context feed, explicit web-clip context feed, explicit recent-chat context feed, explicit provenance-claim context feed, explicit graph-related-note context feed, normalized explicit context metadata/source labels/status fields, required-source availability metadata that survives data-feed refresh, manifest-hash-stamped context feeds, structured prompt context records, stable drag context IDs, validated read-only context drops, feed-envelope source/query/limit matching, selected-preview prompt targeting, target-aware stream loading preview, source-kind compact feed status with required-source availability, preview-side context rail, source-visible in-preview context picker, source-visible preview context sidebar, sheet-and-preview context shortcut fetching, selected-preview drop targets, native payload-aware generated section drop zones with honest unmatched-drop status, generated dashboard section nav, generated data-backed context picker with pick-to-pin, in-memory generated pinned-context tray, generated drag-to-pin context tray, generated data-backed sorting, scoped context-tab visible counts, and clickable tabbed vault result detail panes, app bridge demo/runtime with promise requests, ok/error response detail, and structured event attributes, JS console/window-error/rejection capture, hover-highlighted DOM picker/style inspection with focused selectors and unsafe CSS value rejection, and build-vendored Pyodide demo/runtime with bounded run input plus local/blob worker CSP remain gated until a launchable build proves them."
    }
}
