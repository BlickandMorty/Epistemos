...
// static (the app message-bridge handler is an empty stub, the JS console can't capture errors, DOM
// "inspection" is a static regex of source rather than the live runtime DOM, there is no Python, and
// chat does incremental patches only — no full-surface regenerate). The codebase has a GateStatus
// honesty convention, but the HTML workspace shipped silently as if complete.
//
...
enum HTMLWorkspaceCapabilityStatus {
    ...
    static let capabilities: [Capability] = [
        ...
        Capability(name: "JS console / error capture", isLive: false, note: "No window.onerror bridge — deferred"),
        ...
        Capability(name: "Full-surface regenerate", isLive: false, note: "Incremental patches only today — research"),
    ]
    ...
    static var summary: String {
        "\(liveCount) live, \(deferredCount) static/deferred — renders, edits, and agent-patches; the app-bridge, live-DOM inspection, console capture, Python, and full-surface regenerate are deferred (it's a static renderer today, not a full web-app builder yet)."
    }
}
