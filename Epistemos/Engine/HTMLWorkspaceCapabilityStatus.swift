...
enum HTMLWorkspaceCapabilityStatus {
    ...
    static let capabilities: [Capability] = [
        ...
        Capability(name: "App message-bridge", isLive: false, note: "Sandbox-gated safe API parses bounded ping/status/event.record messages with structured event attributes, records diagnostics, dispatches command-matched correlated response events that reject unsupported requests, exposes a promise request helper, and has manual probe + insertable demo scaffold paths; awaits current in-app proof"),
