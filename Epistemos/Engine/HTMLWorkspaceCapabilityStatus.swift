...
// a source-quad patch primitive so far — no general streaming/provenance/multi-route scaffold yet). The
...
enum HTMLWorkspaceCapabilityStatus {
    ...
    static var summary: String {
        "\(liveCount) live, \(deferredCount) static/deferred — renders, edits, agent-patches, and can opt into Vault-backed data.json refresh; the app-bridge, live-DOM inspection, console capture, Python, and full-surface regenerate UX are deferred (partial artifact runtime, not a full web-app builder yet)."
    }
}
