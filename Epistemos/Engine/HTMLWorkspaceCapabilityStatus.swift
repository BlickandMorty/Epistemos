...
enum HTMLWorkspaceCapabilityStatus {
    ...
    static let capabilities: [Capability] = [
        ...
        Capability(name: "Full-surface regenerate", isLive: false, note: "replaceDocument/regenerate patch plumbing exists, with one-click stream-to-preview/apply, visible revert, pixel-captioned presets, advanced manual fallback, workspace-context grounding, source-labelled bounded context payloads, preview-side context rail, in-preview context picker, and selected-preview drop targets; awaits current in-app proof"),
        ...
        Capability(name: "DOM picker / style inspector", isLive: false, note: "Click-pick inspector bridge, hover-highlighted picker, escaped selector copy, style payload parser, focused style edits, and open-panel probe path are wired; awaits current in-app proof"),
        Capability(name: "Python (Pyodide / WASM)", isLive: false, note: "Build-vendored Pyodide assets, sandbox-gated runtime path, local/blob worker CSP, insertable demo scaffold, and runtime probe are wired; awaits current in-app proof"),
    ]
