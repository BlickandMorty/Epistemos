...
// nonPersistent() on WKWebView hosts; BrowserTrackerContentBlocker WKContentRuleList wiring; zero
// customUserAgent / obscura-dep references).
enum BrowserCapabilityStatus {
    ...
    static var summary: String {
        "\(liveCount) live, \(deferredCount) cut/stub/unbuilt — real HTTP fetch/extract/crawl/schema extraction, private in-app web views, and WKContentRuleList tracker/ad blocking work today; the native Obscura stealth path is owner-cut/superseded by browser-use, and anti-fingerprint spoofing is not built."
    }
}
