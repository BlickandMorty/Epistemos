...
struct GooseWebOnlySurfaceSourceTests {
    ...
    func nativeAgentWindowKeepsGoosePrimary() throws {
        ...
        #expect(!FileManager.default.fileExists(atPath: sourceRoot.appendingPathComponent(
            "Epistemos/Goose/GooseSurfaceWindowController.swift",
            isDirectory: false
        ).path))
        ...
        #expect(reskin.contains("--epistemos-claude-soft-shadow:"))
        #expect(reskin.contains("box-shadow: var(--epistemos-claude-soft-shadow) !important;"))
...
"""))
        ...
        #expect(stageScript.contains("scripts/stage-goose-native-reskin.mjs"))
        #expect(stageScript.contains("epistemos-native-final-flat-pixel-audit"))
        ...
        #expect(reskin.contains(".goose-epistemos [class~='bg-[var(--epistemos-accent)]']"))
        #expect(reskin.contains("color: var(--color-text-inverse) !important;"))
        #expect(reskin.contains("bg-background-secondary/70"))
        ...
        for requiredReskinSnippet in [
            "bg-background-primary/40",
            "bg-background-primary/54 pb-2",
            "bg-background-primary/54 p-3",
            "bg-[var(--epistemos-accent)]/12",
            "focus:bg-background-secondary/72 focus:outline-none focus:ring-0",
            "focus-visible:bg-[var(--epistemos-accent)]/10 focus-visible:ring-0",
            "rounded-[9px] p-1 space-y-0.5",
            "select__menu z-[9999] absolute overflow-hidden",
            "rounded-[11px] bg-background-primary/54",
            "min-h-[128px] overflow-hidden bg-background-primary/54",
            "absolute -bottom-2 right-2 whitespace-nowrap rounded-[8px] bg-background-secondary/72",
            "mt-2 flex flex-wrap gap-2 bg-background-primary/24 p-4",
            "rounded-[10px] bg-background-secondary/56 object-cover",
            "rounded-[10px] bg-background-secondary/56 px-3 py-2"
        ] {
            #expect(reskin.contains(requiredReskinSnippet), "Missing Goose reskin snippet: \(requiredReskinSnippet)")
        }
        ...
        #expect(reskin.contains("bg-background-primary/24"))
        ...
        #expect(reskin.contains("rounded-[12px] bg-background-secondary/56 px-4 py-3"))
        #expect(reskin.contains("rounded-[12px] bg-background-secondary/32 px-4 py-3"))
        #expect(reskin.contains("flex max-h-[90vh] w-[900px] max-w-[90vw] flex-col overflow-hidden rounded-[14px] bg-background-primary/92 p-6"))
        #expect(reskin.contains("flex max-h-[90vh] w-[800px] max-w-[90vw] flex-col overflow-hidden rounded-[14px] bg-background-primary/92 p-6"))
        #expect(reskin.contains("rounded-[10px] bg-background-secondary/60"))
        #expect(reskin.contains("w-full cursor-pointer rounded-[10px] p-3 transition-colors"))
        #expect(reskin.contains("mt-3 rounded-[12px] bg-background-secondary/44 p-3"))
        #expect(reskin.contains("rounded-[12px] bg-background-primary/34 px-3 py-2"))
        ...
        #expect(support.contains("--epistemos-quiet-ring: \\(quietFocusRing) !important;"))
        // June ontology: keyboard focus is June's quiet 2px themed outline and
        // the native injection feeds the June seam (--ep-*) per resolved theme.
        #expect(support.contains("outline: 2px solid var(--focus-ring, \\(quietFocusRing)) !important;"))
        #expect(support.contains("--ep-accent: \\(uiAccent) !important;"))
        #expect(support.contains("--ep-surface-card: \\(surfaceStrong) !important;"))
        #expect(!support.contains("box-shadow: 0 0 0 2px var(--epistemos-quiet-ring)"))
    ...
    func landingExposesGooseWebOnlyEntry() throws {
        ...
        #expect(landing.contains("ui.homeContent = .goose"))
        #expect(landing.contains("AgentSurfaceRootView(theme: ui.theme)"))
