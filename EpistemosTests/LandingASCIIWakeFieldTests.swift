...
struct LandingASCIIWakeFieldTests {
    ...
    @Test("landing wake drag haptics require sustained movement outside the throttle window")
    func landingWakeDragHapticsRequireSustainedMovementOutsideThrottleWindow() {
        var state = LandingCursorBrushHapticState(
            minimumInterval: 0.05,
            baseDistanceThreshold: 14
        )
        let configuration = LandingASCIIWakeFieldConfiguration()

        let initial = state.update(
            location: CGPoint(x: 0, y: 0),
            now: 0.00,
            configuration: configuration
        )
        let tooSmall = state.update(
            location: CGPoint(x: 5, y: 0),
            now: 0.01,
            configuration: configuration
        )
        let firstPulse = state.update(
            location: CGPoint(x: 17, y: 0),
            now: 0.07,
            configuration: configuration
        )
        let throttled = state.update(
            location: CGPoint(x: 34, y: 0),
            now: 0.09,
            configuration: configuration
        )
        let secondPulse = state.update(
            location: CGPoint(x: 52, y: 0),
            now: 0.16,
            configuration: configuration
        )
        ...
        #expect(initial == nil)
        #expect(tooSmall == nil)
        #expect(firstPulse == .whisper)
        #expect(throttled == nil)
        #expect(secondPulse != nil)
    }

    @Test("landing wake drag haptics reset when hover ends")
    func landingWakeDragHapticsResetWhenHoverEnds() {
        var state = LandingCursorBrushHapticState(
            minimumInterval: 0.05,
            baseDistanceThreshold: 14
        )
        ...
        let reentry = state.update(
            location: CGPoint(x: 0, y: 0),
            now: 0.20,
            configuration: configuration
        )
        let resumedDrag = state.update(
            location: CGPoint(x: 18, y: 0),
            now: 0.28,
            configuration: configuration
        )
