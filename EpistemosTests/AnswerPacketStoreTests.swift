...
struct AnswerPacketStoreTests {
    ...
    func persistedRecentAndHonestStatus() throws {
        ...
        #expect(store.contains("openStoreFileForWriting(truncate: false, appendingBytes: line.count)"))
        #expect(store.contains("UInt64(appendingBytes) <= UInt64(Self.maxLogBytes) - byteCount"))
