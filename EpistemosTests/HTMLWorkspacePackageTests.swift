...
nonisolated struct HTMLWorkspacePackageTests {
    ...
    func vaultSearchDashboardTemplateSeedsLiveDataFeedShell() throws {
        ...
        #expect(package.indexHTML.contains(#"data-context-section="Pinned""#))
        #expect(package.indexHTML.contains(#"data-context-section="Rank chart""#))
        #expect(package.indexHTML.contains(#"data-context-section="Detail""#))
        #expect(package.indexHTML.contains(#"data-context-section="Results""#))
        ...
        #expect(package.styleCSS.contains(".section-context-badge"))
        ...
        #expect(package.styleCSS.contains(".result-card.is-section-bound"))
        #expect(package.styleCSS.contains(".result-card.is-selected.is-section-bound"))
        ...
        #expect(package.scriptJS.contains("let sectionContextLabels = {};"))
        #expect(package.scriptJS.contains("sectionContextLabels[sectionID] = dropzoneContextLabel(dropzone);"))
        ...
        #expect(package.scriptJS.contains("workspace.context.pin"))
        #expect(package.scriptJS.contains("workspace.context.drop"))
        ...
        #expect(package.scriptJS.contains("let sectionContextKeys = {};"))
        ...
        #expect(package.scriptJS.contains("function contextStateScope()"))
        #expect(package.scriptJS.contains("vault-context.${source}.${query}.${limit}"))
        #expect(package.scriptJS.contains("function syncPersistedContextState()"))
        #expect(package.scriptJS.contains("function savePersistedContextState()"))
        #expect(package.scriptJS.contains("HTMLWorkspace.contextState?.load(scope)"))
        #expect(package.scriptJS.contains("HTMLWorkspace.contextState?.save(scope, {"))
        #expect(package.scriptJS.contains("uniqueBoundedContextKeys(state.pinnedContextKeys)"))
        #expect(package.scriptJS.contains("boundedStoredContextMap(state.sectionContextKeys)"))
        #expect(package.scriptJS.contains("boundedStoredContextKey(state.contextDropKey)"))
        #expect(package.scriptJS.contains("Stored context state unavailable for this preview."))
        #expect(package.scriptJS.contains("Context pins are active for this session only."))
        #expect(package.scriptJS.contains("shouldValidateContextState(allResults, meta)"))
        #expect(package.scriptJS.contains("Pinned sources waiting for the current feed refresh."))
        ...
        #expect(package.scriptJS.contains("function renderPinnedContext(allResults, meta = {})"))
        #expect(package.scriptJS.contains("function selectPinnedContext(key)"))
        #expect(package.scriptJS.contains("function sectionBoundResult(sectionID, allResults)"))
        #expect(package.scriptJS.contains("function sectionAugmentedResults(sectionID, results, allResults)"))
        #expect(package.scriptJS.contains("return allResults.find((result) => resultKey(result) === key) || null;"))
        #expect(package.scriptJS.contains("return [boundResult].concat(results.filter((result) => resultKey(result) !== boundKey));"))
        ...
        #expect(package.scriptJS.contains("function dropzoneContextID(dropzone)"))
        #expect(package.scriptJS.contains("function dropzoneContextLabel(dropzone)"))
        #expect(package.scriptJS.contains("function bindDropzoneContext(dropzone, key)"))
        #expect(package.scriptJS.contains("sectionContextKeys[dropzoneContextID(dropzone)] = key;"))
        #expect(package.scriptJS.contains("function forgetContextKey(key)"))
        #expect(package.scriptJS.contains("if (selectedResultKey === key)"))
        #expect(package.scriptJS.contains("pinnedContextKeys.splice(index, 1);"))
        #expect(package.scriptJS.contains("function refreshContextDropStatus(allResults, meta = {})"))
        #expect(package.scriptJS.contains("Dropped context is no longer in the current data.json feed."))
        #expect(package.scriptJS.contains("const staleKey = sectionContextKeys[sectionID];"))
        #expect(package.scriptJS.contains("forgetContextKey(staleKey);"))
        #expect(package.scriptJS.contains("Dropped context for ${sectionLabel} is no longer in the current data.json feed."))
        #expect(package.scriptJS.contains("syncPersistedContextState();"))
        #expect(package.scriptJS.contains("refreshContextDropStatus(allResults, meta);"))
        ...
        #expect(package.scriptJS.contains("const isNativeContextPayload = raw.includes('drop_action: regenerate_preview_context') && raw.includes('readonly: true');"))
        #expect(package.scriptJS.contains("if (!isNativeContextPayload) { return ''; }"))
        ...
        #expect(package.scriptJS.contains("Context selected for ${dropzoneContextLabel(dropzone)} from current data.json feed."))
        ...
        #expect(package.scriptJS.contains("renderPinnedContext(allResults, meta)"))
        #expect(package.scriptJS.contains("selectedContextKind = 'all';"))
        #expect(package.scriptJS.contains("const selected = selectedResultKey"))
        #expect(package.scriptJS.contains("? allResults.find((result) => resultKey(result) === selectedResultKey)"))
        #expect(package.scriptJS.contains(": activeResult(sortedResults(visibleResults(allResults)));"))
        ...
        #expect(package.scriptJS.contains("pinDroppedContextPayload(payload, dropzone);"))
        ...
        #expect(package.scriptJS.contains("const displayedResults = sectionAugmentedResults('vault-results', results, allResults);"))
        #expect(package.scriptJS.contains("const resultsSectionBoundKey = sectionContextKeys['vault-results'] || null;"))
        #expect(package.scriptJS.contains("const selectedResult = activeResult(displayedResults);"))
        #expect(package.scriptJS.contains("renderContextPicker(displayedResults, selectedResult)"))
        #expect(package.scriptJS.contains("renderResultDetail(displayedResults, allResults)"))
        #expect(package.scriptJS.contains("displayedResults.forEach((result, index) => {"))
        #expect(package.scriptJS.contains("const isSectionBound = resultsSectionBoundKey && key === resultsSectionBoundKey;"))
        #expect(package.scriptJS.contains("isSectionBound ? 'is-section-bound' : ''"))
        #expect(package.scriptJS.contains("'data-section-bound-result': isSectionBound ? 'true' : 'false'"))
        #expect(package.scriptJS.contains("const resultOrdinal = isSectionBound ? 'Dropped section source' : `#${index + 1}`;"))
        #expect(package.scriptJS.contains("renderContextTabs(allResults, meta)"))
        #expect(package.scriptJS.contains("renderResultChart(results, allResults)"))
        #expect(package.scriptJS.contains("renderResultDetail(displayedResults, allResults)"))
    ...
    func offlinePreviewInjectsCSP() {
        ...
        #expect(srcdoc.contains(#"const workspaceID = "html-workspace-test";"#))
        #expect(srcdoc.contains("contextState,"))
        #expect(srcdoc.contains("epistemos.htmlWorkspace.contextState.v1."))
        #expect(srcdoc.contains("const store = window.localStorage;"))
        #expect(srcdoc.contains("if (!store) { return false; }"))
        #expect(srcdoc.contains("const boundedKeyList = (values) =>"))
        #expect(srcdoc.contains("return keys.slice(-16);"))
        #expect(srcdoc.contains("Object.entries(value).slice(0, 24)"))
        #expect(srcdoc.contains("const normalizedContextState = (value) =>"))
        #expect(srcdoc.contains("pinnedContextKeys: boundedKeyList(value.pinnedContextKeys)"))
        #expect(srcdoc.contains("sectionContextLabels: boundedRecord(value.sectionContextLabels, 160)"))
        #expect(srcdoc.contains("if (!normalized) { return false; }"))
        #expect(srcdoc.contains("store.getItem(storageKey(scope))"))
        #expect(srcdoc.contains("store.setItem(storageKey(scope), raw)"))
    ...
    func appBridgeHelperRequiresSandboxOptIn() {
        ...
        #expect(srcdoc.contains("const nextRequestId = () =>"))
        #expect(srcdoc.contains("window.crypto.randomUUID"))
        #expect(srcdoc.contains("window.crypto.getRandomValues(values)"))
        #expect(srcdoc.contains("requestId: nextRequestId()"))
    ...
    func vaultSearchDashboardCardsSelectRealResultDetail() {
        ...
        #expect(package.scriptJS.contains("function sectionBoundResult(sectionID, allResults)"))
        #expect(package.scriptJS.contains("const boundResult = sectionBoundResult('rank-signal', allResults);"))
        #expect(package.scriptJS.contains("const chartResults = sectionAugmentedResults('rank-signal', results, allResults);"))
        #expect(package.scriptJS.contains("Dropped section source has no numeric rank available for charting."))
        ...
        #expect(package.scriptJS.contains("const boundResult = sectionBoundResult('selected-source', allResults);"))
        #expect(package.scriptJS.contains("const selected = boundResult || activeResult(results);"))
        #expect(package.scriptJS.contains("boundResult ? 'Dropped section source' : 'Selected source'"))
