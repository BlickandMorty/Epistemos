...
// SS-VIS step-2 (owner 2026-06-20): the capability launchers surface tools/cowork on every chat
// surface (shipped: landing/mini-chat/graph/note). The remaining piece is "start off using a tool" —
// which operating mode the new chat begins in. These pin that decision: a plain tool keeps the current
// mode (it's already armed via the shared command center), while a cowork/Act selection starts in Act
// mode so a multi-step run is enabled from the first turn. Pure + total over every mode.
...
struct SSVISLaunchHandoffTests {
    ...
    @Test("a cowork / Act selection starts the chat in Act mode (.agent) from any current mode")
