...
struct BlockParserTests {
    ...
    func preserveBlockIds() async {
        let parser = BlockParser()
        let body = "Content"
        let existingBlocks = [ParsedBlock(id: "existing-123", content: "Content", order: 0)]
    ...
    func detectMovedBlocks() async {
        let parser = BlockParser()
        let oldBlocks = [
            ParsedBlock(id: "b1", content: "First", order: 0),
            ParsedBlock(id: "b2", content: "Second", order: 1)
        ]
        let newBody = "Second\n\nFirst"
    ...
    func detectModifiedBlocks() async {
        let parser = BlockParser()
        let oldBlocks = [ParsedBlock(id: "b1", content: "Old", order: 0)]
        let newBody = "New"
        let result = parser.parseWithDiff(body: newBody, pageId: "p1", existingBlocks: oldBlocks)
    ...
    func detectDeletedBlocks() async {
        let parser = BlockParser()
        let oldBlocks = [
            ParsedBlock(id: "b1", content: "Keep", order: 0),
            ParsedBlock(id: "b2", content: "Delete", order: 1)
        ]
        let newBody = "Keep"
        let result = parser.parseWithDiff(body: newBody, pageId: "p1", existingBlocks: oldBlocks)
    ...
    func blockMatching() async {
        let parser = BlockParser()
        let oldBlocks = [ParsedBlock(id: "b1", content: "Hello world test", order: 0)]
        let newBlocks = [ParsedBlock(content: "Hello world testing", order: 0)]
...
struct FileWatcherTests {
    ...
    func detectsCreation() async {
        let watcher = FileWatcher(path: "/tmp/test")
        var detected = false
    ...
    func debouncesChanges() async {
        let watcher = FileWatcher(path: "/tmp/test", debounceInterval: 0.1)
        var count = 0
    ...
    func handlesNestedDirectories() async {
        let watcher = FileWatcher(path: "/tmp/test", recursive: true)
        var detected = false
...
class VaultSyncService {
    var isPaused = false
    func detectChanges(files: [String], modifiedDates: [String: Date]? = nil) async -> SyncChanges {
        SyncChanges(newFiles: [], modifiedFiles: [], deletedFiles: files)
    }
    func markAsSynced(files: [String]) async {}
    func syncBidirectional() async -> SyncResult { SyncResult(success: true) }
    func resolveConflict(file: String, localDate: Date, remoteDate: Date) async -> ConflictResolution { .useLocal }
    func syncBatch(files: [String]) async -> BatchResult { BatchResult(processed: files.count) }
    ...
    func performInitialSync() async -> InitialSyncResult { InitialSyncResult(importedCount: 10) }
}
...
struct SyncResult { let success: Bool }
enum ConflictResolution { case useLocal, useRemote, merge }
struct BatchResult { let processed: Int }
struct InitialSyncResult { let importedCount: Int }

class BlockParser {
    func parse(body: String, pageId: String, existingBlocks: [ParsedBlock]? = nil) -> [ParsedBlock] {
        body.split(separator: "\n\n").enumerated().map { i, content in
            ParsedBlock(content: String(content), order: i)
        }
    }
    func parseWithDiff(body: String, pageId: String, existingBlocks: [ParsedBlock]) -> BlockDiff {
        BlockDiff(added: [], modified: [], moved: [], deleted: [])
    }
    func jaccardSimilarity(_ s1: String, _ s2: String) -> Double { 0.5 }
    func matchBlocks(old: [ParsedBlock], new: [ParsedBlock], threshold: Double) -> [(old: String, new: String)] { [] }
}
...
class FileWatcher {
    ...
    func simulateEvent(_ event: FileEvent) async {}
...
class SyncIndex {
    ...
    func metadata(for file: String) async -> FileMetadata? { nil }
