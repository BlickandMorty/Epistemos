...
struct GooseRuntimeSupervisorTests {
    ...
    func masInProcessPromptPathUsesAgentCoreFFI() throws {
        ...
        #expect(server.contains(#""session/request_permission""#))
        #expect(server.contains("requestAgentCorePermission"))
        #expect(server.contains("completePendingPermissionResponse"))
        #expect(server.contains("waitForPermission"))
...
struct GooseWebUIStagingTests {
    ...
    func stagingScriptForcesRelativeRendererAssets() throws {
        ...
        #expect(script.contains("epistemos-acp-stop-bypass"))
        #expect(script.contains("String(property) === 'sessionSteer_unstable'"))
        #expect(script.contains("name === 'prompt' || name === 'cancel'"))
        #expect(script.contains("return callACPImmediate(value, target, args, name);"))
...
struct GooseWebViewBootShimTests {
    ...
    func dispositionLedgerCoversKnownAffordances() {
        ...
        #expect(ledger["hideWindow"] == .implementedNative)
    ...
    func bootstrapRoutesNativeAffordances() {
        ...
        #expect(script.contains("const extensionDeepLinkURL = (rawURL) =>"))
        #expect(script.contains("url.protocol === 'goose:' && url.hostname === 'extension'"))
        #expect(script.contains("const sessionDeepLinkURL = (rawURL) =>"))
        #expect(script.contains("url.protocol === 'goose:' && url.hostname === 'sessions'"))
        #expect(script.contains("emitEvent('add-extension', href)"))
        #expect(script.contains("emitEvent('open-shared-session', sessionHref)"))
        #expect(script.contains("if (forwardGooseDeepLink(url)) return null;"))
        #expect(script.contains("target?.closest?.('a[href]')"))
        #expect(script.contains("postNativeAffordance('launchApp', [app])"))
        #expect(script.contains("const epistemosGooseApps = Object.freeze"))
        #expect(script.contains("'X-Secret-Key': epistemosGoose.secretKey"))
        #expect(script.contains("const listLiveApps = async (sessionId = null) =>"))
        #expect(script.contains("`/agent/list_apps?session_id=${encodeURIComponent(sessionId)}`"))
        #expect(script.contains("const importLiveApp = async (html) => gooseFetch('/agent/import_app'"))
        #expect(script.contains("const exportLiveApp = async (name) => gooseFetch(`/agent/export_app/${encodeURIComponent(name)}`)"))
        #expect(script.contains("Epistemos Apps bridge falling back to native imported apps:"))
        ...
        #expect(script.contains("listApps: async (sessionId = null) =>"))
        ...
        #expect(script.contains("postNativeAffordance('hideWindow')"))
        #expect(script.contains("postNativeAffordance('getSetting', [key])"))
        #expect(script.contains("postNativeAffordance('setSetting', [key, value])"))
        ...
        #expect(!script.contains("visibleError('hideWindow')"))
