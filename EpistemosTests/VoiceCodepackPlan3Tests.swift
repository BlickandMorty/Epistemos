...
struct VoiceCodepackPlan3Tests {
    ...
    func kokoroTTSInputHelpersRejectEmptyAndOversizedTextBeforePlayback() throws {
        ...
        #expect(
            EpistemosSpeechSynthesizer
                .textToSpeechStatusMessage(for: oversized, environment: [:], modelRoot: nil)
                .contains("limited to \(EpistemosSpeechSynthesizer.maxTextToSpeechInputCharacters) characters")
        )
    ...
    func readAloudEffectsAreGlobalOptionalAndAppliedToKokoroPCM() throws {
        ...
        let synth = try loadMirroredSourceTextFile("Epistemos/Engine/EpistemosSpeechSynthesizer.swift")
        let agentReadAloud = try loadMirroredSourceTextFile("Epistemos/Engine/EpistemosAgentReadAloud.swift")
        ...
        let experimental = try loadMirroredSourceTextFile("Epistemos/ExperimentalAgent/ExperimentalSurfaceView.swift")
        ...
        #expect(agentReadAloud.contains("enum EpistemosAgentReadAloud"))
        ...
        #expect(experimental.contains("refreshReadAloudAvailability()"))
        #expect(experimental.contains("window.__EPISTEMOS_READALOUD_REFRESH__"))
        #expect(experimental.contains("epistemosExperimentalKokoroInstallRequested"))
        #expect(experimental.contains("KokoroVoiceInstallPrompt()"))
        #expect(experimental.contains("Install voice"))
        #expect(experimental.contains("post(\"install\", \"\")"))
        #expect(experimental.contains("guard EpistemosSpeechSynthesizer.isTextToSpeechAvailable() else"))
        #expect(!juneBridge.contains("String(text.prefix(EpistemosSpeechSynthesizer.maxTextToSpeechInputCharacters))"))
        #expect(!experimental.contains("String(text.prefix(EpistemosSpeechSynthesizer.maxTextToSpeechInputCharacters))"))
    }

    @Test("Kokoro install completion refreshes active read-aloud surfaces without restart")
    func kokoroInstallCompletionRefreshesActiveReadAloudSurfacesWithoutRestart() throws {
        let readAloud = try loadMirroredSourceTextFile("Epistemos/Views/Shared/ReadAloudButton.swift")
        let settings = try loadMirroredSourceTextFile("Epistemos/Views/Settings/VoicePreferencesSection.swift")
        let onboarding = try loadMirroredSourceTextFile("Epistemos/Views/Onboarding/SetupAssistantView.swift")
        let juneNav = try loadMirroredSourceTextFile("Epistemos/JuneAgent/JuneAgentNavBar.swift")
        let experimental = try loadMirroredSourceTextFile("Epistemos/ExperimentalAgent/ExperimentalSurfaceView.swift")

        for source in [readAloud, settings, onboarding, juneNav, experimental] {
            #expect(source.contains(".onChange(of: kokoroDownloader.phase)")
                || source.contains(".onChange(of: downloader.phase)"))
            #expect(source.contains("case .installed")
                || source.contains("if case .installed = newPhase"))
        }

        #expect(readAloud.contains("isShowingKokoroInstallPrompt = false"))
        #expect(settings.contains("showingKokoroInstallPrompt = false"))
        #expect(onboarding.contains("kokoroStatus = status"))
        #expect(onboarding.contains("kokoroInstallMessage = installedKokoroMessage(for: status)"))

        for source in [juneNav, experimental] {
            #expect(source.contains("refreshReadAloudAvailability()"))
            #expect(source.contains("window.__EPISTEMOS_READALOUD_REFRESH__"))
            #expect(source.contains("__EPISTEMOS_TTS_AVAILABLE__"))
        }

        #expect(juneNav.contains("showingKokoroInstallPrompt = false"))
    ...
    func voiceSettingsSurfaceUsesNativeControlsAndThemeTint() throws {
        ...
        #expect(settings.contains("@State private var showingKokoroInstallPrompt = false"))
        #expect(settings.contains("@State private var kokoroDownloader = KokoroModelDownloadService.shared"))
        #expect(settings.contains("KokoroVoiceInstallPrompt()"))
        #expect(settings.contains("previewVoice(preview)"))
        ...
        #expect(settings.contains("showingKokoroInstallPrompt = false"))
        #expect(!settings.contains(".disabled(!EpistemosSpeechSynthesizer.isTextToSpeechAvailable())"))
    ...
    func kokoroNativeRuntimeLoaderFeedsPlaybackBridge() throws {
        ...
        #expect(project.components(separatedBy: "- package: KokoroPipeline\n        product: KokoroPipeline").count - 1 >= 2)
        #expect(project.contains("Epistemos-Experimental:\n    build:\n      targets:\n        Epistemos: all\n      config: Experimental"))
        #expect(project.contains("Epistemos-AppStore:\n    build:\n      targets:\n        Epistemos-AppStore: all"))
        #expect(appStoreScheme.contains("BlueprintName = \"Epistemos-AppStore\""))
        #expect(appStoreScheme.contains("BuildableName = \"Epistemos-AppStore.app\""))
        #expect(experimentalScheme.contains("buildConfiguration = \"Experimental\""))
        #expect(experimentalScheme.contains("BlueprintName = \"Epistemos\""))
        #expect(experimentalScheme.contains("BuildableName = \"Epistemos.app\""))
    ...
    func kokoroSettingsRowIsMASVisibleAndGateBacked() throws {
        ...
        #expect(appleSection.contains("KokoroVoiceInstallPrompt()"))
        ...
        #expect(section.contains("return \"Retry \\(selectedTier.title) Voice\""))
        #expect(section.contains("Retry downloading and installing the \\(selectedTier.title.lowercased()) Kokoro voice package"))
        #expect(section.contains("title: \"Cancel\""))
        #expect(section.contains("downloader.cancel()"))
        ...
        #expect(downloader.contains("case .highestQuality: return \"Highest quality\""))
        #expect(downloader.contains("var approximateSizeLabel: String"))
        #expect(downloader.contains("ByteCountFormatter.string(fromByteCount: approximateByteCount"))
        ...
        #expect(onboarding.contains("KokoroVoiceInstallPresentation.installHelp(for: selectedKokoroTier)"))
        #expect(!onboarding.contains("idleButtonTitle: \"Download Voice\""))
