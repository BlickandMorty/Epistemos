...
struct PipelineServiceTests {
    ...
    // Patch 1 (BLOCKER fix) — verifies that Pro mode with a cloud model selected
    // resolves to cloud agent provider strings that can run tools.
    //
...
struct UserFacingChatErrorKindTests {
    ...
    @Test("local act no-model failures classify as modelNotReady with actionable copy")
    func localActNoModelFailuresAreDiagnosable() {
        // The canonical errorDescriptions of the local no-model failure paths.
        let descriptions = [
...
/// stands in for local model errors so the content-based classifier is exercised
/// on their real strings.
