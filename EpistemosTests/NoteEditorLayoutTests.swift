...
struct NoteEditorLayoutTests {
    ...
    @Test("fragile note save wiring keeps editor flushes on the file-first export path")
    func fragileNoteSaveWiringKeepsEditorFlushesOnFileFirstExportPath() throws {
        let workspaceSource = try loadRepoTextFile("Epistemos/Views/Notes/NoteDetailWorkspaceView.swift")
        let proseSource = try loadRepoTextFile("Epistemos/Views/Notes/ProseEditorView.swift")
        let syncSource = try loadRepoTextFile("Epistemos/Sync/VaultSyncService.swift")

        #expect(workspaceSource.contains("private enum NoteEditorFlushResult"))
        #expect(workspaceSource.contains("private func flushCurrentEditor(reason: NoteSessionSaveReason = .explicitSave) async -> NoteEditorFlushResult"))
        #expect(syncSource.contains("page.applyInteractiveDerivedState(from: stagedBody)"))
        #expect(syncSource.contains("page.updatedAt = .now"))
        #expect(!workspaceSource.contains("guard stageBodyWrite(pageId: pageId, fullText: fullText) else"))
        #expect(workspaceSource.contains("let saved = await vaultSync.savePageBodyFileFirst(pageId: pageId, body: fullText)"))
        #expect(workspaceSource.contains("await MarkdownDocumentSurfaceSaveRegistry.shared.flush(pageId: page.id)"))
        #expect(workspaceSource.contains("await saveMarkdownDocumentSurfaceContent(page: page, markdown: markdown)"))
        #expect(workspaceSource.contains("let saved = await vaultSync.savePageBodyFileFirst(pageId: pageId, body: markdown)"))
        #expect(workspaceSource.contains("guard flushResult != .blocked else { return }"))
        #expect(workspaceSource.contains("await flushCurrentEditor(reason: .disappear)\n                        noteSession.close()"))
        #expect(!workspaceSource.contains("noteSession.close()\n                wordCountDebounce?.cancel()"))
        #expect(workspaceSource.contains("case .unchanged:\n            vaultSync.savePage(pageId: pageId)"))
        #expect(!workspaceSource.contains("_ = await AppBootstrap.shared?.vaultSync.savePageBodyFileFirst(pageId: pageId, body: fullText)"))
        #expect(!workspaceSource.contains("flushCurrentEditor()\n        vaultSync.savePage(pageId: pageId)"))
        ...
        #expect(proseSource.contains("Self.stageBodyWrite(pageId: pageId, currentBody: currentBody, reason: \"flushIfNeeded\")"))
        #expect(proseSource.contains("page.applyInteractiveDerivedState(from: currentBody)"))
        #expect(proseSource.contains("vaultSync.savePageBodyFileFirst(pageId: pageId, body: newValue)"))
        ...
        #expect(syncSource.contains("scheduleVersionCaptureIfNeeded(pageId: pageId, context: context)"))
        #expect(syncSource.contains("NoteFileStorage.requestFlush(pageId: pageId)"))
        #expect(syncSource.contains("latestAvailableBody(for: page, pageId: pageId)"))
    ...
    func outlineContentAndNavigationBelongToActiveSurface() throws {
        ...
        #expect(workspace.contains("let outlineMarkdown = activeOutlineMarkdown(for: page)"))
        #expect(workspace.contains("externalItems: activeOutlineExternalItems(for: page)"))
        #expect(workspace.contains("blockItems: activeOutlineBlockItems(for: page)"))
    ...
    func topSpacingStaysTightBelowToolbar() {
        ...
        #expect(NoteEditorPerformancePolicy.renderedTableOverlayRefreshDelay == .milliseconds(220))
        #expect(NoteEditorPerformancePolicy.transclusionOverlayRefreshDelay == .milliseconds(160))
    }
    ...
    func editorDefersPersistedBodyReadsUntilAsyncLoad() throws {
        ...
        #expect(source.contains("private func loadBodyIfNeeded(force: Bool, preferDisk: Bool = false) async"))
    ...
    func directMarkdownSourceSavesKeepPageBodyFrontMatterFree() throws {
        ...
        #expect(page.wordCount == cleanBody.split(separator: " ").count)
    ...
    func transclusionAndModeFlushPathsLogSaveFailures() throws {
        ...
        #expect(workspaceSource.contains("NoteDetailWorkspaceView: failed to stage flushed editor body"))
    ...
    func interactiveNoteFlushPathsAvoidSynchronousDurableWrites() throws {
        ...
        #expect(proseSource.contains("NoteFileStorage.stageBodyForImmediateRead(pageId: pageId, content: currentBody)"))
        #expect(proseSource.contains("guard await vaultSync.savePageBodyFileFirst(pageId: pageId, body: newValue)"))
        ...
        #expect(syncSource.contains("NoteFileStorage.stageBodyForImmediateRead(pageId: pageId, content: body)"))
        #expect(!workspaceSource.contains("guard stageBodyWrite(pageId: pageId, fullText: fullText) else"))
        #expect(workspaceSource.contains("vaultSync.savePageBodyFileFirst(pageId: pageId, body: fullText)"))
        ...
        #expect(inspectorSource.contains("formattedMarkdownView(editorText)"))
        #expect(!inspectorSource.contains("NoteFileStorage.stageBodyForImmediateRead(pageId: pageId, content: editorText)"))
        #expect(!inspectorSource.contains("vaultSync.savePageBodyFileFirst(pageId: pageId, body: editorText)"))
        #expect(!inspectorSource.contains("vaultSync.savePageBodyFileFirst(pageId: pageId, body: text)"))
        #expect(!inspectorSource.contains("flushEditorIfNeeded(pageId:"))
        #expect(!inspectorSource.contains("debouncedEditorSave(pageId:"))
    ...
    func vaultSavesPrepareLiveEditorStateBeforeExport() throws {
        ...
        #expect(source.contains("NoteFileStorage.requestFlush(pageId: pageId)"))
        #expect(source.contains("latestAvailableBody(for: page, pageId: pageId)"))
    ...
    func pageBodyReadRequestsStageEditorTextWithoutFullMetadataFlush() throws {
        ...
        #expect(source.contains("Self.stageBodyWrite(pageId: pageId, currentBody: currentBody, reason: \"stagePendingBodyForReadIfNeeded\")"))
        #expect(source.contains("NoteFileStorage.stageBodyForImmediateRead(pageId: pageId, content: currentBody)"))
