...
nonisolated struct HTMLWorkspaceSourceGuardTests {
    ...
    func previewUsesOfflineWKWebViewDefaults() throws {
        ...
        #expect(previewSource.contains(#"\(messagePrefix): \(status)"#))
        ...
        let dataJSONResetRange = try #require(previewSource.range(of: "lastRenderedDataJSON = nil"))
        let webViewResetRange = try #require(previewSource.range(of: "self.webView = nil"))
        let stopLoadingRange = try #require(previewSource.range(of: "webView.stopLoading()"))
        #expect(dataJSONResetRange.upperBound <= webViewResetRange.lowerBound)
    ...
    func existingWorkspaceSelectionUsesHTMLWorkspaceOpenHelper() throws {
        ...
        #expect(!graphSource.contains("openDocument(withContentsOf: url"),
                "Graph pages must not fall back to the generic AppKit document opener for HTML Workspace packages.")
    }
    ...
    func editorDebouncesPreviewAndCollapsesDiagnostics() throws {
        let editorViewSource = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceEditorView.swift")
        let editorSource = try loadHTMLWorkspaceEditorSourceBundle()
        let regenerationSource = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceEditorRegeneration.swift")
        ...
        let editorImplementationSource = editorSource + "\n" + regenerationSource
        ...
        #expect(editorSource.contains("var previewRouteName: String?"))
        ...
        #expect(editorImplementationSource.contains("HTMLWorkspaceRegeneratePreview.loadingPackage"))
        #expect(editorImplementationSource.contains("HTMLWorkspaceRegeneratePreview.candidatePackage"))
        #expect(editorImplementationSource.contains("restorePreviewAfterRegenerate()"))
        #expect(editorSource.contains("@State var pythonProbeNonce"))
        #expect(editorSource.contains(#"Button("Test Python Runtime""#))
        #expect(editorSource.contains("func testPythonRuntime()"))
        ...
        #expect(editorSource.contains("liveDOMSnapshot"))
        ...
        #expect(editorViewSource.split(separator: "\n", omittingEmptySubsequences: false).count < 1_000)
    }

    @Test("HTML Workspace chrome stays labeled, token-based, and large enough to use")
    func htmlWorkspaceChromeIsLabeledTokenBasedAndUsable() throws {
        let documentSource = try loadMirroredSourceTextFile("Epistemos/Engine/HTMLWorkspaceDocument.swift")
        let editorSource = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceEditorView.swift")
        let supportSource = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceEditorSupport.swift")
        let regenerateSource = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceRegenerateSurface.swift")

        #expect(documentSource.contains("width: 1320, height: 820"))
        #expect(documentSource.contains("window.minSize = NSSize(width: 980, height: 620)"))
        #expect(editorSource.contains("Label(\"Save\", systemImage: \"square.and.arrow.down\")"))
        #expect(editorSource.contains("Label(\"Regenerate\", systemImage: isRegenerating ? \"hourglass\" : \"wand.and.sparkles\")"))
        #expect(editorSource.contains("Label(\"Tools\", systemImage: \"slider.horizontal.3\")"))
        #expect(editorSource.contains("Label(\"Console\", systemImage: consoleExpanded ? \"terminal.fill\" : \"terminal\")"))
        #expect(editorSource.contains("Label(\"Inspector\", systemImage: \"sidebar.right\")"))
        #expect(!editorSource.contains("Label(\"Artifacts\", systemImage: \"shippingbox.and.arrow.backward\")"))
        #expect(supportSource.contains("agentTokenEstimateText(for source: String)"))
        #expect(supportSource.contains("agent tokens"))
        #expect(!supportSource.contains("lines / \\(source.count) chars"))
        #expect(regenerateSource.contains("private var chatPrompt: some View"))
        #expect(regenerateSource.contains("\"Generate preview\""))
        #expect(regenerateSource.contains("private var presetMenu: some View"))
        #expect(!regenerateSource.contains("private func presetSection"))
    }
    ...
    func htmlWorkspaceLiveDataFeedIsExplicitAndProvenanceVisible() throws {
        ...
        #expect(panelsSource.contains("HTMLWorkspaceDataFeedStatusStrip(package: package, compact: true)"))
    ...
    func htmlWorkspaceRoutesStructuredPatches() throws {
        let routerSource = try loadMirroredSourceTextFile("Epistemos/Engine/HTMLWorkspacePatchRouter.swift")
        let editorSource = try loadHTMLWorkspaceEditorSourceBundle()
        let regenerateSupportSource = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceRegenerateSupport.swift")
        ...
        #expect(regenerateSupportSource.contains("HTMLWorkspacePatchCommandParser.containsPatchBlock(in: response)"))
        #expect(regenerateSupportSource.contains("String(flattened.prefix(limit + 32))"))
        #expect(regenerateSupportSource.contains("String(bounded.prefix(limit - 3)) + \"...\""))
        #expect(previewSource.contains("String(instruction.prefix(240 + 32))"))
        #expect(previewSource.contains("String(trimmed.prefix(240 - 3)) + \"...\""))
        #expect(!regenerateSupportSource.contains("response.contains(\"```\\(HTMLWorkspacePatchCommandParser.fencedLanguage)\""))
        ...
        #expect(editorSource.contains("@State var selectedRouteName"))
        ...
        #expect(editorSource.contains("func addRoute()"))
        #expect(editorSource.contains("func removeRoute()"))
