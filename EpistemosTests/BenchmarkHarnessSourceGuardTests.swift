...
nonisolated struct BenchmarkHarnessSourceGuardTests {
    ...
    func r15PR2FixtureBaselineRunnerIsRealAndNotPlaceholderGated() throws {
        ...
        let recorderSource = try loadMirroredSourceTextFile("EpistemosTests/Benchmarks/BenchmarkRunRecorder.swift")
        #expect(!recorderSource.contains("options: [.atomic]"))
        #expect(recorderSource.contains("configuredResultsDirectory("))
        #expect(recorderSource.contains("epistemos-benchmark-results-"))
        #expect(!recorderSource.contains("appendingPathComponent(\"benchmarks\", isDirectory: true)"))
        #expect(!source.contains("benchmarks\", isDirectory: true)"))
