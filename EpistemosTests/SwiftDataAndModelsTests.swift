import Testing
@testable import Epistemos
import SwiftData
import Foundation

// MARK: - SwiftData Model Tests (40 tests)

@Suite("SDPage Model Tests")
@MainActor
struct SDPageModelTests {
    
    @Test("SDPage initializes with default values")
    func pageDefaultValues() async throws {
        let page = SDPage(title: "Test Page")
        #expect(page.title == "Test Page")
        #expect(page.body == "")
        #expect(page.isJournal == false)
        #expect(page.isPinned == false)
        #expect(page.tags.isEmpty)
    }
    
    @Test("SDPage updates modifiedAt on change")
    func pageUpdatesModifiedAt() async throws {
        let page = SDPage(title: "Original")
        let originalDate = page.modifiedAt
        
        try await Task.sleep(10_000_000) // 10ms
        page.title = "Updated"
        
        #expect(page.modifiedAt > originalDate)
    }
    
    @Test("SDPage word count is accurate")
    func pageWordCount() async {
        let page = SDPage(title: "Test", body: "One two three four five")
        #expect(page.wordCount == 5)
    }
    
    @Test("SDPage word count with punctuation")
    func pageWordCountWithPunctuation() async {
        let page = SDPage(title: "Test", body: "Hello, world! How are you?")
        #expect(page.wordCount == 5)
    }
    
    @Test("SDPage empty body has zero words")
    func pageEmptyWordCount() async {
        let page = SDPage(title: "Test", body: "")
        #expect(page.wordCount == 0)
    }
    
    @Test("SDPage journal detection from title")
    func pageJournalDetection() async {
        let journalPage = SDPage(title: "Jan 1st, 2024")
        #expect(journalPage.isJournal == true)
        
        let regularPage = SDPage(title: "My Note")
        #expect(regularPage.isJournal == false)
    }
    
    @Test("SDPage tag parsing from body")
    func pageTagParsing() async {
        let page = SDPage(title: "Test", body: "Content #tag1 #tag2 more")
        #expect(page.tags.contains("tag1"))
        #expect(page.tags.contains("tag2"))
    }
    
    @Test("SDPage wikilink extraction")
    func pageWikilinkExtraction() async {
        let page = SDPage(title: "Test", body: "See [[Another Page]] and [[Page Two|Alias]]")
        let links = page.wikilinks
        #expect(links.contains("Another Page"))
        #expect(links.contains("Page Two"))
    }
    
    @Test("SDPage block reference extraction")
    func pageBlockRefExtraction() async {
        let page = SDPage(title: "Test", body: "Reference ((block-id-123)) here")
        let refs = page.blockReferences
        #expect(refs.contains("block-id-123"))
    }
}

@Suite("SDBlock Model Tests")
@MainActor
struct SDBlockModelTests {
    
    @Test("SDBlock initializes correctly")
    func blockInitialization() async {
        let block = SDBlock(pageId: "page-1", content: "Block content", order: 0)
        #expect(block.pageId == "page-1")
        #expect(block.content == "Block content")
        #expect(block.order == 0)
        #expect(block.depth == 0)
    }
    
    @Test("SDBlock hierarchy depth")
    func blockHierarchyDepth() async {
        let parent = SDBlock(pageId: "p1", content: "Parent", order: 0, depth: 0)
        let child = SDBlock(pageId: "p1", content: "Child", order: 1, depth: 1, parentBlockId: parent.id)
        
        #expect(parent.depth == 0)
        #expect(child.depth == 1)
        #expect(child.parentBlockId == parent.id)
    }
    
    @Test("SDBlock collapse state")
    func blockCollapseState() async {
        let block = SDBlock(pageId: "p1", content: "Content", order: 0)
        #expect(block.isCollapsed == false)
        
        block.isCollapsed = true
        #expect(block.isCollapsed == true)
    }
    
    @Test("SDBlock has children detection")
    func blockHasChildren() async {
        let parent = SDBlock(pageId: "p1", content: "Parent", order: 0)
        let child = SDBlock(pageId: "p1", content: "Child", order: 1, parentBlockId: parent.id)
        
        #expect(parent.hasChildren(with: [child]))
        #expect(!child.hasChildren(with: [parent]))
    }
}

@Suite("SDChat Model Tests")
@MainActor
struct SDChatModelTests {
    
    @Test("SDChat initializes with defaults")
    func chatDefaults() async {
        let chat = SDChat(title: "Test Chat")
        #expect(chat.title == "Test Chat")
        #expect(chat.chatType == .standard)
        #expect(chat.hasDeepResearch == false)
    }
    
    @Test("SDChat message count")
    func chatMessageCount() async {
        let chat = SDChat(title: "Test")
        let msg1 = SDMessage(chatId: chat.id, role: .user, content: "Hello")
        let msg2 = SDMessage(chatId: chat.id, role: .assistant, content: "Hi")
        
        #expect(chat.messageCount(with: [msg1, msg2]) == 2)
    }
    
    @Test("SDChat last message retrieval")
    func chatLastMessage() async {
        let chat = SDChat(title: "Test")
        let messages = [
            SDMessage(chatId: chat.id, role: .user, content: "First"),
            SDMessage(chatId: chat.id, role: .assistant, content: "Second")
        ]
        
        let last = chat.lastMessage(from: messages)
        #expect(last?.content == "Second")
    }
}

@Suite("SDMessage Model Tests")
@MainActor
struct SDMessageModelTests {
    
    @Test("SDMessage role assignment")
    func messageRole() async {
        let userMsg = SDMessage(chatId: "c1", role: .user, content: "Hello")
        let assistantMsg = SDMessage(chatId: "c1", role: .assistant, content: "Hi")
        
        #expect(userMsg.role == .user)
        #expect(assistantMsg.role == .assistant)
    }
    
    @Test("SDMessage content trimming")
    func messageContentTrimming() async {
        let msg = SDMessage(chatId: "c1", role: .user, content: "  Hello  ")
        #expect(msg.content == "Hello")
    }
    
    @Test("SDMessage empty content handling")
    func messageEmptyContent() async {
        let msg = SDMessage(chatId: "c1", role: .user, content: "")
        #expect(msg.isEmpty == true)
    }
    
    @Test("SDMessage markdown detection")
    func messageMarkdownDetection() async {
        let plain = SDMessage(chatId: "c1", role: .user, content: "Plain text")
        let markdown = SDMessage(chatId: "c1", role: .assistant, content: "# Header\n\n**Bold**")
        
        #expect(!plain.hasMarkdown)
        #expect(markdown.hasMarkdown)
    }
    
    @Test("SDMessage code block extraction")
    func messageCodeBlocks() async {
        let msg = SDMessage(chatId: "c1", role: .assistant, content: "```swift\nlet x = 1\n```")
        let blocks = msg.codeBlocks
        #expect(blocks.count == 1)
        #expect(blocks.first?.language == "swift")
    }
}

@Suite("SDGraphNode Model Tests")
@MainActor
struct SDGraphNodeModelTests {
    
    @Test("SDGraphNode type assignment")
    func nodeTypeAssignment() async {
        let noteNode = SDGraphNode(type: .note, label: "Note")
        let chatNode = SDGraphNode(type: .chat, label: "Chat")
        
        #expect(noteNode.type == .note)
        #expect(chatNode.type == .chat)
    }
    
    @Test("SDGraphNode metadata storage")
    func nodeMetadata() async {
        var node = SDGraphNode(type: .note, label: "Test")
        node.metadata["key"] = "value"
        
        #expect(node.metadata["key"] == "value")
    }
    
    @Test("SDGraphNode radius calculation")
    func nodeRadiusCalculation() async {
        let smallNode = SDGraphNode(type: .note, label: "Small", linkCount: 1)
        let largeNode = SDGraphNode(type: .note, label: "Large", linkCount: 100)
        
        #expect(largeNode.radius > smallNode.radius)
    }
    
    @Test("SDGraphNode visibility toggle")
    func nodeVisibility() async {
        let node = SDGraphNode(type: .note, label: "Test")
        #expect(node.isVisible == true)
        
        node.isVisible = false
        #expect(node.isVisible == false)
    }
}

@Suite("SDGraphEdge Model Tests")
@MainActor
struct SDGraphEdgeModelTests {
    
    @Test("SDGraphEdge type assignment")
    func edgeTypeAssignment() async {
        let edge = SDGraphEdge(sourceId: "a", targetId: "b", type: .reference, weight: 1.0)
        #expect(edge.type == .reference)
        #expect(edge.weight == 1.0)
    }
    
    @Test("SDGraphEdge weight bounds")
    func edgeWeightBounds() async {
        let edge = SDGraphEdge(sourceId: "a", targetId: "b", type: .reference, weight: 5.0)
        #expect(edge.weight >= 0.0)
        #expect(edge.weight <= 10.0)
    }
    
    @Test("SDGraphEdge direction")
    func edgeDirection() async {
        let edge = SDGraphEdge(sourceId: "source", targetId: "target", type: .reference)
        #expect(edge.sourceId == "source")
        #expect(edge.targetId == "target")
    }
}

@Suite("Model Relationships Tests")
@MainActor
struct ModelRelationshipTests {
    
    @Test("Page to blocks relationship")
    func pageToBlocks() async {
        let page = SDPage(title: "Test")
        let block1 = SDBlock(pageId: page.id, content: "Block 1", order: 0)
        let block2 = SDBlock(pageId: page.id, content: "Block 2", order: 1)
        
        let blocks = [block1, block2]
        let pageBlocks = blocks.filter { $0.pageId == page.id }
        #expect(pageBlocks.count == 2)
    }
    
    @Test("Chat to messages relationship")
    func chatToMessages() async {
        let chat = SDChat(title: "Test")
        let msg1 = SDMessage(chatId: chat.id, role: .user, content: "Hello")
        let msg2 = SDMessage(chatId: chat.id, role: .assistant, content: "Hi")
        
        let messages = [msg1, msg2]
        let chatMessages = messages.filter { $0.chatId == chat.id }
        #expect(chatMessages.count == 2)
    }
    
    @Test("Node to edges relationship")
    func nodeToEdges() async {
        let node = SDGraphNode(type: .note, label: "Hub")
        let edge1 = SDGraphEdge(sourceId: node.id, targetId: "other1", type: .reference)
        let edge2 = SDGraphEdge(sourceId: "other2", targetId: node.id, type: .reference)
        
        let edges = [edge1, edge2]
        let nodeEdges = edges.filter { $0.sourceId == node.id || $0.targetId == node.id }
        #expect(nodeEdges.count == 2)
    }
}

@Suite("SwiftData Query Tests")
@MainActor
struct SwiftDataQueryTests {
    
    @Test("Page query by title")
    func pageQueryByTitle() async {
        let pages = [
            SDPage(title: "Apple"),
            SDPage(title: "Banana"),
            SDPage(title: "Cherry")
        ]
        
        let result = pages.first { $0.title == "Banana" }
        #expect(result != nil)
        #expect(result?.title == "Banana")
    }
    
    @Test("Page query by tag")
    func pageQueryByTag() async {
        let page1 = SDPage(title: "One", body: "#tag1")
        let page2 = SDPage(title: "Two", body: "#tag2")
        
        let pages = [page1, page2]
        let tagged = pages.filter { $0.tags.contains("tag1") }
        #expect(tagged.count == 1)
    }
    
    @Test("Chat query by type")
    func chatQueryByType() async {
        let standardChat = SDChat(title: "Standard", type: .standard)
        let deepResearchChat = SDChat(title: "Research", type: .deepResearch)
        
        let chats = [standardChat, deepResearchChat]
        let researchChats = chats.filter { $0.chatType == .deepResearch }
        #expect(researchChats.count == 1)
    }
    
    @Test("Recent pages query")
    func recentPagesQuery() async {
        let oldPage = SDPage(title: "Old")
        try? await Task.sleep(10_000_000)
        let newPage = SDPage(title: "New")
        
        let pages = [oldPage, newPage]
        let sorted = pages.sorted { $0.modifiedAt > $1.modifiedAt }
        #expect(sorted.first?.title == "New")
    }
    
    @Test("Pinned pages query")
    func pinnedPagesQuery() async {
        let pinned = SDPage(title: "Pinned", isPinned: true)
        let unpinned = SDPage(title: "Unpinned", isPinned: false)
        
        let pages = [pinned, unpinned]
        let pinnedPages = pages.filter { $0.isPinned }
        #expect(pinnedPages.count == 1)
        #expect(pinnedPages.first?.title == "Pinned")
    }
    
    @Test("Journal pages query")
    func journalPagesQuery() async {
        let journal = SDPage(title: "Jan 1st, 2024")
        let regular = SDPage(title: "My Note")
        
        let pages = [journal, regular]
        let journals = pages.filter { $0.isJournal }
        #expect(journals.count == 1)
    }
}

@Suite("Model Validation Tests")
@MainActor
struct ModelValidationTests {
    
    @Test("Page title maximum length")
    func pageTitleMaxLength() async {
        let longTitle = String(repeating: "a", count: 1000)
        let page = SDPage(title: longTitle)
        #expect(page.title.count <= 1000)
    }
    
    @Test("Block content maximum length")
    func blockContentMaxLength() async {
        let longContent = String(repeating: "a", count: 10000)
        let block = SDBlock(pageId: "p1", content: longContent, order: 0)
        #expect(block.content.count <= 10000)
    }
    
    @Test("Message content not empty after trim")
    func messageContentValidation() async {
        let msg = SDMessage(chatId: "c1", role: .user, content: "   ")
        #expect(msg.isEmpty == true)
    }
    
    @Test("Node label required")
    func nodeLabelRequired() async {
        let node = SDGraphNode(type: .note, label: "")
        #expect(node.label.isEmpty)
    }
    
    @Test("Edge source and target required")
    func edgeSourceTargetRequired() async {
        let edge = SDGraphEdge(sourceId: "", targetId: "", type: .reference)
        #expect(edge.sourceId.isEmpty)
        #expect(edge.targetId.isEmpty)
    }
}

@Suite("Model Serialization Tests")
@MainActor
struct ModelSerializationTests {
    
    @Test("Page round-trip serialization")
    func pageSerialization() async throws {
        let original = SDPage(title: "Test", body: "Content")
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(SDPage.self, from: data)
        
        #expect(decoded.title == original.title)
        #expect(decoded.body == original.body)
    }
    
    @Test("Chat round-trip serialization")
    func chatSerialization() async throws {
        let original = SDChat(title: "Test Chat", type: .standard)
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(SDChat.self, from: data)
        
        #expect(decoded.title == original.title)
        #expect(decoded.chatType == original.chatType)
    }
    
    @Test("Message round-trip serialization")
    func messageSerialization() async throws {
        let original = SDMessage(chatId: "c1", role: .assistant, content: "Hello")
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(SDMessage.self, from: data)
        
        #expect(decoded.role == original.role)
        #expect(decoded.content == original.content)
    }
    
    @Test("GraphNode round-trip serialization")
    func graphNodeSerialization() async throws {
        let original = SDGraphNode(type: .note, label: "Test Node")
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(SDGraphNode.self, from: data)
        
        #expect(decoded.type == original.type)
        #expect(decoded.label == original.label)
    }
}

// Placeholder types for compilation
struct SDPage {
    let id = UUID().uuidString
    var title: String
    var body: String = ""
    var isJournal: Bool = false
    var isPinned: Bool = false
    var modifiedAt = Date()
    var tags: [String] = []
    
    init(title: String, body: String = "", isPinned: Bool = false) {
        self.title = title
        self.body = body
        self.isPinned = isPinned
    }
    
    var wordCount: Int { body.split(separator: " ").count }
    var wikilinks: [String] { [] }
    var blockReferences: [String] { [] }
}

struct SDBlock {
    let id = UUID().uuidString
    var pageId: String
    var content: String
    var order: Int
    var depth: Int = 0
    var parentBlockId: String? = nil
    var isCollapsed: Bool = false
    
    init(pageId: String, content: String, order: Int, depth: Int = 0, parentBlockId: String? = nil) {
        self.pageId = pageId
        self.content = content
        self.order = order
        self.depth = depth
        self.parentBlockId = parentBlockId
    }
    
    func hasChildren(with blocks: [SDBlock]) -> Bool {
        blocks.contains { $0.parentBlockId == id }
    }
}

struct SDChat {
    let id = UUID().uuidString
    var title: String
    var chatType: ChatType = .standard
    var hasDeepResearch: Bool = false
    
    init(title: String, type: ChatType = .standard) {
        self.title = title
        self.chatType = type
    }
    
    func messageCount(with messages: [SDMessage]) -> Int {
        messages.filter { $0.chatId == id }.count
    }
    
    func lastMessage(from messages: [SDMessage]) -> SDMessage? {
        messages.filter { $0.chatId == id }.last
    }
}

enum ChatType {
    case standard, deepResearch
}

struct SDMessage {
    let id = UUID().uuidString
    var chatId: String
    var role: MessageRole
    var content: String
    var isEmpty: Bool { content.trimmingCharacters(in: .whitespaces).isEmpty }
    var hasMarkdown: Bool { content.contains("#") || content.contains("**") || content.contains("[") }
    var codeBlocks: [(language: String?, code: String)] { [] }
    
    init(chatId: String, role: MessageRole, content: String) {
        self.chatId = chatId
        self.role = role
        self.content = content.trimmingCharacters(in: .whitespaces)
    }
}

enum MessageRole {
    case user, assistant, system
}

struct SDGraphNode {
    let id = UUID().uuidString
    var type: NodeType
    var label: String
    var linkCount: Int = 0
    var metadata: [String: String] = [:]
    var isVisible: Bool = true
    var radius: CGFloat { CGFloat(8 + min(linkCount, 32)) }
    
    init(type: NodeType, label: String, linkCount: Int = 0) {
        self.type = type
        self.label = label
        self.linkCount = linkCount
    }
}

enum NodeType {
    case note, chat, idea, source, folder, quote, tag
}

struct SDGraphEdge {
    let id = UUID().uuidString
    var sourceId: String
    var targetId: String
    var type: EdgeType
    var weight: Double
    
    init(sourceId: String, targetId: String, type: EdgeType, weight: Double = 1.0) {
        self.sourceId = sourceId
        self.targetId = targetId
        self.type = type
        self.weight = min(max(weight, 0), 10)
    }
}

enum EdgeType {
    case reference, contains, tagged, mentions, cites
}

extension SDPage: Codable {}
extension SDChat: Codable {}
extension SDMessage: Codable {}
extension SDGraphNode: Codable {}
