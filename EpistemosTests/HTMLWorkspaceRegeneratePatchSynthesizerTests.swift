...
nonisolated struct HTMLWorkspaceRegeneratePatchSynthesizerTests {
    ...
    func regenerateLoadingPreviewIsTransientEscapedAndNotPersisted() throws {
        ...
        let loadingData = try #require(loading.dataJSON.data(using: .utf8))
        let loadingJSON = try #require(JSONSerialization.jsonObject(with: loadingData) as? [String: String])
        ...
        #expect(loadingJSON["status"] == "regenerating")
        #expect(loadingJSON["request"] == "Explain <script>alert(1)</script> as an animated dashboard")
    ...
    func regenerateLoadingPreviewCarriesSelectedPreviewTarget() throws {
        ...
        #expect(loadingJSON["selected_surface_context"] == "selected_surface.selector: section.hero\nselected_surface.text: Hello <b>world</b>")
    }
    ...
    func regenerateSheetExposesTargetWorkspaceAndHashBeforeReplace() throws {
        ...
        let support = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceRegenerateSupport.swift")
        let editorView = try loadHTMLWorkspaceEditorSourceBundle()
        let editorRegeneration = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceEditorRegeneration.swift")
        ...
        #expect(sheet.contains("Label(isRegenerating ? \"Generating\" : \"Generate preview\", systemImage: \"wand.and.sparkles\")"))
        #expect(sheet.contains("TextField(\"Tell Epistemos what to change, add, simplify, or rebuild.\", text: $instruction, axis: .vertical)"))
        ...
        #expect(support.contains("Load read-only workspace context for \\(query)"))
        #expect(support.contains("nonisolated struct HTMLWorkspaceRegenerateContextShortcut"))
        ...
        #expect(sheet.contains(".buttonStyle(.plain)"))
        #expect(sheet.contains("ZStack(alignment: .topLeading)"))
        ...
        #expect(support.contains("var contextDescriptor: String"))
        #expect(support.contains("var rankDescriptor: String"))
        #expect(support.contains("var provenanceDescriptor: String"))
        #expect(support.contains("nonisolated struct HTMLWorkspaceRegenerateContextItem"))
        #expect(support.contains("var contextID: String"))
        #expect(support.contains("context_id: \\(safeContextID)"))
        #expect(support.contains("readonly: true"))
        #expect(support.contains("var promptPayload: String"))
        #expect(support.contains("var promptRecordLines: [String]"))
        #expect(!support.contains("drop_action: regenerate_preview_context"))
        #expect(!support.contains("previewDropUTType"))
        #expect(!support.contains("dragItemProvider"))
        #expect(!support.contains("verifiedPreviewDropPayload"))
        #expect(sheet.contains("HTMLWorkspaceRegeneratePreset.Family.allCases"))
        #expect(sheet.contains("FlowLayout(spacing: 6)"))
        #expect(sheet.contains("ForEach(contextItems.prefix(5))"))
        #expect(sheet.contains("private func contextItemButton(_ item: HTMLWorkspaceRegenerateContextItem) -> some View"))
        #expect(sheet.contains("onFocusContextItem(item)"))
        #expect(sheet.contains(".help(item.promptPayload)"))
        ...
        #expect(editor.contains("var regenerateContextItems: [HTMLWorkspaceRegenerateContextItem]"))
        #expect(editor.contains("var canApplyPendingRegeneratePreview: Bool"))
        ...
        #expect(editor.contains("var restoreSnapshotName: String?"))
        #expect(editor.contains("guard package.snapshots[name] != nil else { return nil }"))
        #expect(editor.contains("var restorePreviousSurfaceHelpText: String"))
        ...
        #expect(!editor.contains("HTMLWorkspaceRegenerateContextItem.previewDropUTTypes"))
        ...
        #expect(sheet.contains("Label(\"Revert\", systemImage: \"clock.arrow.circlepath\")"))
        #expect(editor.contains(".help(restorePreviousSurfaceHelpText)"))
        #expect(editor.contains("@State var regenerateContextQuery = \"\""))
        #expect(editor.contains("@State var regenerateContextTask: Task<Void, Never>?"))
        #expect(editor.contains("@State var regenerateContextRefreshNonce = 0"))
        #expect(editor.contains("@State var pendingRegeneratePatchResponse: String?"))
        #expect(editor.contains("@State var pendingRegenerateExpectedContentHash: String?"))
        #expect(editor.contains("var regenerateContextStatusLine: String?"))
        #expect(editor.contains("func refreshRegenerateVaultContext()"))
        #expect(editor.contains("func expirePendingRegeneratePreviewIfNeeded(for newPackage: HTMLWorkspacePackage)"))
        #expect(editor.contains("func stampPackageContentRevision()"))
        ...
        #expect(editor.contains("func clearRegenerateVaultContext()"))
        #expect(editor.contains("package.dataJSON = HTMLWorkspaceDataFeedStatus.clearedDataJSON(from: package.dataJSON)"))
        #expect(editor.contains("func focusRegenerateContextItem(_ item: HTMLWorkspaceRegenerateContextItem)"))
        ...
        #expect(editor.contains("boundedRegenerateContext(item.promptPayload)"))
        ...
        #expect(editor.components(separatedBy: "regenerateErrorText = nil\n            statusText = \"Pick a current Epistemos workspace context item\"").count >= 2)
        #expect(!editor.contains("func handlePreviewContextDrop(_ providers: [NSItemProvider]) -> Bool"))
        #expect(!editor.contains("statusText = \"Finish the current regenerate before dropping context\""))
        #expect(!editor.contains("statusText = \"Finish loading context before dropping context\""))
        #expect(!editor.contains("previewContextDropProvider"))
        ...
        #expect(!editor.contains("func applyDroppedPreviewContext(_ payload: String)"))
        #expect(!editor.contains("HTMLWorkspaceRegenerateContextItem.verifiedPreviewDropPayload"))
        ...
        #expect(!editor.contains("func applyPreviewContextItem(_ item: HTMLWorkspaceRegenerateContextItem)"))
        #expect(!editor.contains("Use this picked read-only workspace context item as a primary source"))
        #expect(editor.contains("func isCurrentRegenerateContextItem(_ item: HTMLWorkspaceRegenerateContextItem) -> Bool"))
        #expect(editor.contains("regenerateContextItems.contains { $0.contextID == item.contextID }"))
        #expect(editor.contains("func refreshPreviewContextShortcut(_ shortcut: HTMLWorkspaceRegenerateContextShortcut)"))
        #expect(editor.contains("regenerateContextQuery = shortcut.query"))
        #expect(editor.contains("refreshRegenerateVaultContext()"))
        #expect(editor.contains("func startRegenerateWithContextDirective(_ directive: String, status: String)"))
        #expect(editor.contains("func startRegenerateWithContextDirective(_ directive: String, status: String) {\n        regenerateErrorText = nil"))
        #expect(editor.contains("regenerateContextStatusText = status"))
        #expect(editor.contains("func selectedPreviewTargetDirective() -> String"))
        ...
        #expect(editor.contains("func selectedRegenerateSurfaceContext() -> String?"))
        ...
        #expect(editor.contains("func beginRegenerateSurfaceAttachingContextIfNeeded(instructionOverride: String?)"))
        #expect(editor.contains("func shouldAttachRegenerateContextBeforeStreaming("))
        ...
        #expect(editor.contains("func runRegeneratePreset(_ preset: HTMLWorkspaceRegeneratePreset)"))
        #expect(editor.contains("if preset.family == .vaultData"))
        #expect(editor.contains("func runVaultDataRegeneratePreset(_ preset: HTMLWorkspaceRegeneratePreset)"))
        #expect(editor.contains("func attachRegenerateVaultContext("))
        ...
        #expect(editor.contains("func applyPendingRegeneratePreview()"))
        ...
        #expect(editor.contains("func copyRegeneratePrompt()"))
        ...
        #expect(editor.contains("func previewRegenerateStreamText()"))
        #expect(editor.contains("func applyRegenerateStreamText()"))
    ...
    func regenerateContextItemsDecodeFeedResultsAsDraggableReadOnlySources() {
        ...
        #expect(items.first?.promptPayload.contains("Workspace context: Alpha Note [note-a]") == true)
        #expect(items.first?.promptPayload.contains("context_id: context_kind:vault_record|source:Vault search result|page_id:note-a|provenance:VaultSyncService.searchFullAsync") == true)
        #expect(items.first?.promptPayload.contains("drop_action: regenerate_preview_context") == false)
        #expect(items.first?.promptPayload.contains("readonly: true") == true)
        #expect(items.first?.promptPayload.contains("page_id: note-a") == true)
        #expect(items.first?.promptPayload.contains("title: Alpha Note") == true)
        #expect(items.first?.promptPayload.contains("context_kind: vault_record") == true)
        #expect(items.first?.promptPayload.contains("source_label: Vault search result") == true)
        #expect(items.first?.promptPayload.contains("rank: 0.9000") == true)
        #expect(items.first?.promptPayload.contains("Source: Vault search result / vault_record") == true)
        #expect(items.first?.promptPayload.contains("Provenance: VaultSyncService.searchFullAsync") == true)
        #expect(items.first?.promptPayload.contains("snippet: alpha snippet") == true)
    ...
    @Test("regenerate context items bound oversized UI and prompt payload text")
    func regenerateContextItemsBoundOversizedUIAndPromptPayloadText() throws {
        ...
        #expect(item.promptPayload.count < 1_500)
        #expect(item.promptPayload.contains("readonly: true"))
        #expect(!item.promptPayload.contains("drop_action: regenerate_preview_context"))
    }
    ...
    func contextAndFeedMutationsInvalidatePendingRegeneratePreviews() throws {
        ...
        let editorRegeneration = try loadMirroredSourceTextFile("Epistemos/Views/HTMLWorkspace/HTMLWorkspaceEditorRegeneration.swift")
        let editor = editorView + "\n" + editorRegeneration
        let refreshStart = try #require(editor.range(of: "func refreshRegenerateVaultContext()"))
        let attachContextStart = try #require(editor.range(of: "func attachRegenerateVaultContext"))
        let clearStart = try #require(editor.range(of: "func clearRegenerateVaultContext()"))
        let focusStart = try #require(editor.range(of: "func focusRegenerateContextItem"))
        let presetStart = try #require(editor.range(of: "func runVaultDataRegeneratePreset"))
        let attachStart = try #require(editor.range(of: "func attachStandaloneRegenerateContext"))
        ...
        let restoreStart = try #require(editor.range(of: "func restorePreviousSurface()"))
        let bridgeStart = try #require(editor.range(of: "func testAppBridge()"))
    ...
    func copyableRegeneratePromptIncludesSystemPromptAndTargetHash() {
        ...
        #expect(prompt.contains("compact in-surface context picker/filter"))
        #expect(prompt.contains("picking a record should visibly add/pin that real record into the workspace"))
        #expect(prompt.contains("Do not create drag/drop-only context controls"))
        #expect(prompt.contains("unavailable context must show an honest unavailable status"))
    ...
    func regenerateApplicationAppliesVisiblePackageReplacement() throws {
        ...
        let snapshotName = try #require(result.package.manifest.generationProvenance?.reversibleSnapshotName)
        #expect(snapshotName == HTMLWorkspacePatchApplier.reversibleSnapshotName(for: package))
        #expect(result.package.snapshots[snapshotName] != nil)

        let restored = try HTMLWorkspacePatchApplier.apply(.restoreSnapshot(name: snapshotName), to: result.package)
        #expect(restored.indexHTML == package.indexHTML)
        #expect(restored.styleCSS == package.styleCSS)
        #expect(restored.scriptJS == package.scriptJS)
        #expect(restored.dataJSON == package.dataJSON)
