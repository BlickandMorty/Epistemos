...
struct ReleasePackagingHardeningTests {
    ...
    func runtimeAssetBundlerShipsDefaultSkillsFromAgentCanon() throws {
        ...
        #expect(bundler.contains("DEFAULT_SKILLS_SOURCE_DIR=\"$SRCROOT/.agents/skills\""))
        #expect(bundler.contains("DEFAULT_SKILLS_DIR=\"$RESOURCES_DIR/DefaultSkills\""))
    ...
    func appStoreSettingsHideProOnlySurfaces() throws {
        ...
        #expect(settings.contains("static var visibleSections: [SettingsSection]"))
        #expect(!settings.contains("                .skills,"))
        #expect(settings.contains("safeDetailSelection(for section: SettingsSection?)"))
        #expect(settings.contains("return visibleSections.contains(section) ? section : .general"))
        #expect(settings.contains("case .skills: GeneralDetailView()"))
        #expect(!settings.contains("ExtensionsDetailView"))
	...
	func appStoreBootstrapSkipsProOnlyRuntimeStartup() throws {
        ...
        #expect(reopen.contains("AppStoreFirstWindowPresenter.shared.ensureHomeWindow()"))
    ...
    func debugAndTestsKeepLocalSigningWhileReleaseConfigsKeepPaidTeamSupport() throws {
        let project = try loadProductionHardeningRepoTextFile("Epistemos.xcodeproj/project.pbxproj")
        let projectSpec = try loadProductionHardeningRepoTextFile("project.yml")
        ...
        #expect(projectSpec.contains("DEVELOPMENT_TEAM: 3BNL2669SL"))
...
struct AuditHardeningRegressionTests {
    ...
    func vaultDestructiveUIFlowsUseAsyncSnapshotSwitching() throws {
        ...
        #expect(vaultSync.contains("func stopWatchingAsync(preserveData: Bool = false, skipRecoverySnapshot: Bool = false) async -> Bool"))
        ...
        #expect(vaultSync.contains("let didClear = await vaultSync.stopWatchingAsync("))
        #expect(vaultSync.contains("skipRecoverySnapshot: true"))
    ...
    @MainActor func omegaPlannerSchemasStayAligned() throws {
        ...
        #expect(schemas.count == OmegaToolRegistry.surfacedTools(distribution: .proResearch).count)
    ...
    func pageAndJournalCreationFailHonestlyOnPersistenceErrors() throws {
        ...
        #expect(vaultSync.contains("page.lastSyncedBodyHash = nil"))
        #expect(vaultSync.contains("page.needsVaultSync = true"))
        #expect(vaultSync.contains("new page export tracking"))
        #expect(vaultSync.contains("page.needsVaultSync = false"))
    ...
    func vaultIndexImportCleansUpPendingArtifactsOnSaveFailure() throws {
        ...
        #expect(vaultIndexActor.contains("validatedSearchMutationContext("))
        #expect(vaultIndexActor.contains("let receipt = try searchContext.service.delete("))
        #expect(vaultIndexActor.contains("pageId: pageID"))
        #expect(vaultIndexActor.contains("notifyObservers: searchContext.notifyObservers"))
        #expect(vaultIndexActor.contains("recordSearchIndexMutation("))
        #expect(!vaultIndexActor.contains("try? searchService?.deleteBlocksForPage"))
        #expect(vaultIndexActor.contains("discardPendingImportedPages("))
        #expect(vaultIndexActor.contains("failedSaveLabel: label"))
        #expect(vaultIndexActor.contains("searchMutationBatchID: searchMutationBatchID"))
    }
