...
struct Stash16And19EditorCloseoutTests {
    ...
    @Test("current code editor keeps SourceEditor path instead of minimal test shell")
    func currentCodeEditorKeepsSourceEditorPathInsteadOfMinimalTestShell() throws {
    ...
    func invisibleTextDonorIsRepresentedByXcodeColorsWithoutOldRewrite() throws {
        ...
        #expect(codeEditor.contains("static func flatLight("))
        #expect(codeEditor.contains("static func flatDark("))
        #expect(codeEditor.contains("? .flatDark(accent: accent, background: background, text: text, subtle: subtle)"))
        #expect(codeEditor.contains(": .flatLight(accent: accent, background: background, text: text, subtle: subtle)"))
