...
#[cfg(not(target_arch = "wasm32"))]
fn read_pdftotext_bbox_layout_lines(_path: &Path) -> Option<(f64, Vec<BBoxLayoutLine>)> {
    // Epistemos MAS patch: upstream EdgeParse optionally shells out to Poppler's
    // `pdftotext` for a few layout-specialized render heuristics. The app parser path
    // must stay pure in-process, so this optional helper is disabled here.
    None
}

#[cfg(all(test, not(target_arch = "wasm32")))]
fn parse_pdftotext_bbox_layout_xml(xml: &str) -> Option<(f64, Vec<BBoxLayoutLine>)> {
...
#[cfg(not(target_arch = "wasm32"))]
fn read_pdftotext_layout_lines(_path: &Path) -> Option<Vec<String>> {
    // Epistemos MAS patch: keep Markdown rendering in-process; never call Poppler.
    None
}
