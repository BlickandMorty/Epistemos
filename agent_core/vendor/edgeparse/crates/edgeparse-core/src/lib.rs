...
#[cfg(all(feature = "external-ocr", not(target_arch = "wasm32")))]
...
pub fn convert(
    ...
    for (&page_num, &page_id) in &pages_map {
        ...
        #[cfg(not(all(feature = "external-ocr", not(target_arch = "wasm32"))))]
        let recovered_text_chunks = Vec::new();
        #[cfg(not(all(feature = "external-ocr", not(target_arch = "wasm32"))))]
    ...
    let phase_start = Instant::now();
    if cfg!(feature = "external-ocr") && config.raster_table_ocr_enabled() {
        #[cfg(all(feature = "external-ocr", not(target_arch = "wasm32")))]
        {
