...
fn augment_preserves_per_index_labels() {
    ...
    for (i, (a, b)) in out.iter().zip(v.iter()).enumerate() {
        assert_eq!(
            a.is_hallucination, b.is_hallucination,
...
fn auc_preservation_under_perfect_separation() {
    ...
    let mut v: Vec<LabeledScore> = (0..20).map(|i| obs((i as f32) * 0.02, false)).collect();
    v.extend((0..20).map(|i| obs(0.6 + (i as f32) * 0.02, true)));

    let raw = auc_roc(&v).unwrap();
    let aug = auc_on_augmented(&v).unwrap();
    assert!(approx(raw, aug, 1e-5), "raw_auc={} aug_auc={}", raw, aug);
...
fn auc_preservation_under_perfect_inversion() {
    ...
    let mut v: Vec<LabeledScore> = (0..15).map(|i| obs((i as f32) * 0.03, true)).collect();
...
fn augmented_potential_values_are_monotone_in_raw() {
    ...
    let mut v: Vec<LabeledScore> = (0..30)
        .map(|i| obs(0.01 + (i as f32) * 0.03, i % 2 == 0))
        .collect();
    // Already sorted by score (construction).
    let out = augment(&v).unwrap();
    for w in out.windows(2) {
        assert!(
            w[0].potential.value() <= w[1].potential.value(),
            "non-monotone: {} > {}",
            w[0].potential.value(),
            w[1].potential.value()
        );
