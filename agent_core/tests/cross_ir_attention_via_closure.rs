...
use agent_core::research::eml::{
    closure_attention_score, closure_categorical_softmax_pinned, closure_categorical_softmax_slot,
    evaluate_closure, EmlClosure,
};
...
fn attention_weights_from_softmax_sum_to_one() {
    ...
    let p0 = eval(closure_categorical_softmax_slot(0, &[0, 1]), scores.clone());
    let p1 = eval(closure_categorical_softmax_slot(1, &[0, 1]), scores.clone());
    let pp = eval(closure_categorical_softmax_pinned(&[0, 1]), scores.clone());
...
fn attention_head_end_to_end_3_keys_2_values_2_dims() {
    ...
    let values = [vec![1.0_f64, 0.0], vec![0.0, 1.0], vec![0.5, 0.5]];
    let inv_sqrt_d = 1.0 / 2.0_f64.sqrt();
    ...
    assert!(
        ...
        "Σ y = {} ≠ 1 — softmax weights don't sum correctly",
        sum_components
    );
    ...
    assert!(
        y_a > y_b,
        "y_a = {}, y_b = {}: aligned key should win",
        y_a,
        y_b
    );
}
...
fn attention_weights_uniform_at_zero_scores() {
    ...
    let p0 = eval(closure_categorical_softmax_slot(0, &[0, 1]), vec![0.0, 0.0]);
    let p1 = eval(closure_categorical_softmax_slot(1, &[0, 1]), vec![0.0, 0.0]);
    let pp = eval(closure_categorical_softmax_pinned(&[0, 1]), vec![0.0, 0.0]);
...
fn attention_temperature_high_inv_sqrt_d_sharpens() {
    ...
    // inv_sqrt_d = 0.1 (soft).
    let q = vec![1.0_f64, 0.0];
    let keys = [vec![2.0_f64, 0.0], vec![0.0, 2.0]];

    // Sharp: scale = 2.0.
    let slot_sharp = vec![
        q[0], q[1], keys[0][0], keys[0][1], keys[1][0], keys[1][1], 2.0,
    ];
    let s0_sharp = eval(
        closure_attention_score(&[0, 1], &[2, 3], 6),
        slot_sharp.clone(),
    );
    ...
    // Soft: scale = 0.1.
    let slot_soft = vec![
        q[0], q[1], keys[0][0], keys[0][1], keys[1][0], keys[1][1], 0.1,
    ];
    let s0_soft = eval(
        closure_attention_score(&[0, 1], &[2, 3], 6),
        slot_soft.clone(),
