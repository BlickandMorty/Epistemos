...
fn lse_via_eml_is_ge_max() {
    ...
    let pairs = [(1.0_f64, 0.0), (-1.0, 2.0), (3.0, -3.0)];
    for &(a, b) in &pairs {
...
fn scaled_lse_converges_to_tropical_max_at_large_beta() {
    ...
    let pairs: Vec<(f64, f64)> = vec![(1.0_f64, 0.0), (0.5, 0.5), (-1.0, 2.0), (3.0, -3.0)];
    let betas = [1.0_f64, 10.0, 100.0];
    for &(a, b) in &pairs {
        // Tropical-IR computes max(a, b) exactly.
        let tropical_max = tropical_evaluate(
            &TropicalExpr::max(vec![TropicalExpr::constant(a), TropicalExpr::constant(b)]),
            &[],
        )
        .unwrap();
        ...
        if (a - b).abs() > 1e-3 {
            assert!(
                residual_100 < 0.1,
                "β=100 residual too large for distinct args (a,b)=({},{}): {}",
                a,
                b,
                residual_100
            );
