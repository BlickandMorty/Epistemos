...
pub const F_VAULT_RECALL_50_FIXTURE: &[FVaultRecallRow] = &[
    ...
    FVaultRecallRow {
        // Iter-429: mixed Greek + Hebrew interaction. Prior rows pin
        // both scripts independently; this row requires a Greek word
        // and an RTL Hebrew word in one <=3-token AND query.
        query: "Mamba μνήμη זיכרון",
        expected_paths: &["notes/mamba_greek_hebrew.md"],
        forbidden_paths: &["notes/mamba_english_only.md"],
        category: FVaultRecallCategory::Unicode,
        top_n: 5,
        note: "Fifty-fifth Unicode row (iter-429): mixed-script \
               Greek+Hebrew query — \"Mamba μνήμη זיכרון\". \
               Extends the post-floor Unicode interaction matrix \
               to a European non-Latin alphabetic word plus an RTL \
    ...
    FVaultRecallRow {
        // Iter-431: mixed Cyrillic + Hebrew interaction. This pairs a
        // Cyrillic alphabetic token with an RTL Hebrew word in one
        // <=3-token AND query.
        query: "Mamba кэш זיכרון",
        expected_paths: &["notes/mamba_cyrillic_hebrew.md"],
        forbidden_paths: &["notes/mamba_english_only.md"],
        category: FVaultRecallCategory::Unicode,
        top_n: 5,
        note: "Fifty-seventh Unicode row (iter-431): mixed-script \
               Cyrillic+Hebrew query — \"Mamba кэш זיכרון\". \
               Extends the post-floor Unicode interaction matrix \
               to a Cyrillic alphabetic token plus an RTL Hebrew \
               word in one top-5 membership contract. The English-\
               only Mamba decoy carries the Latin anchor but neither \
               non-Latin term.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        // Iter-434: mixed Devanagari + Hebrew interaction. This pairs
        // a Brahmic abugida token with an RTL Hebrew word in one
        // <=3-token AND query.
        query: "Mamba कैश זיכרון",
        expected_paths: &["notes/mamba_devanagari_hebrew.md"],
        forbidden_paths: &["notes/mamba_english_only.md"],
        category: FVaultRecallCategory::Unicode,
        top_n: 5,
        note: "Fifty-ninth Unicode row (iter-434): mixed-script \
               Devanagari+Hebrew query — \"Mamba कैश זיכרון\". \
               Extends the post-floor Unicode interaction matrix \
               to a Brahmic abugida token plus an RTL Hebrew word \
               in one top-5 membership contract. The English-only \
               Mamba decoy carries the Latin anchor but neither non-\
               Latin term.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        // Iter-437: mixed Japanese katakana + Hebrew interaction. This
        // pairs an East Asian syllabary token with an RTL Hebrew word
        // in one <=3-token AND query.
        query: "Mamba メモリ זיכרון",
        expected_paths: &["notes/mamba_katakana_hebrew.md"],
        forbidden_paths: &["notes/mamba_english_only.md"],
        category: FVaultRecallCategory::Unicode,
        top_n: 5,
        note: "Sixty-second Unicode row (iter-437): mixed-script \
               katakana+Hebrew query — \"Mamba メモリ זיכרון\". \
               Extends the post-floor Unicode interaction matrix \
               to a Japanese katakana syllabary token plus an RTL \
               Hebrew word in one top-5 membership contract. The \
               English-only Mamba decoy carries the Latin anchor but \
               neither non-Latin term.",
    },
    ...
    FVaultRecallRow {
        // Iter-441: mixed Armenian + Hebrew interaction. This pairs an
        // Indo-European Armenian alphabet token with an RTL Hebrew word
        // in one <=3-token AND query.
        query: "Mamba կեշ זיכרון",
        expected_paths: &["notes/mamba_armenian_hebrew.md"],
        forbidden_paths: &["notes/mamba_english_only.md"],
        category: FVaultRecallCategory::Unicode,
        top_n: 5,
        note: "Sixty-fourth Unicode row (iter-441): mixed-script \
               Armenian+Hebrew query — \"Mamba կեշ זיכרון\". \
               Extends the post-floor Unicode interaction matrix \
               from standalone Armenian coverage to an Armenian \
               alphabet token plus an RTL Hebrew word in one top-5 \
               membership contract. The English-only Mamba decoy \
               carries the Latin anchor but neither non-Latin term.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        ...
        // shape — "please can you tell me what where when how
        // some of my notes are about stuff things files thing
        // note file". Extends long-input cardinality
        // progression to 21 (iter-378 was 20). LONGEST
        // PureChatter query in the fixture. ALL 7 noun-class
        // members from QUERY_CHATTER_WORDS appear at the tail
        // (notes + stuff + things + files + thing + note +
        // file — full noun-class coverage in one query). Pin
        // closes the chatter-noun-class survey. All 21 tokens
        // in QUERY_CHATTER_WORDS.
        query: "please can you tell me what where when how some of my notes are about stuff things files thing note file",
        expected_paths: &[],
        forbidden_paths: &[
            "notes/totally_unrelated_a.md",
            "notes/totally_unrelated_b.md",
        ...
        category: FVaultRecallCategory::PureChatter,
        top_n: 7,
        note: "Forty-sixth PureChatter row (iter-385): 21-\
               token long-input. Extends cardinality to 21 \
               (iter-378 = 20). LONGEST PureChatter query in \
               the fixture. ALL 7 chatter-noun-class members \
               at the tail (notes + stuff + things + files + \
               thing + note + file — full noun-class \
               coverage). Closes the chatter-noun-class \
               survey. All 21 tokens in QUERY_CHATTER_WORDS. \
               Brings PureChatter to depth 46 — first \
               category past depth-45 horizon.",
    },
    FVaultRecallRow {
        // 45th PureChatter row (iter-378): 20-token long-input
        // shape — "please can you tell me what where when how
        // some of my notes are about stuff things files thing
        // note". Extends long-input cardinality progression to
        // 20 (iter-371 was 19). LONGEST PureChatter query in
        // the fixture. ALL 7 noun-class members from QUERY_
        // CHATTER_WORDS appear at the tail (notes + stuff +
        // things + files + thing + note + file are the 7
        // chatter-nouns; "file" still pending but already
        // demonstrated). Six of the 7 in this row. All 20
        // tokens in QUERY_CHATTER_WORDS.
        query: "please can you tell me what where when how some of my notes are about stuff things files thing note",
        expected_paths: &[],
        forbidden_paths: &[
            "notes/totally_unrelated_a.md",
            "notes/totally_unrelated_b.md",
        ...
        category: FVaultRecallCategory::PureChatter,
        top_n: 7,
        note: "Forty-fifth PureChatter row (iter-378): 20-\
               token long-input. Extends cardinality to 20 \
               (iter-371 = 19). LONGEST PureChatter query in \
               the fixture. SIX of 7 chatter-noun-class \
               members at the tail (notes + stuff + things + \
               files + thing + note). All 20 tokens in QUERY_\
               CHATTER_WORDS. Brings PureChatter to depth 45 \
               — first category past depth-44 horizon.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        ...
        // Extends long-input cardinality progression to 18
        // (iter-357 was 17). LONGEST PureChatter query in the
        // fixture. Mixes 13 chatter sub-classes including
        // QUADRUPLE chatter-nouns (notes + stuff + things +
        // files — full noun-class membership plus the
        // alternate-plural "files"). All 18 tokens (please /
        // can / you / tell / me / what / where / when / how /
        // some / of / my / notes / are / about / stuff /
        // things / files) are in QUERY_CHATTER_WORDS.
        query: "please can you tell me what where when how some of my notes are about stuff things files",
        expected_paths: &[],
        forbidden_paths: &[
            "notes/totally_unrelated_a.md",
            "notes/totally_unrelated_b.md",
        ...
        category: FVaultRecallCategory::PureChatter,
        top_n: 7,
        note: "Forty-third PureChatter row (iter-364): 18-token \
               long-input (\"please can you tell me what where \
               when how some of my notes are about stuff things \
               files\"). Extends cardinality to 18 (iter-357 = \
               17). LONGEST PureChatter query in the fixture. \
               QUADRUPLE chatter-nouns (notes + stuff + things \
               + files). All 18 tokens in QUERY_CHATTER_WORDS. \
               Brings PureChatter to depth 43 — first category \
               past depth-42 horizon.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        ...
        // 11 (iter-307 was 10). LONGEST PureChatter query in
        // the fixture. Mixes 9 chatter sub-classes (politeness +
        // modal + 2 pronouns + imperative + 2 wh-words +
        // quantifier + preposition + chatter-noun).
        query: "please can you tell me what where some of my notes",
        expected_paths: &[],
        forbidden_paths: &[
            "notes/totally_unrelated_a.md",
            "notes/totally_unrelated_b.md",
        ...
        category: FVaultRecallCategory::PureChatter,
        top_n: 7,
        note: "Thirty-sixth PureChatter row (iter-314): 11-\
               token long-input (\"please can you tell me what \
               where some of my notes\"). Extends cardinality \
               to 11 (iter-307 = 10). LONGEST PureChatter \
               query. Mixes 9 chatter sub-classes (politeness + \
               modal + pronouns + imperative + 2 wh-words + \
               quantifier + preposition + chatter-noun). Brings \
               PureChatter to depth 36 — first category past \
               depth-35 horizon. All 11 tokens in QUERY_CHATTER_\
               WORDS.",
    },
    FVaultRecallRow {
        // 35th PureChatter row (iter-307): 10-token long-input
        // shape — "please can you tell me what some of my
        // notes". Extends long-input cardinality progression
        // to 10 (iter-278 was 9). LONGEST PureChatter query in
        // the fixture. Mixes 8 chatter sub-classes (politeness +
        // modal + 2 pronouns + imperative + wh + quantifier +
        // preposition + chatter-noun). All 10 tokens in
        // QUERY_CHATTER_WORDS.
        query: "please can you tell me what some of my notes",
        expected_paths: &[],
        forbidden_paths: &[
            "notes/totally_unrelated_a.md",
            "notes/totally_unrelated_b.md",
        ...
        category: FVaultRecallCategory::PureChatter,
        top_n: 7,
        note: "Thirty-fifth PureChatter row (iter-307): 10-\
               token long-input (\"please can you tell me what \
               some of my notes\"). Extends long-input \
               cardinality to 10 (iter-278 = 9). LONGEST \
               PureChatter query. Mixes 8 chatter sub-classes \
               (politeness + modal + pronouns + imperative + \
               wh + quantifier + preposition + chatter-noun). \
               Brings PureChatter to depth 35 — second \
               category past depth-34 horizon. All 10 tokens \
               in QUERY_CHATTER_WORDS.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        ...
        // {runtime, system} 2-term-AND. Both pair-partners
        // carry both tokens. Other seeds with "runtime": runtime_
        // old_design (no system). Other seeds with "system":
        // design_system_hover_spec, system_overview (no
        // runtime). All blocked at AND-on-2. iter-373 +
        // iter-380 = 2 of 4 new C(5,2) subsets surveyed on
        // agent-runtime 5-element vocab. iter-380 milestone.
        // Zero new seeds.
        ...
        category: FVaultRecallCategory::Synthesis,
        top_n: 2,
        note: "Forty-sixth Synthesis row (iter-380, milestone): \
               SECOND new C(5,2) subset on agent-runtime 5-\
               element pair. {runtime, system}. Pair-unique \
               AND match: runtime_old_design has runtime only, \
               design_system + system_overview have system \
               only. 2 of 4 new C(5,2) subsets surveyed. \
               iter-380 milestone. Brings Synthesis to depth \
               46 — first category past depth-45 horizon. \
               Zero new seeds.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        ...
        // pattern_v1.md + design_pattern_v1_copy.md both have
        // both tokens. "specific" appears only in iter-11 pair-
        // partners among all corpus. AND-on-2 matches only the
        // pair (design_system_hover_spec.md has "design" but no
        // "specific"; design_general_system.md has "design" but
        // no "specific"). Opens C(3,2) survey on the FOURTH 3-
        // element pair (after Metal + neural-cache-layer + tier-
        // compression). All 3 subsets expected to be achievable
        // since each pair token has at least one seed where the
        // 2-subset is exclusive.
        ...
        category: FVaultRecallCategory::Synthesis,
        top_n: 2,
        note: "Thirty-seventh Synthesis row (iter-316): FIRST 2-\
               term-AND subset on iter-11 design-pattern near-\
               duplicate pair. {specific, design} — \"specific\" \
               unique to pair. AND-on-2 matches only the pair. \
               Opens C(3,2) survey on the FOURTH 3-element pair \
               (after Metal + neural-cache-layer + tier-\
               compression). Brings Synthesis to depth 37 — \
               first category past depth-36 horizon. Zero new \
               seeds.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        ...
        // only iter-271's mamba_sinhala.md. SEVENTEENTH non-
        // ASCII ChattyPrefix domain. SEVENTH and FINAL Brahmic-
        // family ChattyPrefix (after Devanagari + Thai + Tibetan
        // + Khmer + Lao + Myanmar) — Brahmic family completes
        // in ChattyPrefix, matching iter-325's Brahmic-completion
        // in SignalOnly. Parallels iter-325 single-term-AND on
        // the same Sinhala token "ක" (U+0D9A).
        query: "Show me my Mamba ක notes",
        expected_paths: &["notes/mamba_sinhala.md"],
        forbidden_paths: &["notes/mamba_english_only.md"],
        category: FVaultRecallCategory::ChattyPrefix,
        top_n: 7,
        note: "Thirty-eighth ChattyPrefix row (iter-327): \
               Sinhala-multilingual signal domain — 28th \
               distinct lexical universe. Survivors {Mamba, ක} \
               after strip. 2-term AND matches only iter-271 \
               mamba_sinhala.md. Seventeenth non-ASCII \
               ChattyPrefix domain. Seventh and FINAL Brahmic-\
               family ChattyPrefix (after Devanagari + Thai + \
               Tibetan + Khmer + Lao + Myanmar) — Brahmic \
               family completes in ChattyPrefix, matching \
               iter-325 SignalOnly completion. Brings \
               ChattyPrefix to depth 38 — first category past \
               depth-37 horizon. Zero new seeds.",
    },
    FVaultRecallRow {
        // 37th ChattyPrefix row (iter-320, milestone iteration):
        // Myanmar-multilingual signal domain — 27th distinct
        // lexical universe. Survivors after strip {Mamba, က}.
        // 2-term AND matches only iter-179's mamba_myanmar.md.
        // SIXTEENTH non-ASCII ChattyPrefix domain. SIXTH
        // Brahmic-family ChattyPrefix (after Devanagari + Thai +
        // Tibetan + Khmer + Lao).
        query: "Show me my Mamba က notes",
        expected_paths: &["notes/mamba_myanmar.md"],
        forbidden_paths: &["notes/mamba_english_only.md"],
        category: FVaultRecallCategory::ChattyPrefix,
        top_n: 7,
        note: "Thirty-seventh ChattyPrefix row (iter-320, \
               milestone): Myanmar-multilingual signal domain — \
               27th distinct lexical universe. Survivors \
               {Mamba, က} after strip. 2-term AND matches only \
               iter-179 mamba_myanmar.md. Sixteenth non-ASCII \
               ChattyPrefix domain. Sixth Brahmic-family \
               ChattyPrefix. Brings ChattyPrefix to depth 37 — \
               first category past depth-36 horizon. Zero new \
               seeds.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        ...
        // only iter-158's mamba_khmer.md. FOURTEENTH non-ASCII
        // ChattyPrefix domain. FOURTH Brahmic-family signal-
        // domain in ChattyPrefix (after Devanagari + Thai +
        // Tibetan).
        query: "Show me my Mamba ខែ notes",
        expected_paths: &["notes/mamba_khmer.md"],
        forbidden_paths: &["notes/mamba_english_only.md"],
        category: FVaultRecallCategory::ChattyPrefix,
        top_n: 7,
        note: "Thirty-fifth ChattyPrefix row (iter-306): Khmer-\
               multilingual signal domain — 25th distinct \
               lexical universe. Survivors {Mamba, ខែ} after \
               strip. 2-term AND matches only iter-158 \
               mamba_khmer.md. Fourteenth non-ASCII ChattyPrefix \
               domain. Fourth Brahmic-family ChattyPrefix \
               (after Devanagari + Thai + Tibetan). Brings \
               ChattyPrefix to depth 35 — second category past \
               depth-34 horizon. Zero new seeds.",
    },
    FVaultRecallRow {
    ...
    FVaultRecallRow {
        ...
        // tail tokens (ranking + ir from canonical's tail
        // "...ranking ir search relevance scoring notes"). 6
        // terms OR. Canonical body has all 6 (2-3× primaries +
        // 1× ranking + 1× ir); stuffed-decoy has only
        // "saturation" → 1 of 6; other decoys ≤1 of 6.
        // FOURTH iter-84 Adversarial row (after 4-term iter-84
        // + 4-term drop-2 iter-252 + 5-term iter-281). THIRD
        // canonical at 6-term coverage (after agent-runtime
        // iter-120 + vault iter-317). BM25/IR corpus now
        // exercised at 4-term + 5-term + 6-term — second
        // canonical with 3 query-length coverage points.
        query: "bm25 saturation length penalty ranking ir",
        expected_paths: &["notes/bm25_saturation_length_penalty.md"],
        forbidden_paths: &[
            "notes/saturation_stuffed_decoy.md",
            "notes/bm25_overview.md",
        ...
        category: FVaultRecallCategory::Adversarial,
        top_n: 1,
        note: "Thirty-eighth Adversarial row (iter-324): BM25/IR \
               6-term long-query \"bm25 saturation length penalty \
               ranking ir\". Keeps all 4 primaries + 2 IR-context \
               tail tokens. Third canonical at 6-term coverage \
               (after agent-runtime iter-120 + vault iter-317). \
               BM25/IR now exercised at 4-term + 5-term + 6-term \
               — second canonical with 3 query-length coverage \
               points. Canonical 6/6; decoys ≤1 of 6. Brings \
               Adversarial to depth 38 — first category past \
               depth-37 horizon. Zero new seeds.",
    },
    FVaultRecallRow {
