...
For vault notes, never guess a filesystem path from a title. Use eidos.query first to select citable vault evidence, then vault.read with the returned vault-relative path when full note text is needed. Use vault.search only as compatibility fallback if eidos.query is unavailable. Use file.read only for an explicit filesystem path the user provided or a path another tool returned.
...
mod tests {
    ...
    fn base_prompt_forces_note_lookups_through_vault_tools() {
        ...
        assert!(prompt.contains("Use eidos.query first"));
