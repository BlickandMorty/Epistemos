...
impl ShadowAnswerabilitySummary {
    ...
    pub fn cap_fields_match_contract(&self) -> bool {
        self.exact_escalation_target_limit == SHADOW_EXACT_ESCALATION_TARGET_LIMIT
            && self.exact_escalation_query_count_limit == SHADOW_EXACT_ESCALATION_QUERY_COUNT_LIMIT
...
mod tests {
    ...
    #[test]
    fn synthesis_validation_dedupes_markup_title_against_path_backed_title() {
        let mut trace = sufficient_trace();
        trace.candidates[0].path = "Research/Vault Recall Alpha.md".to_string();
        trace.candidates[0].title = " <b>Vault</b> Recall\u{2026}Alpha ".to_string();
        let mut duplicate = selected_candidate();
        duplicate.path.clear();
        duplicate.title = "vault recall alpha".to_string();
        duplicate.rank = 2;
    ...
    fn shadow_first_trace_allows_answer_after_visible_exact_verification() {
        ...
        assert!(!trace.answer_allowed());
        assert!(
            trace.answer_allowed_after_exact_verification(vec![shadow_exact_hit(
                "dense-alpha",
                "Vault Recall Alpha",
                Some("Exact body evidence."),
            )])
    ...
    fn shadow_residual_decode_outcome_bridges_enriched_exact_verification() {
        ...
        assert!(
            outcome.answer_allowed_after_exact_verification(vec![shadow_exact_hit(
                "dense-alpha",
                "Vault Recall Alpha",
                Some("verified body evidence"),
            )])
