//! Gemma QAT E2B route AnswerPacket visibility gate.
//!
//! This primitive consumes the E2B System G dry-run route packet gate and
//! defines the fail-closed visibility packet required before future Gemma route
//! evidence can be shown in product surfaces. It is metadata-only: no
//! AnswerPacket is emitted to a user, no route is admitted, no runtime is
//! executed, and no model/provider bytes are loaded.
...
use crate::uas::{
    ...
    GEMMA_QAT_E2B_EXPECTED_FILE_BYTES, GEMMA_QAT_E2B_SOURCE_REVISION,
...
pub const GEMMA_QAT_E2B_ROUTE_ANSWER_PACKET_VISIBILITY_GATE_ID: &str =
    "F-GemmaQATE2BRouteAnswerPacketVisibilityGate";
pub const GEMMA_QAT_E2B_ROUTE_ANSWER_PACKET_VISIBILITY_GATE_CURSOR: &str =
...
const ARTIFACT_ROOT_PREFIX: &str =
    "artifacts/falsifiers/gemma_qat_e2b_route_answer_packet_visibility_gate/";
const VISIBILITY_CARD_ID: &str = "gemma-e2b-gguf-route-answer-packet-visibility-gate";
const FUTURE_VISIBILITY_PACKET_NAME: &str = "gemma-e2b-gguf-route-answer-packet-visibility-v1";
...
const REQUIRED_REJECTION_POLICIES: &[&str] = &[
    "missing_upstream_dry_run_route_packet",
    "dry_run_route_packet_digest_mismatch",
    ...
    "missing_visible_no_default_model_mutation",
    "missing_visible_no_hidden_authority",
    "missing_visible_non_promotion",
...
// UAS: uas:gemma-qat-e2b-route-answer-packet-visibility-gate:status
// Plane: Verification.
// Residency: metadata-only visibility status; no user-visible packet emitted.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GemmaQatE2bRouteAnswerPacketVisibilityGateStatus {
    VisibilityPacketContractOnly,
}

// UAS: uas:gemma-qat-e2b-route-answer-packet-visibility-gate:spec
// Plane: Controller + Verification.
// Residency: future route AnswerPacket visibility contract only.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct GemmaQatE2bRouteAnswerPacketVisibilityGate {
    pub upstream_dry_run_route_packet_ref: String,
    pub upstream_dry_run_route_packet_id: String,
    ...
    pub upstream_dry_run_route_packet_digest_required: bool,
    ...
    pub status: GemmaQatE2bRouteAnswerPacketVisibilityGateStatus,
    pub next_cursor: String,
}

impl GemmaQatE2bRouteAnswerPacketVisibilityGate {
    pub fn canonical(upstream_dry_run_route_packet_ref: impl Into<String>) -> Self {
        Self {
            upstream_dry_run_route_packet_ref: upstream_dry_run_route_packet_ref.into(),
            upstream_dry_run_route_packet_id: GEMMA_QAT_E2B_SYSTEM_G_DRY_RUN_ROUTE_PACKET_GATE_ID
                .to_string(),
            upstream_admission_packet_ref:
                GEMMA_QAT_E2B_SYSTEM_G_DRY_RUN_ROUTE_PACKET_GATE_UPSTREAM_REF.to_string(),
            ...
            required_rejection_policies: REQUIRED_REJECTION_POLICIES
                ...
                .map(|value| (*value).to_string())
                .collect(),
            upstream_dry_run_route_packet_digest_required: true,
            ...
            visible_scope_rex_ref: "scope_rex:gemma_e2b_route_answer_packet_visibility_gate"
                .to_string(),
            visible_sovereign_gate_ref:
                "sovereign_gate:gemma_e2b_route_answer_packet_visibility_gate".to_string(),
            ...
            visible_cancellation_ref: "cancel:gemma_e2b_route_answer_packet_visibility_gate"
                .to_string(),
            visible_rollback_ref: "rollback:gemma_e2b_route_answer_packet_visibility_gate"
                .to_string(),
            visible_run_event_log_ref:
                "run_event_log:gemma_e2b_route_answer_packet_visibility_gate".to_string(),
            ...
            status: GemmaQatE2bRouteAnswerPacketVisibilityGateStatus::VisibilityPacketContractOnly,
            next_cursor: GEMMA_QAT_E2B_ROUTE_ANSWER_PACKET_VISIBILITY_GATE_NEXT_CURSOR.to_string(),
        }
    }

    pub fn validate(&self) -> Result<(), GemmaQatE2bRouteAnswerPacketVisibilityGateError> {
        if !self
            .upstream_dry_run_route_packet_ref
            .starts_with(UPSTREAM_DRY_RUN_ROUTE_PREFIX)
            || self.upstream_dry_run_route_packet_id
                != GEMMA_QAT_E2B_SYSTEM_G_DRY_RUN_ROUTE_PACKET_GATE_ID
        {
            return Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::BadUpstreamRef);
        }
        validate_exact(
            "upstream_admission_packet_ref",
        ...
        {
            return Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::BadSelectedLane);
        }
        if self.product_build != ProductBuild::Pro
            || self.pro_status != ProStatus::Gated
            || self.status
                != GemmaQatE2bRouteAnswerPacketVisibilityGateStatus::VisibilityPacketContractOnly
            || self.metadata_bytes > MAX_METADATA_BYTES
        {
            return Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::UnsafeState);
        }
        if !self.upstream_dry_run_route_packet_digest_required
        ...
        {
            return Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::ProofBoundaryBroken);
        }
        validate_prefix(
        ...
        {
            return Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::VisibilityActionLeak);
        }
        ...
        {
            return Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::RuntimeActionLeak);
        }
        if self.raw_prompt_bytes_captured != 0
            || self.raw_output_bytes_captured != 0
            || self.answer_packet_suppressed
        {
            return Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::PrivacyLeak);
        }
        ...
        {
            return Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::PromotionClaim);
        }
        validate_exact(
            "next_cursor",
            &self.next_cursor,
    ...
    pub fn metrics(&self) -> GemmaQatE2bRouteAnswerPacketVisibilityGateMetrics {
    ...
    pub fn visibility_gate_address(&self, created_at_ms: u64) -> UasAddress {
        UasAddress::new(
            UasKind::Other(GEMMA_QAT_E2B_ROUTE_ANSWER_PACKET_VISIBILITY_GATE_CURSOR.to_string()),
    ...
    fn preimage(&self) -> String {
        ...
        policies.sort();
        format!(
            "gemma-e2b-route-answer-packet-visibility-gate:v1:{}:{}:{}:{}:{}:{}:{}:{}",
            self.upstream_dry_run_route_packet_ref,
...
// UAS: uas:gemma-qat-e2b-route-answer-packet-visibility-gate:metrics
// Plane: Verification.
// Residency: zero-action counters for visibility and route proof.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct GemmaQatE2bRouteAnswerPacketVisibilityGateMetrics {
...
pub fn required_gemma_qat_e2b_route_answer_packet_visibility_fields() -> Vec<String> {
    REQUIRED_VISIBILITY_FIELDS
        .iter()
        .map(|value| (*value).to_string())
        .collect()
...
pub fn required_gemma_qat_e2b_route_answer_packet_visibility_rejection_policies() -> Vec<String> {
    REQUIRED_REJECTION_POLICIES
        .iter()
        .map(|value| (*value).to_string())
        .collect()
...
// UAS: uas:gemma-qat-e2b-route-answer-packet-visibility-gate:error
// Plane: Verification.
// Residency: fail-closed diagnostics only.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum GemmaQatE2bRouteAnswerPacketVisibilityGateError {
    BadUpstreamRef,
    BadSelectedLane,
...
impl fmt::Display for GemmaQatE2bRouteAnswerPacketVisibilityGateError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::BadUpstreamRef => f.write_str("bad upstream dry-run route packet reference"),
...
impl std::error::Error for GemmaQatE2bRouteAnswerPacketVisibilityGateError {}

fn validate_unique_exact_set(
    field_name: &'static str,
    actual: &[String],
    expected: &[&str],
) -> Result<(), GemmaQatE2bRouteAnswerPacketVisibilityGateError> {
    if actual.len() != expected.len() {
        return Err(
            GemmaQatE2bRouteAnswerPacketVisibilityGateError::DuplicateOrMissingField(field_name),
        );
    }
...
fn validate_prefix(
    ...
    if actual.starts_with(expected_prefix) {
        ...
        Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::BadField(
            field_name,
        ))
    }
}
...
mod tests {
    ...
    fn canonical_visibility_gate_validates_zero_actions() {
        ...
        gate.validate()
            .expect("canonical route AnswerPacket visibility gate should validate");
        let metrics = gate.metrics();
        assert_eq!(metrics.required_visibility_field_count, 30);
        assert_eq!(metrics.required_rejection_policy_count, 63);
    ...
    fn required_set_drift_is_rejected() {
        let mut gate = GemmaQatE2bRouteAnswerPacketVisibilityGate::canonical(
            GEMMA_QAT_E2B_ROUTE_ANSWER_PACKET_VISIBILITY_GATE_UPSTREAM_REF,
        );
        ...
        assert!(matches!(
            gate.validate(),
            Err(
                GemmaQatE2bRouteAnswerPacketVisibilityGateError::DuplicateOrMissingField(
                    "required_visibility_fields"
                )
            )
        ));
    ...
    fn user_visible_packet_and_route_mutation_are_rejected() {
        ...
        assert!(matches!(
            gate.validate(),
            Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::VisibilityActionLeak)
        ));
        let mut gate = GemmaQatE2bRouteAnswerPacketVisibilityGate::canonical(
            GEMMA_QAT_E2B_ROUTE_ANSWER_PACKET_VISIBILITY_GATE_UPSTREAM_REF,
        );
    ...
    fn runtime_execution_and_promotion_are_rejected() {
        ...
        assert!(matches!(
            gate.validate(),
            Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::RuntimeActionLeak)
        ));
        let mut gate = GemmaQatE2bRouteAnswerPacketVisibilityGate::canonical(
            GEMMA_QAT_E2B_ROUTE_ANSWER_PACKET_VISIBILITY_GATE_UPSTREAM_REF,
        );
        ...
        assert!(matches!(
            gate.validate(),
            Err(GemmaQatE2bRouteAnswerPacketVisibilityGateError::PromotionClaim)
        ));
    }
