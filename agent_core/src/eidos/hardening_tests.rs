...
//! permutations.
//!
//! Closed-citation contract hardening session (iters 127-400+) — the
...
/// Shared helper for iter-number-string array validation. Consolidates
/// iters 248 (8-char string-length lock) + 249 (numeric range [100,
/// 999]) into one place. Used by both the vector and shape-lock drift
/// detectors so the canonical "iter NNN" format is enforced once.
///
/// Returns the parsed iter number on success; panics with an explicit
/// `source` label on any deviation.
fn assert_iter_format_canonical(s: &str, source: &str) -> u32 {
    assert_eq!(
        s.len(),
        8,
        "{source} entry {s:?} is not the canonical 8-char `iter NNN` \
         format. Current closed-citation hardening uses 3-digit iter \
         numbers (iter 100-999); a 2- or 4-digit drift suggests \
...
fn assert_lexical_near_duplicate_adversarial_fixture_tie_break_is_deterministic_and_closed() {
    use super::adversarial::{
        adversarial_query_fixture_for_outcome, AdversarialQueryExpectedOutcome,
    };
    ...
    .expect("DeterministicTieBreak adversarial fixture exists");
    let mut lex = InMemoryLexicalIndex::new(manifest());
    lex.insert(
        doc("z-near-dup"),
        format!("{} with trailing variant beta", fixture.query_text),
        EidosSourceKind::Note,
    )
    .unwrap();
    lex.insert(
        doc("a-near-dup"),
        format!("{} with trailing variant alpha", fixture.query_text),
        EidosSourceKind::Note,
    )
    .unwrap();
...
#[test]
fn adversarial_query_fixture_label_slice_matches_fixture_rows() {
    use super::adversarial::adversarial_query_fixture_catalog_labels_match_fixture_rows;
...
#[test]
fn adversarial_query_fixture_labels_are_ascii_lowercase_kebab_case() {
    use super::adversarial::adversarial_query_fixture_labels_are_ascii_lowercase_kebab_case;

    assert!(
        adversarial_query_fixture_labels_are_ascii_lowercase_kebab_case(),
        "fixture labels must remain lowercase ASCII kebab-case wire tokens"
    );
}
...
#[test]
fn adversarial_query_fixture_query_text_slice_matches_fixture_rows() {
    use super::adversarial::adversarial_query_fixture_catalog_query_texts_match_fixture_rows;

    assert!(
        adversarial_query_fixture_catalog_query_texts_match_fixture_rows(),
        "fixture query-text enumerator must remain byte-equal to fixture row query_text values"
    );
}

#[test]
fn adversarial_query_fixture_query_texts_are_nonempty_trimmed_and_control_free() {
    use super::adversarial::adversarial_query_fixture_query_texts_are_nonempty_trimmed_and_control_free;

    assert!(
        adversarial_query_fixture_query_texts_are_nonempty_trimmed_and_control_free(),
        "fixture query texts must stay nonempty, trimmed, and free of control characters"
    );
}

#[test]
fn adversarial_query_fixture_kinds_are_ordered_unique_and_lookup_complete() {
    use super::adversarial::{
        adversarial_query_fixture_for_kind, adversarial_query_fixture_kinds,
        AdversarialQueryFixtureKind, ADVERSARIAL_QUERY_FIXTURES,
    };

    let kinds = adversarial_query_fixture_kinds();
    assert_eq!(
        kinds,
        [
            AdversarialQueryFixtureKind::TypoTransposition,
        ...
        "fixture kind order is a stable typed harness contract"
    );
    assert_eq!(kinds.len(), ADVERSARIAL_QUERY_FIXTURES.len());

    let unique: std::collections::BTreeSet<&str> = kinds
        .iter()
        .map(|kind| match kind {
            AdversarialQueryFixtureKind::TypoTransposition => "TypoTransposition",
            AdversarialQueryFixtureKind::Bm25Saturation => "Bm25Saturation",
            AdversarialQueryFixtureKind::NearDuplicateParagraphTie => "NearDuplicateParagraphTie",
        })
        .collect();
    assert_eq!(
        unique.len(),
        kinds.len(),
        "fixture kinds must remain pairwise unique"
    );

    for kind in kinds {
        assert!(
            adversarial_query_fixture_for_kind(*kind).is_some(),
            "every enumerated fixture kind must resolve through exact lookup"
        );
    }
}

#[test]
fn adversarial_query_fixture_kind_slice_matches_fixture_rows() {
    use super::adversarial::adversarial_query_fixture_catalog_kinds_match_fixture_rows;
...
#[test]
fn adversarial_query_fixture_kind_tokens_are_ascii_lowercase_kebab_case() {
    use super::adversarial::adversarial_query_fixture_kind_tokens_are_ascii_lowercase_kebab_case;
...
#[test]
fn adversarial_query_fixture_kind_tokens_are_ordered_unique_and_complete() {
    use super::adversarial::{
        adversarial_query_fixture_kind_tokens, adversarial_query_fixture_kinds,
        ADVERSARIAL_QUERY_FIXTURES,
    };

    let tokens = adversarial_query_fixture_kind_tokens();
    assert_eq!(
        tokens,
        [
            "typo-transposition",
        ...
        "fixture kind tokens are a stable category wire contract"
    );
    assert_eq!(tokens.len(), adversarial_query_fixture_kinds().len());
    ...
    assert_eq!(
        unique.len(),
        tokens.len(),
        "fixture kind tokens must remain pairwise unique"
    );
}

#[test]
fn adversarial_query_fixture_kind_token_slice_matches_kind_tokens() {
    use super::adversarial::adversarial_query_fixture_catalog_kind_tokens_match_kinds;

    assert!(
        adversarial_query_fixture_catalog_kind_tokens_match_kinds(),
        "fixture kind token slice must remain byte-equal to enum token() output"
    );
}

#[test]
fn adversarial_query_fixture_lookup_by_kind_token_is_exact() {
    use super::adversarial::adversarial_query_fixture_for_kind_token;

    assert_eq!(
        adversarial_query_fixture_for_kind_token("typo-transposition")
            .expect("typo fixture")
            .label,
        "typo-transposition"
    );
    assert_eq!(
        adversarial_query_fixture_for_kind_token("bm25-saturation")
            .expect("bm25 fixture")
            .label,
        "bm25-saturation"
    );
    assert_eq!(
        adversarial_query_fixture_for_kind_token("near-duplicate-paragraph-tie")
            .expect("near-duplicate fixture")
            .label,
        "near-duplicate-paragraph-tie"
    );
    assert!(
        adversarial_query_fixture_for_kind_token("TypoTransposition").is_none(),
        "kind-token lookup must not case-fold enum names"
    );
    assert!(
        adversarial_query_fixture_for_kind_token(" typo-transposition ").is_none(),
        "kind-token lookup must not trim caller input"
    );
}
...
#[test]
fn adversarial_query_fixture_expected_outcome_tokens_are_ascii_lowercase_kebab_case() {
    use super::adversarial::adversarial_query_fixture_expected_outcome_tokens_are_ascii_lowercase_kebab_case;
...
#[test]
fn adversarial_query_fixture_expected_outcome_token_slice_matches_outcome_tokens() {
    use super::adversarial::adversarial_query_fixture_catalog_expected_outcome_tokens_match_outcomes;
...
#[test]
fn adversarial_query_fixture_token_lookup_surface_is_complete() {
    use super::adversarial::adversarial_query_fixture_token_lookup_surface_is_complete;

    assert!(
        adversarial_query_fixture_token_lookup_surface_is_complete(),
        "every adversarial fixture wire token must resolve through exact lookup"
    );
}

#[test]
fn adversarial_query_fixture_catalog_dispatch_surface_is_complete() {
    use super::adversarial::adversarial_query_fixture_catalog_dispatch_surface_is_complete;

    assert!(
        adversarial_query_fixture_catalog_dispatch_surface_is_complete(),
        "adversarial fixture catalog dispatch surface must keep static rows and exact token lookups complete"
    );
}

#[test]
fn adversarial_query_fixture_token_lookups_reject_empty_input() {
    use super::adversarial::adversarial_query_fixture_token_lookups_reject_empty_input;

    assert!(
        adversarial_query_fixture_token_lookups_reject_empty_input(),
        "adversarial fixture token lookups must explicitly fail closed for empty input"
    );
}

#[test]
fn adversarial_query_fixture_token_lookups_reject_whitespace_input() {
    use super::adversarial::adversarial_query_fixture_token_lookups_reject_whitespace_input;

    assert!(
        adversarial_query_fixture_token_lookups_reject_whitespace_input(),
        "adversarial fixture token lookups must explicitly fail closed for whitespace-only input"
    );
}

#[test]
fn adversarial_query_fixture_token_lookups_reject_invisible_input() {
    use super::adversarial::adversarial_query_fixture_token_lookups_reject_invisible_input;

    assert!(
        adversarial_query_fixture_token_lookups_reject_invisible_input(),
        "adversarial fixture token lookups must explicitly fail closed for invisible-only input"
    );
}

#[test]
fn adversarial_query_fixture_token_lookups_reject_control_input() {
    use super::adversarial::adversarial_query_fixture_token_lookups_reject_control_input;

    assert!(
        adversarial_query_fixture_token_lookups_reject_control_input(),
        "adversarial fixture token lookups must explicitly fail closed for control-only input"
    );
}

#[test]
fn adversarial_query_fixture_token_lookups_reject_bidi_input() {
    use super::adversarial::adversarial_query_fixture_token_lookups_reject_bidi_input;
...
#[test]
fn adversarial_query_fixture_token_smuggling_surface_is_complete() {
    use super::adversarial::adversarial_query_fixture_token_smuggling_surface_is_complete;
...
#[test]
fn adversarial_query_fixture_token_smuggling_input_labels_are_ascii_lowercase() {
    use super::adversarial::adversarial_query_fixture_token_smuggling_input_labels_are_ascii_lowercase;

    assert!(
        adversarial_query_fixture_token_smuggling_input_labels_are_ascii_lowercase(),
        "adversarial fixture token-smuggling input labels must remain ASCII lowercase wire labels"
    );
}

#[test]
fn adversarial_query_fixture_token_smuggling_labels_match_inputs() {
    use super::adversarial::adversarial_query_fixture_token_smuggling_labels_match_inputs;

    assert!(
        adversarial_query_fixture_token_smuggling_labels_match_inputs(),
        "adversarial fixture token-smuggling labels must stay paired with their byte-exact inputs"
    );
}

#[test]
fn adversarial_query_fixture_token_smuggling_input_count_matches_surface() {
    use super::adversarial::{
    ...
    assert_eq!(
        adversarial_query_fixture_token_smuggling_input_count(),
        5,
        "adversarial fixture token-smuggling input count must stay pinned"
    );
    assert_eq!(
        adversarial_query_fixture_token_smuggling_input_count(),
        adversarial_query_fixture_token_smuggling_inputs().len(),
        "adversarial fixture token-smuggling count must match input slice length"
    );
    assert_eq!(
        adversarial_query_fixture_token_smuggling_input_count(),
        adversarial_query_fixture_token_smuggling_input_labels().len(),
        "adversarial fixture token-smuggling count must match label slice length"
    );
}

#[test]
fn adversarial_query_fixture_token_smuggling_label_count_matches_surface() {
    use super::adversarial::{
        adversarial_query_fixture_token_smuggling_input_count,
        adversarial_query_fixture_token_smuggling_input_labels,
    ...
    assert_eq!(
        adversarial_query_fixture_token_smuggling_label_count(),
        5,
        "adversarial fixture token-smuggling label count must stay pinned"
    );
    assert_eq!(
        adversarial_query_fixture_token_smuggling_label_count(),
        adversarial_query_fixture_token_smuggling_input_labels().len(),
        "adversarial fixture token-smuggling label count must match label slice length"
    );
    assert_eq!(
        adversarial_query_fixture_token_smuggling_label_count(),
        adversarial_query_fixture_token_smuggling_input_count(),
...
#[test]
fn adversarial_query_fixture_token_smuggling_cases_are_stable_and_complete() {
    use super::adversarial::{
        adversarial_query_fixture_token_smuggling_case_at,
    ...
    let cases = adversarial_query_fixture_token_smuggling_cases();
    assert_eq!(
        cases,
        [
            ("empty", ""),
        ...
        "adversarial fixture token-smuggling cases must stay ordered and byte-exact"
    );
    assert_eq!(
        cases.len(),
        adversarial_query_fixture_token_smuggling_input_count(),
...
#[test]
fn adversarial_query_fixture_token_smuggling_cases_reject_through_token_lookups() {
    use super::adversarial::adversarial_query_fixture_token_smuggling_cases_reject_through_token_lookups;

    assert!(
        adversarial_query_fixture_token_smuggling_cases_reject_through_token_lookups(),
        "adversarial fixture token-smuggling cases must reject through both token lookup paths with labels preserved"
    );
}

#[test]
fn adversarial_query_fixture_token_smuggling_cases_match_label_and_input_slices() {
    use super::adversarial::adversarial_query_fixture_token_smuggling_cases_match_label_and_input_slices;

    assert!(
        adversarial_query_fixture_token_smuggling_cases_match_label_and_input_slices(),
        "adversarial fixture token-smuggling case slice must stay aligned with label and input slices"
    );
}

#[test]
fn adversarial_query_fixture_token_smuggling_case_labels_are_ascii_lowercase() {
    use super::adversarial::adversarial_query_fixture_token_smuggling_case_labels_are_ascii_lowercase;

    assert!(
        adversarial_query_fixture_token_smuggling_case_labels_are_ascii_lowercase(),
        "adversarial fixture token-smuggling case labels must remain ASCII lowercase wire labels"
    );
}

#[test]
fn adversarial_query_fixture_token_smuggling_case_inputs_are_ordered_unique() {
    use super::adversarial::adversarial_query_fixture_token_smuggling_case_inputs_are_ordered_unique;
...
fn adversarial_query_fixture_descriptions_are_ordered_unique_and_nonempty() {
    ...
    assert_eq!(
        ...
        "fixture descriptions are stable catalog-facing descriptors"
    );
    assert_eq!(descriptions.len(), ADVERSARIAL_QUERY_FIXTURES.len());

    let unique: std::collections::BTreeSet<&str> = descriptions.iter().copied().collect();
    assert_eq!(
        unique.len(),
        descriptions.len(),
        "fixture descriptions must remain pairwise unique"
    );

    for description in descriptions {
        assert!(
            !description.trim().is_empty(),
            "fixture descriptions must stay non-empty and human-readable"
        );
    }
}

#[test]
fn adversarial_query_fixture_descriptions_are_trimmed_and_control_free() {
    use super::adversarial::adversarial_query_fixture_descriptions_are_trimmed_and_control_free;

    assert!(
        adversarial_query_fixture_descriptions_are_trimmed_and_control_free(),
        "fixture descriptions must stay trimmed and free of control characters"
    );
}

#[test]
fn adversarial_query_fixture_description_slice_matches_fixture_rows() {
    use super::adversarial::adversarial_query_fixture_catalog_descriptions_match_fixture_rows;
...
#[test]
fn adversarial_query_fixture_lookup_by_index_is_ordered_and_bounded() {
    use super::adversarial::adversarial_query_fixture_at;

    assert_eq!(
        adversarial_query_fixture_at(0)
            .expect("fixture at index 0")
            .label,
        "typo-transposition"
    );
    assert_eq!(
        adversarial_query_fixture_at(1)
            .expect("fixture at index 1")
            .label,
        "bm25-saturation"
    );
    assert_eq!(
        adversarial_query_fixture_at(2)
            .expect("fixture at index 2")
            .label,
        "near-duplicate-paragraph-tie"
    );
    assert!(
...
#[test]
fn adversarial_query_fixture_index_slice_matches_fixture_rows() {
    use super::adversarial::adversarial_query_fixture_catalog_indices_match_fixture_rows;

    assert!(
        adversarial_query_fixture_catalog_indices_match_fixture_rows(),
        "fixture index enumerator must remain ordinal-aligned to fixture rows"
    );
}

#[test]
fn adversarial_query_fixture_catalog_static_surface_is_complete() {
    use super::adversarial::adversarial_query_fixture_catalog_static_surface_is_complete;
...
#[test]
fn adversarial_query_fixture_catalog_static_surface_is_wire_safe() {
    use super::adversarial::adversarial_query_fixture_catalog_static_surface_is_wire_safe;

    assert!(
        adversarial_query_fixture_catalog_static_surface_is_wire_safe(),
        "fixture catalog static helper surface must remain row-aligned and wire-safe"
    );
}
...
#[test]
fn lexical_near_duplicate_adversarial_fixture_tie_break_is_deterministic_and_closed() {
    assert_lexical_near_duplicate_adversarial_fixture_tie_break_is_deterministic_and_closed();
}
...
fn closed_citation_named_smuggling_vector_tests_are_all_present() {
    ...
    // Format + range check delegated to the shared module-level
    // helper introduced in iter 254 (consolidates iters 248-249's
    // inline checks). Single source of truth for "iter NNN" format
    // across both VECTOR + SHAPE_LOCK iter-number arrays.
    for s in &VECTOR_ITER_NUMBERS {
        let _ = assert_iter_format_canonical(s, "VECTOR_ITER_NUMBERS");
    }
...
fn closed_citation_structural_shape_locks_are_all_present() {
    ...
    // Format + range check delegated to the shared module-level
    // helper (iter 254 consolidation, parallel to vector usage above).
    for s in &SHAPE_LOCK_ITER_NUMBERS {
        let _ = assert_iter_format_canonical(s, "SHAPE_LOCK_ITER_NUMBERS");
    }
...
/// Multilingual `EidosHit::clone()` byte-perfect pin — completes
/// the Clone-trilogy multilingual closure across all three closed-
/// citation surface types:
///
///   - iter 663: EidosCitation (chat-layer INPUT)
///   - iter 668: EidosContextPacket (full packet for replay)
///   - this iter (669): EidosHit (retriever OUTPUT)
///
/// Hits flow through cloning paths in fusion retrievers (HybridRetriever,
/// HybridRetrieverN), the ProvenanceVerified admit-set wrapper, and the
/// replay-bundle serialization layer. A custom Clone that canonicalized
/// / trimmed / case-folded the source_id during clone would silently
/// break the byte-strict floor — a retriever emits a hit with byte-X
/// non-Latin source_id, the fusion wrapper clones it for re-emission,
/// and the cloned bytes differ from the original at the downstream
/// validate_citation.
///
/// Parallel to iter 193's smuggling-vector EidosHit Clone test but
/// in the multilingual-positive-control dimension. Five script
/// families (Han, Hangul, Arabic, Devanagari, mixed Han+Latin) —
/// each with full Clone byte-perfect coverage across every nested
/// string field of the hit (source_id, document_id, nested
...
fn eidos_hit_clone_is_byte_perfect_for_multilingual_non_latin_ids() {
    ...
    for (label, src_id, doc_id, provenance_manifest) in cases {
        let pmid = EidosIndexManifestId::new(*provenance_manifest).unwrap();
        let original = EidosHit {
            source_id: EidosChunkId::new(*src_id).unwrap(),
            ...
            score: EidosScoreComponents::default(),
            provenance: EidosProvenance {
                manifest_id: pmid.clone(),
                mode: EidosRetrievalMode::Lexical,
                retrieved_at_unix_ms: 1_700_000_000_000,
            },
        };

        // Fixture sanity — every non-Latin string field is genuinely
        // non-ASCII so the iter exercises the multilingual surface.
        assert!(
            !original.source_id.as_str().is_ascii(),
            "{label}: source_id must be non-ASCII"
        );
        assert!(
            !original.document_id.as_str().is_ascii(),
            "{label}: document_id must be non-ASCII"
        );
        assert!(
            !original.provenance.manifest_id.as_str().is_ascii(),
            "{label}: provenance.manifest_id must be non-ASCII"
        );

        let cloned = original.clone();

        // (1) Full PartialEq round-trip on the whole hit.
        assert_eq!(
            cloned, original,
            "{label}: cloned multilingual hit must equal original"
        );

        // (2) source_id bytes preserved.
        assert_eq!(
            cloned.source_id.as_str().as_bytes(),
            original.source_id.as_str().as_bytes(),
            "{label}: cloned hit source_id bytes must match original \
             verbatim — a custom Clone that NFC-normalized / casefolded \
             non-Latin payloads would silently break the byte-strict \
             floor pinned in iters 658-664"
        );

        // (3) document_id bytes preserved (the other string field on
        // EidosHit besides source_id + the nested provenance).
        assert_eq!(
            cloned.document_id.as_str().as_bytes(),
            original.document_id.as_str().as_bytes(),
            "{label}: cloned document_id bytes must match original"
        );

        // (4) Nested provenance.manifest_id bytes preserved (the
        // nested field most likely to drift if a refactor flattens
        // provenance Clone or implements a custom field-by-field
        // Clone).
        assert_eq!(
            cloned.provenance.manifest_id.as_str().as_bytes(),
            original.provenance.manifest_id.as_str().as_bytes(),
            "{label}: nested provenance.manifest_id bytes preserved \
             — most-likely drift surface in a Clone refactor"
        );

        // (5) Multi-byte UTF-8 survival: byte count > codepoint count
        // for non-ASCII (catches a future Clone that truncates to
        // ASCII).
        assert!(
            cloned.source_id.as_str().len() > src_id.chars().count(),
            "{label}: cloned source_id retains multi-byte UTF-8 sequences"
        );
    }
}
