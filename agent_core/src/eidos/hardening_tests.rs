...
/// Iter 684 — catalog range continuation pin.
...
fn status_md_closed_citation_iter_range_tip_tracks_latest_catalog_pin() {
    ...
    assert!(
        status.contains("Closed-citation contract hardening (iters 127-684)"),
        "STATUS.md closed-citation hardening catalog must advance its iter \
         range tip to iter 684 when the catalog-continuation pin lands"
    );
}
