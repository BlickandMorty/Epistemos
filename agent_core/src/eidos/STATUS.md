...
- 268 unit tests in `agent_core/src/eidos/*`, all green.
...
- Scoring formulas algebraically pinned by-example at all 4 surfaces: Recency `1/(1+age_days)` at 5 ages (iter 100), Lexical `n/(1+n)` at 4 counts (iter 102), Semantic cosine at 4 angles (iter 104), Hybrid 2-way `rrf_sum/max_rrf` at single-mode rank-1 = 0.5 + both-rank-1 = 1.0 (iter 105 + existing). Score component pass-through pinned at all 3 wrapper retrievers: PV preserves every inner field (iter 95), Hybrid_N N=1 + Hybrid 2-way pass through inner score values exactly (iters 82 + 97).
