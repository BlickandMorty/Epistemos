//! `falsify_small_model_runtime_harness_fresh_product_runtime_wrv_probe`.
//!
//! This source/metadata witness proves the fresh small-model product-runtime
//! AnswerPacket route is wired, reachable, visible, and verified at the
//! app-source level while preserving the red L2 capability ceiling and avoiding
//! any new runtime/model-byte load.

use std::collections::BTreeMap;
use std::path::{Path, PathBuf};

#[cfg(test)]
use agent_core::falsifier_artifacts::axes::SMALL_MODEL_RUNTIME_HARNESS_FRESH_PRODUCT_RUNTIME_WRV_PROBE_AXES;
...
use agent_core::uas::{
    ...
    ProStatus, ProductBuild, SmallModelFreshProductRuntimeWrvProbeError,
    SmallModelFreshProductRuntimeWrvSourceRef, SmallModelFreshProductRuntimeWrvSurface,
    SmallModelFreshProductRuntimeWrvTestRef, SmallModelFreshProductRuntimeWrvWitness,
...
const FALSIFIER_ID: &str = "F-SmallModelRuntimeHarnessFreshProductRuntimeWrvProbe";
const ADVANCED_RELEASE_AUDIT_CURSOR: &str =
    "release_audit_distribution_compliance_and_three_uninterrupted_zero_fail_passes";
const FIXTURE_ID: &str = "small_model_runtime_harness_fresh_product_runtime_wrv_probe_v1";
const COMMAND: &str =
...
#[derive(Clone, Copy)]
// UAS: uas:small-model-runtime-harness-fresh-product-runtime-wrv-probe:marker-spec
// Plane: Verification
...
#[derive(Debug)]
// UAS: uas:small-model-runtime-harness-fresh-product-runtime-wrv-probe:witness-error
// Plane: Verification
// Residency: source/fresh product-runtime WRV falsifier errors.
enum FreshProductRuntimeWrvWitnessError {
    Primitive(SmallModelFreshProductRuntimeWrvProbeError),
    Io(String),
    Json(String),
    MissingMarker { path: String, marker: String },
}

impl std::fmt::Display for FreshProductRuntimeWrvWitnessError {
...
impl std::error::Error for FreshProductRuntimeWrvWitnessError {}

impl From<SmallModelFreshProductRuntimeWrvProbeError> for FreshProductRuntimeWrvWitnessError {
    fn from(value: SmallModelFreshProductRuntimeWrvProbeError) -> Self {
...
fn build_artifact(
) -> Result<agent_core::falsifier_artifacts::FalsifierArtifact, FreshProductRuntimeWrvWitnessError>
{
    let evidence = EvidenceSnapshot::read()?;
    let witness = fresh_product_runtime_wrv_witness(&evidence)?;
    let deterministic =
        witness.address() == fresh_product_runtime_wrv_witness(&evidence)?.address();
    ...
    let bool_axes = [
        ...
        (
            ...
            witness.route_authority == "fresh_product_runtime_wrv_source_only_no_route_authority",
        ),
        (
            "upstream_answer_packet_sidecar_bound",
            evidence.answer_packet_sidecar_bound,
        ),
        (
            "upstream_run_event_log_sidecar_bound",
            evidence.run_event_log_sidecar_bound,
        ),
        (
            "upstream_runtime_bytes_loaded_nonzero",
            evidence.upstream_runtime_bytes_loaded > 0,
        ),
        (
            "upstream_model_bytes_loaded_nonzero",
            evidence.upstream_model_bytes_loaded > 0,
        ),
        (
            "packetization_runtime_bytes_zero",
            evidence.packetization_runtime_bytes_loaded == 0,
        ),
        (
            "packetization_model_bytes_zero",
            evidence.packetization_model_bytes_loaded == 0,
        ),
        ...
        (
            ...
            metrics.phase_count == required_fresh_product_runtime_wrv_phases().len() as u64,
        ),
        (
        ...
        (
            "metadata_bound",
            witness.metadata_bytes <= fresh_product_runtime_wrv_metadata_budget_bytes(),
        ),
        (
    ...
    let count_axes = [
        ...
        (
            "phase_count",
            metrics.phase_count,
            required_fresh_product_runtime_wrv_phases().len() as u64,
            "count",
        ),
    ];
    ...
    for (axis, value) in [
        (
            ...
            evidence.packetization_runtime_bytes_loaded,
        ),
        (
            "packetization_model_bytes_loaded",
            evidence.packetization_model_bytes_loaded,
        ),
    ] {
        measurements.insert(
            axis.to_string(),
        ...
        pass_per_axis.insert(axis.to_string(), value == ZERO_BYTES);
        thresholds.insert(
            axis.to_string(),
            AcceptanceThreshold {
                operator: "==".to_string(),
    ...
    thresholds.insert(
        ...
        AcceptanceThreshold {
            operator: "<=".to_string(),
            value: serde_json::json!(fresh_product_runtime_wrv_metadata_budget_bytes()),
            unit: "bytes".to_string(),
        },
    );
    let address = witness.address();
    measurements.insert(
        "small_model_runtime_harness_fresh_product_runtime_wrv_probe_address".to_string(),
        Measurement {
    ...
    thresholds.insert(
        ...
        AcceptanceThreshold {
            operator: "==".to_string(),
            value: measurements
                .get("small_model_runtime_harness_fresh_product_runtime_wrv_probe_address")
    ...
    let anomalies = vec![serde_json::json!({
        "kind": "small_model_fresh_product_runtime_wrv_source_only",
        "detail": "Fresh product-runtime WRV is source/test visible: TriageService, LLMService, NoteChatState, LocalInferenceSerialController, AnswerPacketEmitter, MessageBubble, Settings diagnostics, System G replay, and focused tests are bound to the fresh AnswerPacket/RunEventLog sidecars. This opens no new model/runtime bytes and keeps L2 red."
    })];
    ...
    Ok(ArtifactBuilder {
        ...
        anomalies,
        notes: "L1/L3-source F-SmallModelRuntimeHarnessFreshProductRuntimeWrvProbe: binds fresh product-runtime WRV source/test evidence for the practical small-model route without loading model/runtime bytes, without MAS live-agent overclaim, without 70B/128K promotion, and without L2 capability green."
            .to_string(),
...
fn fresh_product_runtime_wrv_witness(
    evidence: &EvidenceSnapshot,
) -> Result<SmallModelFreshProductRuntimeWrvWitness, FreshProductRuntimeWrvWitnessError> {
    SmallModelFreshProductRuntimeWrvWitness::new(
        "small-model-runtime-harness-fresh-product-runtime-wrv:source-proof",
        ...
        ProductBuild::Pro,
        ProStatus::Gated,
        "fresh_product_runtime_wrv_source_only_no_route_authority",
        evidence.source_refs.clone(),
        product_surfaces(),
        evidence.test_refs.clone(),
        required_fresh_product_runtime_wrv_phases().to_vec(),
        true,
        true,
        true,
        true,
    ...
    .map_err(FreshProductRuntimeWrvWitnessError::from)
}

fn product_surfaces() -> Vec<SmallModelFreshProductRuntimeWrvSurface> {
    ["note-chat", "message-bubble", "settings-diagnostics"]
        .iter()
        .map(|id| SmallModelFreshProductRuntimeWrvSurface {
            surface_id: format!("surface:{id}"),
            admission_ref: "admission:scope_rex:small_model_fresh_product_runtime_wrv_source_only"
                .to_string(),
...
// UAS: uas:small-model-runtime-harness-fresh-product-runtime-wrv-probe:evidence-snapshot
// Plane: Verification
...
struct EvidenceSnapshot {
    ...
    answer_packet_sidecar_bound: bool,
    run_event_log_sidecar_bound: bool,
    upstream_runtime_bytes_loaded: u64,
    upstream_model_bytes_loaded: u64,
    packetization_runtime_bytes_loaded: u64,
    packetization_model_bytes_loaded: u64,
    source_refs: Vec<SmallModelFreshProductRuntimeWrvSourceRef>,
    test_refs: Vec<SmallModelFreshProductRuntimeWrvTestRef>,
...
impl EvidenceSnapshot {
    fn read() -> Result<Self, FreshProductRuntimeWrvWitnessError> {
        ...
        Ok(Self {
            ...
            answer_packet_sidecar_bound: Path::new(ANSWER_PACKET_SIDECAR_PATH).exists(),
            run_event_log_sidecar_bound: Path::new(RUN_EVENT_LOG_SIDECAR_PATH).exists(),
            upstream_runtime_bytes_loaded: measurement_u64(&answer_packet, "upstream_runtime_bytes_loaded")
                .unwrap_or(0),
            upstream_model_bytes_loaded: measurement_u64(&answer_packet, "upstream_model_bytes_loaded")
                .unwrap_or(0),
            packetization_runtime_bytes_loaded: measurement_u64(
                &answer_packet,
                "packetization_runtime_bytes_loaded",
            )
            .unwrap_or(u64::MAX),
            packetization_model_bytes_loaded: measurement_u64(
                &answer_packet,
                "packetization_model_bytes_loaded",
            )
            .unwrap_or(u64::MAX),
            ...
            living_index_has_current_cursor: living_index
                .contains(SMALL_MODEL_RUNTIME_HARNESS_FRESH_PRODUCT_RUNTIME_WRV_PROBE_CURSOR),
...
#[derive(Default)]
// UAS: uas:small-model-runtime-harness-fresh-product-runtime-wrv-probe:invalid-axes
// Plane: Verification
// Residency: invalid fixture coverage for fresh product-runtime WRV rejection paths.
struct InvalidAxes {
...
fn invalid_rejections(witness: &SmallModelFreshProductRuntimeWrvWitness) -> InvalidAxes {
    let mutate = |mutator: fn(&mut SmallModelFreshProductRuntimeWrvWitness)| {
    ...
    InvalidAxes {
        ...
        metadata_budget_rejected: mutate(|candidate| {
            candidate.metadata_bytes = fresh_product_runtime_wrv_metadata_budget_bytes() + 1;
        }),
    }
}

fn mutate_surface(
    witness: &SmallModelFreshProductRuntimeWrvWitness,
    mutator: fn(&mut SmallModelFreshProductRuntimeWrvSurface),
) -> bool {
    let mut candidate = witness.clone();
...
fn collect_source_refs(
    specs: &[MarkerSpec],
) -> Result<Vec<SmallModelFreshProductRuntimeWrvSourceRef>, FreshProductRuntimeWrvWitnessError> {
    specs
        .iter()
        .map(|spec| {
            let markers = collect_markers(spec)?;
            Ok(SmallModelFreshProductRuntimeWrvSourceRef {
...
fn collect_test_refs(
    specs: &[MarkerSpec],
) -> Result<Vec<SmallModelFreshProductRuntimeWrvTestRef>, FreshProductRuntimeWrvWitnessError> {
    specs
        .iter()
        .map(|spec| {
            let markers = collect_markers(spec)?;
            Ok(SmallModelFreshProductRuntimeWrvTestRef {
                ref_id: spec.ref_id.to_string(),
                path: spec.path.to_string(),
                markers,
            })
...
fn collect_markers(spec: &MarkerSpec) -> Result<Vec<String>, FreshProductRuntimeWrvWitnessError> {
    let text = read_to_string(Path::new(spec.path))?;
    let mut markers = Vec::with_capacity(spec.markers.len());
    for marker in spec.markers {
        if !text.contains(marker) {
            return Err(FreshProductRuntimeWrvWitnessError::MissingMarker {
                path: spec.path.to_string(),
...
fn read_to_string(path: &Path) -> Result<String, FreshProductRuntimeWrvWitnessError> {
    std::fs::read_to_string(path).map_err(|error| {
        FreshProductRuntimeWrvWitnessError::Io(format!("{}: {error}", path.display()))
    })
}

fn read_json(path: &Path) -> Result<serde_json::Value, FreshProductRuntimeWrvWitnessError> {
    let text = read_to_string(path)?;
    serde_json::from_str(&text).map_err(|error| {
        FreshProductRuntimeWrvWitnessError::Json(format!("{}: {error}", path.display()))
    })
}
...
fn json_bool(
    ...
    key: &str,
) -> Result<bool, FreshProductRuntimeWrvWitnessError> {
    value
        .get(key)
        .and_then(serde_json::Value::as_bool)
        .ok_or_else(|| FreshProductRuntimeWrvWitnessError::Json(format!("missing bool `{key}`")))
}
...
fn measurement_u64(value: &serde_json::Value, key: &str) -> Option<u64> {
    value.get("measurements")?.get(key)?.get("value")?.as_u64()
}
...
mod tests {
    ...
    fn declared_axes_are_non_empty_and_unique() {
        ...
        for axis in SMALL_MODEL_RUNTIME_HARNESS_FRESH_PRODUCT_RUNTIME_WRV_PROBE_AXES {
