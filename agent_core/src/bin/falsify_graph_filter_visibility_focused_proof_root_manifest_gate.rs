//! `falsify_graph_filter_visibility_focused_proof_root_manifest_gate`.
//!
//! Metadata-only manifest gate for the first focused graph-filter proof root.
//! This does not run Xcode, open test products, mutate source, or claim release
//! readiness; it binds the parser contract that future focused evidence must
//! satisfy before it can count.

use std::collections::BTreeMap;
use std::path::PathBuf;

use agent_core::falsifier_artifacts::axes::GRAPH_FILTER_VISIBILITY_FOCUSED_PROOF_ROOT_MANIFEST_GATE_AXES;
...
use agent_core::uas::{
    ...
    GraphFilterFocusedProofRootManifestGate,
...
const FALSIFIER_ID: &str = "F-GraphFilterVisibilityFocusedProofRootManifestGate";
const FIXTURE_ID: &str = "graph_filter_visibility_focused_proof_root_manifest_gate_v1";
const COMMAND: &str =
...
fn build_artifact(
    ...
    let witness = GraphFilterVisibilityFocusedProofRootManifestGateWitness::new(
    ...
    for (axis, pass) in [
        ("upstream_evidence_envelope_pass", upstream.overall_pass),
        (
            "upstream_ref_bound",
        ...
        (
            "manifest_name_bound",
            witness.spec.manifest_name == "focused-proof-root-manifest.json",
        ),
        (
            "required_manifest_fields_bound",
            witness.metrics.required_manifest_field_count
                == required_graph_filter_focused_proof_root_manifest_fields().len(),
        ),
        (
            "selected_test_product_kinds_bound",
            witness.metrics.selected_test_product_kind_count
                == required_graph_filter_focused_proof_root_selected_product_kinds().len(),
        ),
        (
            "required_rejection_policies_bound",
            witness.metrics.required_rejection_policy_count
                == required_graph_filter_focused_proof_root_rejection_policies().len(),
        ),
        (
        ...
        (
            ...
            witness.spec.selected_product_digest_required,
        ),
        (
        ...
        (
            ...
            witness.spec.selected_test_product_bytes_opened == 0,
        ),
        (
            "no_model_or_app_runtime_bytes",
            witness.spec.model_runtime_bytes_loaded == 0
                && witness.spec.app_runtime_bytes_loaded == 0,
        ),
        (
    ...
    add_count_eq_axis(
        ...
        required_graph_filter_focused_proof_root_manifest_fields().len() as u64,
        "fields",
    );
    add_count_eq_axis(
        &mut measurements,
        ...
        required_graph_filter_focused_proof_root_selected_product_kinds().len() as u64,
        "kinds",
    );
    add_count_eq_axis(
        &mut measurements,
        ...
        required_graph_filter_focused_proof_root_rejection_policies().len() as u64,
        "policies",
    );
    add_count_eq_axis(
        &mut measurements,
    ...
    measurements.insert(
        "graph_filter_visibility_focused_proof_root_manifest_gate".to_string(),
        Measurement {
    ...
    pass_per_axis.insert(
        "next_cursor".to_string(),
        witness.next_cursor == GRAPH_FILTER_VISIBILITY_FOCUSED_PROOF_ROOT_MANIFEST_GATE_NEXT_CURSOR,
    );

    for axis in GRAPH_FILTER_VISIBILITY_FOCUSED_PROOF_ROOT_MANIFEST_GATE_AXES {
        measurements
    ...
    Ok(ArtifactBuilder {
        ...
        anomalies: Vec::new(),
        notes: "metadata-only F-GraphFilterVisibilityFocusedProofRootManifestGate: consumes the fresh test-products evidence envelope and binds the future focused proof-root manifest/parser contract: source status digests, selected test-product digest, enumeration digest, focused xcresult digest, nonzero executed tests, pre-action accounting, full automated-check row preservation, rollback, RunEventLog, AnswerPacket, zero Xcode execution, zero test-product/model/runtime bytes, and no L2/L3/product/release/large-model promotion.".to_string(),
...
fn red_fixture_results(
    witness: &GraphFilterVisibilityFocusedProofRootManifestGateWitness,
) -> Vec<(&'static str, bool)> {
    ...
    for fixture in red_fixture_cases() {
        let rejected = match fixture {
            ...
            RedFixture::MutateSpec { mutate, .. } => {
                let mut spec = GraphFilterFocusedProofRootManifestGate::canonical();
                mutate(&mut spec);
...
// UAS: F-GraphFilterVisibilityFocusedProofRootManifestGate red-fixture set.
// Plane: Verification.
// Residency: metadata-only fixture contract; no Xcode or test-product bytes are loaded.
enum RedFixture {
    UpstreamFail,
    ...
    MutateSpec {
        name: &'static str,
        mutate: fn(&mut GraphFilterFocusedProofRootManifestGate),
    },
}
...
fn red_fixture_cases() -> Vec<RedFixture> {
    vec![
        ...
        RedFixture::MutateSpec {
            ...
            mutate: |spec| {
                spec.required_manifest_fields
                    .retain(|field| field != "focused_result_bundle_digest")
            },
        },
        RedFixture::MutateSpec {
            name: "missing_selected_product_kind_rejected",
            mutate: |spec| {
                spec.selected_test_product_kinds
                    .pop()
                    .map(|_| ())
                    .unwrap_or(())
            },
        ...
        RedFixture::MutateSpec {
            ...
            mutate: |spec| {
                spec.required_rejection_policies
                    .retain(|policy| policy != "focused_proof_replaces_full_row")
            },
        },
        RedFixture::MutateSpec {
        ...
        RedFixture::MutateSpec {
            ...
            mutate: |spec| spec.selected_product_digest_required = false,
        },
        RedFixture::MutateSpec {
        ...
        RedFixture::MutateSpec {
            name: "focused_result_bundle_digest_missing_rejected",
            mutate: |spec| spec.focused_result_bundle_digest_required = false,
        },
        RedFixture::MutateSpec {
            name: "scheme_pre_action_accounting_missing_rejected",
...
// UAS: uas:graph-filter-visibility-focused-proof-root-manifest-gate:upstream-envelope
// Plane: Verification.
// Residency: parsed upstream metadata only.
struct UpstreamEvidenceEnvelope {
    overall_pass: bool,
    address: String,
    next_cursor: String,
}

fn read_upstream() -> Result<UpstreamEvidenceEnvelope, Box<dyn std::error::Error>> {
    ...
    let address = value["measurements"]["automated_checks_fresh_test_products_envelope_address"]
        ["value"]
        .as_str()
        .unwrap_or_default()
        .to_string();
