//! `falsify_small_model_runtime_harness_answer_packet_runtime_probe`.
//!
//! This witness packetizes the retained first-token runtime sidecar into the
...
#[cfg(test)]
use agent_core::falsifier_artifacts::axes::SMALL_MODEL_RUNTIME_HARNESS_ANSWER_PACKET_RUNTIME_PROBE_AXES;
use agent_core::falsifier_artifacts::axes::SMALL_MODEL_RUNTIME_HARNESS_FIRST_TOKEN_RUNTIME_PROBE_AXES;
...
use agent_core::uas::{
    redacted_first_token_run_event_log, required_answer_packet_runtime_probe_phases, ProStatus,
    ProductBuild, SmallModelAnswerPacketRuntimeProbePacket,
...
#[derive(Debug)]
// UAS: uas:small-model-runtime-harness-answer-packet-runtime-probe:witness-error
// Plane: Verification
// Residency: retained packetization rejection taxonomy.
enum AnswerPacketWitnessError {
    Primitive(SmallModelRuntimeHarnessAnswerPacketProbeError),
    Io(String),
    Json(String),
}
...
impl From<SmallModelRuntimeHarnessAnswerPacketProbeError> for AnswerPacketWitnessError {
    fn from(value: SmallModelRuntimeHarnessAnswerPacketProbeError) -> Self {
...
fn build_artifact(
    ...
    let deterministic = SmallModelRuntimeHarnessAnswerPacketProbeWitness::new(
        "small-model-answer-packet-runtime-probe",
    ...
    let bool_axes = [
        ...
        (
            ...
            witness.route_authority == "packetized_first_token_visible_proof_only",
        ),
        (
        ...
        (
            ...
            metrics.phase_count == required_answer_packet_runtime_probe_phases().len() as u64,
        ),
        (
    ...
    let count_axes = [
        ...
        (
            "phase_count",
            metrics.phase_count,
            required_answer_packet_runtime_probe_phases().len() as u64,
            "count",
        ),
    ...
    measurements.insert(
        "small_model_runtime_harness_answer_packet_runtime_probe_address".to_string(),
        Measurement {
    ...
    thresholds.insert(
        ...
        AcceptanceThreshold {
            operator: "==".to_string(),
            value: measurements
                .get("small_model_runtime_harness_answer_packet_runtime_probe_address")
    ...
    let anomalies = vec![serde_json::json!({
        ...
        "detail": "The retained first-token sidecar is packetized into a real Rust AnswerPacket and dense RunEventLog with redacted token text, no new runtime/model bytes, rollback, admission, privacy, budget, and L1/L2/L3 separation. This advances L1 only; L2 capability and L3 product WRV remain red."
    })];
    ...
    Ok(ArtifactBuilder {
        ...
        anomalies,
        notes: "retained L1 F-SmallModelRuntimeHarnessAnswerPacketRuntimeProbe: packetizes the Qwen3-4B first-token sidecar through real AnswerPacket + RunEventLog proof only; no new inference, no new model bytes, no MAS/product route, no 70B, no 128K shard, no hidden cloud, and no L2/L3 promotion."
            .to_string(),
...
> {
    ...
    let mut answer_packet = produce_turn_completion_packet(TurnCompletionInputs {
        packet_id: "answer_packet:qwen3_4b:first-token-runtime:packetized".to_string(),
        stop_reason: "end_turn".to_string(),
        output_tokens: 1,
        attention_mode: AttentionMode::Dynamic,
        vrm_label: VrmLabel::PlausibleButUnverified,
        witnessed_state_id: "witnessed_state:first_token:qwen3_4b:packetized".to_string(),
    ...
    .push_claim(
        Claim::new(
            ClaimId::new("qwen3_4b_first_token_packetized_code_invariant"),
    ...
    let run_event_log = redacted_first_token_run_event_log(&token_digest_ref);
    ...
    let packet = SmallModelAnswerPacketRuntimeProbePacket::new(
        "small_model_answer_packet_runtime_probe_packet",
        "artifact:small_model_runtime_harness_first_token_runtime_probe:result",
        format!("artifact:small_model_runtime_harness_first_token_runtime_probe:live_probe:{packet_sha}"),
        ...
        run_event_log,
        packet_sha,
        evidence.live_probe.runtime_bytes_loaded,
        evidence.live_probe.model_bytes_loaded,
    )?;
    let witness = SmallModelRuntimeHarnessAnswerPacketProbeWitness::new(
        "small-model-answer-packet-runtime-probe",
        "artifact:small_model_runtime_harness_first_token_runtime_probe:result",
        ...
        vec![
            ...
            surface(
                ...
                vec![
                    "Epistemos is a local cognitive substrate".to_string(),
                    "small_model_runtime_harness_answer_packet_runtime_probe".to_string(),
                    "F-SmallModelRuntimeHarnessFirstTokenRuntimeProbe".to_string(),
                ],
            )?,
        ],
...
) -> Result<SmallModelAnswerPacketRuntimeProbeSurface, AnswerPacketWitnessError> {
    Ok(SmallModelAnswerPacketRuntimeProbeSurface::new(
        surface_id,
        path,
        observed_text,
        required_markers,
...
#[derive(Clone)]
// UAS: Aggregates upstream first-token and canon evidence for the AnswerPacket runtime probe.
// Plane: Verification.
// Residency: Reads retained sidecars and docs only; no model/runtime bytes are opened.
...
impl EvidenceSnapshot {
    fn read() -> Result<Self, AnswerPacketWitnessError> {
        ...
        let first_token = read_json(Path::new(FIRST_TOKEN_PATH))?;
        Ok(Self {
            ...
            capability_next_bottleneck: measurement_string(&capability, "next_bottleneck")
                .unwrap_or_else(|| "unset".to_string()),
            first_token_pass: artifact_all_axes_true(
                &first_token,
...
#[derive(Clone, Debug)]
// UAS: Retained redacted sidecar schema consumed by the AnswerPacket runtime probe.
// Plane: Verification.
// Residency: CurrentApp first-token runtime bytes are inherited as evidence, not reloaded.
...
#[derive(Default)]
// UAS: Negative-fixture ledger proving malformed packetized first-token evidence rejects.
// Plane: Verification.
...
fn invalid_fixture_axes(
    ...
    let mutate_packet = |mutator: fn(&mut SmallModelAnswerPacketRuntimeProbePacket)| {
        let mut packet = witness.packets[0].clone();
        mutator(&mut packet);
        packet.validate().is_err()
    };
    let mutate_witness = |mutator: fn(&mut SmallModelRuntimeHarnessAnswerPacketProbeWitness)| {
        let mut candidate = witness.clone();
        mutator(&mut candidate);
        candidate.validate().is_err()
    };
    Ok(InvalidAxes {
        missing_first_token_artifact_rejected: mutate_packet(|packet| {
            packet.first_token_artifact_ref.clear();
        }),
        ...
        run_event_log_error_rejected: mutate_packet(|packet| {
            let mut log = redacted_first_token_run_event_log(&packet.token_digest_ref);
...
mod tests {
    ...
    fn declared_axes_have_no_duplicates() {
        ...
        for axis in SMALL_MODEL_RUNTIME_HARNESS_ANSWER_PACKET_RUNTIME_PROBE_AXES {
