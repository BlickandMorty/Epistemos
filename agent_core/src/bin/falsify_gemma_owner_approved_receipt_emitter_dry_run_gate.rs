//! `falsify_gemma_owner_approved_receipt_emitter_dry_run_gate`
//!
//! Metadata-only witness for the Gemma receipt-emitter dry-run boundary. It
//! proves the emitter shape exists without writing a receipt, touching files,
//! executing tools, loading model bytes, or promoting a route.

use std::collections::BTreeMap;
use std::path::PathBuf;

use agent_core::falsifier_artifacts::axes::GEMMA_OWNER_APPROVED_RECEIPT_EMITTER_DRY_RUN_GATE_AXES;
...
const FALSIFIER_ID: &str = GEMMA_OWNER_APPROVED_RECEIPT_EMITTER_DRY_RUN_GATE_ID;
const FIXTURE_ID: &str = "gemma_owner_approved_receipt_emitter_dry_run_gate_v1";
const COMMAND: &str = "Tools/falsifiers/f_gemma_owner_approved_receipt_emitter_dry_run_gate.sh";
const RESULT: &str =
...
fn main() -> std::process::ExitCode {
    ...
    println!(
        "{FALSIFIER_ID}: overall_pass={} sections={} fields={} outputs={} red_fixture_rejection_count={} artifact={RESULT}",
        artifact.overall_pass,
        artifact.measurements["emitter_section_count"].value,
        artifact.measurements["receipt_field_count"].value,
        artifact.measurements["dry_run_output_count"].value,
...
fn build_artifact(
    ...
    for (name, passed) in [
        ("upstream_receipt_intake_gate_pass", upstream_pass),
        (
            "upstream_receipt_intake_gate_ref_bound",
            gate.upstream_intake_gate_ref
                == GEMMA_OWNER_APPROVED_RECEIPT_EMITTER_DRY_RUN_GATE_UPSTREAM_REF,
        ),
        (
            "emitter_sections_bound",
            metrics.emitter_section_count == 7
                && red_pass(&red_results, "missing_emitter_section")
                && red_pass(&red_results, "duplicate_emitter_section"),
        ),
        (
            "receipt_fields_bound",
            metrics.receipt_field_count == 24
        ...
        (
            "dry_run_outputs_bound",
            metrics.dry_run_output_count == 9
                && red_pass(&red_results, "missing_dry_run_output")
                && red_pass(&red_results, "duplicate_dry_run_output"),
        ),
        (
            "shortcut_and_rejection_policy_bound",
            metrics.denied_shortcut_count == 16
                && metrics.rejection_policy_count == 37
        ...
        (
            "receipt_payload_not_written_or_read",
        ...
        (
            "gemma_owner_approved_receipt_emitter_dry_run_gate_address_deterministic",
            gate.address() == GemmaOwnerApprovedReceiptEmitterDryRunGate::canonical().address(),
        ),
        (
            "next_cursor_bound",
            gate.next_cursor == GEMMA_OWNER_APPROVED_RECEIPT_EMITTER_DRY_RUN_GATE_NEXT_CURSOR,
        ),
    ] {
        add_bool_axis(
            &mut measurements,
    ...
    for (name, value, operator, expected, unit) in [
        (
            "emitter_section_count",
            metrics.emitter_section_count,
            "==",
            7,
            "sections",
        ),
        ...
        (
            "dry_run_output_count",
            metrics.dry_run_output_count,
            "==",
            9,
            "outputs",
        ),
    ...
    add_text_axis(
        ...
        &mut thresholds,
        &mut pass_per_axis,
        "gemma_owner_approved_receipt_emitter_dry_run_gate_address",
...
fn red_fixture_results(
    gate: &GemmaOwnerApprovedReceiptEmitterDryRunGate,
) -> Vec<(&'static str, bool)> {
    let mut fixtures: Vec<(
        ...
        (
            ...
            Box::new(|mut g| {
                g.required_emitter_sections.pop();
                g
            }),
        ),
        (
            ...
            Box::new(|mut g| {
                g.required_emitter_sections
                    .push(g.required_emitter_sections[0].clone());
                g
            }),
        ),
        (
        ...
        (
            ...
            Box::new(|mut g| {
                g.required_dry_run_outputs.pop();
                g
            }),
        ),
        (
            ...
            Box::new(|mut g| {
                g.required_dry_run_outputs
                    .push(g.required_dry_run_outputs[0].clone());
                g
            }),
        ),
        (
            ...
            Box::new(|mut g| {
                g.denied_emitter_shortcuts.pop();
                g
            }),
        ),
        (
            ...
            Box::new(|mut g| {
                g.denied_emitter_shortcuts
                    .push(g.denied_emitter_shortcuts[0].clone());
                g
            }),
        ),
        (
...
fn assert_axis_coverage(measurements: &BTreeMap<String, Measurement>) {
    for axis in GEMMA_OWNER_APPROVED_RECEIPT_EMITTER_DRY_RUN_GATE_AXES {
        assert!(
