//! `falsify_gemma_direct_harness_owner_approved_command_envelope_gate`
//!
//! Metadata-only command envelope gate for a future owner-approved Gemma
//! direct-harness dry-run receipt attempt. It reads only the upstream preflight
//! packet artifact, writes no command envelope, opens no paths, arms no command,
//! spawns no process, and promotes no route.

use std::collections::BTreeMap;
use std::path::{Path, PathBuf};

use agent_core::falsifier_artifacts::axes::GEMMA_DIRECT_HARNESS_OWNER_APPROVED_COMMAND_ENVELOPE_GATE_AXES;
...
const FALSIFIER_ID: &str = GEMMA_DIRECT_HARNESS_OWNER_APPROVED_COMMAND_ENVELOPE_GATE_ID;
const FIXTURE_ID: &str = "gemma_direct_harness_owner_approved_command_envelope_gate_v1";
const COMMAND: &str =
...
fn main() -> std::process::ExitCode {
    ...
    println!(
        "{FALSIFIER_ID}: overall_pass={} command_fields={} command_executed={} process_spawned={} raw_stdout_bytes={} red_fixture_rejection_count={} artifact={RESULT}",
        artifact.overall_pass,
        artifact.measurements["required_command_envelope_field_count"].value,
        artifact.measurements["command_executed_count"].value,
        artifact.measurements["process_spawned_count"].value,
        artifact.measurements["raw_stdout_bytes"].value,
...
fn build_artifact(
    ...
    for (name, passed) in [
        ("upstream_preflight_packet_gate_pass", upstream_pass),
        (
            "upstream_preflight_packet_ref_bound",
            gate.upstream_preflight_ref
                == GEMMA_DIRECT_HARNESS_OWNER_APPROVED_COMMAND_ENVELOPE_GATE_UPSTREAM_REF,
        ),
        (
            "command_fields_and_abort_conditions_bound",
            metrics.required_command_envelope_field_count == 35
                && metrics.required_abort_condition_count == 58
                && red_pass(&red_results, "missing_command_field")
                && red_pass(&red_results, "duplicate_command_field")
                && red_pass(&red_results, "missing_abort_condition")
                && red_pass(&red_results, "duplicate_abort_condition"),
        ),
        (
            "owner_path_model_llama_identity_bound",
        ...
        (
            "hardware_memory_command_policy_bound",
            gate.hardware_and_memory_verdict_required
                && gate.argv_and_env_allowlists_required
                && red_pass(&red_results, "hardware_memory_missing")
                && red_pass(&red_results, "argv_env_allowlists_missing"),
        ),
        (
            "shell_network_hub_denials_bound",
            gate.shell_string_denied
                && gate.network_and_hub_download_denied
                && red_pass(&red_results, "shell_string_allowed")
                && red_pass(&red_results, "network_hub_allowed"),
        ),
        (
            "prompt_timeout_stdio_output_token_sampler_bound",
            gate.prompt_and_grammar_digest_required
                && gate.timeout_cancel_teardown_required
                && gate.stdio_redaction_required
                && gate.output_byte_cap_required
                && gate.token_digest_policy_required
                && gate.memory_sampler_required
                ...
                && red_pass(&red_results, "output_token_sampler_missing"),
        ),
        (
        ...
        (
            "no_execution_and_no_promotion_bound",
            gate.no_execution_bound
                && gate.no_promotion_bound
                && red_pass(&red_results, "no_execution_missing")
                && red_pass(&red_results, "no_promotion_missing"),
        ),
        (
            "command_envelope_write_read_deferred",
            metrics.future_command_envelope_written_count == 0
                && metrics.future_command_envelope_bytes_written == 0
                && metrics.future_command_envelope_bytes_read == 0
                && red_pass(&red_results, "command_envelope_written")
                && red_pass(&red_results, "command_envelope_bytes_written")
                && red_pass(&red_results, "command_envelope_bytes_read"),
        ),
        (
            "zero_owner_path_command_process_file_model_runtime_provider_actions",
        ...
        (
            "gemma_direct_harness_command_envelope_address_deterministic",
    ...
    for (name, value, operator, expected, unit) in [
        (
            "required_command_envelope_field_count",
            metrics.required_command_envelope_field_count,
            "==",
            35,
            "fields",
        ),
        ...
        (
            "future_command_envelope_written_count",
            metrics.future_command_envelope_written_count,
            "==",
            0,
            "envelopes",
        ),
        (
            "future_command_envelope_bytes_written",
            metrics.future_command_envelope_bytes_written,
            "==",
            0,
            "bytes",
        ),
        (
            "future_command_envelope_bytes_read",
            metrics.future_command_envelope_bytes_read,
            "==",
            0,
            "bytes",
        ),
    ...
    add_text_axis(
        ...
        &mut thresholds,
        &mut pass_per_axis,
        "gemma_direct_harness_command_envelope_gate_address",
        &gate
            .command_envelope_gate_address(CREATED_AT_MS)
            .to_string(),
        "non_empty",
    );
    add_text_axis(
    ...
    Ok(ArtifactBuilder {
        ...
        anomalies: Vec::new(),
        notes: "metadata-only F-GemmaDirectHarnessOwnerApprovedCommandEnvelopeGate: consumes the landed Gemma direct-harness owner-approved preflight packet gate and freezes the inert command envelope contract before any dry-run receipt can approach execution. It binds owner/path/model/llama.cpp identity, hardware and memory verdicts, argv/environment allowlists, shell/network/hub-download denials, prompt/grammar policy, timeout/cancel/teardown, stdio redaction, output byte caps, token digest policy, memory sampling, rollback, RunEventLog, AnswerPacket, abstention, human confirmation, no-execution proof, and non-promotion. It writes zero command envelopes, opens zero owner/model/llama-cli paths, arms or executes zero commands, spawns zero processes, captures zero raw path/prompt/output/stdout/stderr/token bytes, mutates no RuntimeRouter/System G/settings/default state, and makes no Gemma live/default/L2/L3/T4/user-facing, quality, live dense 70B, or SSD-as-RAM claim.".to_string(),
...
fn red_fixture_results(
    gate: &GemmaDirectHarnessOwnerApprovedCommandEnvelopeGate,
) -> Vec<(&'static str, bool)> {
    let cases: Vec<(
        &'static str,
        Box<dyn Fn(&mut GemmaDirectHarnessOwnerApprovedCommandEnvelopeGate)>,
    )> = vec![
        (
            "missing_command_field",
            Box::new(|g| {
                g.required_command_envelope_fields.pop();
            }),
        ),
        (
            "duplicate_command_field",
            Box::new(|g| {
                g.required_command_envelope_fields[0] =
                    g.required_command_envelope_fields[1].clone();
            }),
        ),
        (
        ...
        (
            ...
            Box::new(|g| {
                g.artifact_root_prefix =
                    "artifacts/falsifiers/gemma_direct_harness_wrong_command_gate/".to_string();
            }),
        ),
        (
            "bad_next_cursor",
        ...
        (
            "hardware_memory_missing",
            Box::new(|g| g.hardware_and_memory_verdict_required = false),
        ),
        (
            "argv_env_allowlists_missing",
            Box::new(|g| g.argv_and_env_allowlists_required = false),
        ),
        (
            "shell_string_allowed",
            Box::new(|g| g.shell_string_denied = false),
        ),
        (
            "network_hub_allowed",
            Box::new(|g| g.network_and_hub_download_denied = false),
        ),
        (
            "prompt_grammar_missing",
            Box::new(|g| g.prompt_and_grammar_digest_required = false),
        ),
        (
            "timeout_cancel_teardown_missing",
            Box::new(|g| g.timeout_cancel_teardown_required = false),
        ),
        (
            "stdio_redaction_missing",
            Box::new(|g| g.stdio_redaction_required = false),
        ),
        (
            "output_token_sampler_missing",
            Box::new(|g| {
                g.output_byte_cap_required = false;
                g.token_digest_policy_required = false;
                g.memory_sampler_required = false;
            }),
        ),
        (
        ...
        (
            "no_execution_missing",
            Box::new(|g| g.no_execution_bound = false),
        ),
        (
        ...
        (
            ...
            Box::new(|g| g.future_command_envelope_written_count = 1),
        ),
        (
            "command_envelope_bytes_written",
            Box::new(|g| g.future_command_envelope_bytes_written = 1),
        ),
        (
            "command_envelope_bytes_read",
            Box::new(|g| g.future_command_envelope_bytes_read = 1),
        ),
        (
            "owner_path_opened",
...
fn assert_axis_coverage(measurements: &BTreeMap<String, Measurement>) {
    for axis in GEMMA_DIRECT_HARNESS_OWNER_APPROVED_COMMAND_ENVELOPE_GATE_AXES {
        assert!(
