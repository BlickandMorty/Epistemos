//! `falsify_small_model_runtime_harness_product_route_capability_recheck`.
//!
//! This witness rechecks product-route capability after retained AnswerPacket
//! handoff evidence and records the exact red L2/L3 blockers before any fresh
//! product runtime safety lease can be attempted.
...
use agent_core::uas::{
    ...
    ProStatus, ProductBuild, SmallModelProductRouteCapabilityBlocker,
    SmallModelProductRouteCapabilityRecheckError, SmallModelProductRouteCapabilityRecheckWitness,
...
const FALSIFIER_ID: &str = "F-SmallModelRuntimeHarnessProductRouteCapabilityRecheck";
const ADVANCED_RELEASE_AUDIT_CURSOR: &str =
    "release_audit_distribution_compliance_and_three_uninterrupted_zero_fail_passes";
const FIXTURE_ID: &str = "small_model_runtime_harness_product_route_capability_recheck_v1";
const COMMAND: &str =
...
const PRODUCT_HANDOFF_PATH: &str =
    "artifacts/falsifiers/small_model_runtime_harness_product_answer_packet_live_probe/result.json";
const EXPECTED_BLOCKER_COUNT: u64 = 6;
const ZERO_BYTES: u64 = 0;

#[derive(Debug)]
// UAS: uas:small-model-runtime-harness-product-route-capability-recheck:witness-error
// Plane: Verification
// Residency: falsifier IO/JSON/primitive error wrapper.
enum ProductRouteCapabilityRecheckWitnessError {
    Primitive(SmallModelProductRouteCapabilityRecheckError),
    Io(String),
    Json(String),
}

impl std::fmt::Display for ProductRouteCapabilityRecheckWitnessError {
...
impl std::error::Error for ProductRouteCapabilityRecheckWitnessError {}

impl From<SmallModelProductRouteCapabilityRecheckError>
    for ProductRouteCapabilityRecheckWitnessError
{
    fn from(value: SmallModelProductRouteCapabilityRecheckError) -> Self {
...
fn build_artifact() -> Result<
    ...
    ProductRouteCapabilityRecheckWitnessError,
> {
    let evidence = EvidenceSnapshot::read()?;
    let witness = product_route_capability_recheck_witness(&evidence)?;
    let deterministic =
        witness.address() == product_route_capability_recheck_witness(&evidence)?.address();
    ...
    let required_blockers_present = required_product_route_capability_blockers()
        .into_iter()
        .all(|required| {
            witness
                .blockers
    ...
    let bool_axes = [
        ...
        (
            ...
            witness.route_authority == "capability_recheck_no_route_authority",
        ),
        (
        ...
        (
            ...
            evidence.fresh_product_runtime_bytes_loaded == 0,
        ),
        (
        ...
        (
            ...
            metrics.phase_count == required_product_route_capability_recheck_phases().len() as u64,
        ),
        (
        ...
        (
            ...
            invalid_axes.missing_product_handoff_artifact_rejected,
        ),
        (
    ...
    measurements.insert(
        "small_model_runtime_harness_product_route_capability_recheck_address".to_string(),
        Measurement {
    ...
    thresholds.insert(
        ...
        AcceptanceThreshold {
            operator: "==".to_string(),
            value: measurements
                .get("small_model_runtime_harness_product_route_capability_recheck_address")
    ...
    thresholds.insert(
        ...
        AcceptanceThreshold {
            operator: "==".to_string(),
            value: serde_json::json!(required_product_route_capability_blockers()),
            unit: "ids".to_string(),
        },
    );

    let anomalies = vec![serde_json::json!({
        "kind": "small_model_product_route_capability_recheck_red",
        "detail": "Retained Qwen3-4B AnswerPacket handoff evidence is present, but product capability remains red. Fresh app runtime, MAS live-agent, live 70B, and KV-Direct 128K claims stay gated until a separate safety lease and runtime witness exist."
    })];
    ...
    Ok(ArtifactBuilder {
        ...
        anomalies,
        notes: "L1 F-SmallModelRuntimeHarnessProductRouteCapabilityRecheck: records the red L2/L3 product-route blockers after retained AnswerPacket handoff, keeps all fresh product runtime/model bytes at zero, and advances only to a fresh product runtime safety lease."
            .to_string(),
...
fn product_route_capability_recheck_witness(
    evidence: &EvidenceSnapshot,
) -> Result<SmallModelProductRouteCapabilityRecheckWitness, ProductRouteCapabilityRecheckWitnessError>
{
    ...
    .map_err(ProductRouteCapabilityRecheckWitnessError::from)
}

// UAS: uas:small-model-runtime-harness-product-route-capability-recheck:evidence-snapshot
// Plane: Verification
// Residency: kernel/guard/handoff state consumed by the red recheck.
...
impl EvidenceSnapshot {
    fn read() -> Result<Self, ProductRouteCapabilityRecheckWitnessError> {
        let guard = read_json(Path::new(GUARD_PATH))?;
        let capability = read_json(Path::new(CAPABILITY_PATH))?;
        let product_handoff = read_json(Path::new(PRODUCT_HANDOFF_PATH))?;
        Ok(Self {
            ...
            capability_next_bottleneck: measurement_string(&capability, "next_bottleneck")
                .unwrap_or_default(),
            product_handoff_pass: artifact_all_axes_true(&product_handoff),
            retained_runtime_bytes_loaded: measurement_u64(
                &product_handoff,
                "retained_runtime_bytes_loaded",
            )
            .unwrap_or(0),
            fresh_product_runtime_bytes_loaded: measurement_u64(
                &product_handoff,
                "fresh_product_runtime_bytes_loaded",
            )
            .unwrap_or(0),
            ...
            seventy_b_route_pass: measurement_bool(&capability, "seventy_b_route_pass")
                .unwrap_or(false),
            metadata_bytes: std::fs::metadata(PRODUCT_HANDOFF_PATH)
...
fn capability_blockers(
    ...
    [
        ...
        (
            ...
            "controller",
            "evidence:product_runtime:fresh_bytes_zero",
        ),
        (
            "blocker:l3_fresh_app_runtime_unverified",
            "verification",
            "evidence:l3:fresh_app_runtime_unverified",
        ),
        (
    ...
    .into_iter()
    .map(
        |(blocker_id, plane, evidence_ref)| SmallModelProductRouteCapabilityBlocker {
            blocker_id: blocker_id.to_string(),
            plane: plane.to_string(),
            product_build: ProductBuild::Pro,
            pro_status: ProStatus::Gated,
            evidence_ref: evidence_ref.to_string(),
            answer_packet_ref: "answer_packet:product_route_capability_recheck:red".to_string(),
            rollback_ref: "rollback:no_product_route_promotion".to_string(),
            budget_ref: "budget:zero_fresh_product_runtime_bytes".to_string(),
...
#[derive(Default)]
// UAS: uas:small-model-runtime-harness-product-route-capability-recheck:invalid-axes
// Plane: Verification
// Residency: invalid fixture coverage for product route recheck rejection paths.
struct InvalidAxes {
    missing_product_handoff_artifact_rejected: bool,
...
fn invalid_rejections(witness: &SmallModelProductRouteCapabilityRecheckWitness) -> InvalidAxes {
    let mutate = |mutator: fn(&mut SmallModelProductRouteCapabilityRecheckWitness)| {
        let mut candidate = witness.clone();
        mutator(&mut candidate);
        candidate.validate().is_err()
    };
    InvalidAxes {
        missing_product_handoff_artifact_rejected: mutate(|candidate| {
            candidate.product_answer_packet_artifact_ref.clear();
        }),
        missing_required_blocker_rejected: mutate(|candidate| {
            candidate.blockers.retain(|blocker| {
                blocker.blocker_id != "blocker:fresh_product_runtime_route_missing"
            });
        }),
        ...
        metadata_budget_rejected: mutate(|candidate| {
            candidate.metadata_bytes = product_route_capability_recheck_metadata_budget_bytes() + 1;
        }),
    }
}

fn mutate_blocker(
    witness: &SmallModelProductRouteCapabilityRecheckWitness,
...
fn read_json(path: &Path) -> Result<serde_json::Value, ProductRouteCapabilityRecheckWitnessError> {
    let text = std::fs::read_to_string(path).map_err(|error| {
        ProductRouteCapabilityRecheckWitnessError::Io(format!(
            "failed to read {}: {error}",
            path.display()
        ))
    })?;
    serde_json::from_str(&text).map_err(|error| {
        ProductRouteCapabilityRecheckWitnessError::Json(format!(
...
fn json_bool(
    ...
    key: &str,
) -> Result<bool, ProductRouteCapabilityRecheckWitnessError> {
    value
        .get(key)
        .and_then(serde_json::Value::as_bool)
        .ok_or_else(|| {
            ProductRouteCapabilityRecheckWitnessError::Json(format!("missing bool `{key}`"))
        })
}
...
mod tests {
    ...
    fn blocker_ids_match_required_contract() {
        let evidence = EvidenceSnapshot {
            guard_next_existing_work:
                SMALL_MODEL_RUNTIME_HARNESS_PRODUCT_ROUTE_CAPABILITY_RECHECK_CURSOR.to_string(),
            ...
            capability_next_bottleneck:
                SMALL_MODEL_RUNTIME_HARNESS_PRODUCT_ROUTE_CAPABILITY_RECHECK_CURSOR.to_string(),
            product_handoff_pass: true,
            retained_runtime_bytes_loaded: 1,
            fresh_product_runtime_bytes_loaded: 0,
    ...
    #[test]
    fn measurement_bool_reads_nested_value() {
