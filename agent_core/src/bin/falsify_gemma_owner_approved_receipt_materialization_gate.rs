//! `falsify_gemma_owner_approved_receipt_materialization_gate`
//!
//! Metadata-only witness for the Gemma owner-approved receipt materialization
//! boundary. It proves the materializer contract exists without writing a
//! receipt, touching files, executing tools, loading model bytes, or promoting
//! a route.

use std::collections::BTreeMap;
use std::path::PathBuf;

use agent_core::falsifier_artifacts::axes::GEMMA_OWNER_APPROVED_RECEIPT_MATERIALIZATION_GATE_AXES;
...
const FALSIFIER_ID: &str = GEMMA_OWNER_APPROVED_RECEIPT_MATERIALIZATION_GATE_ID;
const FIXTURE_ID: &str = "gemma_owner_approved_receipt_materialization_gate_v1";
const COMMAND: &str = "Tools/falsifiers/f_gemma_owner_approved_receipt_materialization_gate.sh";
const RESULT: &str =
...
fn main() -> std::process::ExitCode {
    ...
    println!(
        "{FALSIFIER_ID}: overall_pass={} fields={} modes={} safety_checks={} red_fixture_rejection_count={} artifact={RESULT}",
        artifact.overall_pass,
        artifact.measurements["materialization_field_count"].value,
        artifact.measurements["materialization_mode_count"].value,
        artifact.measurements["safety_check_count"].value,
...
fn build_artifact(
    ...
    for (name, passed) in [
        ("upstream_receipt_emitter_dry_run_gate_pass", upstream_pass),
        (
            "upstream_receipt_emitter_dry_run_gate_ref_bound",
            gate.upstream_emitter_dry_run_ref
                == GEMMA_OWNER_APPROVED_RECEIPT_MATERIALIZATION_GATE_UPSTREAM_REF,
        ),
        (
            "materialization_fields_bound",
            metrics.materialization_field_count == 18
                && red_pass(&red_results, "missing_materialization_field")
                && red_pass(&red_results, "duplicate_materialization_field"),
        ),
        (
            "materialization_modes_bound",
            metrics.materialization_mode_count == 4
                && red_pass(&red_results, "missing_materialization_mode")
                && red_pass(&red_results, "unknown_materialization_mode"),
        ),
        (
            "safety_checks_bound",
            metrics.safety_check_count == 12
                && red_pass(&red_results, "missing_safety_check")
                && red_pass(&red_results, "duplicate_safety_check"),
        ),
        (
            "denied_shortcuts_bound",
        ...
        (
            "receipt_materialization_deferred",
            metrics.materialized_receipt_bytes == 0
                && metrics.receipt_payload_bytes_read == 0
                && red_pass(&red_results, "materialized_receipt_bytes")
        ...
        (
            "gemma_owner_approved_receipt_materialization_gate_address_deterministic",
            gate.address() == GemmaOwnerApprovedReceiptMaterializationGate::canonical().address(),
        ),
        (
            "next_cursor_bound",
            gate.next_cursor == GEMMA_OWNER_APPROVED_RECEIPT_MATERIALIZATION_GATE_NEXT_CURSOR,
        ),
    ] {
        add_bool_axis(
            &mut measurements,
    ...
    for (name, value, operator, expected, unit) in [
        (
            "materialization_field_count",
            metrics.materialization_field_count,
            "==",
            18,
            "fields",
        ),
        (
            "materialization_mode_count",
            metrics.materialization_mode_count,
            "==",
            4,
            "modes",
        ),
        ...
        (
            "materialized_receipt_bytes",
            metrics.materialized_receipt_bytes,
            "==",
            0,
            "bytes",
        ),
    ...
    add_text_axis(
        ...
        &mut thresholds,
        &mut pass_per_axis,
        "gemma_owner_approved_receipt_materialization_gate_address",
    ...
    Ok(ArtifactBuilder {
        ...
        anomalies: Vec::new(),
        notes: "metadata-only F-GemmaOwnerApprovedReceiptMaterializationGate: consumes the receipt-emitter dry-run gate and freezes the future owner-approved receipt materializer contract. It materializes zero receipt bytes, opens zero files, hashes zero files, executes zero llama-cli commands, loads zero model/runtime/provider bytes, mutates zero RuntimeRouter/System G/settings state, and makes no Gemma L2/L3/T4/user-facing claim.".to_string(),
...
fn red_fixture_results(
    gate: &GemmaOwnerApprovedReceiptMaterializationGate,
) -> Vec<(&'static str, bool)> {
    let mut fixtures: Vec<(
        ...
        (
            "missing_materialization_field",
            Box::new(|mut g| {
                g.required_materialization_fields.pop();
                g
            }),
        ),
        (
            "duplicate_materialization_field",
            Box::new(|mut g| {
                g.required_materialization_fields
                    .push(g.required_materialization_fields[0].clone());
                g
            }),
        ),
        (
            "missing_materialization_mode",
            Box::new(|mut g| {
                g.allowed_materialization_modes.pop();
                g
            }),
        ),
        (
            "unknown_materialization_mode",
            Box::new(|mut g| {
                g.allowed_materialization_modes[0] = "hf_cache_auto_receipt".to_string();
                g
            }),
        ),
        (
            ...
            Box::new(|mut g| {
                g.required_safety_checks.pop();
                g
            }),
        ),
        (
            "duplicate_safety_check",
            Box::new(|mut g| {
                g.required_safety_checks
                    .push(g.required_safety_checks[0].clone());
                g
            }),
        ),
        (
        ...
        (
            ...
            Box::new(|mut g| {
                g.denied_shortcuts.push(g.denied_shortcuts[0].clone());
                g
            }),
        ),
        (
        ...
        (
            "materialized_receipt_bytes",
            Box::new(|mut g| {
                g.materialized_receipt_bytes = 1;
                g
            }),
        ),
        (
...
fn assert_axis_coverage(measurements: &BTreeMap<String, Measurement>) {
    for axis in GEMMA_OWNER_APPROVED_RECEIPT_MATERIALIZATION_GATE_AXES {
        assert!(
