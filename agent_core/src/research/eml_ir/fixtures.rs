...
pub const ADVERSARIAL_FIXTURE_COUNT: usize = 28;
...
pub fn adversarial_fixture(index: usize) -> AdversarialFixture {
    ...
    match index {
        ...
        20 => adversarial(
            index,
            "ln_fp16_min_negative_subnormal",
            AdversarialOperation::Ln,
            1.0,
        ...
        26 => adversarial(
            index,
            "eml_ln_fp16_one_minus_ulp",
            AdversarialOperation::Eml,
            1.0,
            Fp16Bits::from_bits(0x3bff).to_f64(),
        ),
        _ => adversarial(
            index,
            "ln_fp16_one_minus_ulp",
            AdversarialOperation::Ln,
            1.0,
            Fp16Bits::from_bits(0x3bff).to_f64(),
        ),
...
fn adversarial_negative_zero_fixture_count() -> usize {
    adversarial_fixture_count_matching(|fixture| {
        is_negative_zero(fixture.x) || is_negative_zero(fixture.y)
    })
}

#[cfg(test)]
fn adversarial_positive_zero_fixture_count() -> usize {
    adversarial_fixture_count_matching(|fixture| {
        is_positive_zero(fixture.x) || is_positive_zero(fixture.y)
    })
}
...
#[cfg(test)]
fn adversarial_eml_ln_zero_branch_fixture_count() -> usize {
    adversarial_fixture_count_matching(|fixture| {
        fixture.operation == AdversarialOperation::Eml && fixture.y == 0.0
    })
}
...
#[cfg(test)]
fn adversarial_x_nan_fixture_count() -> usize {
    adversarial_fixture_count_matching(|fixture| fixture.x.is_nan())
}
...
fn adversarial_positive_infinity_fixture_count() -> usize {
    adversarial_fixture_count_matching(|fixture| {
        is_positive_infinity(fixture.x) || is_positive_infinity(fixture.y)
    })
}

#[cfg(test)]
fn adversarial_negative_infinity_fixture_count() -> usize {
    adversarial_fixture_count_matching(|fixture| {
        is_negative_infinity(fixture.x) || is_negative_infinity(fixture.y)
    })
}
...
fn adversarial_subnormal_fixture_count() -> usize {
    adversarial_fixture_count_matching(|fixture| {
        is_subnormal_probe(fixture.x) || is_subnormal_probe(fixture.y)
    })
}

#[cfg(test)]
fn adversarial_fixture_count_matching(
    matches_fixture: impl Fn(AdversarialFixture) -> bool,
) -> usize {
    (0..ADVERSARIAL_FIXTURE_COUNT)
        .map(adversarial_fixture)
        .filter(|fixture| matches_fixture(*fixture))
        .count()
}
...
#[cfg(test)]
fn is_negative_infinity(value: f64) -> bool {
...
fn is_subnormal_probe(value: f64) -> bool {
    value.is_subnormal()
        || matches!(Fp16Bits::from_f64(value).bits(), 0x0001..=0x03ff | 0x8001..=0x83ff)
}
...
mod tests {
    ...
    #[test]
    fn adversarial_fixtures_pin_negative_infinity_count() {
        assert_eq!(adversarial_negative_infinity_fixture_count(), 1);
    }

    #[test]
    fn adversarial_fixtures_pin_subnormal_count() {
        assert_eq!(adversarial_subnormal_fixture_count(), 6);
    }
    ...
    fn adversarial_fixtures_cover_eml_exact_zero_exp_branch() {
        assert_eq!(ADVERSARIAL_FIXTURE_COUNT, 28);
    ...
    fn adversarial_fixtures_cover_ln_one_exact_zero() {
        ...
        assert_eq!(ADVERSARIAL_FIXTURE_COUNT, 28);
        let one_minus_ulp = adversarial_fixture(27);
        assert_eq!(one_minus_ulp.label, "ln_fp16_one_minus_ulp");
        assert_eq!(one_minus_ulp.operation, AdversarialOperation::Ln);
        assert_eq!(one_minus_ulp.x, 1.0);
        assert_eq!(one_minus_ulp.y, Fp16Bits::from_bits(0x3bff).to_f64());
    }
    ...
    fn adversarial_fixtures_cover_ln_negative_fp16_subnormal() {
        ...
        assert_eq!(negative_subnormal.x, 1.0);
        assert_eq!(negative_subnormal.y, Fp16Bits::from_bits(0x8001).to_f64());
        assert!(negative_subnormal.y.is_sign_negative());
    }
    ...
    fn adversarial_fixtures_cover_eml_negative_fp16_subnormal() {
        ...
        assert_eq!(negative_subnormal.operation, AdversarialOperation::Eml);
        assert_eq!(negative_subnormal.x, 1.0);
        assert_eq!(negative_subnormal.y, Fp16Bits::from_bits(0x8001).to_f64());
        assert!(negative_subnormal.y.is_sign_negative());
    }
    ...
    #[test]
    fn adversarial_fixtures_cover_eml_ln_one_exact_zero() {
        assert_eq!(ADVERSARIAL_FIXTURE_COUNT, 28);
        let ln_one = adversarial_fixture(25);
        assert_eq!(ln_one.label, "eml_ln_one_exact_zero");
        assert_eq!(ln_one.operation, AdversarialOperation::Eml);
        assert_eq!(ln_one.x, 1.0);
        assert_eq!(ln_one.y, 1.0);

        let one_minus_ulp = adversarial_fixture(26);
        assert_eq!(one_minus_ulp.label, "eml_ln_fp16_one_minus_ulp");
        assert_eq!(one_minus_ulp.operation, AdversarialOperation::Eml);
        assert_eq!(one_minus_ulp.x, 1.0);
        assert_eq!(one_minus_ulp.y, Fp16Bits::from_bits(0x3bff).to_f64());
    }
    ...
    fn adversarial_fixtures_cover_finite_negative_ln_branch_cut() {
        ...
        assert_eq!(ln_negative_one.x, 1.0);
        assert_eq!(ln_negative_one.y, -1.0);

        let eml_negative_one = adversarial_fixture(22);
        assert_eq!(eml_negative_one.label, "eml_ln_negative_one");
        assert_eq!(eml_negative_one.operation, AdversarialOperation::Eml);
        assert_eq!(eml_negative_one.x, 1.0);
        assert_eq!(eml_negative_one.y, -1.0);
    }
