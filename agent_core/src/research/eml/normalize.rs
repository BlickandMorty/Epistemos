...
fn evaluate_closure_expr(expr: &EmlClosureExpr, consts: &[f64]) -> Result<f64, NormalizeError> {
    match expr {
        ...
        EmlClosureExpr::Eml(_, _) => {
            ...
            if expr.is_slot_free() {
                let bare = expr
                    .clone()
                    .try_into_bare_expr()
                    .expect("is_slot_free guarantees try_into_bare_expr succeeds");
...
fn is_canonical_subtree(expr: &EmlClosureExpr) -> bool {
    match expr {
        ...
        | EmlClosureExpr::Mul(l, r) => is_canonical_subtree(l) && is_canonical_subtree(r),
    }
}
...
mod tests {
    ...
    fn bare_is_normalized_always_true() {
        ...
        assert!(is_normalized_expr(&EmlExpr::eml(
            EmlExpr::One,
            EmlExpr::One
        )));
    }
    ...
    fn from_normalize_error_carries_eval_error() {
        let inner = EmlEvalError::DepthExceeded {
            depth: 100,
            cap: 32,
        };
    ...
    fn closure_eval_plus_with_slot_children() {
        ...
        let c = EmlClosure::new(tree, vec![std::f64::consts::PI, std::f64::consts::E]).unwrap();
        let v = evaluate_closure(&c).unwrap();
    ...
    fn closure_eval_ln_via_minus_and_eml() {
        ...
        let tree = EmlClosureExpr::minus(EmlClosureExpr::one(), EmlClosureExpr::slot(0));
        let c = EmlClosure::new(tree, vec![std::f64::consts::E]).unwrap();
        let v = evaluate_closure(&c).unwrap();
    ...
    fn closure_eval_divide_with_slots() {
        ...
        let tree = EmlClosureExpr::divide(EmlClosureExpr::slot(0), EmlClosureExpr::slot(1));
