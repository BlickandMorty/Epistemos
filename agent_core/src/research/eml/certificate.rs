//! Source:
//! - iter-1 audit `docs/audits/EML_IR_AUDIT_2026_05_17.md` §6 item 4
//!   ("Lean certificate emission ... a function
//!    `lean_certificate(&EmlExpr) → String` that emits a Lean 4 term
//!    whose typechecking proves the tree well-formed under the
//!    branch-safe typing").
//! - Phase A close-out `docs/audits/PHASE_A_CLOSEOUT_2026_05_17.md`
//!   §3 (iter-13 deliverable).
//! - Doctrine `docs/fusion/PRIMITIVE_IR_STACK_DOCTRINE_2026_05_17.md`
//!   §3 ("Lean schema authority — every IR has a Lean 4 schema
//!    module ... per-tree certificate emission").
//! - Companion: [`super::branched`] (the typestate this module
//!   emits certificates for); [`super::grammar`] (the bare EmlExpr
//!   the emitter walks).
//!
//! # Lean 4 certificate emission
//!
//! This module emits Lean 4 source as a String targeting
//! `Epistemos.EML`. The committed schema module built with explicit
//! `~/.elan/bin` PATH, and certificate obligations were sharpened
//! through iter-629 with zero sorries in committed Lean sources. The
//! emitter's live obligation is schema-shaped Lean source for the
//! branch-safety, evaluator equality, and positive-value proof rows.
//!
//! The emitted term for a tree is:
//!
//! - `(1 : ℝ)` for an [`super::grammar::EmlExpr::One`] leaf.
//! - `(Real.exp <left> - Real.log <right>)` for an `Eml(l, r)`.
//!
//! The full certificate for a [`super::branched::PositiveEmlExpr`]
//! is a Lean source block targeting `Epistemos.EML.CertificateTarget`:
//!
//! ```text
//! -- Generated by EML-IR certificate.rs (iter-13)
//! import Epistemos.EML
//! namespace Epistemos.EML.Generated
//! noncomputable def eml_expr_<hash> : Epistemos.EML.Expr := ...
//! theorem eml_branch_safe_<hash> :
//!     Epistemos.EML.BranchSafe eml_expr_<hash> := by
//!   exact Epistemos.EML.one_branch_safe
//! noncomputable def eml_certificate_<hash> :
//!     Epistemos.EML.CertificateTarget := ...
//! ```
//!
//! Symbolic branch-safety and evaluator equalities close from schema
//! lemmas where available. Non-symbolic branch-safety or evaluator
//! equalities are exposed as explicit runtime witness binders rather
//! than hidden generated proof bodies.
//!
//! ## What the emitter guarantees today
//!
//! - **Schema targeting.** The emitted source imports
//!   `Epistemos.EML` and constructs an `Epistemos.EML.Expr` plus
//!   `Epistemos.EML.CertificateTarget`.
//! - **Round-trip-with-evaluator.** The emitted cached value
//!   matches `PositiveEmlExpr::value()` exactly.
//! - **Determinism.** Same input → same emitted string (no
//!   floating-point formatting drift in the term portion).

use super::branched::PositiveEmlExpr;
use super::grammar::EmlExpr;

/// Lean 4 term for an [`EmlExpr`] subtree.
///
/// Recursive: `One` lowers to `(1 : ℝ)`; `Eml(l, r)` lowers to
/// `(Real.exp <l-term> - Real.log <r-term>)`.
pub fn lean_term(expr: &EmlExpr) -> String {
    match expr {
        EmlExpr::One => "(1 : ℝ)".to_string(),
        EmlExpr::Eml(l, r) => {
            format!("(Real.exp {} - Real.log {})", lean_term(l), lean_term(r))
        }
    }
}

/// Lean schema constructor term for an [`EmlExpr`] subtree.
///
/// This targets `lean/Epistemos/Epistemos/EML.lean`, not the
/// real-valued semantic expression. Generated certificates use this
/// term to construct `Epistemos.EML.Expr` values.
pub fn lean_expr_term(expr: &EmlExpr) -> String {
    match expr {
        EmlExpr::One => "Epistemos.EML.Expr.one".to_string(),
        EmlExpr::Eml(l, r) => {
            format!(
                "(Epistemos.EML.Expr.eml {} {})",
                lean_expr_term(l),
                lean_expr_term(r)
            )
        }
    }
}

/// Stable per-tree hash for the certificate name. Uses Rust's
/// default `Hasher` indirectly via `format!`-based serialization
/// of the bare expression (deterministic since `EmlExpr` is a
/// simple ADT with `#[derive(Debug)]`).
fn tree_hash_suffix(expr: &EmlExpr) -> String {
    // Stable structural hash: walk the tree and accumulate an
    // FNV-1a 64-bit hash of the canonical "1" / "E" symbols.
    // FNV-1a chosen for determinism + no_std-friendliness.
    const FNV_OFFSET: u64 = 0xcbf29ce484222325;
    const FNV_PRIME: u64 = 0x100000001b3;
    fn step(expr: &EmlExpr, h: &mut u64) {
        match expr {
            EmlExpr::One => {
                *h ^= b'1' as u64;
                *h = h.wrapping_mul(FNV_PRIME);
            }
            EmlExpr::Eml(l, r) => {
                *h ^= b'E' as u64;
                *h = h.wrapping_mul(FNV_PRIME);
                step(l, h);
                *h ^= b',' as u64;
                *h = h.wrapping_mul(FNV_PRIME);
                step(r, h);
                *h ^= b')' as u64;
                *h = h.wrapping_mul(FNV_PRIME);
            }
        }
    }
    let mut h = FNV_OFFSET;
    step(expr, &mut h);
    format!("{:016x}", h)
}

fn indent_lean_term_by(term: &str, spaces: usize) -> String {
    let pad = " ".repeat(spaces);
    term.lines()
        .map(|line| format!("{pad}{line}"))
        .collect::<Vec<_>>()
        .join("\n")
}

fn positive_eval_proof_source(expr: &EmlExpr) -> Option<String> {
    match expr {
        EmlExpr::One => Some("(by norm_num [Epistemos.EML.Expr.eval])".to_string()),
        EmlExpr::Eml(l, r) if matches!(&**r, EmlExpr::One) => Some(format!(
            "(Epistemos.EML.eval_eml_right_one_positive {})",
            lean_expr_term(l)
        )),
        EmlExpr::Eml(_, _) => None,
    }
}

fn closed_branch_safe_term(expr: &EmlExpr) -> Option<String> {
    match expr {
        EmlExpr::One => Some("Epistemos.EML.one_branch_safe".to_string()),
        EmlExpr::Eml(l, r) => {
            let left_expr = lean_expr_term(l);
            let right_expr = lean_expr_term(r);
            let left_safe = indent_lean_term_by(&closed_branch_safe_term(l)?, 6);
            let right_safe = indent_lean_term_by(&closed_branch_safe_term(r)?, 6);
            let right_positive = indent_lean_term_by(&positive_eval_proof_source(r)?, 6);
            Some(format!(
                "(Epistemos.EML.BranchObligation.discharge\n\
                 \x20 {{ left := {left_expr}\n\
                 \x20   right := {right_expr}\n\
                 \x20   left_safe :=\n\
                 {left_safe}\n\
                 \x20   right_safe :=\n\
                 {right_safe}\n\
                 \x20   right_positive :=\n\
                 {right_positive} }})"
            ))
        }
    }
}

fn branch_safe_proof_source(expr: &EmlExpr) -> String {
    closed_branch_safe_term(expr).map_or_else(
        || "exact Epistemos.EML.RuntimeBranchSafeWitness.carries branchWitness".to_string(),
        |term| format!("exact {term}"),
    )
}

fn needs_runtime_branch_witness(expr: &EmlExpr) -> bool {
    closed_branch_safe_term(expr).is_none()
}

fn runtime_branch_witness_binder(expr: &EmlExpr, suffix: &str) -> String {
    if needs_runtime_branch_witness(expr) {
        format!(
            "\n\x20   (branchWitness : Epistemos.EML.RuntimeBranchSafeWitness eml_expr_{suffix})"
        )
    } else {
        String::new()
    }
}

fn runtime_branch_witness_arg(expr: &EmlExpr) -> &'static str {
    if needs_runtime_branch_witness(expr) {
        " branchWitness"
    } else {
        ""
    }
}

fn eval_matches_proof_source(expr: &EmlExpr) -> &'static str {
    match expr {
        EmlExpr::One => "exact Epistemos.EML.Expr.eval_one",
        EmlExpr::Eml(_, _) => "exact Epistemos.EML.RuntimeEvalWitness.carries evalWitness",
    }
}

fn needs_runtime_eval_witness(expr: &EmlExpr) -> bool {
    !matches!(expr, EmlExpr::One)
}

fn runtime_eval_witness_binder(expr: &EmlExpr, suffix: &str) -> String {
    if needs_runtime_eval_witness(expr) {
        format!(
            "\n\x20   (evalWitness : Epistemos.EML.RuntimeEvalWitness eml_expr_{suffix} eml_value_{suffix})"
        )
    } else {
        String::new()
    }
}

fn runtime_eval_witness_arg(expr: &EmlExpr) -> &'static str {
    if needs_runtime_eval_witness(expr) {
        " evalWitness"
    } else {
        ""
    }
}

/// Full Lean 4 certificate for a runtime-validated tree. The
/// theorem statement asserts the tree's value is strictly positive;
/// branch-safety closes from schema lemmas where possible, and
/// non-leaf evaluator equality is supplied as a RuntimeEvalWitness.
pub fn lean_certificate(p: &PositiveEmlExpr) -> String {
    let expr_term = lean_expr_term(p.as_expr());
    let semantic_term = lean_term(p.as_expr());
    let suffix = tree_hash_suffix(p.as_expr());
    let value = p.value();
    let branch_safe_proof = branch_safe_proof_source(p.as_expr());
    let branch_witness_binder = runtime_branch_witness_binder(p.as_expr(), &suffix);
    let branch_witness_arg = runtime_branch_witness_arg(p.as_expr());
    let eval_matches_proof = eval_matches_proof_source(p.as_expr());
    let eval_witness_binder = runtime_eval_witness_binder(p.as_expr(), &suffix);
    let eval_witness_arg = runtime_eval_witness_arg(p.as_expr());
    format!(
        "-- Generated by EML-IR certificate.rs (T5 Phase B1 iter-13; Lean-first iter-607)\n\
         -- Source: docs/fusion/PRIMITIVE_IR_STACK_DOCTRINE_2026_05_17.md §3\n\
         -- Schema: lean/Epistemos/Epistemos/EML.lean\n\
         -- Schema module built with explicit ~/.elan/bin PATH; certificate obligations sharpened through iter-629.\n\
         -- Generated non-leaf eval proofs close from RuntimeEvalWitness.\n\
         -- Generated non-symbolic branch proofs close from RuntimeBranchSafeWitness.\n\
         -- Runtime-validated value: {value}\n\
         -- Semantic term: {semantic_term}\n\
         import Epistemos.EML\n\
         \n\
         namespace Epistemos.EML.Generated\n\
         \n\
         noncomputable def eml_expr_{suffix} : Epistemos.EML.Expr :=\n\
         \x20   {expr_term}\n\
         \n\
         noncomputable def eml_value_{suffix} : ℝ :=\n\
         \x20   {value}\n\
         \n\
         theorem eml_branch_safe_{suffix}{branch_witness_binder} :\n\
         \x20   Epistemos.EML.BranchSafe eml_expr_{suffix} := by\n\
         \x20 {branch_safe_proof}\n\
         \n\
         theorem eml_eval_matches_{suffix}{eval_witness_binder} :\n\
         \x20   Epistemos.EML.Expr.eval eml_expr_{suffix} = eml_value_{suffix} := by\n\
         \x20 {eval_matches_proof}\n\
         \n\
         theorem eml_positive_value_{suffix} :\n\
         \x20   0 < eml_value_{suffix} := by\n\
         \x20 norm_num [eml_value_{suffix}]\n\
         \n\
         noncomputable def eml_certificate_{suffix}{branch_witness_binder}{eval_witness_binder} : Epistemos.EML.CertificateTarget :=\n\
         \x20   {{ expr := eml_expr_{suffix}\n\
         \x20     value := eml_value_{suffix}\n\
         \x20     branch_safe := eml_branch_safe_{suffix}{branch_witness_arg}\n\
         \x20     value_matches := eml_eval_matches_{suffix}{eval_witness_arg}\n\
         \x20     positive_value := eml_positive_value_{suffix}\n\
         \x20     sourceRow := \"docs/fusion/PRIMITIVE_IR_STACK_DOCTRINE_2026_05_17.md §5 EML-IR\" }}\n\
         \n\
         theorem eml_certificate_data_fields_{suffix}{branch_witness_binder}{eval_witness_binder} :\n\
         \x20   (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).expr = eml_expr_{suffix} ∧\n\
         \x20     (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).value = eml_value_{suffix} := by\n\
         \x20 exact Epistemos.EML.CertificateTarget.dataFieldsMatch\n\
         \x20   (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg})\n\
         \x20   eml_expr_{suffix} eml_value_{suffix} rfl rfl\n\
         \n\
         theorem eml_certificate_source_row_{suffix}{branch_witness_binder}{eval_witness_binder} :\n\
         \x20   (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).sourceRow =\n\
         \x20     \"docs/fusion/PRIMITIVE_IR_STACK_DOCTRINE_2026_05_17.md §5 EML-IR\" := by\n\
         \x20 exact Epistemos.EML.CertificateTarget.sourceRowMatches\n\
         \x20   (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg})\n\
         \x20   \"docs/fusion/PRIMITIVE_IR_STACK_DOCTRINE_2026_05_17.md §5 EML-IR\"\n\
         \x20   rfl\n\
         \n\
         theorem eml_branch_and_eval_{suffix}{branch_witness_binder}{eval_witness_binder} :\n\
         \x20   Epistemos.EML.BranchSafe (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).expr ∧\n\
         \x20     Epistemos.EML.Expr.eval (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).expr =\n\
         \x20       (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).value := by\n\
         \x20 exact Epistemos.EML.CertificateTarget.branchSafeAndEvalMatches\n\
         \x20   (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg})\n\
         \n\
         theorem eml_certificate_positive_value_{suffix}{branch_witness_binder}{eval_witness_binder} :\n\
         \x20   0 < (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).value := by\n\
         \x20 exact Epistemos.EML.CertificateTarget.positiveValueCarries\n\
         \x20   (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg})\n\
         \n\
         theorem eml_eval_positive_{suffix}{branch_witness_binder}{eval_witness_binder} :\n\
         \x20   0 < Epistemos.EML.Expr.eval eml_expr_{suffix} := by\n\
         \x20 exact Epistemos.EML.CertificateTarget.eval_positive (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg})\n\
         \n\
         theorem eml_full_obligations_{suffix}{branch_witness_binder}{eval_witness_binder} :\n\
         \x20   Epistemos.EML.BranchSafe (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).expr ∧\n\
         \x20     Epistemos.EML.Expr.eval (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).expr =\n\
         \x20       (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).value ∧\n\
         \x20     0 < Epistemos.EML.Expr.eval (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg}).expr := by\n\
         \x20 exact Epistemos.EML.CertificateTarget.fullObligations\n\
         \x20   (eml_certificate_{suffix}{branch_witness_arg}{eval_witness_arg})\n\
         \n\
         end Epistemos.EML.Generated\n",
        value = value,
        suffix = suffix,
        expr_term = expr_term,
        semantic_term = semantic_term,
        branch_safe_proof = branch_safe_proof,
        branch_witness_binder = branch_witness_binder,
        branch_witness_arg = branch_witness_arg,
        eval_matches_proof = eval_matches_proof,
        eval_witness_binder = eval_witness_binder,
        eval_witness_arg = eval_witness_arg,
    )
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn lean_term_one_leaf() {
        assert_eq!(lean_term(&EmlExpr::One), "(1 : ℝ)");
    }

    #[test]
    fn lean_term_eml_one_one() {
        let e = EmlExpr::eml(EmlExpr::One, EmlExpr::One);
        assert_eq!(lean_term(&e), "(Real.exp (1 : ℝ) - Real.log (1 : ℝ))");
    }

    #[test]
    fn lean_term_nested_eml() {
        // eml(eml(One, One), One)
        let inner = EmlExpr::eml(EmlExpr::One, EmlExpr::One);
        let outer = EmlExpr::eml(inner, EmlExpr::One);
        let lt = lean_term(&outer);
        // Outer wraps inner; check both Real.exp and Real.log present.
        assert!(lt.contains("Real.exp (Real.exp"));
        assert!(lt.contains("Real.log (1 : ℝ)"));
    }

    #[test]
    fn lean_term_is_deterministic() {
        let e = EmlExpr::eml(EmlExpr::eml(EmlExpr::One, EmlExpr::One), EmlExpr::One);
        let a = lean_term(&e);
        let b = lean_term(&e);
        assert_eq!(a, b);
    }

    #[test]
    fn lean_expr_term_one_leaf_targets_schema_constructor() {
        assert_eq!(lean_expr_term(&EmlExpr::One), "Epistemos.EML.Expr.one");
    }

    #[test]
    fn certificate_targets_eml_schema_module() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("import Epistemos.EML"));
        assert!(c.contains("namespace Epistemos.EML.Generated"));
        assert!(c.contains("Epistemos.EML.CertificateTarget"));
    }

    #[test]
    fn tree_hash_is_stable_across_calls() {
        let e = EmlExpr::eml(EmlExpr::One, EmlExpr::One);
        assert_eq!(tree_hash_suffix(&e), tree_hash_suffix(&e));
    }

    #[test]
    fn tree_hash_differs_for_different_trees() {
        let a = EmlExpr::eml(EmlExpr::One, EmlExpr::One);
        let b = EmlExpr::eml(EmlExpr::eml(EmlExpr::One, EmlExpr::One), EmlExpr::One);
        assert_ne!(tree_hash_suffix(&a), tree_hash_suffix(&b));
    }

    #[test]
    fn tree_hash_distinguishes_left_vs_right_nest() {
        // eml(eml(One, One), One)  vs  eml(One, eml(One, One))
        // Two distinct trees that both evaluate to non-trivial values.
        let left_nest = EmlExpr::eml(EmlExpr::eml(EmlExpr::One, EmlExpr::One), EmlExpr::One);
        let right_nest = EmlExpr::eml(EmlExpr::One, EmlExpr::eml(EmlExpr::One, EmlExpr::One));
        assert_ne!(tree_hash_suffix(&left_nest), tree_hash_suffix(&right_nest));
    }

    #[test]
    fn certificate_contains_term() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("(1 : ℝ)"));
    }

    #[test]
    fn certificate_contains_runtime_value() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("Runtime-validated value: 1"));
    }

    #[test]
    fn certificate_routes_non_leaf_eval_through_runtime_witness() {
        let b = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        assert!(c.contains("Epistemos.EML.RuntimeEvalWitness"));
        assert!(c.contains("exact Epistemos.EML.RuntimeEvalWitness.carries evalWitness"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_closes_positive_value_from_runtime_number() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("eml_positive_value_"));
        assert!(c.contains("norm_num [eml_value_"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_projects_eval_positive_from_schema_target() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("theorem eml_eval_positive_"));
        assert!(c.contains("Epistemos.EML.CertificateTarget.eval_positive"));
        assert!(c.contains("eml_certificate_"));
    }

    #[test]
    fn certificate_projects_branch_and_eval_from_schema_target() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("theorem eml_branch_and_eval_"));
        assert!(c.contains("Epistemos.EML.CertificateTarget.branchSafeAndEvalMatches"));
        assert!(c.contains("eml_certificate_"));
    }

    #[test]
    fn certificate_projects_full_obligations_from_schema_target() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("theorem eml_full_obligations_"));
        assert!(c.contains("Epistemos.EML.CertificateTarget.fullObligations"));
        assert!(c.contains("eml_certificate_"));
    }

    #[test]
    fn certificate_projects_target_data_fields() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains(
            "sourceRow := \"docs/fusion/PRIMITIVE_IR_STACK_DOCTRINE_2026_05_17.md §5 EML-IR\""
        ));
        assert!(c.contains("theorem eml_certificate_source_row_"));
        assert!(c.contains("Epistemos.EML.CertificateTarget.sourceRowMatches"));
        assert!(c.contains("theorem eml_certificate_data_fields_"));
        assert!(c.contains(".expr = eml_expr_"));
        assert!(c.contains(".value = eml_value_"));
        assert!(c.contains("Epistemos.EML.CertificateTarget.dataFieldsMatch"));
    }

    #[test]
    fn certificate_closes_one_branch_safe_source() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("theorem eml_branch_safe_"));
        assert!(c.contains("exact Epistemos.EML.one_branch_safe"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_uses_schema_one_branch_safe_source() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("Epistemos.EML.one_branch_safe"));
        assert!(!c.contains("Epistemos.EML.BranchSafe.one"));
    }

    #[test]
    fn certificate_closes_one_eval_matches_source() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("theorem eml_eval_matches_"));
        assert!(c.contains("exact Epistemos.EML.Expr.eval_one"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_uses_schema_one_eval_source() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("Epistemos.EML.Expr.eval_one"));
        assert!(!c.contains("\n  rfl\n"));
    }

    #[test]
    fn certificate_closes_eml_one_one_branch_safe_source() {
        let b = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        assert!(c.contains("Epistemos.EML.BranchObligation.discharge"));
        assert!(c.contains("norm_num [Epistemos.EML.Expr.eval]"));
        assert!(!c.contains("runtime typestate"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_closes_left_chain_branch_safe_source() {
        let left = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let b = super::super::branched::BranchedEmlExpr::eml(left, PositiveEmlExpr::one());
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        assert!(
            c.matches("Epistemos.EML.BranchObligation.discharge")
                .count()
                >= 2
        );
        assert!(!c.contains("runtime typestate"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_closes_right_one_one_branch_safe_source() {
        let right = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        )
        .try_into_positive()
        .unwrap();
        let outer_left = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let b = super::super::branched::BranchedEmlExpr::eml(outer_left, right);
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        assert!(
            c.matches("Epistemos.EML.BranchObligation.discharge")
                .count()
                >= 2
        );
        assert!(c.contains("Epistemos.EML.eval_eml_right_one_positive"));
        assert!(!c.contains("runtime typestate"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_closes_right_left_chain_branch_safe_source() {
        let inner_left = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let right =
            super::super::branched::BranchedEmlExpr::eml(inner_left, PositiveEmlExpr::one())
                .try_into_positive()
                .unwrap();
        let outer_left = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let b = super::super::branched::BranchedEmlExpr::eml(outer_left, right);
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        assert!(
            c.matches("Epistemos.EML.BranchObligation.discharge")
                .count()
                >= 3
        );
        assert!(c.contains("Epistemos.EML.eval_eml_right_one_positive"));
        assert!(!c.contains("runtime typestate"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_uses_schema_right_one_positive_source() {
        let right = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        )
        .try_into_positive()
        .unwrap();
        let outer_left = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let b = super::super::branched::BranchedEmlExpr::eml(outer_left, right);
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        assert!(c.contains("Epistemos.EML.eval_eml_right_one_positive"));
        assert!(!c.contains("Real.exp_pos"));
    }

    #[test]
    fn certificate_uses_schema_branch_obligation_discharge() {
        let b = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        assert!(c.contains("Epistemos.EML.BranchObligation.discharge"));
        assert!(!c.contains("Epistemos.EML.BranchSafe.eml\n"));
    }

    #[test]
    fn certificate_routes_non_symbolic_branch_safe_through_runtime_witness() {
        let inner_right = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        )
        .try_into_positive()
        .unwrap();
        let hard_right = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            inner_right,
        )
        .try_into_positive()
        .unwrap();
        let b = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            hard_right,
        );
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        assert!(c.contains("Epistemos.EML.RuntimeBranchSafeWitness"));
        assert!(c.contains("exact Epistemos.EML.RuntimeBranchSafeWitness.carries branchWitness"));
        assert_eq!(c.matches("sorry").count(), 0);
    }

    #[test]
    fn certificate_carries_positivity_goal() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("0 < eml_value_"));
        assert!(c.contains("theorem eml_certificate_positive_value_"));
        assert!(c.contains("Epistemos.EML.CertificateTarget.positiveValueCarries"));
    }

    #[test]
    fn certificate_header_cites_phase_b1_iter_13() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains("iter-13"));
    }

    #[test]
    fn certificate_header_tracks_schema_build_and_generated_obligations() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        assert!(c.contains(
            "Schema module built with explicit ~/.elan/bin PATH; certificate obligations sharpened through iter-629"
        ));
        assert!(c.contains("Generated non-leaf eval proofs close from RuntimeEvalWitness"));
    }

    #[test]
    fn certificate_for_eml_one_one_emits_e_value() {
        // eml(One, One) evaluates to e ≈ 2.718.
        let b = super::super::branched::BranchedEmlExpr::eml(
            super::super::branched::BranchedEmlExpr::one(),
            PositiveEmlExpr::one(),
        );
        let p = b.try_into_positive().unwrap();
        let c = lean_certificate(&p);
        // The cached value formatted into the comment should be e.
        assert!(c.contains("2.71828"));
    }

    #[test]
    fn certificate_theorem_name_uses_tree_hash() {
        let p = PositiveEmlExpr::one();
        let c = lean_certificate(&p);
        // Format: theorem eml_branch_safe_<16-hex-chars>
        assert!(c.contains("theorem eml_branch_safe_"));
        // Hex suffix is 16 characters of [0-9a-f].
        let needle = "eml_branch_safe_";
        let pos = c.find(needle).unwrap();
        let after = &c[pos + needle.len()..];
        let suffix: String = after.chars().take(16).collect();
        assert_eq!(suffix.len(), 16);
        assert!(suffix.chars().all(|ch| ch.is_ascii_hexdigit()));
    }

    #[test]
    fn certificate_is_deterministic_for_same_input() {
        let p = PositiveEmlExpr::one();
        let c1 = lean_certificate(&p);
        let c2 = lean_certificate(&p);
        assert_eq!(c1, c2);
    }
}
