...
#[async_trait]
pub trait SubAgentResearcher: Sync {
...
pub async fn run_plan<R: SubAgentResearcher>(
    plan: &ResearchPlan,
    researcher: &R,
    ...
    for layer in layers {
        ...
        let prior = all.clone();
        let layer_results: Vec<SubResult> = stream::iter(layer.into_iter())
            .map(|sq| researcher.research(sq, &prior))
            .buffered(cap)
            .collect()
            .await;
...
mod tests {
    ...
    async fn runs_all_subquestions_in_plan_order() {
        let p = diamond();
        let r = probe();
        let results = run_plan(&p, &r, 4).await.unwrap();
    ...
    async fn a_parallel_layer_actually_runs_concurrently() {
        let p = diamond();
        let r = probe();
        let _ = run_plan(&p, &r, 4).await.unwrap();
    ...
    async fn concurrency_cap_one_serializes() {
        let p = diamond();
        let r = probe();
        let _ = run_plan(&p, &r, 1).await.unwrap();
    ...
    async fn cyclic_plan_errors_without_running() {
        ...
        let r = probe();
        let err = run_plan(&p, &r, 4).await.unwrap_err();
