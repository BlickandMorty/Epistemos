...
#   ./scripts/verify/omega_verify.sh --task 3       # Verify legacy Omega-1 task
#   ./scripts/verify/omega_verify.sh --task omega5-2 # Verify Sprint Omega-5 task
#   ./scripts/verify/omega_verify.sh --recursive    # 3-pass recursive loop
#
# Exit codes: 0 = all pass, 1 = failures found

set -uo pipefail
# NOTE: -e removed intentionally — verification checks use grep pipelines
# that return exit 1 on no-match; we handle failures via check_pass/check_fail
...
normalize_count() { printf '%s\n' "$1" | tail -n1 | tr -d '[:space:]'; }
...
run_cargo_test_filter() {
  ...
  local output
  if output=$(cargo test --manifest-path "$manifest" -- "$filter" 2>&1); then
    printf '%s\n' "$output" | tail -5
    check_pass "$label"
  else
    printf '%s\n' "$output" | tail -20
...
run_cargo_check_manifest() {
  local manifest="$1" label="$2"
  local output
  if output=$(cargo check --manifest-path "$manifest" 2>&1); then
    printf '%s\n' "$output" | tail -5
    check_pass "$label"
  else
    printf '%s\n' "$output" | tail -20
...
run_cargo_test_manifest() {
  local manifest="$1" label="$2"
  local output
  if output=$(cargo test --manifest-path "$manifest" 2>&1); then
    printf '%s\n' "$output" | tail -5
    check_pass "$label"
  else
    printf '%s\n' "$output" | tail -20
...
layer_0() {
  ...
  if [ -f "docs/AGENT_PROGRESS.md" ]; then
    local done todo
    done=$(normalize_count "$(grep -c '^\- \[x\]' docs/AGENT_PROGRESS.md 2>/dev/null || true)")
    todo=$(normalize_count "$(grep -c '^\- \[ \]' docs/AGENT_PROGRESS.md 2>/dev/null || true)")
...
layer_1() {
  ...
  unsafe_blocks=$(grep -rn 'unsafe' --include="*.rs" agent_core/src omega-mcp/src omega-ax/src 2>/dev/null | wc -l | tr -d ' ')
  if [ "$unsafe_blocks" -gt 0 ]; then
    unsafe_with_safety=$(normalize_count "$(grep -rn -B1 'unsafe' --include="*.rs" agent_core/src omega-mcp/src omega-ax/src 2>/dev/null | grep -c 'SAFETY' || true)")
...
layer_2() {
  ...
  check_file "agent_core/src/storage/diff_engine.rs" 30 "★ Living vault diff engine"
...
layer_3() {
  ...
  if command -v xcodebuild &>/dev/null && [ -f "Epistemos.xcodeproj/project.pbxproj" ]; then
    ...
    for suite in "${focused_suites[@]}"; do
      only_args+=("-only-testing:EpistemosTests/$suite")
    done
    ...
    if echo "$focused_output" | grep -qE "TEST FAILED|TEST EXECUTE FAILED"; then
      ...
      local passed
      passed=$(normalize_count "$(printf '%s\n' "$focused_output" | awk '/✔ Test "/ { count++ } END { print count + 0 }')")
      if [ "$passed" -eq 0 ]; then
        passed=$(normalize_count "$(printf '%s\n' "$focused_output" | sed -n 's/.*Test run with \([0-9][0-9]*\) tests passed.*/\1/p' | tail -n1)")
      fi
      if [ -z "$passed" ] || [ "$passed" -eq 0 ]; then
        passed=$(normalize_count "$(printf '%s\n' "$focused_output" | sed -n 's/.*Executed \([0-9][0-9]*\) tests.*/\1/p' | tail -n1)")
      fi
...
verify_task() {
  ...
  case "$task_num" in
    1)
      ...
      run_cargo_test_filter "agent_core/Cargo.toml" "prompt_caching" "prompt_caching tests"
      ;;
    2)
      ...
      run_cargo_test_filter "agent_core/Cargo.toml" "think" "think tool tests"
      ;;
    3)
      ...
      run_cargo_test_filter "agent_core/Cargo.toml" "compaction" "compaction tests"
      ;;
    4)
      ...
      run_cargo_test_filter "agent_core/Cargo.toml" "security" "security tests"
      ;;
    5)
      ...
      run_cargo_test_filter "omega-mcp/Cargo.toml" "transport" "omega-mcp transport tests"
      ;;
    6)
      ...
      ;;
    omega5-1)
      check_file "agent_core/src/storage/diff_engine.rs" 30 "diff_engine.rs"
      check_pattern "agent_core/src/storage/diff_engine.rs" "generate_text_diff" "generate_text_diff function"
      check_pattern "agent_core/src/storage/diff_engine.rs" "generate_json_diff" "generate_json_diff function"
      check_pattern "agent_core/src/storage/diff_engine.rs" "apply_text_patch" "apply_text_patch function"
      check_pattern "agent_core/src/lib.rs" "diff_engine" "Diff engine wired in lib.rs"
      run_cargo_test_filter "agent_core/Cargo.toml" "diff_engine" "diff engine tests"
      ;;
    omega5-2)
      check_file "agent_core/src/storage/memory_classifier.rs" 40 "memory_classifier.rs"
      check_pattern "agent_core/src/storage/memory_classifier.rs" "MemoryOperation" "MemoryOperation enum"
      check_pattern "agent_core/src/storage/memory_classifier.rs" "classify_memory_operation" "classify_memory_operation function"
      check_pattern "agent_core/src/storage/memory_classifier.rs" "VaultFact" "VaultFact struct"
      check_pattern "agent_core/src/storage/memory_classifier.rs" "build_classification_prompt" "Classification prompt builder"
      check_pattern "agent_core/src/storage/memory_classifier.rs" "ClaudeHaiku" "Haiku fallback routing"
      check_pattern "agent_core/src/lib.rs" "memory_classifier" "Memory classifier wired in lib.rs"
      run_cargo_test_filter "agent_core/Cargo.toml" "memory_classifier" "memory classifier tests"
      ;;
    omega5-3)
      check_file "agent_core/src/storage/memory_decay.rs" 40 "memory_decay.rs"
      check_pattern "agent_core/src/storage/memory_decay.rs" "NodeStrength" "NodeStrength struct"
      check_pattern "agent_core/src/storage/memory_decay.rs" "collect_garbage" "collect_garbage function"
      check_pattern "agent_core/src/storage/memory_decay.rs" "batch_decay" "batch_decay function"
      run_cargo_test_filter "agent_core/Cargo.toml" "memory_decay" "memory decay tests"
      ;;
    omega5-4)
      check_file "agent_core/src/storage/cross_propagation.rs" 40 "cross_propagation.rs"
      check_pattern "agent_core/src/storage/cross_propagation.rs" "PropagationResult" "PropagationResult struct"
      check_pattern "agent_core/src/storage/cross_propagation.rs" "scan_for_references" "scan_for_references function"
      check_pattern "agent_core/src/storage/cross_propagation.rs" "generate_propagation_diffs" "generate_propagation_diffs function"
      run_cargo_test_filter "agent_core/Cargo.toml" "cross_propagation" "cross propagation tests"
      ;;
    omega5-5)
      check_file "agent_core/src/storage/vault_git.rs" 40 "vault_git.rs"
      check_pattern "agent_core/src/storage/vault_git.rs" "VaultGit" "VaultGit struct"
      check_pattern "agent_core/src/storage/vault_git.rs" "commit_diffs" "commit_diffs function"
      check_pattern "agent_core/src/storage/vault_git.rs" "history" "history function"
      check_pattern "agent_core/Cargo.toml" "git2" "git2 dependency"
      run_cargo_test_filter "agent_core/Cargo.toml" "vault_git" "vault git tests"
      ;;
    omega5-6)
      check_file "Epistemos/Vault/ConversationPersistence.swift" 40 "ConversationPersistence.swift"
      check_pattern "Epistemos/Vault/ConversationPersistence.swift" "ConversationPersistence" "ConversationPersistence actor"
      check_pattern "Epistemos/Vault/ConversationPersistence.swift" "appendTurn" "appendTurn function"
      check_pattern "Epistemos/Vault/ConversationPersistence.swift" "generateCompanionMarkdown" "generateCompanionMarkdown function"
      run_swift_build_check "Conversation persistence build"
      ;;
    omega5-7)
      check_file "Epistemos/Vault/VaultChatMutator.swift" 40 "VaultChatMutator.swift"
      check_pattern "Epistemos/Vault/VaultChatMutator.swift" "VaultChatMutator" "VaultChatMutator type"
      check_pattern "Epistemos/Vault/VaultChatMutator.swift" "mutate" "mutate function"
      check_pattern "Epistemos/Vault/VaultChatMutator.swift" "DiffResult" "DiffResult usage"
      run_swift_build_check "Vault chat mutator build"
      ;;
    omega5-8)
      check_file "agent_core/src/vault_registry.rs" 40 "vault_registry.rs"
      check_file "Epistemos/Vault/VaultRegistry.swift" 40 "VaultRegistry.swift"
      check_pattern "agent_core/src/vault_registry.rs" "VaultIdentity" "VaultIdentity enum"
      check_pattern "agent_core/src/vault_registry.rs" "merge_vaults" "merge_vaults function"
      check_pattern "Epistemos/Vault/VaultRegistry.swift" "VaultSwitcher" "VaultSwitcher view"
      run_cargo_test_filter "agent_core/Cargo.toml" "vault_registry" "vault registry tests"
      ;;
    omega5-9)
      bold "Running Sprint Omega-5 verification..."
      check_file "agent_core/src/storage/diff_engine.rs" 30 "diff_engine.rs"
      check_file "agent_core/src/storage/memory_classifier.rs" 40 "memory_classifier.rs"
      check_file "agent_core/src/storage/memory_decay.rs" 40 "memory_decay.rs"
      check_file "agent_core/src/storage/cross_propagation.rs" 40 "cross_propagation.rs"
      check_file "agent_core/src/storage/vault_git.rs" 40 "vault_git.rs"
      check_file "agent_core/src/vault_registry.rs" 40 "vault_registry.rs"
      check_file "Epistemos/Vault/ConversationPersistence.swift" 80 "ConversationPersistence.swift"
      check_file "Epistemos/Vault/VaultChatMutator.swift" 40 "VaultChatMutator.swift"
      check_file "Epistemos/Vault/VaultRegistry.swift" 40 "VaultRegistry.swift"
      check_pattern "agent_core/src/lib.rs" "diff_engine" "Diff engine wired in lib.rs"
      check_pattern "agent_core/src/lib.rs" "memory_classifier" "Memory classifier wired in lib.rs"
      check_pattern "agent_core/src/lib.rs" "memory_decay" "Memory decay wired in lib.rs"
      check_pattern "agent_core/src/lib.rs" "cross_propagation" "Cross propagation wired in lib.rs"
      check_pattern "agent_core/src/lib.rs" "vault_git" "Vault git wired in lib.rs"
      check_pattern "agent_core/src/lib.rs" "vault_registry" "Vault registry wired in lib.rs"
      run_cargo_check_manifest "agent_core/Cargo.toml" "agent_core cargo check"
      run_cargo_test_filter "agent_core/Cargo.toml" "diff_engine" "diff engine tests"
      run_cargo_test_filter "agent_core/Cargo.toml" "memory_classifier" "memory classifier tests"
      run_cargo_test_filter "agent_core/Cargo.toml" "memory_decay" "memory decay tests"
      run_cargo_test_filter "agent_core/Cargo.toml" "cross_propagation" "cross propagation tests"
      run_cargo_test_filter "agent_core/Cargo.toml" "vault_git" "vault git tests"
      run_cargo_test_filter "agent_core/Cargo.toml" "vault_registry" "vault registry tests"
      run_cargo_test_manifest "omega-mcp/Cargo.toml" "omega-mcp regression tests"
      run_swift_build_check "Sprint Omega-5 Swift build"
      check_pattern "docs/AGENT_PROGRESS.md" "Task 8: VaultRegistry\\.swift / vault_registry\\.rs" "AGENT_PROGRESS includes Task 8"
      ;;
    *)
      red "Unknown task: $task_num"
      yellow "Known task selectors: 1-6 (legacy Omega-1) or omega5-1 ... omega5-9"
      ;;
  esac
}
