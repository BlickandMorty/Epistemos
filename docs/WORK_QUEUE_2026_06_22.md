...
- [~] **0.48 ONE native recent-chat system (Act+Work)** — AUDIT DONE (iter59): NOT 3 stores. SDChat is ALREADY the
  unified native store (chatType incl "worker"); ALL popovers read recentChatsDescriptor; Osaurus ChatSessionsManager is
  DEAD in product; MiniChat persistMiniChatSession mirrors into SDChat. REAL GAP: new main-act runActOsaurusTurn (0.40)
  ...
  DONE (gap C complete): iter60 main-act persists to SDChat; iter61 0.48b ACT+WORK two-section popover; iter62
  0.48b-part2 Work sessions persist as SDChat "worker" rows (AppBootstrap.persistWorkSession on live work launch)
  + reopen via .openWorkSession notification (HomeRouter flips to .work; loadChatIntoSession branches worker rows).
  ONE native SDChat store; main act + mini + work all save + reopen. ChatSessionsManager dead-in-product (as found).
  →addendum #3/#5 + 0.38.
...
## iter (2026-06-24) RESOLUTION of focus-1 — honest no-vault rooting DONE + bookmark root cause FOUND
- **Honest rooting DONE** (statically verified; full build owed): `BundledWorkOpenCodeShell.launchSpec` now GATES fusion on a non-nil active vault (`if let vaultURL = epistemosVaultRoot`). No active vault → fusion OMITTED entirely (no OPENCODE_CONFIG / EPISTEMOS_VAULT_ROOT) → omega_mcp_stdio honestly reports no vault. NEVER roots at the empty `~/Documents/Epistemos` default anymore. Source-guard tests updated in lockstep. Full detail + verification: `docs/handoffs/HARDENING_NO_VAULT_HONESTY_LEDGER_2026_06_24.md`.
- **"No vault after rebuild" ROOT CAUSE (runtime-confirmed via `defaults read`):** DUAL preference domains. The vault is connected under the **App Store sandboxed build** (`com.epistemos.appstore`: lastVaultPath=/Users/jojo/Downloads/openclaw-main + 84-byte security-scoped bookmark). The locally-rebuilt **Debug** app (`com.epistemos.app`) has a SEPARATE empty domain (no bookmark, no lastVaultPath) → vaultURL nil. Vault selection does NOT carry across the two build targets; the legacy-suite migration doesn't bridge them. NOT a bookmark-revocation bug.
- **Owner decision needed:** should Debug + AppStore SHARE a vault (app-group/migration), or just connect the vault once in the Debug build? Until then, honest no-vault is correct.
- **Re the "OPENCODE MCP NOT CONNECTING / 0 MCP" item above:** likely the SAME no-vault state on the Debug build (no vault → no fusion config written → 0 servers), not necessarily an absent binary. Re-check after connecting a vault in the Debug build.
