...
|---|---|---|---|---|---|---|---|---|
| 1 | UasAddress | UASA identity | UAS | Verified Floor | driver §4.G mission G.B1 | `agent_core/src/uas/address.rs` (landed iter 21 4f74a7e9) | F-UAS-ZeroCopy-Spine + F-ACS-Anchor-Addressing | **landed (substrate-floor; Phase B.G.B1.a iter 21)** |
| 2 | ResidencyLease | substrate-tier lease | UAS | Verified Floor | driver §4.G G.B1 | `agent_core/src/uas/residency_lease.rs` (landed iter 24 ce06b9f6) | F-UAS-ZeroCopy-Spine | **landed (substrate-floor; Phase B.G.B1.d iter 24)** |
| 3 | UasKind | substrate-typed identity tag (T1 coordinates) | UAS | Verified Floor | driver §4.G G.B1 + T1 scope | `agent_core/src/uas/kind.rs` (landed iter 22 0ac612c3; 8 known variants + Other(String) escape) | F-UAS-ZeroCopy-Spine | **landed (substrate-floor; Phase B.G.B1.b iter 22; T1 variant review pending — coord doc §2)** |
...
| 5 | ACS Anchor | typed anchor (theorem tag · plane coord · residency tier · source hash · active packet id) | ACS | Verified Floor | driver §4.G G.B3 | `agent_core/src/research/acs/anchor.rs` (landed iter 27 01f4ab53) + `agent_core/src/research/acs/anchor_registry.rs` (landed iter 28 2deb32f1) + 4-stage harness `agent_core/tests/acs_anchor_addressing.rs` (landed iter 29 0368d76b) | F-ACS-Anchor-Addressing | **landed (substrate-floor; Phase B.G.B3 iters 27-29; full ClaimLedger/ReplayBundle integration deferred to iter 38+)** |
...
| 8 | HeliosPage | sketch + residual + exact escalation | SHA | Verified Floor | Shadow Memory canon (INT8 sketch + Metal scoring + top-k + exact decode) | `agent_core/src/research/page_gather/{helios_page, sketch_topk, residual_rescore, escalation_policy}.rs` (iters 43-46 b487f26d → e7cc9080) + integration harness `agent_core/tests/page_gather_shadow_escalation.rs` (iter 47 17db5bcb) | F-ShadowFirst-PageEscalation | **landed (substrate-floor; three-stage type + sketch_topk Stage 1 + residual_rescore Stage 2 + EscalationPolicy Stage 3 + harness shape proof; KL drift measurement vs oracle Phase C)** |
...
| 18 | PacketRouter1bit | 1-bit dispatch (MoE-style binary specialization) | KER | Verified Floor | Shazeer et al. arXiv:1701.06538 (sparse MoE) + Fedus et al. arXiv:2101.03961 (Switch Transformer top-1) + `docs/fusion/helios v6.2.md` §4 | `agent_core/src/helios/packet_router.rs` (CPU ref, 439 LOC) + integration harness `agent_core/tests/packet_router_dispatch.rs` (iter 48 c8e0eb16) + Metal stub `Epistemos/Shaders/PacketRouter1bit.metal` | F-PacketRouter1bit-Dispatch | **landed (substrate-floor; CPU-ref at-scale + cross-distribution; Metal kernel + p99 < 100 µs Phase B.G.B5)** |
| 19 | ControllerKernelPack | 6 fused micro-kernels (scalar_add · scalar_mul · max_reduce · argmax · copy_range · zero_fill) | KER | Verified Floor | `docs/fusion/helios v6.2.md` 8-stage §5 | `agent_core/src/helios/controller_pack.rs` (CPU ref, 343 LOC) + integration harness `agent_core/tests/controller_kernel_pack.rs` (iter 49 9e5707c9) | F-ControllerKernelPack | **landed (substrate-floor; 7-size sweep + 6-kernel sequence correctness; Metal kernel + p99 < 50 µs Phase B.G.B5)** |
...
| 26 | Active Assembly Runtime (AAR) | active-pull selector for packets / components / model mechanisms | AAR | Verified Floor | driver §4.G hierarchy + G.B6 | `agent_core/src/research/active_assembly/{packet, selector}.rs` (iter 37-38 d52214ce, 43db76df) + integration harness `agent_core/tests/active_assembly_minimal.rs` (iter 39 79762a98) | F-ActiveAssembly-Minimal | **landed (substrate-floor; PacketGraph DAG + MarginAnchoredGreedyPull selector + 5-invariant shape proof on 200-node graph × 50 queries; full two-sided constraint Phase C)** |
