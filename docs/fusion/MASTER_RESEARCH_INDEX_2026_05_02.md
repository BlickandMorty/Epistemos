...
| `F-ProviderRoute-CopySourceGuard` | PASS metadata-only witness on 2026-06-04. Proves Living Index and lattice HTML copy keep provider-reference, KV-Direct 128K, dense 70B, live sparse 70B, and practical MLX routing source-only with no provider calls, prompt manifests, source laundering, hidden cloud fallback, route-policy mutation, hidden authority, runtime/model bytes, or L2/L3 promotion; downstream `F-TransportTrace-AnswerPacket`, `F-SSD-WearBudget`, `F-ColdStream-vs-Mmap`, `F-SlabArena-CopyCount`, `F-MetalIO-FeatureGate`, `F-CodecStage-Latency`, `F-TransportCancellation`, and `F-CachePolicy-Pollution` now pass metadata-only, so current L1 cursor is `cold_panic_fallback`. |
