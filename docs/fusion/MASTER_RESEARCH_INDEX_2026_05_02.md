...
> Guard-owned product cursor remains automated checks; L2 and L3 remain red.
> Deep Research Pass 110 in
> `docs/fusion/DEEP_RESEARCH_BREAKTHROUGH_SYNTHESIS_2026_06_06.md` converts
> the body-read checksum handoff into the next buildable source-card blueprint:
> `F-SearchIndex-ReleaseBlockerCard`. It treats search freshness as the Eidos
> evidence supply line for large-model replay and requires exact refs for
> `SearchIndexService`, `RRFFusionQuery`, `ReadableBlocksIndex`,
> `ReadableBlocksProjector`, `VaultSyncService`, `QueryRuntime`, graph state/
> store, RRF fusion tests, search-index fusion tests, and query-runtime tests.
> Required proof families include FTS external-content trigger integrity,
> rebuild/fallback behavior, query-parser fallback, RRF k parity, BM25/rank
> convention, recency half-life, vault scope, graph/evidence caveats, privacy,
> rollback, RunEventLog, AnswerPacket, no hidden Eidos route authority, and no
...
| `F-SearchIndex-ReleaseBlockerCard` | T0 build-plan from Deep Research Pass 110 on 2026-06-07. Should prove retained `search_index` release-audit failure is bound before page/block/readable-block FTS, RRF fusion, graph/evidence retrieval, Eidos priors, Gemma QAT replay, KV/cache reuse, TurboVec compressed retrieval, or local research/coding/writing routes can treat search output as fresh evidence. Required refs include `SearchIndexService`, `RRFFusionQuery`, `ReadableBlocksIndex`, `ReadableBlocksProjector`, `VaultSyncService`, `QueryRuntime`, `QueryTypes`, graph state/store, SearchIndex/RRF/QueryRuntime tests, and RRF design docs; required axes include upstream body-read proof, external-content FTS trigger/rebuild/fallback integrity, query parser fallback, RRF k parity, BM25/rank convention, recency half-life, vault scope, graph/evidence no-hidden-authority, privacy/no raw query/body/snippet, rollback, RunEventLog, AnswerPacket, zero DB/body/model/runtime/cache/provider bytes, and no L2/L3/product/live-70B promotion. Not landed yet; no product code changed and no capability promoted. |
