...
The Swift UI includes vault selection, security-scoped bookmarks, LAContext biometric unlock, agent dashboard, Resonance Gate table, and Simulation v1.6 Landing Farm.
...
## Simulation Mode v1.6 acceptance bar

The Swift shell contains Landing Farm, companion creation shell, archive/delete biometric gate, restore view, LoRA adapter unwrap timing view, reduce-motion static fallback, and audit-readable state labels. `AgentEvent` includes the six v1.6 variants: `SteerRequested`, `SummaryStarted`, `SummaryDelta`, `SummaryCompleted`, `VaultCreated`, and `VaultArchived`.
