...
Hermes is modeled as a non-authoritative XPC helper with leased provider capabilities, shared arena metadata, provenance rows, and zero durable memory ownership.
...
## Hackathon Block A acceptance bar

- `/help core` returns the Core-tier parity slate.
- `/calc 2*pi` returns `6.283185...` using the deterministic local evaluator.
- `/ask <prompt>` routes through the active `HermesBoundary` provider and marks provenance pending.
- `/run echo hello` is blocked in Core and becomes a structured Pro/Research passthrough intent; no MAS shell execution occurs.
- Provider switching changes the next `/ask` route.
