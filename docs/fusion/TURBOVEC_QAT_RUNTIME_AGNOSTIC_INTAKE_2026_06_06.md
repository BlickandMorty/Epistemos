...
7. `F-SmallCompressedModel-LiveHarness`
   ...
   - Current next research-to-build model-ladder unit after the owner gate, command card, and model-path readiness card: `small_compressed_model_owner_approved_runtime_probe`. It must be explicitly owner-approved, must use the visible llama-cli card and a real model path, must start with the smallest eligible compressed model, and must not rerun 128K shards, dense 70B, 12B/31B, mmap/SSD stress, hidden server endpoints, or any hidden provider fallback by default.
