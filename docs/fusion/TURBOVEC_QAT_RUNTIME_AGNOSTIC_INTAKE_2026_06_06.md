...
7. `F-SmallCompressedModel-LiveHarness`
   ...
   - Current next research-to-build model-ladder unit after the owner gate: `small_compressed_model_owner_approved_runtime_probe`. It must be explicitly owner-approved and must start with the smallest eligible compressed model; it must not rerun 128K shards, dense 70B, 12B/31B, mmap/SSD stress, or any hidden provider fallback by default.
