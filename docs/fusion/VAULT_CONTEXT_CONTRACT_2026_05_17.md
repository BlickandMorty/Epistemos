...
8. If confidence is low or ambiguous, ask the user or broaden the search. Adversarial lanes must treat a near-tied top fused score as `ADVERSARIAL_CONFUSION`. Do not silently answer from weak retrieval, and do not collapse rejected weak hits into a false "no results" claim.
...
10. Emit a trace for every vault lookup: searched-note count, candidate count, selected count, selected distinct-note count for synthesis checks, top reasons, confidence band, contract-sufficient/high/medium/low confidence counts for the full candidate pool and selected context where applicable, and any degraded signals.
...
13. Shadow-first retrieval may propose candidates from Model2Vec/HNSW/RRF, but dense-only, low-score, ambiguous-margin, evidence-hidden, or empty-query Shadow traces are not answerable until exact lexical/body escalation verifies them. If exact escalation is unavailable, report insufficient evidence.
