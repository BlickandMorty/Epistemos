...
4. Chat enforcement: indexed fallback searches a 50-200 candidate pool, rejects source-rank-only matches, emits per-hit reasons, and appends an explicit `Vault provenance:` block with parseable bold title/path bullets plus `Why:` lines for every grounded fallback note. Prompt contracts require title/path/snippet/body evidence plus that visible block. Low top-score margin and `exact_escalation_required=true` fused-search metadata are ambiguity guards in both cloud/chat and local-agent prompt surfaces, and prompt contracts now require using bounded `exact_escalation_queries` as the verification candidates when fused search provides them while forbidding answers from exact-escalation targets or queries before visible exact verification returns. Single-result indexed fallback now refuses to promote one note when the top two contract-sufficient candidates are separated only by source-order noise; multi-hit fallback keeps the useful candidate context but labels the top match as ambiguous instead of presenting the list as decisive. If indexed hits exist but all fail the evidence contract, the user sees an explicit insufficient-evidence response instead of a no-results claim. Synthesis prompts require at least two independently retrieved vault notes or an honest insufficient-evidence response.
