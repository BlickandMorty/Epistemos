...
5. Notes UI provenance cards: surface why each indexed fallback note was loaded; RRF results carry renderable reasons for callers that surface fused results, and the note chat parser recognizes explicit `Vault provenance:` blocks. Repeated visible-match and explicit-block citations for the same title/path merge into one provenance card with combined reasons, and multiple `Why:` lines under one citation merge without duplicating reasons.
