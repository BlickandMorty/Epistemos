...
| `F-SSD-WearBudget` | PASS metadata-only witness on 2026-06-04. Proves repeated ColdStream plans bind read/write volume, burst, energy, cache-pollution, write-amplification, reuse-horizon, rollback, RunEventLog, SCOPE-Rex/SovereignGate admission, compatibility fence, cache policy, visible wear caveats, and zero runtime/model bytes before SSD-backed route claims can promote. Downstream `F-ColdStream-vs-Mmap`, `F-SlabArena-CopyCount`, `F-MetalIO-FeatureGate`, `F-CodecStage-Latency`, `F-TransportCancellation`, and `F-CachePolicy-Pollution` now pass metadata-only; current L1 cursor is `small_model_runtime_harness_dry_run_witness` after `F-SmallModelRuntimeHarnessSafetyPlan`; L2/L3 do not promote. |
| `F-SlabArena-CopyCount` | PASS metadata-only witness on 2026-06-04. Proves CPU slab plans preallocate buffers, bind lease tables, report copy counts, expose zero per-token allocation deltas, carry rollback/RunEventLog/AnswerPacket/admission/cancellation/fallback refs, and reject hidden authority, live benchmarks, runtime bytes, and product promotion. Downstream `F-MetalIO-FeatureGate`, `F-CodecStage-Latency`, `F-TransportCancellation`, and `F-CachePolicy-Pollution` now pass metadata-only; current L1 cursor is `small_model_runtime_harness_dry_run_witness` after `F-SmallModelRuntimeHarnessSafetyPlan`; L2/L3 do not promote. |
...
| `F-TransportCancellation` | PASS metadata-only witness on 2026-06-05. Proves route epochs, cancellation groups/tokens, route-change refs, leases, schedulers, obsolete-read rejection, stale-slab rejection, rollback, RunEventLog, AnswerPacket, SCOPE-Rex/SovereignGate admission, compatibility fence, visible caveats, and zero runtime/model/transport bytes. Downstream `F-CachePolicy-Pollution` now passes metadata-only; current L1 cursor is `small_model_runtime_harness_dry_run_witness` after `F-SmallModelRuntimeHarnessSafetyPlan`; L2/L3 do not promote. |
