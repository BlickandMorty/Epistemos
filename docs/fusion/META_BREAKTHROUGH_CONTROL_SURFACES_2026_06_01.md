...
| `F-SSD-WearBudget` | PASS metadata-only witness on 2026-06-04. Proves repeated ColdStream plans bind read/write volume, burst, energy, cache-pollution, write-amplification, reuse-horizon, rollback, RunEventLog, SCOPE-Rex/SovereignGate admission, compatibility fence, cache policy, visible wear caveats, and zero runtime/model bytes before SSD-backed route claims can promote. Current L1 cursor is `coldstream_vs_mmap`; L2/L3 do not promote. |
