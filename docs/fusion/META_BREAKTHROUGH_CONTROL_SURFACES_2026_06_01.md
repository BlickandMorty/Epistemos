...
| `F-SSD-WearBudget` | PASS metadata-only witness on 2026-06-04. Proves repeated ColdStream plans bind read/write volume, burst, energy, cache-pollution, write-amplification, reuse-horizon, rollback, RunEventLog, SCOPE-Rex/SovereignGate admission, compatibility fence, cache policy, visible wear caveats, and zero runtime/model bytes before SSD-backed route claims can promote. Downstream `F-ColdStream-vs-Mmap`, `F-SlabArena-CopyCount`, `F-MetalIO-FeatureGate`, `F-CodecStage-Latency`, `F-TransportCancellation`, and `F-CachePolicy-Pollution` now pass metadata-only; current L1 cursor is `small_model_runtime_harness_fresh_product_runtime_l3_release_audit_automated_checks_probe` after downstream `F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditZeroFailProbe`; L2/L3 do not promote. |
| `F-SlabArena-CopyCount` | PASS metadata-only witness on 2026-06-04. Proves CPU slab plans preallocate buffers, bind lease tables, report copy counts, expose zero per-token allocation deltas, carry rollback/RunEventLog/AnswerPacket/admission/cancellation/fallback refs, and reject hidden authority, live benchmarks, runtime bytes, and product promotion. Downstream `F-MetalIO-FeatureGate`, `F-CodecStage-Latency`, `F-TransportCancellation`, and `F-CachePolicy-Pollution` now pass metadata-only; current L1 cursor is `small_model_runtime_harness_fresh_product_runtime_l3_release_audit_automated_checks_probe` after downstream `F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditZeroFailProbe`; L2/L3 do not promote. |
...
| `F-TransportCancellation` | PASS metadata-only witness on 2026-06-05. Proves route epochs, cancellation groups/tokens, route-change refs, leases, schedulers, obsolete-read rejection, stale-slab rejection, rollback, RunEventLog, AnswerPacket, SCOPE-Rex/SovereignGate admission, compatibility fence, visible caveats, and zero runtime/model/transport bytes. Downstream `F-CachePolicy-Pollution` now passes metadata-only; current L1 cursor is `small_model_runtime_harness_fresh_product_runtime_l3_release_audit_automated_checks_probe` after downstream `F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditZeroFailProbe`; L2/L3 do not promote. |
...
| `F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditZeroFailProbe` | PASS L1/L3 zero-fail claim-boundary witness on 2026-06-05. Proves the release-audit zero-fail bar remains mandatory and unclaimed: 14 residual blocker cards, 18 phases, upstream runtime/model bytes `16777216`/`2137326367`, zero zero-fail runtime/model bytes, required automated check surfaces for `Epistemos.xcodeproj`, `graph-engine`, `omega-mcp`, and `omega-ax`, no automated/log/manual/distribution evidence claimed, no ship-call/product-capability promotion, no hidden authority, no route mutation, no MAS/L2/L3/70B/128K/autogenous promotion, and next cursor `small_model_runtime_harness_fresh_product_runtime_l3_release_audit_automated_checks_probe`. L2 remains `vault_research_route_with_packetized_mitigation`; L3 release readiness remains unpromoted. |
| `F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditAutomatedChecksProbe` | RED schema-valid release-audit automated-check witness, hardened on 2026-06-06. Build, graph-engine, omega-mcp, and omega-ax checks passed; `xcodebuild_test` failed and is now recorded as 161 issues across 84 unique tests with failure-family counts led by graph/filter visibility `34`, agent route policy `21`, theme/presentation `19`, distribution/project integrity `18`, research tool catalog `16`, editor/epdoc `14`, UI shell source guard `14`, and model vault/catalog `9`. The top family now has focused repair commands for `FilterEngineComprehensiveTests`, `ResourceExhaustionTests`, and `ConcurrencyEdgeCaseTests` plus graph/filter source anchors. This is repair evidence only: L1 cursor stays parked, L2 remains `vault_research_route_with_packetized_mitigation`, L3 release readiness is unpromoted, and no large-model product capability claim follows from this red ledger. |
