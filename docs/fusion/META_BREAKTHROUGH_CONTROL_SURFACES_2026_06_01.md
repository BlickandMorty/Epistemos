...
| `F-SparseRoute-NoHiddenAuthority` | PASS metadata-only witness on 2026-06-04. Proves sparse route control cannot treat source priors, proof traces, oracle labels, PatternBoost motifs, fast-weight deltas, scout proposals, or sparse wake certificates as hidden live authority before route-source claims promote. Current active cursor moves to `F-ColdStream-NoHiddenAuthority`. |
