...
| `F-CodecStage-Latency` | PASS metadata-only witness on 2026-06-05. Proves codec stages separate file-read traces from decode/conversion latency traces, bind checksums, copy counts, CPU/Metal kernel refs, output slabs or MetalBufferLease refs, rollback, RunEventLog, AnswerPacket, SCOPE-Rex/SovereignGate admission, compatibility fence, cancellation, visible caveats, and zero runtime/model/transport bytes. Current L1 cursor is `transport_cancellation`; L2/L3 do not promote. |
