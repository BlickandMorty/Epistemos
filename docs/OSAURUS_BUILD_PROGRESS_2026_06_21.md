...
- [ ] **Deeper reskin → CLARIFIED to "fully restore the old Epistemos UI, driven by Osaurus" (owner 1485):**
  ...
  - **✅ OPTION-(b) PIVOT IMPLEMENTED (pending Pro verify):** (A) `LocalAgentLoop.shouldRouteActThroughOsaurus`
    ...
    - **✅ ACT↔WORK TOGGLE IMPLEMENTED (pending Pro verify; auditor `dfb411271` flagged it as the follow-up):**
      a CLEAN capsule `WorkspaceModeToggle` in `HomeRouter` (NO armed-dot/"experimental" framing — the old-UI
      `WorkspaceModePicker` carries that drift, wrong now that act+work are live). Routes `.work →
      WorkTerminalHostView(home)`, else the act flow (ChatView/Landing); Pro `#if` (MAS act-only, SwiftTerm is
      Pro). Persisted via `WorkspaceModeSelection`. Persistent so you can switch from any surface, but hidden
      during an active act chat so it never overlays the thread. The landing's own `WorkspaceModePicker` stays
      gated off (`isArmed`), so no duplicate. Additive — the act path is untouched. Built once the auditor
      confirmed the act surface PASS (no longer premature). Exact placement/styling = owner's visual refine.
...
## MAS dual-build — SwiftTerm guard (2026-06-22)
...
- The owner's regression was a REVERT: option-(b) re-mounted the old ChatView, so act became "the same chat,
  just with a badge". To prevent that recurring, added ActSurfaceOsaurusUIDirectionGuardTests — a deterministic
...
- The act P0 is at the buildable done-bar; the owner's mandated runtime-verify is the gate. Wrote
  docs/ACT_SURFACE_RUNTIME_VERIFY_CHECKLIST_2026_06_22.md — an ordered, commit-mapped check the owner runs on
...
- Found a robustness flaw in efe95c8dd: it latched the persistent flag BEFORE checking for owner models, so an
  unlucky first-mount race (bridge not registered yet → no models) would permanently skip seeding. Fixed:
