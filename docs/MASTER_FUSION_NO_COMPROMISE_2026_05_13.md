...
| **UAS-ACS substrate inventory (T3 Phase A)** — no-loss register for §4.G concept reconciliation; maps scattered names to layer, residency tier, current file, falsifier dependency, and status before code changes. | `docs/audits/UAS_ACS_SUBSTRATE_INVENTORY_2026_05_17.md` (`4468b09ac`, T3 local branch) | INVESTIGATING — docs-only substrate inventory landed; `agent_core/src/uas/` and active-assembly/page-gather/local-recall implementation homes remain NOT-STARTED until Phase B |
