...
| **Hyper-Dynamic Schemas (Meta-Schemas that repair themselves)** | `acs_meta_layer.md`; T1 `docs/audits/HYPERDYNAMIC_SCHEMAS_AUDIT_2026_05_17.md` (`74099ea58`..`e49288ff3`); T1 `docs/fusion/TRI_FUSION_HYPERDYNAMIC_SCHEMAS_2026_05_17.md` (`ebcfbdd20`..`87d336711`); T1 `e117ef855`; T1 `116257807`; T1 `44936eb9e`; T1 `499130ad9`; T1 `c953fa00e`; T1 `d37833ca4` | IN-FLIGHT — T1 audit/doc pass mapped schema source anchors, round-trip proof obligations, FFI/provenance boundary, doctrine format contract, agent-facing API, model wiring, and editor mutation wiring; branch implementation now has a canonical JSON document floor, path-aware document schema validation, typed mutation witness floor, a Swift-facing UniFFI document handle, a LocalToolGrammar Tri-Fusion mutation tool schema, and a mutation-application bridge acknowledgement path. `agent_core/src/lib.rs` remains a pending scope-exception note and generated artifacts are dirty before merge-readiness | branch implementation pending T1 scope-exception / artifact cleanup |
...
| **F-VaultRecall-50 fallback candidate contract** | T4 `docs/fusion/VAULT_CONTEXT_CONTRACT_2026_05_17.md`; T4 `4e0f74372`; T4 `2c4f0d1bf`; T4 `6998cc41f`; T4 `5bbe32951`; T4 `2657a6469`; T4 `dd11893d2`; T4 `docs/falsifiers/F-VaultRecall-50_2026_05_17.md` | IN-FLIGHT / PARTIAL PASS — branch patch widens indexed fallback candidate pool to at least 50 when inventory supports it, applies true recency half-life in RRF fusion, strips query boilerplate, recovers exact-title/original-note hits by path title, seeds synthesis side titles, and now emits renderable RRF provenance reasons. Falsifier now passes exact-title, Top-5 paraphrase, agent-context proxy, zero first-7 enumeration, synthesis diversity, and adversarial reject bars; visible UI `why selected` remains the final risk because `dd11893d2` touched query/test surfaces, not UI | T4 local branch; push / PR pending |
