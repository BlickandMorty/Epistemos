...
| **GBNF / structured output** | PARTIAL | LocalToolGrammar + MLXStructured + JSONSchema + soft-guidance fallback; T2 `4f7b9df60` model grammar matrix; T2 `ed7ff2531` Settings strict-grammar/schema-drift diagnostics; T2 pushed `46ac80bba` native grammar profiles; T2 `6c5526fa8` task-class model selection (scope note pending for Swift test path); T2 `79cb183ee` AgentBlueprint mission runner; T2 `ea2792cd1` AgentBlueprint model-choice runtime badges; T2 `59abb152a` AgentBlueprint MissionPacket persistence/replay | — | per-tool `&'static str` GBNF / Tool trait JsonSchema macro / ≤256 reasoning enforcement / per-model strict masking fixtures / global per-model capability badges + 36B-on-16GB runtime proof |
