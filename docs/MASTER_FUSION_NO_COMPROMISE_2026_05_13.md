...
| **Phase A — CPU foundation + zero-copy** — 4-week target, **SHIPPED** on this branch (`codex/research-snapshot-2026-05-08`). Per PASS 2 B2-H15 verification: 2629 tests passing post-Phase-A. Rust orchestrates topology and writes positions/velocities into Metal-allocated `.storageModeShared` buffers; uniform-grid repulsion + GraphPOPE-lite warm-start + reveal FSM all in CPU code. | source §"Phase A" + audit verification | **LANDED** |
