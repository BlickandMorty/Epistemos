...
| **LoRA / MambaPEFT fine-tuning of `in_proj` / `x_proj` / `dt_proj` / `out_proj`** — nightly fine-tune on personal notes corpus via existing ODIA pipeline; hot-swap adapters via MoLoRA infrastructure | source §Phase 3 (Ω20) | NOT-STARTED |
...
| **Source papers**: MemMamba (threshold-triggered state summarization, 2025) · Model2Vec (paragraph-level fast embed) · TurboQuant (PolarQuant + QJL residual) · MambaPEFT / Memba PEFT (in_proj/x_proj/dt_proj/out_proj targeting) | source §"Source Papers" | reference |
