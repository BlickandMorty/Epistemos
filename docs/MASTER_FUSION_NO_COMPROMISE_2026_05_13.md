...
### 3.44 EML Integration Substrate — runtime-layer adapters over the EML primitive (T7 §4.B)

**Source:** `docs/audits/EML_AUDIT_2026_05_17.md` (substrate state) + `docs/fusion/EML_INTEGRATION_DOCTRINE_2026_05_17.md` (integration plan + 5 candidate sites) + `docs/audits/EML_AUDIT_OF_AUDIT_2026_05_17.md` (post-implementation verification). Companion code crate area: `agent_core/src/research/eml_integration/` (Rust runtime layer; T5 owns the IR layer, see §4 disambiguation `EML (d) EML-IR`).

| Concept | Source | Status |
|---|---|---|
| **EML primitive substrate floor.** `eml(x, y) = exp(x) − ln(y)` over f64 with grammar `S → 1 \| eml(S, S)`, recursive evaluator (depth ≤ 32), and AnswerPacket-freeze gate guarded by the 1024-sample fp16-ULP smoke oracle at 2-ULP shipping bar. 1,232 LOC across 6 files in `agent_core/src/research/eml/`, 74 in-module property tests. | Odrzywołek arXiv:2603.21852 (universality on Liouvillian-solvable subdomain) + Stachowiak arXiv:2604.23893 (Abelian-group decomposition) + `agent_core/src/research/eml/mod.rs:6-10` | **SHIPPED** (pre-T7; substrate floor) |
| **§5.0 reconciliation row — what EML IS and IS NOT.** EML is a Liouvillian-elementary arithmetic primitive with a Smith-quintic hard fence (`eml/mod.rs:42-45`). EML is **not** a LeCun Energy-Based Model (no sampler, no Z partition, no contrastive divergence), **not** a diffusion / score-matching model (no noise schedule), **not** a Hopfield/RBM (no Gibbs sampling), **not** a probabilistic model at all. When this row uses the word *"potential"* or *"energy"* it means *the f64 value of an `EmlExpr` evaluated by `eml::evaluator::evaluate`, optionally post-composed with a documented monotone normalization*. Any doctrine row calling EML an "EBM" is drift. | `docs/audits/EML_AUDIT_2026_05_17.md` §6 (full IS/IS-NOT enumeration) + grep-verified absence of `Sampler\|langevin\|gibbs\|partition\|train\|gradient_step` in `research/eml/` and `research/eml_integration/` 2026-05-17 | **doctrine boundary** |
| **`eml_integration::potential::EmlPotential` — the encoding primitive.** Monotone encoding `(x, y) = (ln(1+s), 1+s)` → `value = eml(x, y) = (1+s) − ln(1+s)`. Floor `value(0) = 1.0`; strictly monotone-increasing for `s > 0` (derivative `1 − 1/(1+s) > 0`); rejects `s < 0` and non-finite `s` with typed errors. 15 property tests pin the floor, monotonicity, closed-form match, big-`s` asymptote, and error-rejection. | `agent_core/src/research/eml_integration/potential.rs` + `docs/fusion/EML_INTEGRATION_DOCTRINE_2026_05_17.md` §3.2 | **SHIPPED (T7 iter 3, commit `f18627f24`)** |
| **`eml_integration::observatory` — SAE Cognition Observatory anomaly augmentation (the MVP integration site).** Read-only consumer of `cognition_observatory::sae::{LabeledScore, auc_roc}` (§3.36 doctrine pin AUC ≥ 0.90 is invariant under this overlay). Produces `AugmentedObservation` with the raw score, the `EmlPotential`, and the original label. **Cornerstone property** (Hanley & McNeil 1982 rank-based AUC invariance under strictly monotone score transforms): `auc_on_augmented(obs) ≡ auc_roc(obs)` modulo a < 1e-5 f32-cast tolerance. Pinned by 5 distinct property tests across lib + integration suites. | `agent_core/src/research/eml_integration/observatory.rs` + `agent_core/tests/eml_observatory.rs` + Hanley & McNeil 1982 (cited in `cognition_observatory/sae.rs:11-13`) + doctrine §3.3 | **SHIPPED (T7 iter 4 + 6, commits `c2d0aab80` + `01318d76a`)** |
| **`eml_integration::diagnostic::compute_live_readout()` — Settings → Diagnostics payload.** Bundles: ULP smoke-oracle health (max/mean error + within-bar count + fraction + 2.0 shipping bar) · AnswerPacket schema-freeze gate verdict · `EmlPotential::from_score(1.0)` sentinel (= `2 − ln(2) ≈ 1.3068528…` as forward-stable canary) · verbatim Smith-quintic universality fence text (`UNIVERSALITY_FENCE_TEXT` const) so the user-visible row carries the caveat. Serde-roundtrip safe; deterministic across calls. 10 in-module property tests. | `agent_core/src/research/eml_integration/diagnostic.rs` + doctrine §3.4 + `eml/mod.rs:42-45` (the fence text source) | **SHIPPED (T7 iter 5, commit `0920347d6`) — Rust side · Swift `EmlEnergyHealthRow` mirror Phase C** |
| **EML→Tri-Fusion ambiguity resolution (candidate site (a)).** MD ⇄ JSON ⇄ HTML round-trip yielding multiple valid parses → pick `argmin_i eml(NLL_i, recall_i)`. | doctrine §2.(a) | **FORWARD-STAGE — coord T1** (Tri-Fusion module not yet stable enough; tri_fusion is in T7 SCOPE LOCK don't-touch list) |
| **EML→ConfidenceRouter scoring (candidate site (b)).** `(x, y) = (request_complexity_log, model_recall_estimate)`; potential acts as inverse-confidence routing key. | doctrine §2.(b) | **FORWARD-STAGE — coord T2** (agent_runtime ConfidenceRouter not yet exposing extension hook; agent_runtime is in T7 SCOPE LOCK don't-touch list) |
| **EML→Kuramoto coupling tempering (candidate site (c)).** When global order `R(t)` exceeds 0.95 threshold, damp coupling via the `-1/y` partial as a gradient term. | doctrine §2.(c) | **FORWARD-STAGE — coord T3** (acs/kuramoto.rs in T7 SCOPE LOCK don't-touch list; UAS-ACS overall plan in T3's scope) |
| **EML→F-VaultRecall-50 re-ranking (candidate site (d)).** Re-rank query results by `eml(-log(bm25 + ε), recall_fraction)`; smaller-is-better. | doctrine §2.(d) | **FORWARD-STAGE — coord T4** (storage/vault.rs in T7 SCOPE LOCK don't-touch list; F-VaultRecall-50 dataset stabilization is T4's scope) |
| **Acceptance bar (T7 §4.B prompt verbatim):** "EML called by ≥ 2 modules · property-test-backed · diagnostic row visible." Re-grep verification 2026-05-17: 3 sub-modules call into `eml/` (potential direct · diagnostic direct · observatory transitive); 56 new property tests added (target +30); Rust-side diagnostic payload live with serde-roundtrip safety. Default-features cargo baseline 1671/1671 held across all 7 T7 commits. | `docs/audits/EML_AUDIT_OF_AUDIT_2026_05_17.md` §6 | **acceptance bar cleared** |
| **Cross-references** — §3.36 SAE Cognition Observatory (consumer of the MVP overlay) · §3.21 Ternary core (sibling research-tier substrate) · §3.22 Continual learning (separate post-training family) · §4 disambiguation row "EML" (4 distinct meanings; T7 work is meaning (a) Odrzywolek operator). | (this row) | doctrine cross-links |
| **Why this row is canon, not just code.** Per the §3.43 PR-discipline rule landed 2026-05-16: any new substantive Rust module added to `agent_core/` MUST include a MASTER_FUSION §3.X doctrine row in the same commit (or, when added across multiple commits, retroactively before the slice closes). T7's eml_integration crate area (`mod.rs` + 3 sub-modules + 1 integration-test file + 3 doctrine/audit docs) is the kind of substantive addition that needs canon anchoring; this row is that anchor. Future agents land forward-staged candidate sites by adding rows below this one (or extending in place), never by silently widening eml/. | doctrine reconciliation per §3.43 rule + T7 audit-of-audit 2026-05-17 | doctrine reconciliation |
