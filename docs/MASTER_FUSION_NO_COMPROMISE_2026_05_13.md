...
| **Hyper-Dynamic Schemas (Meta-Schemas that repair themselves)** | `acs_meta_layer.md`; T1 `docs/audits/HYPERDYNAMIC_SCHEMAS_AUDIT_2026_05_17.md` (`74099ea58`..`e49288ff3`); T1 `docs/fusion/TRI_FUSION_HYPERDYNAMIC_SCHEMAS_2026_05_17.md` (`ebcfbdd20`..`87d336711`); T1 `e117ef855`; T1 `116257807` | IN-FLIGHT — T1 audit/doc pass mapped schema source anchors, round-trip proof obligations, FFI/provenance boundary, doctrine format contract, agent-facing API, model wiring, and editor mutation wiring; branch implementation now has a canonical JSON document floor and path-aware document schema validation, but `agent_core/src/lib.rs` is a pending scope-exception note before merge-readiness | branch implementation pending T1 scope-exception resolution |
