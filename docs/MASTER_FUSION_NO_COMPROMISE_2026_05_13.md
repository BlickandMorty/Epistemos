...
| **EML integration doctrine / substrate audit (T7 iter32 overlay)** | T7 `bdf991c8d`; T7 `e9314bf04`; `docs/audits/EML_AUDIT_2026_05_17.md`; `docs/fusion/EML_INTEGRATION_DOCTRINE_2026_05_17.md` | INVESTIGATING / AUDIT+DOCTRINE BRANCH START — existing `agent_core/src/research/eml/` remains an island with no external callers; T7 proposes `agent_core/src/research/eml_integration/**` for SAE Cognition Observatory MVP. Live `agent_core/src/research/mod.rs` registration is a narrow exception requiring rationale before PR | T7 local branch; implementation WIP not main-merged |
| **EML-IR doctrine / EML Observatory implementation overlays (T5/T7 iter33)** | T5 `0b14c779b`; T5 `078bbce83`; T5 `201d35515`; T7 `0920347d6`; T7 `01318d76a`; T7 `3476f0629`; `docs/audits/EML_IR_AUDIT_2026_05_17.md`; `docs/fusion/PRIMITIVE_IR_STACK_DOCTRINE_2026_05_17.md`; `docs/audits/EML_AUDIT_OF_AUDIT_2026_05_17.md` | INVESTIGATING / BRANCH IMPLEMENTATION START — T5 added docs-only EML-IR substrate audit + primitive IR stack doctrine. T7 added `eml_integration` diagnostic live-readout surface, `agent_core/tests/eml_observatory.rs`, and audit-of-audit. T7 test path needs explicit rationale before PR; neither branch is main-merged | T5/T7 local branches |
| **EML iter34 custody + CLI/coordination overlay** | T5 `6714761cc`; T5 `f99cdc02d`; T7 `4005f302e`; T7 `0a91d3698`; T7 `2fc19ca6b`; T7 `457ac4cb7`; T7 `9e2c8e90e`; `docs/audits/EML_COORD_DEP_STATUS_2026_05_17.md` | INVESTIGATING / BRANCH SURFACES EXPANDED — T5 created `research_custody/*` skeletons and claims for all six IRs. T7 added `AugmentedSummary` diagnostic surfacing, an `epistemos_eml` CLI readout, EML doctrine implementation log, and coordination-dependency audit. T7 `eml_integration/**` and live `eml/**` work are in-lane; `agent_core/Cargo.toml`, `agent_core/src/bin/epistemos_eml.rs`, and `CLAUDE.md` need explicit scope rationale before PR | T5/T7 branches; not main-merged |
| **EML iter35 IR doctrine + serde overlay** | T5 `9f3fb782e`; T5 `d7cd4a1a4`; T7 `7ddd6763e`; `docs/audits/TROPICAL_IR_RECONCILIATION_PLAN_2026_05_17.md` | INVESTIGATING / BRANCH SURFACES EXPANDED — T5 added the Tropical IR reconciliation plan and primitive-IR cross-composition lattice doctrine. T7 added serde derives for downstream JSON consumption across `eml/operator.rs`, `eml_integration/observatory.rs`, and `eml_integration/potential.rs`. T5 is ahead of origin by 2; T7 is local/no-upstream. Prior T7 CLI manifest/bin and `CLAUDE.md` scope-rationale debt carries | T5/T7 branches; not main-merged |
| **EML iter36 Phase A closeout + CLI/session overlay** | T5 `63eb704f6`; T7 `fee5add02`; T7 `958bfc6b0`; T7 `4312fff8a`; `docs/audits/PHASE_A_CLOSEOUT_2026_05_17.md`; `docs/SESSION_REPORT_T7_2026_05_17.md` | INVESTIGATING / BRANCH SURFACES EXPANDED — T5 closed Phase A with audit-of-audit and Phase B1 entry doctrine, now clean/tracking origin. T7 extended the `epistemos_eml` CLI with `--observations-stdin` and added session docs, but `agent_core/src/bin/epistemos_eml.rs` remains CLI/bin exact-scope debt. Live T7 `eml_integration/**` WIP is in-lane; branch is local/no-upstream | T5/T7 branches; not main-merged |
| **EML iter37 B1 implementation + display overlay** | T5 `df41be778`; T5 `4e0cbf253`; T5 `99f54d506`; T5 `594502e20`; T5 `e5f45c316`; T7 `a934a2e31`; T7 `8fcfce4ff`; T7 `77fda079c`; `docs/audits/EML_AUDIT_OF_AUDIT_2_2026_05_17.md` | INVESTIGATING / BRANCH IMPLEMENTATION EXPANDED — T5 added in-scope EML-IR closure, normalize, branched expression, and certificate modules under `agent_core/src/research/eml/` plus EML custody updates; branch is ahead of origin by 4 with live in-lane certificate WIP. T7 added audit-of-audit #2 and Display impls for EmlPotential / AugmentedSummary in `eml_integration/**`; branch remains local/no-upstream with prior CLI/CLAUDE rationale debt | T5/T7 branches; not main-merged |
...
| **L4** | **Engram (O(1) hash-recall for static knowledge — B2-M12)** — fixed-budget hash table for facts/signatures/dates/API-contracts/never-recompute knowledge. Separates **static** lookup from **dynamic** reasoning (MoE / attention / SSM stack). DeepSeek V4 Preview's *Sparsity Allocation Law*: ~20-25% of resident memory reserved for static-fact recall, ~75-80% for compute / dynamic reasoning. Substrate at `epistemos-research/src/engram.rs` (274 LOC, Lane 3 RESEARCH-ONLY) ships hash-table TYPE substrate (insert / lookup / capacity bounds) + `recommended_static_fraction` constant (heuristic, not theorem). Maps to `agent_core::resonance::lambda::ResidencyLevel::L4Engram` Pro+ tier. **Caveat from preservation source:** "Sparsity Allocation Law is heuristic not proven theorem; O(1) is hash-table-lookup-only and ignores collision resolution + cache effects." | `epistemos_resonance_gate.md §2.2` + `helios_shadow_memory.md` + `EPISTEMOS_RECONCEPTUALIZATION_2026_05_03.md` + `agent_core/src/resonance/lambda.rs:34` (`L4Engram`) + `epistemos-research/src/engram.rs` | PARTIAL — type substrate shipped Lane 3 RESEARCH-ONLY; not yet wired into a residency-routing call site. Pro+ entitlement gates it out of Core/App Store builds (per `lambda.rs:CORE_ALLOWED` 5-tier whitelist). |
| **L5** | Network Cascade (Hermes-4-405B style cloud) | `helios v3.md`, `hermes_gateway_architecture.md` | PARTIAL — cloud providers wired via URLSession; full cascade routing NOT-STARTED |
| **L_SE** | Self-Evolving (Titans-MAC + SEAL-DoRA nightly; maps to `ResidencyLevel::L5Adapter` Pro+ in code) | `epistemos_definitive_master.md` | NOT-STARTED |

> **§3.2 doctrine-vs-code alignment note (B2-M12, recorded 2026-05-16).** The code-side `agent_core::resonance::lambda::ResidencyLevel` enum carries **8 tiers**: L0Working / L1Recent / L2Warm / L3Cold / **L4Engram** (Pro+) / **L5Adapter** (Pro+) / L6Forbidden (Research-only, Secure Enclave seal) / L7Quarantine (Core-allowed, τ=-1 rejected-claim sink). This iter renumbers §3.2's prior L4 (Network Cascade) → L5 to make room for L4 Engram alongside the code's `L4Engram` variant. **Residual drift acknowledged:** the §3.2 doctrine table still doesn't enumerate L6 Forbidden or L7 Quarantine — those are tracked as Pro+/Research-only and Core-allowed-quarantine respectively in `lambda.rs:CORE_ALLOWED` + `lambda.rs:requires_pro_or_research`, but their doctrine rows are deferred to a future slice (post-V1 alignment pass). `lambda.rs:55-56` enforces "A Core path that emits an L4–L6 target is a P0 tier-leakage bug" — that gate is canonical regardless of whether §3.2 enumerates every tier. **PR discipline going forward:** any change to `ResidencyLevel` enum variants MUST update both `lambda.rs` AND §3.2 in lockstep; existing 8-vs-6 gap is the legacy state being closed by this row.
...
| **Hyper-Dynamic Schemas (Meta-Schemas that repair themselves)** | `acs_meta_layer.md`; T1 `docs/audits/HYPERDYNAMIC_SCHEMAS_AUDIT_2026_05_17.md` (`74099ea58`..`e49288ff3`); T1 `docs/fusion/TRI_FUSION_HYPERDYNAMIC_SCHEMAS_2026_05_17.md` (`ebcfbdd20`..`87d336711`); T1 `e117ef855`; T1 `116257807`; T1 `44936eb9e`; T1 `499130ad9`; T1 `c953fa00e`; T1 `d37833ca4`; T1 `20e74e8eb`; T1 `15321659d`; T1 `ed1b8c058`; T1 `ccef1c9ab`; T1 `bf08a43dd`; T1 `6b7e5f46b`; T1 `fc9efe18d`; T1 `b2b6c1ec4`; T1 `c035afd25`; T1 `327a2a42c`; T1 `ec1544b1b`; T1 `2b91f561f`; T1 `f292e3709`; T1 `28eca4075`; T1 `cefcbb89d`; T1 `ff23b1c67`; T1 `7a00d7400`; T1 `06b7822d5`; T1 `c27f92572`; T1 `83b4ea215`; T1 `db5e03e7e`; T1 `e304b6034`; T1 `9d3dff13c`; T1 `2d1dbe9fb` | IN-FLIGHT — T1 audit/doc pass mapped schema source anchors, round-trip proof obligations, FFI/provenance boundary, doctrine format contract, agent-facing API, model wiring, and editor mutation wiring; branch implementation now has a canonical JSON document floor, path-aware document schema validation, typed mutation witness floor, a Swift-facing UniFFI document handle, a LocalToolGrammar Tri-Fusion mutation tool schema, mutation-application bridge acknowledgement, grammar-shaped mutation envelopes, stale base-hash rejection, envelope metadata in witnesses, explicit deferred provenance linkage fields, Swift grammar actors that require `run_id` for agent-authored mutations, Rust integration coverage for mutation envelope preservation / stale-hash rejection / actor decoding, a Swift-side Epdoc mutation receiver gate that validates envelopes before forwarding canonical `applyTriFusionMutation` editor commands, a Swift-generated JS fallback that applies structured mutations to live `window.epdocEditor` JSON and visibly marks model-authored blocks, 200 deterministic JSON corpus cases, mutation-envelope provenance derivation, ClaimLedger witness commits, Cognitive DAG provenance ID / verification coverage, provenance-id FFI exposure, missing-DAG-provenance edge coverage, rationale-required Epdoc receiver enforcement, Rust FFI apply-and-commit provenance coverage for round-trip acceptance tracking, committed Swift FFI document round-trip coverage, Swift provenance mutation-path decoding / witness assertions, committed Tri-Fusion Epdoc prompt guidance, canonical Markdown projection, 50 deterministic byte-equal Markdown fixtures, and Markdown document FFI / canonical projection bridge coverage. `agent_core/src/lib.rs` remains a pending scope-exception note; `EpistemosTests/EpdocEditorBridgeTests.swift`, `RustTriFusionDocumentClient.swift`, `RustTriFusionDocumentClientTests.swift`, `LocalAgentPromptBuilder.swift`, and `HermesPromptBuilderTests.swift` are committed exact-scope / coordination debt; `agent_core/tests/tri_fusion_markdown_round_trip.rs` needs exact test-path rationale; generated artifacts remain before merge-readiness | branch implementation pending T1 scope-exception / Swift-test rationale / Swift FFI client + prompt-builder coordination / Markdown test-path rationale / artifact cleanup |
| **Hyper-Dynamic Schemas iter32 cross-format corpus overlay** | T1 `cd12d1ac7`; T1 `58ddcd4d0`; T1 `a460f399f` | IN-FLIGHT / TEST-CORPUS EXPANDED — branch adds JSON/Markdown/HTML coherence proof, 40 common-subset cross-format fixtures, and 50 deterministic HTML tree fixtures. Live `agent_core/tests/tri_fusion_corpus_report.rs` is mission-aligned but needs exact-path rationale before PR | T1 branch tests; not main-merged |
| **Hyper-Dynamic Schemas iter33 mutation corpus overlay** | T1 `f51cd6bc3`; T1 `f476b9d1c` | IN-FLIGHT / TEST-CORPUS EXPANDED — branch adds corpus acceptance reporting and mutation witness corpus coverage. Live Swift FFI client/test paths are dirty again and remain committed scope-debt surfaces from earlier T1 passes | T1 branch tests; not main-merged |
| **Hyper-Dynamic Schemas iter34 Swift FFI round-trip overlay** | T1 `4fdd9f63c` | IN-FLIGHT / SWIFT FFI COVERAGE EXPANDED — branch adds Swift Markdown/HTML FFI document round-trip coverage in `RustTriFusionDocumentClient.swift` and `RustTriFusionDocumentClientTests.swift`. The paths continue the committed Swift FFI exact-scope / coordination debt line; live Epdoc bridge work is partly in-lane while Swift-test rationale, footer, artifacts, and the historical `lib.rs` exception remain pending | T1 branch Swift FFI tests; not main-merged |
| **Hyper-Dynamic Schemas iter35 Epdoc highlight-gate overlay** | T1 `19daab802` | IN-FLIGHT / EPDOC RECEIVER HARDENED — branch gates model-authored Epdoc highlight markers so they only apply on model-authored mutation paths. `EpdocEditorBridge.swift` is in-lane; `EpistemosTests/EpdocEditorBridgeTests.swift` repeats Swift-test exact-scope rationale debt. Live prompt-builder/test drift, footer mismatch, generated artifacts, and the historical `lib.rs` exception still carry | T1 branch Epdoc bridge/tests; not main-merged |
| **Hyper-Dynamic Schemas iter36 prompt/provenance overlay** | T1 `a9ac16c04`; T1 `739ed2643` | IN-FLIGHT / PROMPT + PROVENANCE TESTS EXPANDED — branch hardens local-agent mutation prompt behavior and proves provenance mirror commit behavior in Tri-Fusion. `agent_core/src/tri_fusion/mod.rs` is in-lane, while `LocalAgentPromptBuilder.swift`, `HermesPromptBuilderTests.swift`, and `LocalToolGrammarTests.swift` are committed prompt-builder / Swift-test exact-scope rationale debt. Generated artifacts, footer mismatch, and the historical `agent_core/src/lib.rs` exception still carry | T1 branch; not main-merged |
| **Hyper-Dynamic Schemas iter37 FFI provenance/corpus overlay** | T1 `d71e19d20`; T1 `1be496d8f`; T1 `59ded876b` | IN-FLIGHT / FFI PROVENANCE + CORPUS EXPANDED — branch proves FFI provenance commit behavior, hardens Swift provenance witness coverage, and adds round-trip corpus shards. `agent_core/src/bridge.rs` and `agent_core/src/tri_fusion/mod.rs` are in-lane; `RustTriFusionDocumentClientTests.swift` repeats Swift-test exact-scope rationale debt. Generated artifacts, footer mismatch, and historical exception debt carry | T1 branch; not main-merged |
...
| **UAS-ACS substrate inventory (T3 Phase A)** — no-loss register for §4.G concept reconciliation; maps scattered names to layer, residency tier, current file, falsifier dependency, and status before code changes. | `docs/audits/UAS_ACS_SUBSTRATE_INVENTORY_2026_05_17.md` (`4468b09ac`; updated by `08137d05f`, `ecaabd5e2`); `docs/fusion/UAS_ACS_CANONICAL_ARCHITECTURE_2026_05_16.md` (`d00d72eb2`; updated by `08137d05f`, `961a32c60`, `ecaabd5e2`); `docs/audits/UAS_ACS_MORPH_DEEP_DIVE_2026_05_17.md` (`08137d05f`); `docs/falsifiers/F-UAS-ZeroCopy-Spine_2026_05_17.md` (`6745c19a7`); `docs/falsifiers/F-ACS-Anchor-Addressing_2026_05_17.md` (`a26a20803`); `docs/falsifiers/F-ShadowFirst-PageEscalation_2026_05_17.md` (`318402afc`); `docs/falsifiers/F-PageGather-M2Pro_2026_05_17.md` (`6ad5c763b`); `docs/falsifiers/F-ActiveAssembly-Minimal_2026_05_17.md` (`ead9302d2`); `docs/falsifiers/F-KV-Direct-Gate_2026_05_17.md` (`72cfcffd9`); `docs/falsifiers/F-SemiseparableBlockScan-Correctness_2026_05_17.md` (`7d5fc2822`); `docs/falsifiers/F-LocalRecallIsland-32K_2026_05_17.md` (`18654d983`); `docs/falsifiers/F-PacketRouter1bit-Dispatch_2026_05_17.md` (`838955711`); `docs/falsifiers/F-ControllerKernelPack_2026_05_17.md` (`a3e302180`); `docs/falsifiers/F-70B-Local-Cocktail-Composition_2026_05_17.md` (`b392a61b7`); `docs/falsifiers/F-ULP-Oracle_2026_05_17.md` (`e432b54f1`); `docs/audits/UAS_ACS_T_TERMINAL_COORDINATION_2026_05_17.md` (`ecaabd5e2`); `docs/audits/UAS_ACS_PHASE_B_BLUEPRINT_2026_05_17.md` (`1a2bbe01d`); `docs/audits/UAS_ACS_AUDIT_OF_AUDIT_2026_05_17.md` (`96c861c72`); `docs/audits/UAS_ACS_PHASE_A_CLOSEOUT_2026_05_17.md` (`52293dfaa`); T3 `4f74a7e98`; T3 `0ac612c3b`; T3 `b2ed9a034`; T3 `ce06b9f6c`; T3 `01f892bf2`; T3 `01f4ab53f`; T3 `2deb32f18`; T3 `0368d76b1`; T3 `b10986cb9`; T3 `912c7b98b`; T3 `7975f57d6`; T3 `0a316f530`; T3 `d5f419b94`; T3 `998835f71`; T3 `8f2bec8b4`; T3 `d52214ce6`; T3 `43db76df2`; T3 `79762a980`; T3 `f4d5baa4e`; T3 `6624225c1`; T3 `90c5484be`; T3 `b487f26de`; T3 `6580e5e53`; T3 `4f42c8a9e`; T3 `c8e0eb16d`; T3 `9e5707c95`; T3 `45132d685` | INVESTIGATING / PHASE B.G.B1 BRANCH-COMPLETE + ACS ANCHOR / ZERO-COPY SUBSTRATE-FLOOR PASS / ACTIVE-ASSEMBLY SUBSTRATE-FLOOR HARNESS / PAGE-GATHER STAGE 1-2 SUBSTRATE / PACKET-ROUTER + CONTROLLER-KERNEL SUBSTRATE-FLOOR — docs-only substrate inventory, canonical architecture/no-loss register, Morph deep-dive correction, 11 §4.G ladder falsifier docs, V6.1 W1 F-ULP-Oracle foundation gate, full 41-row MASTER_FUSION cross-link coverage, T-terminal coordination matrix, Phase B implementation blueprint, audit-of-audit, Phase A closeout, Phase B.G.B1 UAS address / kind / residency tier / residency lease / witness-emission code, local AcsAnchor coordinate object, AnchorRegistry lookup primitives, the F-ACS-Anchor-Addressing 4-stage harness, Phase B iter-30 / iter-40 / iter-50 audit-of-audit, doctrine status catch-up, UAS copy-counter tracking primitives, F-UAS-ZeroCopy path 1/2/3/4/5 acceptance tests, the substrate-floor PASS report, F-ActiveAssembly PacketGraph / MarginAnchoredGreedyPull / substrate-floor harness, PageGather HeliosPage / sketch_top_k / residual_rescore substrate, F-PacketRouter1bit substrate-floor tests, and F-ControllerKernelPack substrate-floor tests landed on T3. UAS / ACS / active_assembly / page_gather paths are in-lane; `agent_core/src/lib.rs`, `agent_core/src/scope_rex/residency.rs`, `agent_core/src/research/mod.rs`, `active_assembly_minimal.rs`, `packet_router_dispatch.rs`, and `controller_kernel_pack.rs` are narrow exception/rationale items. Live escalation-policy WIP remains in the page_gather lane |
| **UAS-ACS iter32 long-context substrate overlay** | T3 `870b692d0`; T3 `19965e65a`; T3 `f72f5ded3` | INVESTIGATING / PHASE B LONG-CONTEXT SUBSTRATE EXPANDED — LocalRecallIsland-32K, SemiseparableBlockScan Track A correctness, and PageGather M2 Pro CPU twin tests landed. `page_gather_m2pro.rs` is in the written lane; `local_recall_island_32k.rs`, `ssd_block_scan_correctness.rs`, live `long_context_harness.rs`, and `agent_core/src/helios/mod.rs` need exact-path / re-export rationale before PR | T3 branch tests; not main-merged |
| **UAS-ACS iter33 RULER+BABILong overlay** | T3 `fdffdf5bf`; T3 `41588ec77` | INVESTIGATING / LONG-CONTEXT SUBSTRATE EXPANDED — RULER+BABILong harness landed in `agent_core/tests/long_context_harness.rs`, and the F-UAS-ZeroCopy PASS report was refreshed. Branch is ahead of origin by 2; `long_context_harness.rs` remains an exact-filename rationale item | T3 branch tests/docs; not main-merged |
| **UAS-ACS iter35 T5/T7 boundary clarification overlay** | T3 `75d6407d8`; `docs/audits/UAS_ACS_T_TERMINAL_COORDINATION_2026_05_17.md` | INVESTIGATING / COORDINATION CLARIFIED — T3 recorded the T5/T7 EML boundary: T5 owns primitive IR doctrine/custody while T7 owns EML integration / observatory implementation. Commit is docs-only and scope-clean; existing UAS-ACS exact-filename / module-surface rationale debt remains unchanged | T3 branch docs; not main-merged |
| **UAS-ACS iter36 ternary/Sherry research-test overlay** | T3 `08865a28a`; T3 `aeb614f2b` | INVESTIGATING / RESEARCH SUBSTRATE TESTS EXPANDED — branch added Wave J1 ternary packing and Wave J7 Sherry 3:4 sparse-codec tests. These align with T3's read-first ternary/Sherry research sources and no-loss register, but `agent_core/tests/ternary_packing.rs` and `agent_core/tests/sherry_3_4_codec.rs` sit outside the written `tests/uas_*`, `tests/acs_*`, `tests/page_gather_*` shorthand and need exact-filename rationale before PR | T3 branch tests; not main-merged |
...
| **State ladder**: implemented → emitted → populated → rendered → canonical-product-surface | V6.2 progress; T2 `9b090203d`; T2 `edb69ec47`; T2 `a3e177e92` | rendered (chip lookups via sink); branch attempts at durable packet persistence and replayable agent-run timelines are NOT accepted as merge-ready until T2 scope violations / T6 UI coordination are resolved |
...
| **Provenance Console** | `EPISTEMOS_FINAL_DOCTRINE` §4, audit reconciliation; T2 `faf76214a`; T2 `119ef24de`; T2 `8a03f71bb`; T6 `fec5c4773`; T6 `a36355d51` | MATCHES / BRANCH PATCH — shipped console plus branch projection of run-completed AnswerPacket, MissionPacket / AgentBlueprint metadata, and local mission execution contract visibility. Branch UI adds manual refresh and load-more pagination. T2 projection commits remain coordination debt because `ProvenanceConsoleProjectionService.swift` overlaps T6 and sits outside T2's written Engine lane | `Epistemos/Views/Provenance/`, `ProvenanceConsoleProjectionService.swift`, `Epistemos/Views/Settings/ProvenanceConsoleView.swift` |
| **Agent replay failure termination overlay (T2 iter33)** | T2 `896bae766` | IN-FLIGHT / COMMITTED SCOPE DEBT — failed local replay runs now terminate in chat state/timeline paths, but the commit uses `ChatCoordinator.swift`, `AgentChatState.swift`, `AgentRunTimelineView.swift`, and Swift tests outside T2's exact lanes. It does not close hardware-tier issue 015 | T2 branch; coordination/rationale pending |
| **Local mission execution contracts overlay (T2 iter36)** | T2 `8a03f71bb` | IN-FLIGHT / COMMITTED SCOPE DEBT — local mission execution contracts and visibility paths landed across `ProvenanceConsoleProjectionService.swift`, `AgentBlueprint.swift`, `AgentRunTimelineView.swift`, and Swift tests. `AgentBlueprint.swift` is in-lane; Provenance Console / timeline surfaces need T6 coordination, Swift tests need exact-scope rationale, and hardware-tier issue 015 remains Investigating | T2 branch; coordination/rationale pending |
| **Local-only mission packet overlay (T2 iter37)** | T2 `8aebaefa2` | IN-FLIGHT / COMMITTED SCOPE DEBT — local-only mission packet enforcement landed across `ChatCoordinator.swift`, `ProvenanceConsoleProjectionService.swift`, `AgentBlueprint.swift`, `AgentRunTimelineView.swift`, and Swift tests. AgentBlueprint work is in-lane, but prompt seam / Provenance Console / timeline / Swift-test scope debt widened. Hardware-tier issue 015 remains Investigating | T2 branch; coordination/rationale pending |
...
| **BiometricWriteGate** | `ternary kernel.md`; T8 `3eadad5c4`; T8 `c6f07c593`; T8 `045f29ae4`; `docs/fusion/BIOMETRIC_LOCK_DOCTRINE_2026_05_17.md` | PHASE 0 DOCTRINE-ONLY / IMPLEMENTATION GATE CLOSED — lockable surfaces, lock session model, and agent isolation refined in docs only. No source implementation started |
| **Biometric lock doctrine iter33 overlay** | T8 `9466b45df`; T8 `7a763435e`; T8 `03432df1d` | PHASE 0 DOCTRINE-ONLY / IMPLEMENTATION GATE CLOSED — lock UI, recovery doctrine, and open theorem obligations refined in docs only. No source implementation started |
| **Biometric lock doctrine iter36 coordination hooks overlay** | T8 `7fa1df06c` | PHASE 0 DOCTRINE-ONLY / IMPLEMENTATION GATE CLOSED — coordination audit hooks added to `BIOMETRIC_LOCK_DOCTRINE_2026_05_17.md`; no source implementation started |
...
| **F-VaultRecall-50 fallback candidate contract** | T4 `docs/fusion/VAULT_CONTEXT_CONTRACT_2026_05_17.md`; T4 `4e0f74372`; T4 `2c4f0d1bf`; T4 `6998cc41f`; T4 `5bbe32951`; T4 `2657a6469`; T4 `dd11893d2`; T4 `7c258ad95`; T4 `6e07a2ed3`; T4 `ffc4c8722`; T4 `7fbb27ad5`; T4 `a22dae6fa`; T4 `4e0aadd3b`; T4 `262b90214`; T4 `20c60ae67`; T4 `627eef6ea`; T4 `f35f5e624`; T4 `93ad1953a`; T4 `719b032e0`; T4 `eacc32d78`; T4 `66881f0c8`; T4 `36d338a8a`; T4 `ee9325094`; T4 `604356040`; T4 `d4e118744`; T4 `2313f9ee1`; T4 `81bcdd5c1`; T4 `e162c7db1`; T4 `53a1c17fa`; T4 `d246b088d`; T4 `910c578ea`; T4 `b44f80373`; T4 `704ab4858`; T4 `5b16257ad`; T4 `5dff1f532`; T4 `32ba74ce9`; T4 `b666b7c13`; T4 `6342246a8`; T4 `e69e6e03d`; T4 `docs/falsifiers/F-VaultRecall-50_2026_05_17.md` | IN-FLIGHT / TRACE CONTRACT BRANCH PATCH — branch patch widens indexed fallback candidate pool to at least 50 when inventory supports it, applies true recency half-life in RRF fusion, strips query boilerplate, recovers exact-title/original-note hits by path title, seeds synthesis side titles, emits renderable RRF/fallback provenance reasons, renders note-chat Vault provenance cards, adds `agent_core::retrieval` trace/contract types, emits `VaultStore::hybrid_search_with_trace` inventory/candidate/confidence/provenance traces, replaces placeholder MMR with real relevance/diversity trace decisions, adds recency-decay, user-priority, graph-proximity trace signals, RRF source-hit breadth / confidence-band / contract-sufficiency metadata, fused-search confidence-count trace telemetry, standardized compact `Vault provenance:` card blocks for fallback and non-fallback grounded answers, rejects low-confidence indexed fallback matches that lack title/path/snippet provenance, enforces prompt evidence thresholds in ChatCoordinator / LocalAgentPromptBuilder so source rank alone is not treated as evidence, rejects rank-only fused evidence in RRF, retains and mirrors fused confidence metrics, reports insufficient indexed evidence instead of false no-results, adds Rust synthesis distinct-note validation with `SynthesisUnderCited`, adds adversarial-margin `AdversarialConfusion` trace validation, maps Shadow-first exact-escalation violations, rejects selected-count mismatches, bounds selected count by candidate pool, rejects selected candidates with invalid fused scores, requires selected candidates to clear the low-confidence band, adds selected-only confidence counts distinct from full candidate-pool counts, and records rank-only / synthesis / adversarial / selected-trace validation status in the Vault Context Contract / F-VaultRecall falsifier docs. Falsifier pass bars are green for retrieval and the UI provenance slice is covered by focused Swift fallback/RRF tests; remaining Vault Context Contract work includes branch push/PR, generated artifact cleanup, and a narrow `agent_core/src/lib.rs` module-registration scope exception | T4 local branch; push / PR pending |
| **F-VaultRecall-50 iter32 Shadow exact overlay** | T4 `7a1241d93`; T4 `b67a8d69e`; T4 `d1fbe8432` | IN-FLIGHT / TRACE CONTRACT BRANCH PATCH — Shadow-first score margin and exact-escalation request handling landed in retrieval + contract/falsifier docs. Live `RRFFusionQuery.swift`, `SearchIndexService.swift`, and F-VaultRecall RRF tests are in-lane. Branch remains local-only with generated artifact cleanup and historical `agent_core/src/lib.rs` exception pending | T4 local branch; push / PR pending |
| **F-VaultRecall-50 iter33 RRF top-score overlay** | T4 `f4b3c2738`; T4 `9359dba3f` | IN-FLIGHT / TRACE CONTRACT BRANCH PATCH — Swift RRF/SearchIndex paths now trace RRF top-score margin and contract/falsifier docs record it. Live ChatCoordinator / LocalAgentPromptBuilder / fallback-test WIP is T4-owned. Branch remains local-only with generated artifacts and historical `agent_core/src/lib.rs` exception pending | T4 local branch; push / PR pending |
| **F-VaultRecall-50 iter34 Shadow snippet/bounded-target overlay** | T4 `939a1c882`; T4 `1130cea3f`; T4 `2989bd550`; T4 `d4be53b1b` | IN-FLIGHT / TRACE CONTRACT BRANCH PATCH — retrieval traces now include Shadow escalation snippets, doctrine records those snippets, and Shadow escalation targets are bounded and documented. Live ChatCoordinator / F-VaultRecall fallback-test WIP is T4-owned. Branch remains local-only with generated artifact cleanup and historical `agent_core/src/lib.rs` exception pending | T4 local branch; push / PR pending |
| **F-VaultRecall-50 iter35 fallback provenance block overlay** | T4 `73d007188` | IN-FLIGHT / TRACE CONTRACT BRANCH PATCH — fallback answers now include the fallback provenance block through `ChatCoordinator.swift` with focused `F_VaultRecall_50_FallbackTests.swift` coverage. Paths are T4-owned; branch remains local-only with generated artifact cleanup and historical `agent_core/src/lib.rs` exception pending | T4 local branch; push / PR pending |
| **F-VaultRecall-50 iter36 fallback provenance docs overlay** | T4 `07ff6d2dd` | IN-FLIGHT / TRACE CONTRACT BRANCH PATCH — Vault Context Contract and F-VaultRecall falsifier now record the fallback provenance block. Live `ChatCoordinator.swift` / fallback-test WIP is T4-owned; branch remains local-only with generated artifact cleanup and historical `agent_core/src/lib.rs` exception pending | T4 local branch; push / PR pending |
| **F-VaultRecall-50 iter37 ambiguous fallback guard overlay** | T4 `601a5ae2b`; T4 `3b04dc4e5` | IN-FLIGHT / TRACE CONTRACT BRANCH PATCH — ambiguous fallback top-hit handling now rejects insufficiently separated indexed fallback candidates, with focused fallback tests and Vault Context Contract / F-VaultRecall doctrine records. Live `ChatCoordinator.swift` / fallback-test WIP remains T4-owned; branch remains local-only with generated artifact cleanup and historical `agent_core/src/lib.rs` exception pending | T4 local branch; push / PR pending |
...
| **Ambient Frequencies + recursive UI/UX audit loop** | T6 `docs/audits/UI_UX_AmbientFrequencies_2026-05-17.md`; `UI_UX_Settings_Diagnostics_2026-05-17.md`; `UI_UX_Halo_ProvenanceConsole_2026-05-17.md`; `UI_UX_CognitiveWeightBadge_2026-05-17.md`; `UI_UX_NotesAskBarError_2026-05-17.md`; `UI_UX_MinorFixes_2026-05-17.md`; `UI_UX_Settings_SubtreeSweep_2026-05-17.md`; `UI_UX_Notes_SubtreeSweep_2026-05-17.md`; `UI_UX_Chat_Graph_SubtreeSweep_2026-05-17.md`; `UI_UX_FinalSweep_Remaining_2026-05-17.md`; T6 `67ba8f17d`..`1ac9448a8`; T6 `66fba2f6f`; T6 `162a13eb4`; T6 `5f92c90f4`; T6 `e19b8118c`; T6 `d849b0aef`; T6 `57fda8c47`; T6 `09668f95c`; T6 `fec5c4773`; T6 `01b4f32af`; T6 `a36355d51`; T6 `169d32d84`; T6 `97cefc1c2`; T6 `6ed7be0fe`; T6 `6fd986def`; T6 `9e0a0cd6d`; T6 `7357c68f5` | IN-FLIGHT — branch patch adds Ambient Frequencies P1 fixes; follow-up docs audit Settings Diagnostics, Halo/Provenance, CognitiveWeightBadge, Notes ask-bar errors, minor fixes, consolidated Settings, Notes, Chat/Graph, and final remaining Views subtrees. T6 reports pass-1 coverage complete across 209 `Epistemos/Views/**` files, has additive Settings Diagnostics accessibility consistency and boilerplate cleanup, fixed the AmbientFrequencyLivePlayer SRR hold-counter stutter, added a Provenance Console refresh control, persisted Halo panel resize, added Provenance Console load-more pagination beyond the 40-entry cap, added accessibility coverage for ProcessMemory / DeploymentProfile rows and Setup Assistant, added silent-escalation toasts for note ask / graph chat auto-routes, hardened AI suggestion / live activity / VaultOrganizer accessibility, added AnswerPacket / ContextWindow / disclosure a11y values, and honors Reduce Motion in TypewriterPlainText. Live non-artifact WIP is clear; not main-merged yet, with generated artifact and footer hygiene still pending |
| **Ambient Frequencies iter32 route-change overlay** | T6 `7983e5b32` | IN-FLIGHT — live player handles `AVAudioEngineConfigurationChange` by rebuilding stale source-node state after route/sample-rate changes. Live `AmbientFrequencySettingsView.swift` WIP is in-lane; generated artifact and footer hygiene remain pending | T6 branch; not main-merged |
| **Ambient Frequencies iter33 audiophile export overlay** | T6 `896159b57`; T6 `7c52a7d56`; T6 `80a4d9cc1`; `docs/audits/UI_UX_Audio_AudiophileUpgrade_2026-05-17.md` | IN-FLIGHT — audiophile upgrade plan, dynamics-chain AppStorage regression coverage, and per-export master gain in dB for Ambient Frequency WAV export landed in T6 branch. Generated artifacts and footer hygiene remain pending | T6 branch; not main-merged |
| **Ambient Frequencies iter34 live-player meter overlay** | T6 `5fa4efdc2`; T6 `381052942` | IN-FLIGHT — legacy live-player sliders now expose accessibility values, and the live-player VU meter has peak-hold + decay behavior in `AmbientFrequencySettingsView.swift`. T6 branch remains generated-artifact/Claude-footer dirty but path-scope clean | T6 branch; not main-merged |
| **Ambient Frequencies iter35 diagnostic error UI overlay** | T6 `0c08066ef`; T6 `bd1825a18`; `docs/audits/UI_UX_Audio_AudiophileUpgrade_2026-05-17.md` | IN-FLIGHT — live-player diagnostics gained Copy + Dismiss error UI in `AmbientFrequencySettingsView.swift`, and the audiophile audit was refreshed through iters 33-37. T6 branch remains generated-artifact/Claude-footer dirty but path-scope clean | T6 branch; not main-merged |
| **Ambient Frequencies iter36 gain/master chain-order overlay** | T6 `775137b83` | IN-FLIGHT — live-player Settings UI now clarifies Gain versus Master volume in the chain order inside `AmbientFrequencySettingsView.swift`. T6 remains path-scope clean, with generated artifact and footer hygiene still pending | T6 branch; not main-merged |
...
### 3.43 `epistemos-code-index` — RAG-for-code retrieval (Wave 9.7, SHIPPED) (B2-M15)

| Concept | Source | Status |
|---|---|---|
| **`epistemos-code-index` crate — workspace code indexer.** Provides RAG-for-code retrieval as the sibling of Halo Shadow vault index (notes side). Per `Cargo.toml` description: "Workspace code indexer — RAG chunking + Model2Vec embeddings + usearch HNSW sidecar. Per-file sidecar lives at `<vault>/.epcache/code/<sha256-hex-of-vault-rel-path>.epcode.json` (path-hash, NOT body-hash; identical to Swift `CodeSidecarPath`). Wave 9.7." | `epistemos-code-index/Cargo.toml` + `epistemos-code-index/src/{lib,sidecar,state,error}.rs` (4 files · 36 KB src) | **SHIPPED (W9.7 base)** — crate present in main; build script `build-epistemos-code-index.sh` wired into Xcode at `Epistemos.xcodeproj/project.pbxproj` lines 779 (Bundle Runtime Assets MAS_SANDBOX path) + 836 (Bundle Runtime Assets standard path); FFI surface (`codeindex_*` entry points wrapped in `std::panic::catch_unwind` per `panic = "unwind"` release profile); `cdylib + staticlib + rlib` crate-types; fat `libepistemos_code_index.a` covers arm64 + x86_64. |
| **Architectural split per `epistemos_code_verdict.md` + brain dump 2026-04-26.** **Swift** owns the live editing UI surface (TextKit 2 + SwiftTreeSitter direct C bindings; live syntax stays in Swift to avoid UTF-16/UTF-8 cross-FFI mapping stutter). **Rust here** owns project-wide background indexing: chunking source into RAG-friendly windows · computing Model2Vec embeddings · building a per-vault usearch HNSW index for the **agent-grep API (W9.9 follow-up)** · populating each file's `<vault>/.epcache/code/<sha256-hex-of-vault-rel-path>.epcode.json` sidecar. | `epistemos-code-index/src/lib.rs:1-26` doctrine comment | architectural boundary — Swift edit-time, Rust background-index-time |
| **Sidecar path canonical layout — bit-for-bit Swift parity.** `<vault-root>/.epcache/code/<sha256-hex-of-vault-rel-path>.epcode.json`. Swift uses `CryptoKit.SHA256.hash(data: vaultRelPath.utf8)`; Rust uses the equivalent `sha2::Sha256` over the **vault-relative path** bytes. Hash is over the *path* (so file rename re-binds on next index pass), NOT the body (so re-saves don't churn the sidecar). Companion Swift class: `Epistemos/Models/CodeArtifactSidecar.swift::CodeSidecarPath.sidecarURL(forVaultRoot:vaultRelativePath:)`. Fixture test in `sidecar.rs` pins the exact hex digest Swift produces. | `epistemos-code-index/src/sidecar.rs:1-15` + `Epistemos/Models/CodeArtifactSidecar.swift` | SHIPPED — bidirectional parity test enforced |
| **W9.7 base scope vs W9.7 follow-up scope.** Base scope (this commit): FFI surface + module skeleton + explicitly-labeled in-memory fallback indexer so Swift side can integrate now. Light deps only (`parking_lot · serde · serde_json · sha2 · thiserror`). Full backend (usearch 2.25 + Model2Vec encoder + tree-sitter symbol extractor) lands in **W9.7 follow-up** once the surface + tests + agent-grep API (W9.9) are green. Splitting keeps W9.7 base build under 30 s per the `Cargo.toml` comment lines 14-17. | `epistemos-code-index/Cargo.toml:14-22` | SHIPPED (base) · PARTIAL (full backend deferred to W9.7-followup) |
| **Why this row is canon, not just code (B2-M15)** — audit-of-audit #3 (iter 30) surfaced that this ~36-KB src / Xcode-wired crate is COMPLETELY ABSENT from PASS 1 + PASS 2 + every canon doc (MASTER_FUSION + NEW_SESSION_HANDOFF + HERMES_AGENT_CORE_2_0_DESIGN + MAS_COMPLETE_FUSION + AGENTS.md + CLAUDE.md FILE MAP all returned zero hits for `epistemos-code-index` / `epistemos_code_index`). Pattern matches the previous orphan-quarantine drift that hit `KaTeXSnippets` / `KIVIQuantization` / `variant_ladder` per RCA-P2-010 / commit `06819a33a`. This row prevents a future agent from re-implementing the indexer OR accidentally quarantining it as orphan. | PASS 2 B2-M15 audit row + audit-of-audit #3 surfacing + commit `06819a33a` orphan-quarantine precedent | doctrine reconciliation |
| **Cross-references** — Halo Shadow (notes-side index sibling, MASTER_FUSION §3.9) · agent-grep API W9.9 (post-W9.7 follow-up, will consume the usearch HNSW sidecar) · `Epistemos/Models/CodeArtifactSidecar.swift` Swift parity surface · `build-epistemos-code-index.sh` Xcode build phase · PASS 1 H-10 Auto-research loops (consumer of agent-grep API in research mode) · B2-M14 §3.42 differential-privacy gate (sibling forward-staged primitive; both gate auto-research data, code-index for source-of-truth fetches, DP for telemetry emit). | (this row) | doctrine cross-links |

> **§3.43 surfacing note (B2-M15, recorded 2026-05-16).** This is the third consecutive recent §5.0 catch reflecting the **W9.7 wave's substantive code-vs-doctrine drift**: substantial Rust crates ship into main while the canon catalog hasn't tracked them. Compare to B2-M6 RuntimePlane (canonical in code, missing from canon), B2-M10 Effect subsystem (722 LOC in main, framed as salvage), B2-M11 JIT defense (canonical in MAS_APP_REVIEW_NOTES, missing from §0 immutable rules), and now B2-M15 `epistemos-code-index` (~36 KB src + 2 Xcode build-phase wires, absent from every canon doc). **PR-discipline rule landed:** any new top-level Rust crate added to `Cargo.toml` workspace MUST include a MASTER_FUSION §3.X doctrine row + FILE MAP entry in the same commit. The legacy gap (`epistemos-code-index` + the orphan-quarantine triplet) is now being closed retroactively; future drift gated.
...
| **GBNF / structured output** | PARTIAL | LocalToolGrammar + MLXStructured + JSONSchema + soft-guidance fallback; T2 `4f7b9df60` model grammar matrix; T2 `ed7ff2531` Settings strict-grammar/schema-drift diagnostics; T2 pushed `46ac80bba` native grammar profiles; T2 `6c5526fa8` task-class model selection (scope note pending for Swift test path); T2 `79cb183ee` AgentBlueprint mission runner; T2 `ea2792cd1` AgentBlueprint model-choice runtime badges; T2 `59abb152a` AgentBlueprint MissionPacket persistence/replay; T2 `a7a39595f` LocalAgent drift diagnostics / Settings health row; T2 `996f0831b` active local constellation Settings row; T2 `05a0133c4` AnswerPacket replay stamping; T2 `dc2d17fb3` power-user local-model risk gate / Settings OOM-risk labels; T2 `6b3dc77a9` local blueprint mission replay; T2 `8a03f71bb` local mission execution contracts; T2 `8aebaefa2` local-only mission packet enforcement | — | per-tool `&'static str` GBNF / Tool trait JsonSchema macro / ≤256 reasoning enforcement / per-model strict masking fixtures / global per-model capability badges + verified 36B-on-16GB runtime proof + Swift-test scope rationale for diagnostics and `LocalModelInfrastructureTests.swift` + committed scope rationale for `05a0133c4` / `6b3dc77a9` / `8a03f71bb` / `8aebaefa2` Provenance Console / ChatCoordinator / AgentRunTimelineView / AgentEventVisibilityTests / LocalAgentLoopTests paths |
...
| **ACS** | INVESTIGATING | T3 `4468b09ac` substrate inventory; T3 `d00d72eb2` canonical architecture; T3 `08137d05f` Morph deep-dive; T3 `6745c19a7` F-UAS-ZeroCopy-Spine gate; T3 `a26a20803` F-ACS-Anchor-Addressing gate; T3 `318402afc` F-ShadowFirst-PageEscalation; T3 `6ad5c763b` F-PageGather-M2Pro; T3 `ead9302d2` F-ActiveAssembly-Minimal gate; T3 `72cfcffd9` F-KV-Direct-Gate; T3 `7d5fc2822` F-SemiseparableBlockScan-Correctness; T3 `18654d983` F-LocalRecallIsland-32K; T3 `838955711` F-PacketRouter1bit-Dispatch; T3 `a3e302180` F-ControllerKernelPack; T3 `b392a61b7` F-70B-Local-Cocktail research ceiling; T3 `e432b54f1` F-ULP-Oracle W1 gate; T3 `52293dfaa` Phase A closeout; T3 `4f74a7e98` UAS address scaffold; T3 `0ac612c3b` UasKind; T3 `b2ed9a034` ResidencyTier; T3 `ce06b9f6c` ResidencyLease; T3 `01f892bf2` UasStateWitness acceptance; T3 `01f4ab53f` AcsAnchor; T3 `2deb32f18` AnchorRegistry; T3 `0368d76b1` ACS anchor round trip; T3 `b10986cb9` Phase B audit-of-audit; T3 `912c7b98b` doctrine catch-up; T3 `7975f57d6` UAS copy counter; T3 `0a316f530` ZeroCopy path 1; T3 `d5f419b94` ZeroCopy path 2; T3 `998835f71` ZeroCopy path 3; T3 `8f2bec8b4` ZeroCopy PASS report; T3 `d52214ce6` PacketGraph; T3 `43db76df2` MarginAnchoredGreedyPull; T3 `79762a980` ActiveAssembly harness; T3 `f4d5baa4e` iter-40 audit-of-audit; T3 `6624225c1` ZeroCopy path 5; T3 `90c5484be` ZeroCopy path 4; T3 `b487f26de` HeliosPage; T3 `6580e5e53` sketch_top_k; T3 `4f42c8a9e` residual_rescore; T3 `c8e0eb16d` PacketRouter1bit substrate-floor; T3 `9e5707c95` ControllerKernelPack substrate-floor; T3 `45132d685` iter-50 audit-of-audit | research-tier; T3 Phase B.G.B1 branch-complete plus AcsAnchor/AnchorRegistry, ACS round-trip harness, F-UAS-ZeroCopy substrate-floor PASS across 5 of 6 paths, F-ActiveAssembly-Minimal substrate-floor harness, second and third mid-loop audit-of-audit, PageGather HeliosPage / sketch-top-K / residual-rescore substrate, F-PacketRouter1bit substrate-floor, and F-ControllerKernelPack substrate-floor, with narrow `agent_core/src/lib.rs`, `agent_core/src/scope_rex/residency.rs`, `agent_core/src/research/mod.rs`, `active_assembly_minimal.rs`, `packet_router_dispatch.rs`, and `controller_kernel_pack.rs` rationale exceptions | page gather escalation policy / local recall island / 4 homeostatic loops / VSM S1-S5 / HealingAction / Markov blanket / Hyper-Dynamic Schemas |
...
| C9 | **V6.2 per-bubble VRMLabelView binding** (state: rendered FULL) — wire emitted AnswerPacket → message bubble so each turn carries its audit chip · ⚠️ **AWAITING USER SIGN-OFF** | `docs/audits/V6_2_PER_BUBBLE_BINDING_RESEARCH_2026_05_12.md` (252L, frontmatter `state: research · verdict: TWO_OPTIONS_RECOMMEND_OPTION_B`). Research surfaces a real race condition: at `StreamingDelegate.onComplete`, the assistant message's `RuntimeId` is committed BEFORE `emit(packet)` await completes — so a side-table sink (Option A) needs a timestamp-match heuristic that can mis-bind. **Option A:** side-table sink with timestamp matching (faster · race-prone · ambiguous when packets arrive out-of-order). **Option B (RECOMMENDED):** pass `packetId` through `AgentStreamEvent.complete` (zero race, packet id committed BEFORE the stream event; small AgentStreamEvent enum delta · 2-3 file diff). PARTIAL state ALREADY LANDED at `Epistemos/Views/Chat/MessageBubble.swift:477` per §3.18 row 297 ("LatestAnswerPacketSink + per-bubble VRMLabelView chip — LANDED"); FULL state requires this binding work. Decision: which of A/B + whether to ship as one commit or two (research §1241 question 1). NO implementation until user signs off. |
...
| H6 | **Graph Toolbar — Cursor Force + Shape Bound buttons** · ⚠️ **AWAITING USER SIGN-OFF** | `docs/audits/GRAPH_TOOLBAR_CURSOR_FORCE_SHAPE_BOUND_SPEC_2026_05_12.md` (203L · frontmatter `state: research-spec · verdict: AWAITING_USER_SIGNOFF_BEFORE_IMPLEMENTING`). **Button 1 — Cursor force:** every node feels a force toward (suck) or away from (repel) the live cursor; knobs = direction toggle + intensity slider (~0–1.0). **Button 2 — Shape bound:** nodes pushed inward toward an invisible geometric formation (circle / triangle / square; "or more" hinted hexagon/star later); knobs = shape picker + scale slider for bounding radius. **Target file:** `Epistemos/Views/Graph/GraphFloatingControls.swift` (existing toolbar pill: Physics → Pixel/Fast → Forces popover → Reset view → Rebuild → Close). Spec proposes adding 2 new chips between "Forces" and "Reset view" using the existing `AnchoredPopoverButton` pattern (matches Forces popover shape). **Decision required:** ship as one PR or two (per-button) + finalize "or more" shape inventory (hexagon/star approved or deferred). NO implementation until user signs off. |
