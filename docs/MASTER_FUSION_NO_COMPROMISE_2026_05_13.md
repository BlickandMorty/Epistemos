...
| **UAS-ACS substrate inventory (T3 Phase A)** — no-loss register for §4.G concept reconciliation; maps scattered names to layer, residency tier, current file, falsifier dependency, and status before code changes. | `docs/audits/UAS_ACS_SUBSTRATE_INVENTORY_2026_05_17.md` (`4468b09ac`); `docs/fusion/UAS_ACS_CANONICAL_ARCHITECTURE_2026_05_16.md` (`d00d72eb2`); `docs/falsifiers/F-UAS-ZeroCopy-Spine_2026_05_17.md` (`6745c19a7`); `docs/falsifiers/F-ACS-Anchor-Addressing_2026_05_17.md` (`a26a20803`); `docs/falsifiers/F-ActiveAssembly-Minimal_2026_05_17.md` (`ead9302d2`); `docs/falsifiers/F-KV-Direct-Gate_2026_05_17.md` (`72cfcffd9`); `docs/falsifiers/F-SemiseparableBlockScan-Correctness_2026_05_17.md` (`7d5fc2822`) | INVESTIGATING — docs-only substrate inventory, canonical architecture/no-loss register, and seven falsifier docs landed on T3; `agent_core/src/uas/` and active-assembly/page-gather/local-recall implementation homes remain NOT-STARTED until Phase B; T3 branch is tracking origin and currently ahead locally |
...
| **State ladder**: implemented → emitted → populated → rendered → canonical-product-surface | V6.2 progress; T2 `9b090203d`; T2 `edb69ec47` | rendered (chip lookups via sink); branch attempt at durable packet persistence plus Settings audit row is NOT accepted as merge-ready until the T2 scope violation is resolved; T2's current uncommitted broad Rust/Swift drift is excluded from this row |
...
| **F-VaultRecall-50 fallback candidate contract** | T4 `docs/fusion/VAULT_CONTEXT_CONTRACT_2026_05_17.md`; T4 `4e0f74372`; T4 `2c4f0d1bf`; T4 `6998cc41f`; T4 `5bbe32951`; T4 `2657a6469`; T4 `dd11893d2`; T4 `7c258ad95`; T4 `6e07a2ed3`; T4 `docs/falsifiers/F-VaultRecall-50_2026_05_17.md` | IN-FLIGHT / PROVENANCE SLICE PASS — branch patch widens indexed fallback candidate pool to at least 50 when inventory supports it, applies true recency half-life in RRF fusion, strips query boilerplate, recovers exact-title/original-note hits by path title, seeds synthesis side titles, emits renderable RRF/fallback provenance reasons, and now renders note-chat Vault provenance cards. Falsifier pass bars are green for retrieval and the UI provenance slice is covered by focused Swift fallback/RRF tests; remaining Vault Context Contract work includes Rust trace types, MMR decisions, graph proximity, confidence bands, and low-confidence enforcement | T4 local branch; push / PR pending |
...
| **ACS** | INVESTIGATING | T3 `4468b09ac` substrate inventory; T3 `d00d72eb2` canonical architecture; T3 `6745c19a7` F-UAS-ZeroCopy-Spine gate; T3 `a26a20803` F-ACS-Anchor-Addressing gate; T3 `ead9302d2` F-ActiveAssembly-Minimal gate; T3 `72cfcffd9` F-KV-Direct-Gate; T3 `7d5fc2822` F-SemiseparableBlockScan-Correctness | research-tier; T3 branch ahead of origin pending push cadence | `agent_core/src/uas/` / active assembly / page gather / local recall island / 4 homeostatic loops / VSM S1-S5 / HealingAction / Markov blanket / Hyper-Dynamic Schemas |
