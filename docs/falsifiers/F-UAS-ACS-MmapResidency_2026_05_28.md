...
scope: file-backed UAS / ACS residency slice
---
...
This falsifier proves one concrete piece of the no-compromise UAS / ACS
...
leased through `ResidencyLease`, and resolved through ACS projection lookup
...
| `acs_projection_lookup` | ACS projection lookup returns the anchored object without field loss. |
...
This witness upgrades UAS / ACS from metadata-only evidence to a real
file-backed mmap residency proof. It still stops below the live model boundary.
Agents may cite it as:

```text
UAS/ACS mmap residency works for a deterministic file-backed KV-page slice.
```
