...
## N267 - Remote Branch Commit SHA Alias
...
Rejection reason: `commit_sha` must store the resolved lowercase hex commit, not a remote-tracking branch alias such as `origin/main`.

## N268 - Tag Name Commit SHA Alias
...
Rejection reason: `commit_sha` must store the resolved 40-character lowercase hex commit, not a tag alias that can be moved or reissued.

## N269 - Trailing Space Commit SHA
...
{
  "field": "commit_sha",
  "commit_sha": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "schema_pattern": "^[0-9a-f]{40}$"
}
```

Rejection reason: `commit_sha` must contain exactly 40 lowercase hex characters; overlong strings are not valid Git object identities for replay.

## N274 - Non-Hex Commit SHA Character
...
{
  "field": "commit_sha",
  "commit_sha": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaag",
  "schema_pattern": "^[0-9a-f]{40}$"
}
```

Rejection reason: `commit_sha` may use only lowercase hex bytes; lowercase non-hex letters such as `g` are invalid.

## N275 - Mixed-Case Commit SHA
...
Rejection reason: `commit_sha` must be all lowercase hex; a single uppercase nibble makes the replay identity non-canonical.

## N276 - Quoted Commit SHA
...
```json
{
  ...
  "commit_sha": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "schema_pattern": "^[0-9a-f]{40}$",
  "semantic_rule": "repo_history_complete_for_commit",
  "repo_history_state": "shallow"
}
```
...
## N290 - Missing Remote Metadata For Commit SHA
...
## N292 - Remote Origin Mismatch For Commit SHA
...
```json
{
  ...
  "commit_sha": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "schema_pattern": "^[0-9a-f]{40}$",
  "semantic_rule": "commit_remote_origin_matches_scope",
  "expected_remote": "epistemos-falsifier-handbook",
  "actual_remote": "unrelated-fork"
}
```

Rejection reason: replay receipts must bind the commit to the scoped repository origin; a valid hash from an unrelated fork is replay-ineligible.

## N293 - Stale Remote Tracking Commit SHA
...
```json
{
  ...
  "commit_sha": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "schema_pattern": "^[0-9a-f]{40}$",
  "semantic_rule": "remote_tracking_ref_fresh_for_commit",
  "remote_tracking_state": "stale"
}
```
