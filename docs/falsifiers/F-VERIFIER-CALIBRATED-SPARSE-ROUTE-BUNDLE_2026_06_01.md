...
| `F-AblationShadowRun` | Current active cursor: claimed useful units survive a counterfactual remove-one-unit comparison on quality, verifier, latency, and bytes. | Baseline/candidate traces plus decision record; must remain shadow-only and AnswerPacket-visible. |
