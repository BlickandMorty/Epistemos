...
Migration notes must encode their migration fields as semicolon-delimited `key=value` tokens inside `notes`, using ASCII keys and values. When `from_schema=` appears, the JSON Schema fragment requires the base migration keys so validator tooling can parse the note without natural-language inference. `from_schema`, `to_schema`, `artifact_path`, `migration_command`, `field_mapping`, `validator`, `reviewer`, and `reviewed_at_utc` migration tokens must be bounded by note start/end or semicolon delimiters; embedded substrings do not satisfy version, path, command, mapping, identity, or review-time attestation.
