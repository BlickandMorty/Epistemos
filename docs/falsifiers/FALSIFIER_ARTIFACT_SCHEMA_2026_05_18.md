    ...
    "notes": {
      ...
      "allOf": [
        ...
        },
        {
          ...
          "then": {
            "pattern": "(?=.*from_schema=\\d{4}-\\d{2}-\\d{2}\\.\\d+)(?=.*to_schema=\\d{4}-\\d{2}-\\d{2}\\.\\d+)(?=.*artifact_path=artifacts/falsifiers/[A-Za-z0-9._/-]+)(?=.*migration_command=[A-Za-z0-9._/-]+)(?=.*field_mapping=[A-Za-z0-9._,/:+-]+)(?=.*provider_data_sent_class_gap_report=[A-Za-z0-9._,/:+-]+)(?=.*provider_replay_permission_gap_report=[A-Za-z0-9._,/:+-]+)(?=.*provider_pass_retention_gap_report=[A-Za-z0-9._,/:+-]+)(?=.*provider_artifact_root_gap_report=[A-Za-z0-9._,/:+-]+)(?=.*provider_artifact_dot_segment_gap_report=[A-Za-z0-9._,/:+-]+)(?=.*reviewer=[A-Za-z0-9._-]+)(?=.*reviewed_at_utc=\\d{4}-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\\d|3[01])T(?:[01]\\d|2[0-3]):[0-5]\\d:[0-5]\\d(?:\\.\\d+)?Z)"
          }
        },
        {
          "if": {
