...
`turbovec_quarantine_adapter_microbench_probe`, because the first actual
adapter microbench must run in quarantine and remain non-authoritative before
compressed retrieval can influence large-local-model context selection.
