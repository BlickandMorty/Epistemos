...
status: F-ColdStream-vs-Mmap, F-SlabArena-CopyCount, F-MetalIO-FeatureGate, and F-CodecStage-Latency passed metadata-only witnesses; no live cold transport promotion without cancellation, cache, panic fallback, and live platform benchmarks
---
...
Current cursor after the 2026-06-05 `F-CodecStage-Latency` metadata-only codec witness: `transport_cancellation`.
...
advances L1 only as codec/read-trace separation evidence; `F-TransportCancellation`,
