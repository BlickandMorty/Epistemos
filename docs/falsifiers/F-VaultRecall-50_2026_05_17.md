...
- `EpistemosTests/F_VaultRecall_50_FallbackTests.swift` verifies indexed fallback answers emit per-hit `Why:` reasons plus a compact `Vault provenance:` block, source-rank-only matches are rejected with explicit insufficient-evidence messaging, note chat sidebar provenance parsing accepts explicit provenance blocks, and prompt contracts reject low-confidence/source-rank-only/low-margin synthesis claims.
