...
are cold-fault learning, oracle baselines, and any ColdStream
no-hidden-authority witness before transport policy can influence live routes.
KV byte-budget cards are now covered by `F-KVByteBudgetCard`; source-to-residency
no-poison propagation is now covered by `F-SourceToResidency-NoPoison`.
