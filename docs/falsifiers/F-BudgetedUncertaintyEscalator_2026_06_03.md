...
- L1 next cursor at landing: `F-SparseWakeProposal-Budget`; current cursor after the 2026-06-04 VerifierBudgetAuction witness is `F-KVPageSketchIndex`
...
This advances L1 only. It does not make RouteScoutSSM, TwoStageRouteScout, or BudgetedUncertaintyEscalator a live router, does not promote sparse wake execution, and does not change MAS/Pro product copy. `F-SparseWakeProposal-Budget` and `F-VerifierBudgetAuction` now pass as metadata-only evidence; the current architecture unit is `F-KVPageSketchIndex`, which must prove page-sketch address, budget, compatibility, hits, misses, and privacy evidence before query-aware page selection can promote.
