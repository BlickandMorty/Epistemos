...
- Next research-to-build unit after latency/memory abstention and runtime shadow planning:
...
Latency, memory, timeout, and abstention envelopes are now covered by `F-TurboVec-LatencyMemoryAbstention`, and runtime shadow/synthetic microbenching are now covered by `F-TurboVec-RuntimeShadowBenchmarkPlan` and `F-TurboVec-QuarantineAdapterMicrobenchProbe`. The next retrieval/index research-to-build unit is `turbovec_quarantine_real_adapter_owner_approval_probe`, because the first real external adapter probe must remain quarantined and non-authoritative before compressed retrieval can help large-local-model context selection.
