...
- Small-model harness fresh product runtime L3 manual verification witness: `docs/falsifiers/F-SmallModelRuntimeHarnessFreshProductRuntimeL3ManualRuntimeVerificationProbe_2026_06_05.md`
- Small-model harness fresh product runtime L3 capability closeout witness: `docs/falsifiers/F-SmallModelRuntimeHarnessFreshProductRuntimeL3CapabilityCloseoutProbe_2026_06_05.md`
- Small-model harness fresh product runtime L3 release-audit preflight witness: `docs/falsifiers/F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditPreflightProbe_2026_06_05.md`
- Small-model harness fresh product runtime L3 release-audit zero-fail gate witness: `docs/falsifiers/F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditZeroFailProbe_2026_06_05.md`
- Small-model harness fresh product runtime L3 release-audit automated-checks witness: `docs/falsifiers/F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditAutomatedChecksProbe_2026_06_05.md`
- Current cursor after the 2026-06-05 RED `F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditAutomatedChecksProbe`: `small_model_runtime_harness_fresh_product_runtime_l3_release_audit_automated_checks_probe`. The gate implementation and ledger artifact exist, but `xcodebuild_test` failed while build, graph-engine, omega-mcp, and omega-ax checks passed. The 2026-06-06 hardening pass now records the retained Swift log as 161 issues across 84 unique tests with top families graph/filter visibility `34`, agent route policy `21`, theme/presentation `19`, distribution/project integrity `18`, research tool catalog `16`, editor/epdoc `14`, UI shell source guard `14`, and model vault/catalog `9`; a follow-up focused repair-plan pass binds the top family to `FilterEngineComprehensiveTests`, `ResourceExhaustionTests`, and `ConcurrencyEdgeCaseTests` with `FilterEngine.swift`, `GraphTypes.swift`, and `GraphState.swift` anchors. L1 queue integrity remains duplicate-free but does not advance; L2 product capability remains `vault_research_route_with_packetized_mitigation`; broader fresh app runtime capability is unchanged; release readiness is still blocked until automated checks are green, runtime logs are correlated, manual runtime evidence and distribution/compliance review land, and three uninterrupted zero-fail passes are recorded.
