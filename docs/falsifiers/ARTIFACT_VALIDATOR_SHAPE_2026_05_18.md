...
| `W-Validator-MigrationIdentityDelimiter` | TBD validator-implementation terminal | Any executable validator accepts `validator` or `reviewer` tokens in migration notes. | Reject embedded identity-token substrings that are not bounded by note start/end or semicolon delimiters before migration acceptance. |
