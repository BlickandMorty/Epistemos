...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N202 - .*?```json\n(.*?)\n```/m,1] || abort("N202 missing"); x=JSON.parse(block); abort("N202 numeric-leading slug missing") unless x["catalog_slug"] == "1-reserved-state"; abort("N202 unexpectedly matches slug grammar") if x["catalog_slug"].match?(/\A[a-z][a-z0-9]*(?:-[a-z0-9]+)*\z/); puts "identity gap numeric-leading slug negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N203 - .*?```json\n(.*?)\n```/m,1] || abort("N203 missing"); x=JSON.parse(block); abort("N203 dotted slug missing") unless x["catalog_slug"] == "reserved.state"; abort("N203 unexpectedly matches slug grammar") if x["catalog_slug"].match?(/\A[a-z][a-z0-9]*(?:-[a-z0-9]+)*\z/); puts "identity gap dotted slug negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N204 - .*?```json\n(.*?)\n```/m,1] || abort("N204 missing"); x=JSON.parse(block); abort("N204 spaced slug missing") unless x["catalog_slug"] == "reserved state"; abort("N204 unexpectedly matches slug grammar") if x["catalog_slug"].match?(/\A[a-z][a-z0-9]*(?:-[a-z0-9]+)*\z/); puts "identity gap spaced slug negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N205 - .*?```json\n(.*?)\n```/m,1] || abort("N205 missing"); x=JSON.parse(block); abort("N205 comma slug missing") unless x["catalog_slug"] == "reserved,state"; abort("N205 unexpectedly matches slug grammar") if x["catalog_slug"].match?(/\A[a-z][a-z0-9]*(?:-[a-z0-9]+)*\z/); puts "identity gap comma slug negative case ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N207 - .*?```json\n(.*?)\n```/m,1] || abort("N207 missing"); x=JSON.parse(block); abort("N207 colon slug missing") unless x["catalog_slug"] == "reserved:state"; abort("N207 unexpectedly matches slug grammar") if x["catalog_slug"].match?(/\A[a-z][a-z0-9]*(?:-[a-z0-9]+)*\z/); puts "identity gap colon slug negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N208 - .*?```json\n(.*?)\n```/m,1] || abort("N208 missing"); x=JSON.parse(block); abort("N208 plus slug missing") unless x["catalog_slug"] == "reserved+state"; abort("N208 unexpectedly matches slug grammar") if x["catalog_slug"].match?(/\A[a-z][a-z0-9]*(?:-[a-z0-9]+)*\z/); puts "identity gap plus slug negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N209 - .*?```json\n(.*?)\n```/m,1] || abort("N209 missing"); x=JSON.parse(block); abort("N209 ampersand slug missing") unless x["catalog_slug"] == "reserved&state"; abort("N209 unexpectedly matches slug grammar") if x["catalog_slug"].match?(/\A[a-z][a-z0-9]*(?:-[a-z0-9]+)*\z/); puts "identity gap ampersand slug negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N210 - .*?```json\n(.*?)\n```/m,1] || abort("N210 missing"); x=JSON.parse(block); abort("N210 at-sign slug missing") unless x["catalog_slug"] == "reserved@state"; abort("N210 unexpectedly matches slug grammar") if x["catalog_slug"].match?(/\A[a-z][a-z0-9]*(?:-[a-z0-9]+)*\z/); puts "identity gap at-sign slug negative case ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/FALSIFIER_ARTIFACT_SCHEMA_2026_05_18.md"); schema=JSON.parse(s[/```json\n(.*?)\n```/m,1]); pat=schema.dig("properties","commit_sha","pattern"); abort("commit_sha pattern drift") unless pat == "^[0-9a-f]{40}$"; puts "commit sha schema pattern ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/FALSIFIER_ARTIFACT_SCHEMA_2026_05_18.md"); schema=JSON.parse(s[/```json\n(.*?)\n```/m,1]); ts=schema.dig("properties","timestamp_utc"); abort("timestamp format drift") unless ts["format"] == "date-time"; abort("timestamp pattern drift") unless ts["pattern"] == "^\\\\d{4}-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\\\\d|3[01])T(?:[01]\\\\d|2[0-3]):[0-5]\\\\d:[0-5]\\\\d(?:\\\\.\\\\d+)?Z$"; puts "timestamp utc schema pattern ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N212 - .*?```json\n(.*?)\n```/m,1] || abort("N212 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N212 uppercase commit missing") unless x["commit_sha"] == "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"; abort("N212 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "uppercase commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N265 - .*?```json\n(.*?)\n```/m,1] || abort("N265 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N265 refspec commit missing") unless x["commit_sha"] == "HEAD~1"; abort("N265 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "refspec commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N266 - .*?```json\n(.*?)\n```/m,1] || abort("N266 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N266 branch commit missing") unless x["commit_sha"] == "main"; abort("N266 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "branch commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N267 - .*?```json\n(.*?)\n```/m,1] || abort("N267 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N267 remote branch commit missing") unless x["commit_sha"] == "origin/main"; abort("N267 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "remote branch commit sha negative case ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N269 - .*?```json\n(.*?)\n```/m,1] || abort("N269 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N269 trailing-space commit missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa "; abort("N269 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "trailing-space commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N270 - .*?```json\n(.*?)\n```/m,1] || abort("N270 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N270 leading-space commit missing") unless x["commit_sha"] == " aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N270 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "leading-space commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N271 - .*?```json\n(.*?)\n```/m,1] || abort("N271 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N271 embedded-newline commit missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaa\naaaaaaaaaaaaaaaaaaaa"; abort("N271 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "embedded-newline commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N272 - .*?```json\n(.*?)\n```/m,1] || abort("N272 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N272 abbreviated commit missing") unless x["commit_sha"] == "aaaaaaaaaaaa"; abort("N272 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "abbreviated commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N273 - .*?```json\n(.*?)\n```/m,1] || abort("N273 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N273 overlong commit missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N273 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "overlong commit sha negative case ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N275 - .*?```json\n(.*?)\n```/m,1] || abort("N275 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N275 mixed-case commit missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaAaaaaaaaaaaaaaaaaaaa"; abort("N275 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "mixed-case commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N276 - .*?```json\n(.*?)\n```/m,1] || abort("N276 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N276 quoted commit missing") unless x["commit_sha"] == "\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\""; abort("N276 unexpectedly matches commit grammar") if x["commit_sha"].match?(pat); puts "quoted commit sha negative case ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N278 - .*?```json\n(.*?)\n```/m,1] || abort("N278 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N278 all-zero commit missing") unless x["commit_sha"] == "0000000000000000000000000000000000000000"; abort("N278 should still match syntax regex before semantic rejection") unless x["commit_sha"].match?(pat); abort("N278 semantic rule missing") unless x["semantic_rule"] == "commit_exists_in_repo"; puts "all-zero commit sha semantic negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N279 - .*?```json\n(.*?)\n```/m,1] || abort("N279 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N279 non-commit sha missing") unless x["commit_sha"] == "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"; abort("N279 should still match syntax regex before semantic rejection") unless x["commit_sha"].match?(pat); abort("N279 semantic object type missing") unless x["semantic_rule"] == "commit_object_type" && x["resolved_object_type"] == "tree"; puts "non-commit object sha semantic negative case ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N282 - .*?```json\n(.*?)\n```/m,1] || abort("N282 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N282 blob sha missing") unless x["commit_sha"] == "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"; abort("N282 should still match syntax regex before semantic rejection") unless x["commit_sha"].match?(pat); abort("N282 semantic object type missing") unless x["semantic_rule"] == "commit_object_type" && x["resolved_object_type"] == "blob"; puts "blob object sha semantic negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N283 - .*?```json\n(.*?)\n```/m,1] || abort("N283 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N283 unreachable sha missing") unless x["commit_sha"] == "ffffffffffffffffffffffffffffffffffffffff"; abort("N283 should still match syntax regex before semantic rejection") unless x["commit_sha"].match?(pat); abort("N283 reachability rule missing") unless x["semantic_rule"] == "commit_reachable_from_branch" && x["repo_lookup_result"] == "commit_exists_unreachable"; puts "unreachable commit sha semantic negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N284 - .*?```json\n(.*?)\n```/m,1] || abort("N284 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N284 dirty worktree sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N284 should still match syntax regex before semantic rejection") unless x["commit_sha"].match?(pat); abort("N284 dirty worktree rule missing") unless x["semantic_rule"] == "repo_worktree_clean_at_run" && x["worktree_state"] == "dirty"; puts "dirty worktree commit sha semantic negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N285 - .*?```json\n(.*?)\n```/m,1] || abort("N285 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N285 detached-head sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N285 should still match syntax regex before semantic rejection") unless x["commit_sha"].match?(pat); abort("N285 detached-head rule missing") unless x["semantic_rule"] == "commit_reachable_from_named_branch" && x["head_state"] == "detached"; puts "detached head commit sha semantic negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N286 - .*?```json\n(.*?)\n```/m,1] || abort("N286 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N286 wrong-branch sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N286 should still match syntax regex before semantic rejection") unless x["commit_sha"].match?(pat); abort("N286 branch rule missing") unless x["semantic_rule"] == "commit_reachable_from_expected_branch" && x["expected_branch"] == "codex/t23b-m2pro-falsifier-handbook-2026-05-18" && x["actual_branch"] == "main"; puts "wrong branch commit sha semantic negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N287 - .*?```json\n(.*?)\n```/m,1] || abort("N287 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N287 shallow clone sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N287 should match syntax regex") unless x["commit_sha"].match?(pat); abort("N287 shallow history rule missing") unless x["semantic_rule"] == "repo_history_complete_for_commit" && x["repo_history_state"] == "shallow"; puts "shallow clone commit sha semantic negative case ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N289 - .*?```json\n(.*?)\n```/m,1] || abort("N289 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N289 invalid-branch sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N289 should match syntax regex") unless x["commit_sha"].match?(pat); abort("N289 invalid expected branch missing") unless x["semantic_rule"] == "commit_reachable_from_expected_branch" && x["expected_branch"] == "codex/t23b m2pro"; puts "invalid branch metadata commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N290 - .*?```json\n(.*?)\n```/m,1] || abort("N290 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N290 remote-metadata sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N290 should match syntax regex") unless x["commit_sha"].match?(pat); abort("N290 missing remote marker") unless x["semantic_rule"] == "commit_remote_origin_recorded" && x.key?("remote_origin_url") && x["remote_origin_url"].nil?; puts "missing remote metadata commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N291 - .*?```json\n(.*?)\n```/m,1] || abort("N291 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N291 invalid-remote sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N291 should match syntax regex") unless x["commit_sha"].match?(pat); abort("N291 invalid remote marker missing") unless x["semantic_rule"] == "commit_remote_origin_recorded" && x["remote_origin_url"] == "not a url"; puts "invalid remote metadata commit sha negative case ok"'
```

```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N292 - .*?```json\n(.*?)\n```/m,1] || abort("N292 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N292 remote-mismatch sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N292 should match syntax regex") unless x["commit_sha"].match?(pat); abort("N292 remote mismatch marker missing") unless x["semantic_rule"] == "commit_remote_origin_matches_scope" && x["expected_remote"] == "epistemos-falsifier-handbook" && x["actual_remote"] == "unrelated-fork"; puts "remote mismatch commit sha negative case ok"'
```
...
```bash
ruby -rjson -e 's=File.read("docs/falsifiers/ARTIFACT_NEGATIVE_EXAMPLES_2026_05_18.md"); block=s[/## N294 - .*?```json\n(.*?)\n```/m,1] || abort("N294 missing"); x=JSON.parse(block); pat=Regexp.new(x["schema_pattern"]); abort("N294 remote-fetch timestamp sha missing") unless x["commit_sha"] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; abort("N294 should match syntax regex") unless x["commit_sha"].match?(pat); abort("N294 missing fetch timestamp marker") unless x["semantic_rule"] == "remote_tracking_fetch_timestamp_recorded" && x.key?("remote_fetch_timestamp_utc") && x["remote_fetch_timestamp_utc"].nil?; puts "missing remote fetch timestamp commit sha negative case ok"'
```
...
```bash
ruby -e 'h=File.read("docs/falsifiers/M2_PRO_VERIFIED_FLOOR_HANDBOOK_2026_05_18.md"); s=File.read("docs/falsifiers/FALSIFIER_ARTIFACT_SCHEMA_2026_05_18.md"); f=File.read("docs/falsifiers/F_PAGE_GATHER_BASELINE_2026_05_18.md"); expected=%w[median_bw_256mb median_bw_512mb median_bw_1gb window_seconds]; expected.each { |axis| abort("F-PageGather baseline axis #{axis} missing") unless h.include?(axis) && s.include?(axis) && f.include?(axis) }; puts "F-PageGather baseline axis floor audit ok"'
```
...
| `W-Validator-MigrationIdentitySlugGrammarNegative` | TBD validator-implementation terminal | Any executable validator accepts at-sign-bearing, ampersand-bearing, plus-bearing, colon-bearing, slash-bearing, comma-bearing, spaced, dotted, underscore, uppercase, title-case, boundary-punctuation, empty-token, or numeric-leading identity-gap slug aliases. | Keep N197/N198/N199/N200/N201/N202/N203/N204/N205/N206/N207/N208/N209/N210 failing so identity-gap slug references use only lowercase hyphenated catalog tokens. |
