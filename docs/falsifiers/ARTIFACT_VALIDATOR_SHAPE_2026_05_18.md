...
| `W-Validator-NotesLengthCap` | TBD validator-implementation terminal | Any executable validator accepts falsifier `notes`. | Reject notes longer than 1024 characters before replay promotion or migration acceptance. |
...
| `W-Validator-MigrationGapTokens` | TBD validator-implementation terminal | Any executable validator accepts `from_schema=` migration notes. | Reject migration notes missing any schema-table gap token, and reject gap tokens absent from both the notes key allowlist and the `from_schema=` regex before migration acceptance. |
