...
- Script: `Tools/falsifiers/f_sparse_wake_certificate_answer_packet.sh`
- Artifact: `artifacts/falsifiers/sparse_wake_certificate_answer_packet/result.json`
- L1 next cursor: `F-LayerKVJointLease`
...
This advances L1 only. It does not make `SparseWakeCertificate` a live router, does not restore live KV pages, does not permit hidden PatternBoost/lattice/Eidos route authority, does not load local model bytes, does not promote the 70B track to product runtime, and does not change MAS/Pro user copy. The next architecture unit is `F-LayerKVJointLease`, which must prove dynamic depth and KV/page choice are leased together with error, byte, latency, fallback, rollback, and AnswerPacket accounting before any sparse route can promote.
