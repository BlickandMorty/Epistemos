...
Runtime shadow benchmark planning and synthetic microbenching are now covered by `F-TurboVec-RuntimeShadowBenchmarkPlan` and `F-TurboVec-QuarantineAdapterMicrobenchProbe`. The next retrieval/index research-to-build unit is `turbovec_quarantine_real_adapter_owner_approval_probe`, because the first real external adapter probe must run in quarantine, remain non-authoritative, and keep route/model-context mutation impossible before compressed retrieval can help large-local-model context selection.
