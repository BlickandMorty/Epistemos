...
| LAKE-BUILD | `lake build` runs with explicit `~/.elan/bin` PATH after the iter-581 Tropical schema repair and iter-592 zero-sorry E1 pass. | `PATH="$HOME/.elan/bin:$PATH"; cd lean/Epistemos && lake build 2>&1` | every 10 iters once `LEAN-TOOLCHAIN` resolves | iter-950 | RESOLVED: cadence retry at iter-950 ran full `lake build`, printed `Build completed successfully.`, and exited 0 after the iter-941 through iter-949 Operator, Geometry, Info, EML, and Tropical source-row schema/certificate carrier passes. | iter-582 |
| EML-LEAN-VENDOR | `tomdif/eml-lean` source is not vendored into the Lean project; gated on network/toolchain/vendor pass. | `test -d lean/Epistemos/eml-lean` | every 20 iters | iter-943 | FAILED: `test -d lean/Epistemos/eml-lean` returned `eml-lean=missing exit=1`; directory is still missing. |  |
...
- `LAKE-BUILD` cadence retry at iter-733 ran with explicit
  ...
  Full `lake build` printed `Build completed successfully.` and
  exited 0.
- `LAKE-BUILD` cadence retry at iter-743 ran with explicit
  `~/.elan/bin` PATH after the PCF-5, PCF-10, PCF-9, and related
  boundary witness passes. Full `lake build` printed
  `Build completed successfully.` and exited 0.
- `LAKE-BUILD` cadence retry at iter-760 ran with explicit
  `~/.elan/bin` PATH after the H12, H13, H14, PCF-4, and
  EML-LEAN-VENDOR ledger passes. Full `lake build` printed
  `Build completed successfully.` and exited 0.
- `LAKE-BUILD` cadence retry at iter-770 ran with explicit
  `~/.elan/bin` PATH after the PCF-5 through PCF-10 threshold
  and witness passes. Full `lake build` printed
  `Build completed successfully.` and exited 0.
- `LAKE-BUILD` cadence retry at iter-780 ran with explicit
  `~/.elan/bin` PATH after the E2 through E7 and Primitive IR
  schema-count passes. Full `lake build` printed
  `Build completed successfully.` and exited 0.
...
- `EML-LEAN-VENDOR` cadence retry at iter-719 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-739 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-759 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1. A first shell wrapper attempt used zsh's read-only
  `status` variable and failed before the actual ledger command; the
  retry above is the ledger command result.
- `EML-LEAN-VENDOR` cadence retry at iter-783 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-803 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-823 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-843 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-863 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-883 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-903 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-923 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `EML-LEAN-VENDOR` cadence retry at iter-943 returned
  `eml-lean=missing exit=1`; `test -d lean/Epistemos/eml-lean`
  exited 1.
- `LAKE-BUILD` cadence retry at iter-790 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Geometry identity-witness and Clifford-witness
  sharpening pass.
- `LAKE-BUILD` cadence retry at iter-800 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Info Bernoulli certificate-target witness pass.
- `LAKE-BUILD` cadence retry at iter-810 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the EML right-one certificate-target witness pass.
- `LAKE-BUILD` cadence retry at iter-820 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Scan and Tropical certificate-alignment pass.
- `LAKE-BUILD` cadence retry at iter-830 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Operator and Info constructor-alignment pass.
- `LAKE-BUILD` cadence retry at iter-840 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after all six generated-shape sample modules were
  imported into the aggregate Lean library.
- `LAKE-BUILD` cadence retry at iter-850 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Geometry, Tropical, Info, and Operator
  certificate witness-binder passes from iter-845 through iter-849.
- `LAKE-BUILD` cadence retry at iter-860 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Tropical, Geometry, and Info schema/certificate
  carrier passes from iter-855 through iter-859.
- `LAKE-BUILD` cadence retry at iter-870 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Operator dim-witness pass and Scan monoid/SSD
  schema-carrier passes from iter-865 through iter-869.
- `LAKE-BUILD` cadence retry at iter-880 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Tropical, Info, and Geometry target-carrier
  passes from iter-871 through iter-879.
- `LAKE-BUILD` cadence retry at iter-890 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
...
- `LAKE-BUILD` cadence retry at iter-910 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the EML, Tropical, Info, Geometry, and Operator
  generated-sample target-carrier passes from iter-901 through iter-909.
- `LAKE-BUILD` cadence retry at iter-920 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Scan, Info, Geometry, Operator, Tropical, and
  EML schema/certificate carrier passes from iter-911 through iter-919.
- `LAKE-BUILD` cadence retry at iter-930 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Scan, Info, and Tropical schema/certificate
  carrier passes from iter-921 through iter-929.
- `LAKE-BUILD` cadence retry at iter-940 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Tropical and Scan source-row
  schema/certificate carrier passes from iter-931 through iter-939.
- `LAKE-BUILD` cadence retry at iter-950 ran the table command with
  explicit `~/.elan/bin` PATH, printed `Build completed successfully.`,
  and exited 0 after the Operator, Geometry, Info, EML, and Tropical
  source-row schema/certificate carrier passes from iter-941 through
  iter-949.
