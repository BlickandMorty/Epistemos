...
| LAKE-BUILD | `lake build` runs with explicit `~/.elan/bin` PATH after the iter-581 Tropical schema repair and iter-592 zero-sorry E1 pass. | `PATH="$HOME/.elan/bin:$PATH"; cd lean/Epistemos && lake build 2>&1` | every 10 iters once `LEAN-TOOLCHAIN` resolves | iter-713 | RESOLVED: cadence retry at iter-713 ran full `lake build`, printed `Build completed successfully.`, and exited 0 after the iter-707 through iter-712 certificate-header alignments. | iter-582 |
| EML-LEAN-VENDOR | `tomdif/eml-lean` source is not vendored into the Lean project; gated on network/toolchain/vendor pass. | `test -d lean/Epistemos/eml-lean` | every 20 iters | iter-699 | FAILED: `test -d lean/Epistemos/eml-lean` returned `eml-lean=missing exit=1`; directory is still missing. |  |
