...
| EML-LEAN-VENDOR | `tomdif/eml-lean` source is not vendored into the Lean project; gated on network/toolchain/vendor pass. | `test -d lean/Epistemos/eml-lean` | every 20 iters | iter-597 | FAILED: `test -d lean/Epistemos/eml-lean` exited 1; directory is still missing. |  |
