...
|---|---|---|---|---|---|---|
| LEAN-TOOLCHAIN | `elan`, `lean`, and `lake` are not in `PATH`; blocks `lake update`, `lake build`, and typechecked Lean claims. Toolchain pin is `leanprover/lean4:v4.16.0`. | `command -v elan && command -v lean && command -v lake` | every 10 iters | iter-527 | FAILED: `elan: NOT_IN_PATH`; `lean: NOT_IN_PATH`; `lake: NOT_IN_PATH`; combined retry exited 1. |  |
