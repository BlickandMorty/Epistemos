...
| Item | Commit | What |
|---|---|---|
| **#1** (canon-merge) | 77463e3a | Merge `CANON_GAPS_AND_ADDENDA` C1 (WRV), C2 (no silent fallback), C3 (BYOK off by default), C4 (UX posture §4.0), C5 (canonical state is the only source of truth — §2.2 invariant #5 + §6 forbidden), C13 (telemetry policy line). Each merged block carries inline `(C#, merged 2026-05-05.)` provenance tag in the doctrine. |
...
| **A2** | pending | Macaroon-derived dispatch capabilities. Currently `cognitive_dag_store` registers a single `system_mirror_capability_hash`; doctrine wants per-mirror macaroon-derived caps. |
| **A3** | pending | Auto-invoke dispatch coverage gaps. 4 mirrors wired today (Skills / Procedural / Provenance / Companion). The other legacy write paths still bypass the DAG. |
...
| **C6, C7, C8, C9, C10, C11, C12, C14, C15** | partially staged | Remaining `CANON_GAPS_AND_ADDENDA` blocks. Some (C7 Phase R / PromptTree, C8 APP_STORE_RELEASE_COMPLETION_STATUS, C11 pre-release evidence package) need separate verification passes against current code state before merging — they reference work that may have moved. |
