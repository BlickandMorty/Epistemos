...
**Date:** 2026-07-04 · **Status: CANONICAL for the Mac App Store build** · Sibling of
[`PROMPT_PLAN_1_PRO_OPENCHAMBER.md`](PROMPT_PLAN_1_PRO_OPENCHAMBER.md).

> 🔴 **ARCHITECTURE CORRECTION 2026-07-04 (owner):** the MAS agent surface is **June,
> VENDORED and run as its real UI — cloned exactly like OpenChamber is on the Pro track —
...
> demo-look **structurally impossible** — you get June because it *is* June, the same reason
> OpenChamber can't look like a demo. This is the owner's original intent ("use June as the
...
build` → static web assets, WKWebView-embeddable like OpenChamber); a **small IPC seam**
...
1. **MAS build** (Epistemos-AppStore scheme, `EPISTEMOS_APP_STORE` + `MAS_SANDBOX`). App
   ...
   sandbox-legal.
2. **THE AGENT SURFACE = VENDORED JUNE** (fork + `epistemos/` overlay, like OpenChamber
   ...
   Native chrome (pill / mascot / all-chats sheet) wraps it, exactly as it wraps OpenChamber
   on Pro. The Epistemos wave / click-to-search **landing (Home) stays native**; June is the
...
6. **Money:** free = local (Apple FM + GGUF, ungated). Paid = cloud agent via StoreKit 2 →
   ...
   tokens in Keychain).
7. **Two-build split:** this is MAS. OpenChamber/OpenCode/browser-use are Pro-only. The
   vendored June fork + agent_core backend are the MAS agent surface; the Pro track never
   touches them.
...
- **Fork June + overlay** (`epistemos/`), like the OpenChamber fork: Epistemos changes as
...
- **De-risk FIRST (the §9 spike):** boot the built June frontend in a plain WKWebView with a
  ...
  through the adapter. **Honest gate:** June is more Tauri-coupled than OpenChamber's
...
- **Selection:** a composer chip in June's UI picks the lane (mirror the Pro engine chip);
...
- **Tool allowlist (from canon):** vault I/O (security-scoped bookmarks); in-app caps
  ...
  cloud calls via the proxy. **Forbidden (Pro-only):** `cli_passthrough`, `terminal`, bash
...
- **Ingest (all MAS-legal, on-device):** receipt/image → **Apple Vision `VNRecognizeTextRequest`**
  ...
  native; messy paste → agent field-inference. Cloud OCR = Pro-only. (Shared with Plan 9's
  ingest.)
...
## §6 VENDORING DISCIPLINE (like OpenChamber)
...
- **Native chrome wraps June:** the toolbar pill (Home/Agent/Notes/Settings), the all-chats
  sheet, the companion **mascot** (present on the Agent room while an agent works, per Plan 5).
  Chrome drives June via injected intent events, never by reloading the SPA URL (kills the
  session — same rule as the Pro pill). The Agent tab mounts June; no stub.
...
- **Phase 0 — Spikes (de-risk, AppStore scheme):**
  ...
  (b) **agent_core turn:** `EPISTEMOS_MAS_GOOSE_V0=1`, drive one `runAgentSession` turn and
      stream deltas into a stub — PASS (already proven; reuse).
  ...
  *If (a) reveals deeper Tauri coupling than the ~30 sites, STOP and report before vendoring.*
...
- Don't touch the Pro/OpenChamber track, the graph, or the editors (Plan 2) beyond the seams.
...
- **"Done" requires the owner to confirm it renders as real June + a working agent_core turn —
  an agent may not declare the look finished.**
