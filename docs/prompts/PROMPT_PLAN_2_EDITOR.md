...
PRIORITY-0 (do these first, before new build-sequence work):
  ...
  3. VISUAL FIDELITY (codepack §3a) — make the embed look like the finished MarkEdit app: inherit FontPicker.defaultFontSize + AppPreferences.Editor.lineHeight (stop substituting a smaller fontSize / hardcoded 1.45), harvest MarkEdit's default-prefs into AppBootstrap, match insets + window size. Acceptance: .md side-by-side with MarkEdit.app matches.
