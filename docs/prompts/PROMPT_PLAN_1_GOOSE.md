...
> ★ **NO NATIVE NAV RAIL (owner 2026-06-30 — supersedes the "native frame = window/nav-rail" wording below):** the native nav rail DUPLICATED Goose's own web sidebar (Chat/Sessions/Models/Skills/Recipes/Extensions/Scheduler/Apps appeared TWICE) and it LAGGED on every click. DROP it. **Native = the window FRAME only** (rounded window + vibrancy + traffic-lights + launcher + permission pop-ups); **Goose's OWN reskinned web UI is the ENTIRE surface AND the navigation.** Do NOT build or keep `AgentNavigationRailView`, the hybrid content router, or per-route native migration — that whole workstream is CUT. Goal = "just the Goose UI in a clean native window," reskinned high-quality-flat per the directive above. This REMOVES a large chunk of the old Phase-1/2/3 native work.
