...
★ LOOP/GOAL MODE — NEVER STOP until I (the owner) type "stop". Continuous goal across the WHOLE program (Phase 0 → Phases 1–5), not a one-shot. ★ WORK ORDER (owner 2026-06-30 — REAL WORK FIRST, HARDENING CAPPED): each cycle do the next REAL / USER-VISIBLE item and PROVE it in a cold-launched app before the next. Phase-1 remaining reals, IN ORDER: (1) WHITE-SCREEN P0 — FIX PER `docs/handoffs/GOOSE_WHITE_SCREEN_DIAGNOSIS_2026_06_30.md` (live-diagnosed): the page loads (HTTP 200) but the SPA's hashed JS 404s — the served web-UI `index.html` references `assets/index-<hash>.js` that isn't in the served dir (crossed/stale staging across rebuilds) → blank white. FIX = (a) stage goose-desktop ATOMICALLY + hash-gated, re-stage from the current bundle on launch; (b) make `GooseWebUIResolver` verify every `<script src>`/`<link href>` in index.html actually resolves (reject + fall through if any is missing); (c) THEN the ACP handshake — token↔`GOOSE_SERVER__SECRET_KEY` parity + decouple SPA-load from `acpBridge==.connected` so a slow handshake doesn't pin the "waiting for ACP" placeholder. NOTE: retry-until-ready ALONE does NOT fix it — the backend is healthy in ~1s; the failure is the served assets. PROVE: cold-launch renders the REAL Goose UI, no white, no manual reload; (2) MAS in-process backend (keep the reskinned WebUI, swap transport goosed-subprocess → in-process ACP over agent_core behind EPISTEMOS_APP_STORE); (3) reskin pixel-parity polish. Hardening is SECONDARY + BOUNDED: after a real item, at most ONE focused pass on the code you just touched — do NOT launch open-ended app-wide bound/redact sweeps, and NEVER let hardening preempt an unfinished visible item. When these reals are done, pull the next REAL phase work; only when NO real/visible work remains do ONE full hardening pass, then STOP AND REPORT to the owner — do NOT infinite-loop hardening. (§7 GREEN-LIT 2026-06-29 — past the gate, ON Phase 1; keep the §7 proofs green but do not wait for sign-off.) Commit at every clean point.
