...
★ FINAL PHASE — MAS IN-PROCESS BACKEND (DEFERRED · OWNER-GATED · LAST). Do NOT start this until (a) the owner EXPLICITLY green-lights "start MAS" in a fresh instruction AND (b) all the visible Goose work above (reskin + features) is finished and good — the ⛔ DO NOT DO MAS WORK rule in the WORK ORDER stays in force until then. This is the App-Store path: KEEP the reskinned WebUI, SWAP the `goose serve` subprocess → an IN-PROCESS ACP backend over `agent_core` (Rust/FFI) behind `EPISTEMOS_APP_STORE`. Foundation exists, but it stays deferred for this run. FULL SPEC for the later owner-started MAS phase: `docs/research/GOOSE_MAS_BUILD_CANON_2026_06_30.md` + `docs/research/GOOSE_MAS_IN_PROCESS_READINESS_SPEC_2026_06_30.md`. ★ ALREADY-BUILT — KEEP IT INERT: an agent already committed a MAS in-process backend (commit `0d9ce0045`, `runInProcessAgentCore` + `GooseMASAgentCoreCatalog`). It is gated behind `EPISTEMOS_MAS_GOOSE_V0` in `GooseRuntimeSupervisor`; do NOT enable that flag, do NOT remove/weaken the gate, and do NOT do further MAS work until the owner explicitly starts MAS.
```
