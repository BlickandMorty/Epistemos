...
★ FINAL PHASE — MAS IN-PROCESS BACKEND (DEFERRED · OWNER-GATED · LAST). Do NOT start this until (a) the owner EXPLICITLY green-lights "start MAS" AND (b) all the visible Goose work above (reskin + features) is finished and good — the ⛔ DO NOT DO MAS WORK rule in the WORK ORDER stays in force until then. This is the App-Store path: KEEP the reskinned WebUI, SWAP the `goose serve` subprocess → an IN-PROCESS ACP backend over `agent_core` (Rust/FFI) behind `EPISTEMOS_APP_STORE`. Foundation already exists (MAS entitlements incl. `network.server` + `cs.allow-jit` + vault bookmarks; `agent_core` UniFFI bridge; the `WorkSPAServer` loopback pattern; the `EPISTEMOS_APP_STORE` gate to re-wire). FULL SPEC = build order · transport · tool-boundary split (Pro-gate cli_passthrough/terminal/stdio_mcp/bash/imessage/apple/code_execution; keep vault/HTTP-MCP/cloud/in-app) · the 4 "truly good" loop rules · the sandboxed-build PROOF BAR → **PRIMARY REFERENCE: `docs/research/GOOSE_MAS_BUILD_CANON_2026_06_30.md`** — the 6-source CONVERGED canon (my deep-research + Gemini×2 + GPT×2 + a Goose-embedding synthesis). It **CORRECTS the transport** to `WKScriptMessageHandlerWithReply` (control plane) + `callAsyncJavaScript` event-push (streaming), NOT a loopback server and NOT `WKURLSchemeHandler`-primary — so the `WorkSPAServer` loopback + `network.server` entitlement mentioned above become the FALLBACK (path B, hardened), not the default; the default is to adapt the Goose WebUI's ACP transport onto the WebKit bridge (path A). It also gives the exact entitlement set (already declared), demands 2.5.2 self-containment incl. **HIDING the Goose WebUI's own add-MCP / `goose://` deeplink extensibility surface**, adds the 5.1.2(i) third-party-AI consent gate, and carries ranked risks + ready-to-submit App-Review notes. Read it FIRST. Companion (build order + proof bar + long-form deep-research §6): `docs/research/GOOSE_MAS_IN_PROCESS_READINESS_SPEC_2026_06_30.md`. When MAS is green-lit, build to the canon and prove it in a real sandboxed/notarized build (not Debug). ★ ALREADY-BUILT — KEEP IT INERT: an agent already committed a MAS in-process backend (commit `0d9ce0045`, `runInProcessAgentCore` + `GooseMASAgentCoreCatalog`). It is now **OWNER-GATED OFF by default** via `EPISTEMOS_MAS_GOOSE_V0` (default off) in `GooseRuntimeSupervisor` — on a MAS build it stays honestly `.unavailable` unless the owner sets the flag. ⛔ Do NOT enable that flag, do NOT remove/weaken the gate, and do NOT do any further MAS work — MAS stays INERT until the owner explicitly green-lights it. (Debug/Pro builds are unaffected; this only keeps MAS builds off.)
```
