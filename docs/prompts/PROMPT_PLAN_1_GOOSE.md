...
★ LOOP/GOAL MODE — NEVER STOP until I (the owner) type "stop". Continuous goal across the WHOLE program (Phase 0 → Phases 1–5), not a one-shot. ★ WORK ORDER (owner 2026-06-30 — REAL WORK FIRST, HARDENING CAPPED): each cycle do the next REAL / USER-VISIBLE item and PROVE it in a cold-launched app before the next. Phase-1 remaining reals, IN ORDER: (1) MAS in-process backend (keep the reskinned WebUI, swap transport goosed-subprocess → in-process ACP over agent_core behind EPISTEMOS_APP_STORE); (2) reskin pixel-parity polish; then pull the next REAL phase work (Phases 2–5) — FINISH THE PLAN first. Hardening is SECONDARY + BOUNDED + LAST: after a real item, at most ONE focused pass on the code you just touched — do NOT launch open-ended app-wide bound/redact sweeps, and NEVER let hardening preempt an unfinished visible item. WHITE-SCREEN: it RENDERS now (owner-confirmed 2026-06-30) ✅ — do NOT re-fix it; its robustness hardening (atomic hash-gated UI staging + `GooseWebUIResolver` verifying every referenced `<script src>`/`<link href>` resolves + ACP-decouple, per `docs/handoffs/GOOSE_WHITE_SCREEN_DIAGNOSIS_2026_06_30.md`) belongs in the FINAL hardening pass so a future rebuild can't regress it — it currently renders only because a rebuild happened to stage consistent assets. When the reals are done, pull the next REAL phase work; only when NO real/visible PLAN work remains do ONE full hardening pass (incl. white-screen robustness), then STOP AND REPORT to the owner — do NOT infinite-loop hardening. (§7 GREEN-LIT 2026-06-29 — past the gate, ON Phase 1; keep the §7 proofs green but do not wait for sign-off.) Commit at every clean point.
