...
BUILD ORDER (verify-first; much is specced-but-unbuilt per the codepack §6 — check live code before building):
  (1) PLUMBING — ⭐ PLAN 2 OWNS BUILDING THIS; PLAN 5 CONSUMES + VERIFIES IT (overlap resolved 2026-06-30). The note-context plumbing (`ActiveEpdocTracker` = frontmost EpdocDocument via NSWindow key + `NoteContextProvider` = bounded head/tail body via the EXISTING ProseMirrorMarkdownProjector → `WorkNativeMCPHost.updateContext` on note change) and the editor DOCK SLOT are Plan 2's build-sequence item 7. FIRST verify they exist in current code (Plan 2 should have landed them): if present → CONSUME them, do NOT rebuild/fork. ONLY IF Plan 2 has not yet landed them (its item 7 is late in its sequence) → build them here as a fallback, matching Plan 2's shape so there is no duplicate/divergent plumbing. Codepack §3 is the spec either way.
