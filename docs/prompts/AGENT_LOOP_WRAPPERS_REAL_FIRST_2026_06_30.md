...
> `docs/prompts/MAS_ONLY_STRATEGIC_PIVOT_2026_07_07.md` first. The loop-wrapper
> DISCIPLINE (real-work-first,
> verify-before-claim, commit-clean-points, autonomous proof) is durable, but any wrapper that
> launches or preserves a PRO/OpenChamber/ProAgent/Experimental/1Code lane is invalid. Current
> active surface is **MAS/June** only. KEELSTONE deletes OpenChamber/ProAgent residue and proves
> parked lanes do not leak into MAS.
...
> Current plans (build order + hard gates + canon) live in the active plan/prompt files named by
> `MASTER_PLAN_INDEX_2026_07_03.md` plus `docs/plans/*`. This file is discipline guidance, not
> a current Plan-1 launch prompt.
>
...
> - **Proof is autonomous:** the owner may test later. Call code done on your own honest self-check / compiling build —
>   do NOT block "done" on a formal in-app proof ceremony. But the build MUST compile so later review is possible, and
>   you must NEVER fake capability (honest gating stands).
...
>   reals, then ONE capped hardening pass on what you touched, then **REPORT A CLEAN CHECKPOINT**. Do not wait on
>   owner in-app testing when an explicit next real item remains; no new app-wide hardening loops.
...
hardening pass on what you touch, then report a clean checkpoint — do NOT infinite-loop hardening or wait on owner testing
when an explicit next real item remains.
⛔ INVALID WRAPPER (2026-07-06): do not launch a Plan-1 Goose or PRO/OpenChamber agent from this
section. `PROMPT_PLAN_1_PRO_OPENCHAMBER.md` is archived/deleted, and
`PROMPT_PLAN_1_GOOSE.md` remains a tombstone. For current agent surfaces, use
`PROMPT_PLAN_1_MAS_JUNE.md`, the Experimental/1Code prompt, and KEELSTONE's deletion plan.
...
MAS — superseded by the MAS-only June/Epdoc Assist lane. Do not revive the old Goose ACP loopback surface for App Store
builds; the former runtime-flag path is retired. The MAS path is now direct native SwiftUI/WK June -> in-process
agent_core (`GooseMASAgentCoreRunner`) with one approval/provenance/session authority:
  - Wire June/Epdoc Assist turns to the REAL agent_core agentic loop over the UniFFI bridge (preserve thinking
    blocks, stream every token, honor stop_reason, DispatchQueue.main.async NEVER .sync in FFI callbacks) — no
    CloudLLMClient passthrough and no loopback ACP listener. Replace empty extensions/recipes/skills/schedules + static catalog with live enumeration.
  - Implement the real tool-boundary split in agent_core (Pro-gate cli_passthrough/terminal/registry-bash/stdio_mcp/imessage/
    ...
    keyword heuristic. Keep MAS fail-closed; no one-flag Goose runtime rollback and no faked agent capability.

PROOF (autonomous): call code done on a compiling build + your own honest self-check; don't wait on
a proof ceremony or owner test. Never fake capability.
...
live everywhere; MAS in-process path runs the REAL agent_core loop behind the gate. Then one capped hardening pass and report
a clean checkpoint; continue only to explicit next real work, never to open-ended hardening.
```
...
report a clean checkpoint; do not wait on owner testing when an explicit next real item remains.
...
PROOF (autonomous): flip isLive on your own honest self-check once it works in-app; don't wait on a
formal ceremony or owner test; never fake.
...
after working in-app; regenerate feels direct/pixel-minimal. Then one capped hardening pass and report a clean checkpoint;
continue only to explicit next real work, never to open-ended hardening.
```
...
★ FINISH MODE — PLAN 3 (Capabilities: MAS-safe arXiv, voice/Kokoro, vault/native work backends; browser-use Pro parked). Don't rush, but CONVERGE: close
the reals, ONE capped hardening pass, then report a clean checkpoint; do not wait on owner testing when an explicit next real
item remains.
...
THEN — remaining REAL work:
 ...
 4. browser-use Pro signed packaging — PARKED by `MAS-ONLY-SHIP-LOCK-2026-07-07`; do not pursue Developer-ID/notarization while MAS-only is active.
...
PROOF (autonomous): flip honest gates on your own self-check once real; do not wait on owner testing; never fake capability.
KEEP CANON: MAS/June is the ONE active user-facing agent while `MAS-ONLY-SHIP-LOCK-2026-07-07`
is active; browser-use/Goose/Developer-ID motifs are parked donor context unless rebuilt through
MAS-safe June + `agent_core`. Graph DO-NOT-TOUCH, honest capability gating. Commit at every clean point.
DONE WHEN: build green; Kokoro voice live (or honestly blocked on model install, reported); AVSpeech off the shipped path;
signed packaging done or Dev-ID/notarization reported as the sole remaining gate. Then one capped hardening pass and report a
clean checkpoint; continue only to explicit next real work, never to open-ended hardening.
```
