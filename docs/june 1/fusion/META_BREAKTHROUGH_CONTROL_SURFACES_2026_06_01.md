...
| `F-ColdStream-vs-Mmap` | PASS metadata-only witness on 2026-06-04. Proves same-fixture mmap-fault, naive pread, and ColdStream benchmark-plan rows bind official Apple mmap/fcntl/Dispatch I/O/Metal source refs, p95/p99 stalls, read amplification, cancellation, fallback, rollback, RunEventLog, SCOPE-Rex/SovereignGate admission, visible AnswerPacket summaries, no SSD-as-RAM claim, no live benchmark attempt, and zero runtime/model bytes. L1 advances to `slab_arena_copy_count`; L2/L3 do not promote. |
...
| `F-SSD-WearBudget` | PASS metadata-only witness on 2026-06-04. Proves repeated ColdStream plans bind read/write volume, burst, energy, cache-pollution, write-amplification, reuse-horizon, rollback, RunEventLog, SCOPE-Rex/SovereignGate admission, compatibility fence, cache policy, visible wear caveats, and zero runtime/model bytes before SSD-backed route claims can promote. Downstream `F-ColdStream-vs-Mmap` now passes metadata-only; current L1 cursor is `slab_arena_copy_count`; L2/L3 do not promote. |
