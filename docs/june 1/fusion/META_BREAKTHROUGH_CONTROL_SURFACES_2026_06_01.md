...
| `F-SparseRoute-NoHiddenAuthority` | PASS metadata-only witness on 2026-06-04. Proves sparse route control cannot treat source priors, proof traces, oracle labels, PatternBoost motifs, fast-weight deltas, scout proposals, or sparse wake certificates as hidden live authority before route-source claims promote. ColdStream now passes metadata-only; current cursor is `large_model_provider_reference_deferred_by_mlx_route`. |
| `F-ColdStream-NoHiddenAuthority` | PASS metadata-only witness on 2026-06-04. Proves ColdStream transport cannot wake bytes, mutate route policy, override SCOPE-Rex/SovereignGate admission, suppress AnswerPacket proof, hide traces, or load runtime/model bytes before SemanticWorkingSetPlan, ResidencyPageTable, leases, rollback, RunEventLog, and AnswerPacket evidence exist. Current cursor is `large_model_provider_reference_deferred_by_mlx_route`; L2/L3 remain unpromoted. |
