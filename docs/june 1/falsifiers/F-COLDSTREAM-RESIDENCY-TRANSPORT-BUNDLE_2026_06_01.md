...
status: F-ColdStream-vs-Mmap, F-SlabArena-CopyCount, and F-MetalIO-FeatureGate passed metadata-only witnesses; no live cold transport promotion without codec, cancellation, cache, panic fallback, and live platform benchmarks
---
...
Current cursor after the 2026-06-04 `F-MetalIO-FeatureGate` metadata-only platform feature-gate witness: `codec_stage_latency`.
...
`F-TransportCancellation`, `F-CachePolicy-Pollution`, and
`F-ColdPanicFallback` remain separate platform/runtime gates before ColdStream
