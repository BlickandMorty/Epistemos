...
- Current cursor after the 2026-06-05 `F-SmallModelRuntimeHarnessFreshProductRuntimeL3ReleaseAuditPreflightProbe`: `small_model_runtime_harness_fresh_product_runtime_l3_release_audit_zero_fail_probe`. This advances L1/L3 release-audit preflight proof only; L2 product capability remains `vault_research_route_with_packetized_mitigation`, broader fresh app runtime capability is unchanged, and release readiness is still blocked until a real log-first zero-fail release audit lands.
