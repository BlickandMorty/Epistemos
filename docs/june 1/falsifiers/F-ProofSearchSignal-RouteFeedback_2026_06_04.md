...
This advances L1 only. It does not make proof feedback a live router, does not allow a proof assistant to bypass tests, citations, SCOPE-Rex, SovereignGate, RunEventLog, or AnswerPacket, does not promote sparse wake execution, does not load local model bytes, does not promote the 70B track to product runtime, and does not change MAS/Pro user copy. `F-ProofPressureSignal` now passes as metadata-only evidence; the current architecture unit is `F-AblationShadowRun`, which must prove claimed useful units survive counterfactual remove-one-unit comparison before route-importance claims can promote.
