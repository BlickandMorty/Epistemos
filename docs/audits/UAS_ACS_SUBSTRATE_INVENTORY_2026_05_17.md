...
|---|---|---|---|---|---|---|---|
| 1 | **UasAddress** | Unified Address Space identity | UAS (BODY) | Verified Floor | `agent_core/src/uas/address.rs` (landed iter 21 4f74a7e9 — 7 unit + 4 integration tests) | F-UAS-ZeroCopy-Spine, F-ACS-Anchor-Addressing | **landed (substrate-floor; Phase B.G.B1.a)** |
| 2 | **ResidencyLease** | substrate-tier lease handle | UAS (BODY) | Verified Floor | `agent_core/src/uas/residency_lease.rs` (landed iter 24 ce06b9f6 — 7 unit tests including RAII drop) | F-UAS-ZeroCopy-Spine | **landed (substrate-floor; Phase B.G.B1.d)** |
| 3 | **UasKind** | substrate-typed identity tag | UAS (BODY) | Verified Floor | `agent_core/src/uas/kind.rs` (landed iter 22 0ac612c3 — 8 known variants + Other(String)) | F-UAS-ZeroCopy-Spine | **landed (substrate-floor; Phase B.G.B1.b — T1 variant review pending)** |
...
| 5 | **ACS Anchor** | typed anchor (theorem tag · plane coord · residency tier · source hash · active packet id) | ACS (COORDINATE SYSTEM) | Verified Floor | `agent_core/src/research/acs/anchor.rs` (landed iter 27 01f4ab53) + `agent_core/src/research/acs/anchor_registry.rs` (landed iter 28 2deb32f1) + integration harness `agent_core/tests/acs_anchor_addressing.rs` (iter 29 0368d76b) | F-ACS-Anchor-Addressing | **landed (substrate-floor; Phase B.G.B3 iters 27-29; full ClaimLedger integration deferred to iter 38+)** |
...
| 8 | **HeliosPage** | sketch + residual + exact escalation | Shadow-first paging (SENSORY FILTER) | Verified Floor | `agent_core/src/research/page_gather/` (iters 43-47, 5 substrate modules + harness) | F-ShadowFirst-PageEscalation | **landed (substrate-floor; Phase B.G.B4 complete)** |
...
| 17 | **PacketRouter1bit** | 1-bit dispatch (MoE-style binary specialization) | Kernels (MUSCLE) | Verified Floor | `agent_core/src/helios/packet_router.rs` (CPU ref, 439 LOC) + `agent_core/tests/packet_router_dispatch.rs` (iter 48 c8e0eb16, 10k batch + cross-distribution) | F-PacketRouter1bit-Dispatch | **landed (substrate-floor; Metal kernel + p99 < 100 µs Phase B.G.B5)** |
| 18 | **ControllerKernelPack** | 6 fused micro-kernels (scalar_add · scalar_mul · max_reduce · argmax · copy_range · zero_fill) | Kernels (MUSCLE) | Verified Floor | `agent_core/src/helios/controller_pack.rs` (CPU ref, 343 LOC) + `agent_core/tests/controller_kernel_pack.rs` (iter 49 9e5707c9, 7-size sweep + sequence) | F-ControllerKernelPack | **landed (substrate-floor; Metal kernel + p99 < 50 µs Phase B.G.B5)** |
...
| 25 | **Active Assembly Runtime (AAR)** | active-pull selector for packets / components / model mechanisms | AAR (NERVOUS SYSTEM) | Verified Floor | `agent_core/src/research/active_assembly/{packet, selector}.rs` (iters 37-38) + harness `agent_core/tests/active_assembly_minimal.rs` (iter 39) | F-ActiveAssembly-Minimal | **landed (substrate-floor; Phase B.G.B6 complete)** |
