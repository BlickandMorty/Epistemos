...
Codex red-team addendum 2026-05-14: four release-gate entries were reopened by fresh verification and operator evidence. Follow-up closure pass: RCA8-P1-004, RCA9-P0-002, and RCA12-P0-002 are structurally patched and scanner/build verified; RCA8-P0-003 is patched with MAS and isolated Pro scratch-vault zero-crash/zero-runtime-issue soak evidence. A later closure pass promoted RCA3-P1-008 from PATCHED PARTIAL to PATCHED by surfacing per-model installed disk footprint in Settings. A MAS scratch note ask-bar follow-up patched the no-runtime silent-drop path so existing model-not-ready guidance is visible, but the full simple-rewrite live smoke still needs a safe scratch vault with a ready runtime. Treat MAS/Pro release readiness as blocked until the remaining live-smoke gates in `docs/CODEX_V1_FINAL_RECURSIVE_RELEASE_AUDIT_2026_05_14.md` close: protected graph spinner, MAS note ask-bar simple rewrite live smoke in a model-ready scratch setup, Pro cloud/provider smoke or waiver, and five consecutive recursive zero-new-blocker passes.
...
Status: PATCHED-2026-05-14 / MAS+PRO SCRATCH SOAK PASS — operator App Store crash evidence reproduced the deferred `FutureBackingData` / background SwiftData relationship-access risk. Follow-up fix snapshots SwiftData page primitives before async body/index reads; targeted vault release-gate tests pass, an isolated MAS scratch-vault import/Notes/schema soak completed with zero new crash reports, and an isolated Pro scratch-vault import/Notes/schema rerun completed with `ZSDPAGE` wiki-link columns present, five indexed pages, runtime diagnostics issues empty, and no audit-bundle crash.
...
| `agent_core/src/credential_pool.rs` (ORPHAN-HERMES-SALVAGE-001, surfaced 2026-05-16 by B-6 verification) | Dead file — present on disk (264 LOC, Apr 23) but NOT declared in `agent_core/src/lib.rs`, so not compiled into the crate. Zero Keychain integration. Hermes-parity worktree salvage that never landed. | None yet — file simply doesn't link, so no production code can reference it | Decide: WIRE into `agent_loop.rs`/provider routing with proper Keychain integration (matches original salvage intent), FORMALLY-SCAFFOLD (declare in `lib.rs` behind a feature flag + add drift-gate test), or DELETE. Not V1-blocking either way. |
| `agent_core/src/error_classifier.rs` (ORPHAN-HERMES-SALVAGE-001, surfaced 2026-05-16 by B-6 verification) | Compiled orphan — declared `pub mod error_classifier;` at `lib.rs:21`, 7 unit tests pass (`cargo test --lib error_classifier` => 7 passed), but `rg "use crate::error_classifier\|error_classifier::\|ErrorClassifier"` across `agent_core/src/` returns ZERO production call-sites. The 2026-05-15 gap audit's claim that this is "wired into `agent_loop.rs:10`" is FALSE in current main. | `error_classifier::tests` suite (7 tests; ensures the classifier doesn't silently lose its patterns) | Decide: WIRE into `agent_loop.rs` error-handling path for better UX surfaces on agent failures, or DELETE. Not a security gate — V1 ships fine without it. |
| `agent_core/src/session_persistence.rs` (ORPHAN-HERMES-SALVAGE-001, surfaced 2026-05-16 by B-6 verification) | Dead file — present on disk (508 LOC, Apr 23) but NOT declared in `agent_core/src/lib.rs`. Hermes-parity worktree salvage that never landed; schema is unverified because the module never links. | None yet — file simply doesn't link | Decide: WIRE as canonical session-state schema (note: `agent_core::session` already exists and is the live caller-chain for `load_session_context` per `context_loader.rs:177`), or DELETE if `session::*` is the canonical path forward. Not V1-blocking. |
...
corresponding feature lands OR is explicitly abandoned. (Exception note 2026-05-16: the three `agent_core/src/{credential_pool,error_classifier,session_persistence}.rs` rows came from the `hermes-parity` worktree salvage and are not documented scaffolding — disposition is genuinely open. Treat them as deferred-decision rather than known-scaffold.)
