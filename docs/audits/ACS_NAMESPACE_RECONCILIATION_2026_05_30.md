...
This correction replaces the temporary "ACS means Active Cold Storage" patch.
...
| ACS Kuramoto / Kuramoto cellular resonance | Research-tier resonance/autopoiesis candidate for coordinating firing subsets. | Nested under Active Assembly / research candidates, not product storage. |
...
| Residency Governor | Policy that decides leases, tiers, residency plans, copy-count limits, page/mmap safety, eviction, defer, or quarantine. | Works with ColdStore; not the same as AcsAnchor. |
| Active Assembly | Waking-set selector for a task. It reads from ColdStore, VaultRecall, graph, KV pages, adapters, parameter anchors, tools, and kernels. | Do not call this ACS. |
...
4. Treat `ACS Kuramoto` as a research/candidate resonance mechanism under
   Active Assembly, not as the product residency layer.
