...
| **W-14** | T2 + T09 verification iter-92/103 (`agent_runtime/function_call.rs` AnswerPacket emission; Swift `AnswerPacketEmitter` substrate) | `AnswerPacketEmitter` + `LatestAnswerPacketSink`; chat rows store `answerPacketId`, with packets available from the emitter ring for chat/Provenance surfaces | user sees AnswerPacket claim_kind + citations + confidence from per-chat-row lookup while the packet remains in the ring; visible badge details remain W-27 | every emission lands in the `AnswerPacketEmitter` ring, increments `emittedTotal`, and is retrievable by packet id through `LatestAnswerPacketSink` until ring eviction; `AnswerPacketHealthRow` shows non-zero emission count; per-row badge UX remains W-27 | P0 | merge T2 for runtime producer; current Swift emission/linkage substrate verified on T09 | PASS for Swift emission/linkage substrate; W-27 remains NOT-STARTED for visible per-row badge |
...
| **W-22** | T4 (extended `hybrid_search`) + T3/T14 (`UasAddress` + five-plane metadata) | retrieval returns `Vec<UasAddress>` instead of `Vec<NoteId>` | downstream consumers (agent context, Halo panel, ChatInputBar) get typed addresses with kind metadata | breaking change: every consumer migrates; cargo lib floor ≥ 1671 maintained | P1 | merge T3 + T4; W-01 | PARTIAL: MAS-safe UAS/five-plane vocabulary exists; vault return-type migration pending |
...
## §4b. Eidos V0 closed-citation retrieval (P0 — T10 Cohort A output, build_2026-05-18)

T10 built `agent_core/src/eidos/` with seven retrieval modes + the closed-citation contract behind the `EidosRetriever` trait. The cross-wirings below take that substrate from `implemented-not-wired` to user-reachable. See `docs/EIDOS_V0_CLOSED_CITATION_DESIGN_2026_05_18.md` for the full surface.

| ID | Source(s) | Consumer | User Surface | Acceptance | Priority | Deps | Status |
|---|---|---|---|---|---|---|---|
| **W-46** | T10 (`agent_core/src/eidos/*`) | `Epistemos/Eidos/EidosBridge.swift` (NEW) over Rust ↔ Swift FFI | Brain Panel + ChatCoordinator can request `EidosContextPacket` without bypassing the closed-citation contract | Swift exposes `Eidos.retrieve(query) -> EidosContextPacket` + `Eidos.validate(citation:against:) -> Result`; FFI binding round-trips JSON byte-equal in unit test; 217 Rust eidos tests still green | P0 | merge T10 | NOT-STARTED-FFI (Rust side ready: 217 tests green, Swift mirror types declared in `Epistemos/Eidos/Eidos.swift`, cross-language parity JSON fixture pinned. FFI plumbing + xcodebuild verification still TODO) |
| **W-47** | T10 (`EidosContextPacket::validate_citations`) | `Epistemos/App/ChatCoordinator.swift` emit-path gate | the "model hallucinated a source" failure becomes IMPOSSIBLE; rejection surfaces in chat as "this answer cited unverified sources" | ChatCoordinator runs every emitted source_id list through `validate_citations` before commit; deliberately injected fabricated id triggers refusal; tests cover all-valid / one-forged / mixed-failure cases | P0 | W-46 | NOT-STARTED-WIRE (Rust contract proven by `chat_layer_emit_gate_all_legitimate_citations_passes` + `chat_layer_emit_gate_refuses_wholesale_on_any_forgery` hardening tests; Swift mirror methods present. Gates on W-46 FFI) |
| **W-48** | T10 (`EidosHit::provenance` + `EidosScoreComponents`) | Brain Panel "Retrieved by Eidos" surface | per-hit badge shows mode (Lexical / Semantic / Hybrid / CodeSymbol / ClaimEvidence / GraphNeighborhood / RawArchive / Recency / ProvenanceVerified) + manifest id + 4-component score breakdown without any cloud round-trip | opening the Brain Panel during a vault query shows the badge with all 4 score components on 50/50 sample queries; offline-mode test passes | P0 | W-46 + T6 (Brain Panel surface from §6) | NOT-STARTED-UI (Rust packets carry every field needed; SwiftUI component build gates on W-46 FFI + T6 Brain Panel chrome) |
| **W-49** | T10 (`eidos::claim_evidence::InMemoryClaimEvidence`) + T3 / Phase 1 (`agent_core::provenance::ledger::ClaimLedger`) | replace `InMemoryClaimEvidence` with a `LedgerBackedClaimEvidence` that wraps `ClaimLedger` | retrieving evidence for any ledger-tracked claim returns matching evidence chunks with stance encoded in `source_id` | `LedgerBackedClaimEvidence` implements `EidosRetriever`; closed-citation contract holds against a populated ledger fixture; retraction propagation removes withdrawn evidence from future packets | P1 | merge T10 + ledger Phase 1 already in main | RUST-LANDED (commit ce69d4f28; 9 tests; snapshot-isolated; not-wired pending W-46 Swift bridge) |
| **W-50** | T10 (`eidos::graph_neighborhood::InMemoryGraphNeighborhood`) + existing `agent_core::cognitive_dag` | replace `InMemoryGraphNeighborhood` with a `DagBackedGraphNeighborhood` over the V2.1 cognitive DAG | 1-hop expansion from a real cognitive DAG node returns its `EdgeKind::DerivesFrom` + `EdgeKind::Contradicts` neighbors; the seed-encoded `source_id` shape (`{neighbor_id}::graph::from::{seed_id}`) survives the wiring | `DagBackedGraphNeighborhood` implements `EidosRetriever`; tests exercise both edge kinds; 2-hop is explicitly deferred to a deep-hardening pass with frontier-order spec | P1 | merge T10 | NOT-STARTED-BACKEND (analogous shape to W-49 LedgerBacked: implement new module, take a snapshot via cognitive_dag's read API, filter by EdgeKind. Self-loop case + closed-citation contract already proven by InMemoryGraphNeighborhood tests) |
| **W-51** | T10 (`eidos::semantic::InMemorySemanticIndex`) + `epistemos-shadow` (usearch HNSW backend) | `ShadowBackedSemanticIndex` over the production HNSW + same embedding model as the indexed corpus | semantic retrieval routes through the same shadow backend that powers RRF fusion in Swift; Eidos's Hybrid mode internally calls `ShadowBackedLexicalIndex` + `ShadowBackedSemanticIndex` with the SAME manifest id; k=60 RRF matches existing Swift `Phase3FusionConsts.K_RRF` | shadow backend exposes `(EidosDocumentId, Vec<f32>) → Vec<EidosHit>` over the same FFI as the lexical backend; cargo test --lib eidos:: still green; closed-citation contract holds end-to-end | P1 | merge T10 + epistemos-shadow already in main | NOT-STARTED-BACKEND (Eidos side ready: trait + closed-citation contract + RRF k=60 invariant pinned. epistemos-shadow side: needs an FFI surface that matches `(EidosDocumentId, Vec<f32>) → Vec<EidosHit>`) |
...
| **W-47** | T09 ledger row §5g (canonical typed events) | `CLAUDE.md` FILE MAP + canon docs | none (internal doc honesty) | Two related drifts surfaced in iter-72 audit: (a) **MutationEnvelope naming collision** — `agent_core/src/mutations/envelope.rs:40` is the canonical 14-field MutationEnvelope (§3.5 four-layer event hierarchy contract, Swift mirror at `Epistemos/Models/MutationEnvelope.swift`); `graph-engine/src/knowledge_core/store.rs:57` is a different concept (graph transaction descriptor with `tx_id`/`touched_block_ids`/`affects_*` flags). Rename graph-engine one to `GraphMutationEnvelope` or `KnowledgeCoreTransaction` to remove the collision. (b) **Canonical alias table missing** — `agent_core/src/scope_rex/answer_packet.rs:27-30` documents `TypedArtifact ≡ MutationEnvelope`, `RunEventLog ≡ provenance/ledger`, `ClaimFrame ≡ Claim`, `EvidenceLedger ≡ ClaimLedger`. Surface this alias table in CLAUDE.md FILE MAP so future audits don't search for nonexistent `pub struct RunEventLog`. | P2 | none (doc-only fix; in-source comment is the truth surface) | NOT-STARTED |
| **W-48** | T09 audit iter-76 — `omega-mcp/src/pty.rs` env-leak through `unistd::fork()`+`libc::execvp()` | `omega-mcp/src/pty.rs::spawn_pty` | **Security-sensitive — PTY shell child** | `spawn_pty` at `omega-mcp/src/pty.rs:305-381` uses raw `unistd::fork()` + `libc::execvp()` (current iter-734 recheck: fork at line 330, `execvp` at line 354), bypassing the standard `Command::new` API. `harden_cli_subprocess` is therefore NOT APPLICABLE to this path. The child inherits **all parent env vars** wholesale across fork; only `TERM=dumb` is `putenv`'d (line 348). `omega-mcp/src/subprocess.rs:39` does provide `hardened_command(program)` with `command.env_clear()`, but PTY does not call it because PTY uses raw fork/exec instead of `Command`. `LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`, `MallocStackLogging`, provider API keys, etc. can inherit into the PTY shell child unless cleared before `execvp`. Acceptance: add PTY-specific env hardening in the child branch before `execvp`: call `libc::clearenv()`, then reinstall only the canonical allowlist (`PATH`, `HOME`, `USER`, `LOGNAME`, `TMPDIR`, `LANG`, `LC_ALL`, `LC_CTYPE`, `TERM`, `TZ`) with `TERM=dumb` preserved/overridden. OR justify in code-comment why PTY shells legitimately need the full parent env. Add a sentinel-env test in `omega-mcp` that proves the PTY child does not inherit a parent-only variable (e.g. set `EPISTEMOS_PTY_ENV_SENTINEL=leak` and/or `LD_PRELOAD=/tmp/x.so` in parent, fork PTY, run `env`, assert sentinels absent and allowlisted essentials present). **P1** (Pro-tier exposure; PTY is reachable by Pro-tier MCP tool callers). | P1 | none — fix is self-contained in `omega-mcp/src/pty.rs` | NOT-STARTED |
| **W-49** | T09 audit iter-82 — `IMessageDriverService.swift` missing file-level `#if !EPISTEMOS_APP_STORE` guard | `Epistemos/Omega/iMessageDriver/IMessageDriverService.swift` | **App Store compliance — Pro-tier symbol leakage** | The `AppEnvironment.swift:39-41` `#if !EPISTEMOS_APP_STORE` guard correctly excludes the env-binding from MAS builds — but `IMessageDriverService.swift` itself has **no file-level guard**. The class compiles into App Store builds, just isn't injected. Compare to `Bridge/ComputerUseBridge.swift:1`, `Bridge/Phase4Bridge.swift:1`, `Harness/CompletionChecker.swift:1` which ALL have file-head `#if !EPISTEMOS_APP_STORE` guards. The asymmetry is the gap. Acceptance: add `#if !EPISTEMOS_APP_STORE` / `#endif` wrapping the entire file (matching the Bridge/* pattern) **OR** add CI build-matrix test that `nm` on `Epistemos.app/Contents/MacOS/Epistemos` lacks the `IMessageDriverService` symbol in App Store flavor. P2 (no current incident, but ship-hardening for Apple App Store review and a defense-in-depth). | P2 | none — fix is self-contained | NOT-STARTED |
| **W-50** | T09 audit iter-84 — `MemoryTier` enum vs prompt-deck canon divergence | `agent_core/src/cognitive_dag/edge.rs:118 pub enum MemoryTier` ⇆ T17B Lattice/WBO Register vocabulary | none (substrate vocabulary discipline) | Code has 5 variants (Hot=L0, Warm=L1, Cool=L2, Cold=L3, SelfEvolving=L_SE) — `docs/NO_COMPROMISE_ENDGAME_PROMPT_DECK_2026_05_18.md` §"Preserve explicitly — Six/seven memory tiers" names 7 (L0 hot, L1 compressed residual, L2 shadow/sketch, L3 SSD oracle, **L4/L5 cascade/adapters**, L_SE self-evolving, **L7 quarantine**). 3-tier gap (L4/L5/L7) is undocumented. Per Jojo's overseer direction iter-84: **T09 must NOT add the missing variants — T17B Lattice/WBO Register canonicalizes the tier vocabulary FIRST**, then cognitive_dag mirrors. Acceptance: T17B publishes the canonical 7-tier mapping in `docs/LATTICE_WYNER_ZIV_WBO_REGISTER_2026_05_18.md`; cognitive_dag/edge.rs's `MemoryTier` enum is then updated to match (adds `Cascade(L4)`/`Adapter(L5)`/`Quarantine(L7)` variants), with serde-compat for old 5-tier `.epbundle` artifacts. | P2 | **DEPENDS ON T17B canonicalization** — cognitive_dag mirror waits for T17B | NOT-STARTED |
| **W-51** | T09 audit iter-94 — Pro-tier capability gating absent in MCP dispatch path | `omega-mcp/src/dispatcher.rs::execute` + catalog entries for `github` / `web_search` / `pty` subsystems | **Product-tier separation — Pro lane is design-only** | `rg "isPro\|ProTier\|pro_tier\|TierPro\|requirePro\|tierGate\|ProSubscription\|SubscriptionTier\|EntitlementService" /Users/jojo/Downloads/Epistemos-t09-product-ledger/Epistemos/` returns **empty** across all 252K lines of Swift. `rg "tier\|capability\|Pro\b\|gate" /Users/jojo/Downloads/Epistemos-t09-product-ledger/omega-mcp/src/catalog.rs` returns only JSON-schema descriptions, no admission gate. The `Subscription` matches in Swift are EventBus subjects (e.g. `AppCoordinator.swift:61` `// MARK: - EventBus Subscriptions`), not app-store subscriptions; no `StoreKit` import in `Epistemos/`. `omega-mcp/src/dispatcher.rs:308` performs only argument-schema validation before dispatch — github / web_search / pty all execute unconditionally for any caller that reaches the dispatcher. So the "Pro" lane labels in `docs/CURRENT_PRODUCT_ARCHITECTURE_LEDGER_2026_05_18.md` reflect **design-intent product separation**, NOT a current code gate. The falsifier `F-OmegaMCP-ProToolGating` is therefore unfalsifiable today because the feature it tests doesn't exist. Acceptance: either (i) implement an `OmegaMCPTierGate` admission layer in `dispatcher.rs::execute` that consults a `Capability::ProTier` token before invoking github/web_search/pty subsystems (plus Swift-side `EntitlementService` reading StoreKit / receipt validation), then add the falsifier as a runtime gate test; OR (ii) drop the Pro-tier lane labels from the ledger until the feature exists. | P2 | none — cross-terminal product-architecture decision; touches both Rust (`omega-mcp/src/dispatcher.rs`) and Swift (new `EntitlementService.swift`) | NOT-STARTED |
| **W-52** | T09 audit iter-134 — `CSISafeguard` implemented + unit-tested but not wired into the distillation pipeline | `Epistemos/KnowledgeFusion/CloudKnowledgeDistillationService.swift::actor CloudKnowledgeDistillationService` (write paths) | **Alignment safety — Catastrophic Subjective Interference guard is orphan** | `Epistemos/KnowledgeFusion/Alignment/CSISafeguard.swift:14 final class CSISafeguard` exists with `recordMeasurement(value:epoch:experimentId:) -> Bool` + `computeCSI() -> Double` + `reset()` + `triggerMeasurement`. `EpistemosTests/OmegaCSISafeguardTests.swift` has 8 `@Test` markers exercising the class in isolation (untriggered start / healthy CSI continues / drop below threshold halts / rapid decline detected / reset clears / computeCSI good/poor/empty arrays). **Gap**: `rg "CSISafeguard\.|safeguard\.recordMeasurement\|CSISafeguard\(" Epistemos/` (excluding `/MOHAWK/` training data) returns **only the class definition file** — zero production callers. The training-data references at `Epistemos/KnowledgeFusion/MOHAWK/*.jsonl` name `Epistemos/Omega/Orchestrator/OmegaTrainingCoordinator.swift` as the caller, but that file **does not exist in the repo** (`find Epistemos -name "OmegaTrainingCoordinator*"` returns empty — training data was generated against an aspirational caller that was never wired). The class is orphan; the distillation pipeline (`CloudKnowledgeDistillationService` actor at line 17) writes adapter shapes without any safeguard evaluation. Acceptance: (a) wire `CSISafeguard.recordMeasurement(...)` into every distillation-write callsite inside `CloudKnowledgeDistillationService` BEFORE the persistence step, AND (b) add an integration test that asserts a low-CSI value short-circuits the write. This satisfies CLAUDE.md "HONEST CAPABILITY GATING" — knowledge-fusion alignment must respect the safeguard, not bypass it. **P2** (no current incident; risk surface for adapter-drift catastrophic regressions). | P2 | none — self-contained in `Epistemos/KnowledgeFusion/` (Swift only) | WIRED-SOURCE-GUARDED-XCODE-BLOCKED (Terminal E adds `CSISafeguard.shared`, calls `recordMeasurement` before `store.save(vault)`, and adds low-CSI short-circuit/source-order tests. Rust targeted tests + direct pro/lsp x86_64 build pass. Targeted Xcode verification stopped after three failures; current blocker is Swift 6 actor isolation in `CloudKnowledgeDistillationTests.makeNote(...)` called from a nonisolated provider closure.) |
| **W-53** | T09 audit iter-158 — Model download `verifySnapshot` is structural-only; no SHA256 LFS hash verification before marking `installed` | `Epistemos/Engine/ModelDownloadManager.swift::verifySnapshot` (line 96-134) + external `HuggingFace.HubClient` download boundary | **Supply-chain integrity — local model weights** | Iter-741 path refresh: this worktree has no local `Epistemos/Engine/HubClient.swift`; `ModelDownloadManager.swift` imports `HuggingFace`, creates `HubCache`, and `makeClient(cache:)` returns `HubClient(session:cache:)` from that package. `rg -n "(BLAKE3\|SHA256\|sha256\|checksum\|verifyHash\|integrityHash\|x-linked-etag)" Epistemos/Engine/ModelDownloadManager.swift` returns no model-download integrity hits. SHA256 is used elsewhere in the codebase (`SDPage.swift:78` body-hash for dirty detection, `Security/CapabilityBridge.swift:234` HMAC capability signing, `MutationEnvelope.swift:295 integrityHash`, `Graph/EntityExtractor.swift:57` entity hashing) but **never in the model-download path**. The `verifySnapshot` function at `ModelDownloadManager.swift:96-134` is structural-only — checks revision-format (40-char SHA hex OR `"main"`), file presence (`config.json`, at least one non-zero `.safetensors`, a tokenizer file). No cryptographic content hash. HuggingFace LFS responses include SHA256 in the `x-linked-etag` header for LFS-tracked files, but the current `downloadSnapshot(...)` wrapper result is ignored and `ModelDownloadManager` never reads or compares that metadata. **Real security gap**: a corrupted or maliciously-substituted weights file would pass `verifySnapshot` as long as it has the right filename + non-zero size + a tokenizer alongside. Acceptance: (a) capture LFS SHA256 / `x-linked-etag` metadata at the HuggingFace download seam or wrap the snapshot download with per-file hash metadata; (b) stream response bodies through a SHA256 hasher (`CryptoKit.SHA256.update(_:)`); (c) reject installation if `streamed_hash != expected_hash`. Add a `@Test` injecting a corrupt-byte mock and asserting the install fails. P2 (no current incident; remediation deferred to security/inference). | P2 | none — local fix centers on `Epistemos/Engine/ModelDownloadManager.swift` plus the HuggingFace client adapter boundary | NOT-STARTED |
