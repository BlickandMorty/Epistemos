...
| **W-48** | T09 audit iter-76 — `omega-mcp/src/pty.rs` env-leak through `unistd::fork()`+`libc::execvp()` | `omega-mcp/src/pty.rs::spawn_pty` | **Security-sensitive — PTY shell child** | `spawn_pty` at `omega-mcp/src/pty.rs:305-381` uses raw `unistd::fork()` + `libc::execvp()` (line 329 + 354), bypassing the standard `Command::new` API. `harden_cli_subprocess` is therefore NOT APPLICABLE to this path. The child inherits **all parent env vars** wholesale across fork; only `TERM=dumb` is `putenv`'d (line 348). `LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`, `MallocStackLogging`, etc. **inherit into the PTY shell child** with no protection — same hijack vectors `agent_core/src/security.rs::SUBPROCESS_DENYLIST` (24 entries) is designed to block. Acceptance: add PTY-specific env hardening in the child branch before `execvp`: call `libc::clearenv()`, then reinstall only the canonical 10-var allowlist (`PATH`, `HOME`, `USER`, `LOGNAME`, `TMPDIR`, `LANG`, `LC_ALL`, `LC_CTYPE`, `TERM`, `TZ`) with `TERM=dumb` preserved/overridden. OR justify in code-comment why PTY shells legitimately need the full parent env. Add a sentinel-env test in `omega-mcp` that proves the PTY child does not inherit a parent-only variable (e.g. set `EPISTEMOS_PTY_ENV_SENTINEL=leak` and/or `LD_PRELOAD=/tmp/x.so` in parent, fork PTY, run `env`, assert sentinels absent and allowlisted essentials present). **P1** (Pro-tier exposure; PTY is reachable by Pro-tier MCP tool callers). | P1 | none — fix is self-contained in `omega-mcp/src/pty.rs` | NOT-STARTED |
