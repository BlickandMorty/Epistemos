...
| **W-53** | T09 audit iter-158 — Model download `verifySnapshot` is structural-only; no SHA256 LFS hash verification before marking `installed` | `Epistemos/Engine/ModelDownloadManager.swift::verifySnapshot` (line 96-134) + `Epistemos/Engine/HubClient.swift` LFS download | **Supply-chain integrity — local model weights** | `grep -nE "(BLAKE3\|SHA256\|sha256\|checksum\|verifyHash\|integrityHash\|x-linked-etag)" Epistemos/Engine/ModelDownloadManager.swift Epistemos/Engine/HubClient.swift` returns **zero matches**. SHA256 is used elsewhere in the codebase (`SDPage.swift:78` body-hash for dirty detection, `Security/CapabilityBridge.swift:234` HMAC capability signing, `MutationEnvelope.swift:295 integrityHash`, `Graph/EntityExtractor.swift:57` entity hashing) but **never in the model-download path**. The `verifySnapshot` function at `ModelDownloadManager.swift:96-134` is structural-only — checks revision-format (40-char SHA hex OR `"main"`), file presence (`config.json`, at least one non-zero `.safetensors`, a tokenizer file). No cryptographic content hash. HuggingFace LFS responses include SHA256 in the `x-linked-etag` header for LFS-tracked files, but neither `ModelDownloadManager` nor `HubClient` reads or compares them. **Real security gap**: a corrupted or maliciously-substituted weights file would pass `verifySnapshot` as long as it has the right filename + non-zero size + a tokenizer alongside. Acceptance: (a) `HubClient` captures `x-linked-etag` SHA256 during download; (b) streams response body through a SHA256 hasher (`CryptoKit.SHA256.update(_:)`); (c) `verifySnapshot` rejects installation if `streamed_hash != x_linked_etag`. Add a `@Test` injecting a corrupt-byte mock and asserting the install fails. P2 (no current incident; remediation deferred to security/inference). | P2 | none — self-contained in `Epistemos/Engine/` (Swift only) | NOT-STARTED |
