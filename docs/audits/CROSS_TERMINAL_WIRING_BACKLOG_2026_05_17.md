...
| ID | Source(s) | Consumer | User Surface | Acceptance | Priority | Deps | Status |
|---|---|---|---|---|---|---|---|
| **W-46** | T10 (`agent_core/src/eidos/*`) | `Epistemos/Eidos/EidosBridge.swift` (NEW) over Rust ↔ Swift FFI | Brain Panel + ChatCoordinator can request `EidosContextPacket` without bypassing the closed-citation contract | Swift exposes `Eidos.retrieve(query) -> EidosContextPacket` + `Eidos.validate(citation:against:) -> Result`; FFI binding round-trips JSON byte-equal in unit test; 84 Rust eidos tests still green | P0 | merge T10 | NOT-STARTED |
