...
| **Wave J Pro Research validation against acceptance bars** | `docs/audits/` — J1-J9 entries | TERMINAL_HANDOFF_SNAPSHOT §"What needs USER ATTENTION" #6: "Wave J Pro Research entries J1-J9 — research-only; not validated against acceptance bars. USER must decide whether to promote any to V1 ship scope or keep deferred for V2." | each J1-J9 either: (a) gets a §4.G-style falsifier doc + harness, OR (b) is explicitly marked V2-deferred in MASTER_FUSION |
...
| **OpenClaw multi-claw MAS framework** | per TF §0 criterion #4: `mas_architecture_research.md` · multi-claw orchestrator · capability-scoped dispatch profiles · per-claw audit trail | OpenClaw was Wave J4 (Pro Research) — TF carved it out from TB to integrate as a multi-claw orchestrator | multi-claw orchestrator lands; capability-scoped profile per claw; audit trail flows to ClaimLedger |
...
| **MCP integration** | TD | TF (Channel Relay MCP servers) · TB (Pro Research MCP servers for Wave J integration) |
...
| **OpenClaw multi-claw MAS framework** | TF | TB (Wave J4 Pro Research source) · TD (per-claw provider routing) · TA (MAS / Pro tier gating) |
