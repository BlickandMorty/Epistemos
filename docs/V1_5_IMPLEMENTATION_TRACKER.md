...
| W9.26 B-tree rope | 🟡 FOUNDATION (PR2 of N) | dcc5521f + e9618ddf | PR1 foundation: `agent_core/src/rope.rs` wraps `crop = "0.4"` with `utf16-metric`. PR2: `agent_core/src/rope_handle.rs` raw FFI handle layer matching the W9.21 honest-FFI pattern (12 extern "C" exports incl. retain/release, insert/delete, utf16↔byte conversion, snapshot/free_string). 6 unit tests pass; 688/688 agent_core test floor green. PR3 Swift `RopeFFIClient` + `~Copyable` handle + PR4 NoteFileStorage migration + PR5 ProseEditorRepresentable2 bridge remain. |
