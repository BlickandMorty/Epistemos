...
| ID | Status | Commit | Notes |
| -- | ------ | ------ | ----- |
| N1 Prompt Tree (JSPF + PTF) | 🟡 FOUNDATION (PR1 of N) | 7316f86b + 1ab15596 + e8c22dbb | PR1 foundation + WRV anchor: PromptTree.swift (typed Prompt + 8 sub-types + composer), PromptRenderer.swift (4 targets + Relocation Trick), PromptCache.swift (4-breakpoint cap + per-provider degradation), PromptTreePersister.swift (PTF directory + GC), 5 prompt-shape entries in StructureRegistry, ChatCoordinator first-turn wire, 8 PromptTreeTests passing. PR1.5 (1ab15596) added Settings toggle (`Epistemos/State/PromptTreePreferences.swift` + StructuredSurfacesView footer). PR1 doc-update (e8c22dbb) reflects toggle landed. **Demoted 2026-04-27 per CRITIQUE_LOG pass #7** — `🟢 SHIPPED` was a status-drift claim; multi-PR pattern matches W9.21/W9.26/W9.27 series. Phase 1 cache-telemetry wire is blocked on the substrate gap discovered in 1f6c575d (session_insights.rs orphan + ReasoningTrajectoryMetricsFFI lacks token fields). Future PRs: substrate fix → cached_tokens_share telemetry → W9.6 dashboard wire → Rust SSE Relocation Trick end-to-end. |
