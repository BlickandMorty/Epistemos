...
| **W-22** | `hybrid_search` returns `Vec<UasAddress>` instead of `Vec<NoteId>` | T3+T4 | PARTIAL | T14 exposes MAS-safe UAS/five-plane vocabulary; vault retrieval return-type migration still pending |
...
| ID | Title | Owner | Status | Acceptance |
|---|---|---|---|---|
| **W-24** | DAG node carries UasAddress + AcsAnchor | T3 | PASS-IN-BRANCH | every NodeKind variant has `uas: Option<UasAddress>` + `anchor: Option<AcsAnchor>` + `RuntimePlane` + `ResidencyTier`; serde defaults preserve legacy JSON |
...
| **W-44** | 6 IR primitives in hyperdynamic_schemas (Tri-Fusion ABI accepts IR-typed expressions) | PARTIAL: `ScanProgram<T>` now carries `Option<LatticeBudget>`; full six-IR ABI still pending |
...
| **F-UAS-CopyCount** | T3/T14 | Zero tensor/data copies on hot path after shared backing; allowed metadata copies enumerated + byte-counted; stack-label coverage per allocation/copy site | PASS on Terminal G M2 Pro run. Harness: `agent_core/src/bin/uas_copy_count.rs`; artifact: `artifacts/falsifiers/uas_copy_count/result.json`; measured tensor copies: 0. Scope is honest: instrumented shared-backing fixture, tensor-copy counter only. |
| **F-ACS-AnchorLookup** | T18B / T3 / T14 | Valid anchor round-trips through lookup/audit/projection with all fields intact (theorem tag / plane coord / residency tier / source hash / active packet ID); invalid theorem IDs fail closed | PASS on Terminal G M2 Pro run. Harness: `agent_core/src/bin/acs_anchor_lookup.rs`; artifact: `artifacts/falsifiers/acs_anchor_lookup/result.json`; 10,000 anchors, average lookup 516 ns, invalid theorem ID rejected. |
