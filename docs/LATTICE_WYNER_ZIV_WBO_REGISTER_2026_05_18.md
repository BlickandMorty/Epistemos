...
| Residency-codec mapping | A row whose memory tier's primary codec differs from `ResidencyTier::primary_coder()` is rejected before it can borrow another tier's falsifier or side information; `ResidencyTier::primary_falsifier()` exposes the tier-owned hook directly. `residency_tier_catalog_maps_every_tier_to_primary_falsifier` asserts that every residency primary falsifier equals its primary codec falsifier. `ledger_validation_rejects_every_nonprimary_codec_for_every_residency_tier` asserts that every residency tier rejects every non-primary codec before side-information or falsifier borrowing; non-primary codecs still fail when borrowing the tier primary side-information and falsifier. |
