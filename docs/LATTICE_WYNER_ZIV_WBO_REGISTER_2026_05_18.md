...
| Standalone budget validation | `LatticeBudget::validate()` rejects empty contribution lists even before the budget is wrapped in a `WboLedgerEntry`; `lattice_budget_measured_status_returns_none_for_empty_public_contributions` asserts that empty public contribution lists cannot report measured success. |
