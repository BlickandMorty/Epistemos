...
| Register doc coverage | `register_doc_names_every_residency_tier_and_wbo_term` and `register_doc_names_every_codec_and_side_information_kind` assert that the Markdown register still names every canonical residency tier, `WboTermCode::ALL` term row, codec variant, side-information kind, exact residency-to-falsifier `F-*` hook set, exact term-to-falsifier `F-*` hook set, exact codec-to-falsifier `F-*` hook set, and exact codec-to-side-information witness set. |
