...
Codex delivered a **real Phase 0 scaffold** (~12 Swift modules, unit tests on mock ACP, build-for-testing green). The 2026-06-27 hardening slices closed the original live transport/WebView/Electron fallback proof holes, the top native file/dialog/external-URL affordances, the read-only custom ACP minimum, typed/unit-proven provider-settings read ACP, live provider config save/read/delete ACP, live read-only WebView route smoke for providers/settings/extensions/skills, structured unhandled-ACP diagnostics, and golden F1-F5 ACP fixtures, but **Phase 0 is still NOT signed off** because full parity/"nothing lost vs real Goose", provider authenticate/settings mutation parity, remaining long-tail shims, MAS/manual/distribution WRV, and owner sign-off remain open.

| Layer | ~Complete |
|-------|-----------|
| Architecture / code shape | ~85% |
| Owner §7 proof gate | ~92% |
...
| 5 | Nothing lost vs real Goose | **FAIL** — read-only `_goose/unstable/*` providers/extensions/preferences/defaults/session-info/diagnostics are live-proven, provider supported-model/config-read/config-status ACP is typed/unit-proven, provider config save/read/delete is live-proven, read-only WebView routes for providers/settings/extensions/skills are live-smoke proven, unsupported custom requests now return structured diagnostics, and F1-F5 ACP fixtures are pinned, but remaining long-tail shims, provider authenticate/settings mutation parity, MAS/manual/distribution WRV, and manual owner sign-off remain open |
...
- `/tmp/epistemos-goose-phase0-acp-custom-readonly.log` — `phase0_live_acp_custom_readonly=pass`, live session, 65 provider entries, 15 config extensions, preferences/defaults present, session info echo, diagnostics object report.
...
- Build/test checkpoint: `xcodebuild ... build-for-testing` passed after adding provider config mutation ACP; `GooseACPClientTests` + `GooseRuntimeSupervisorTests` passed 14/14; `GooseProviderMutationLiveIntegrationTests` passed 1/1 against real `goose serve`; focused Goose ACP codec/client/event-bridge/golden-fixture/shim/native-affordance/runtime suites passed 39/39; `GooseWebRouteLiveIntegrationTests` passed 1/1 after restaging; live `GooseLiveIntegrationTests` passed 6/6 earlier in the same Phase 0 hardening run; golden fixture suite passed 4/4; runtime/resolver/staging/Electron-launcher suites passed earlier in the same Phase 0 hardening run.
...
- Read-only custom ACP subset for providers, config extensions, preferences, defaults, session info, and diagnostics
- Provider-settings read ACP for supported models, config field reads, and configured-status reads, plus provider config save/delete ACP (save/read/delete live-proven in an isolated home; authenticate/settings mutation still open)
...
1. Finish extended ACP beyond the live-proven read-only subset, typed provider-settings reads, provider config save/delete, and read-only route smoke: provider authenticate, settings mutation, deeper Skills/provider/settings parity, or honest blocked UI where not yet wired.
...
```
GOOSE PHASE 0 — LIVE TRANSPORT/WEBVIEW/ELECTRON FALLBACK + TOP NATIVE AFFORDANCES + READ-ONLY CUSTOM ACP + PROVIDER ROUTE SMOKE + PROVIDER CONFIG MUTATION + GOLDEN F1-F5 PROVEN, NOT SIGNED OFF

DO NOT start Epistemos/Agent/* or hybrid AppKit Phase 1 until owner signs §7 proof gate.

NEXT ORDER: (1) provider auth/settings mutation parity and deeper Skills/provider parity or honest blocked UI,
