...
| 5 | Nothing lost vs real Goose | **FAIL** — read-only `_goose/unstable/*` providers/extensions/preferences/defaults/session-info/diagnostics are live-proven, provider supported-model/config-read/config-status ACP is typed/unit-proven, provider config save/read/delete is live-proven, settings preference save/read/remove plus defaults-save are live-proven, read-only WebView routes for providers/settings/extensions/skills are live-smoke proven, unsupported custom requests now return structured diagnostics, and F1-F5 ACP fixtures are pinned, but remaining long-tail shims, provider authenticate parity, deeper Skills/provider/settings parity, MAS/manual/distribution WRV, and manual owner sign-off remain open |
...
- Build/test checkpoint: `xcodebuild ... build-for-testing` passed after adding settings mutation ACP; focused Goose ACP codec/client/event-bridge/golden-fixture/shim/native-affordance/runtime suites passed 40/40; `GooseACPClientTests` passed 6/6; `GooseProviderMutationLiveIntegrationTests` passed 1/1; `GooseSettingsMutationLiveIntegrationTests` passed 1/1 against real `goose serve`; earlier same-day proof remains: `GooseWebRouteLiveIntegrationTests` 1/1, live `GooseLiveIntegrationTests` 6/6, golden fixture suite 4/4, and runtime/resolver/staging/Electron-launcher suites green.
...
- Provider-settings read ACP for supported models, config field reads, and configured-status reads; provider config save/delete ACP live-proven in an isolated home; settings preference/defaults mutation live-proven; provider authenticate still open
...
1. Finish extended ACP beyond the live-proven read-only subset, typed provider-settings reads, provider config save/delete, settings mutation, and read-only route smoke: provider authenticate, deeper Skills/provider/settings parity, or honest blocked UI where not yet wired.
...
```
GOOSE PHASE 0 — LIVE TRANSPORT/WEBVIEW/ELECTRON FALLBACK + TOP NATIVE AFFORDANCES + READ-ONLY CUSTOM ACP + PROVIDER ROUTE SMOKE + PROVIDER CONFIG/SETTINGS MUTATION + GOLDEN F1-F5 PROVEN, NOT SIGNED OFF

DO NOT start Epistemos/Agent/* or hybrid AppKit Phase 1 until owner signs §7 proof gate.

NEXT ORDER: (1) provider auth parity and deeper Skills/provider/settings parity or honest blocked UI,
