...
| M4 | MED | `listGitWorktreeDirs` blocks `@MainActor` up to 3s (`semaphore.wait`) and can pipe-deadlock on >64KB git output (drains the pipe only after the wait) → UI beachball + dishonest empty result | ⏳ DEFERRED: needs an async restructure (drain the pipe concurrently, run the subprocess off the main actor, deliver via the async replyHandler) |
