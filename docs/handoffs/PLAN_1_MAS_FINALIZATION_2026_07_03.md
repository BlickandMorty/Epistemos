...
| Stronger local chat (embedded llama.cpp GGUF) | A | **ENGINE PROVEN** + wired + linked | P0a `scripts/llama-mas-sandbox-spike.sh` (27 tokens, Metal, app-sandbox only). `llama.framework` embedded in the built `Epistemos.app/Contents/Frameworks`. `LocalGGUFQuickChatBackend` wraps the same engine, compiles + links. **App-gated:** live in-app answer needs a downloaded model + app launch. |
| Model download manager (checksum/resume/RAM-gate) | A | CODE-COMPLETE, build green | `QuickChatModelDownloadManager` (HF-published-sha256, atomic, resume, delete-on-corrupt). **Not exercised live** (needs a real download). |
