...
### E. arXiv-surface deep audit — IN FLIGHT at handoff time
A systematic deep pass over `Views/Arxiv/**` + `Arxiv/**` (network/XML resilience, per-keystroke re-publish,
parsing safety, dead code) was running when this handoff was written. **See the tail of `HARDENING_AUDIT.md`**
for its findings + any clean fixes Agent-B landed from it; anything it surfaced that was unsafe/owner-gated is
folded into sections A–D above.

---

## WHAT LANDED (15 findings, all `** BUILD SUCCEEDED **`, committed to `feat/goose-surface`)
