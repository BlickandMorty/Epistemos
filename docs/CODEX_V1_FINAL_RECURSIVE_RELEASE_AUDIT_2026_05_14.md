...
Scope: current Epistemos v1 only, for both Mac App Store (`Epistemos-AppStore`) and Pro/direct (`Epistemos`) builds. Helios/V6.2 migration, Lean verification stack, Pro Research kernels, speculative architecture rewrites, and post-v1 research work are excluded unless they affect the current shipping app.
...
| V1-GATE-LIVE-PRO-001 Pro live smoke incomplete | PATCHED/PARTIAL; cloud-agent smoke BLOCKED by no provider access in isolated app | Runtime UI / Pro routing | MAS non-blocker | Pro blocker until cloud account/API-key access is configured or explicitly waived for this machine | Required by user mission; manifest Pro identity at `docs/MAS_RELEASE_MANIFEST_2026_05_13.md:10-18`; Pass 11 Pro settings/runtime evidence | Computer Use: Pro surfaces, local/cloud routing, provider/tool tiers, no MAS restriction bleed | Pro profile surfaces, local model generation, and local vault-tool routing passed. Cloud provider/tool-agent smoke remains blocked because the isolated app has no stored OpenAI/Anthropic/Google account session or API key. The diagnostics row now reports account sessions as well as legacy API keys. | Yes, only evidence-backed |
...
Result: MAS settings diagnostics, MAS graph inspector simple summarize, MAS graph related-notes escalation, graph-return visual smoke, Pro profile visibility, Pro local chat, and Pro local vault-tool smoke passed. Zero-streak remains 0 because note ask-bar simple rewrite was not executed against user vault data, Pro cloud/agent smoke is blocked by missing provider account/API-key access in the isolated app, the Release-only build hang and broad undefined-import review risk remain unresolved, and five consecutive recursive passes are still incomplete.
...
Result: the MAS broad fork/exec import risk is cleared by excluding the GGUF/llama runtime from the App Store target while preserving it in Pro. Zero-streak remains 0 because note ask-bar simple rewrite remains blocked by user-data safety, Pro cloud-agent smoke remains blocked by missing provider account/API-key access, and five consecutive recursive passes are still incomplete.
...
- Pro cloud live execution remains BLOCKED: no OpenAI/Anthropic/Google provider account session or API key is available in the isolated app. The diagnostics row now distinguishes account sessions from legacy API keys instead of reporting only key state.
...
### Code file agent-write approval-loop closure - 2026-05-14

Changed:

- `RCA10-P0-004` is closed for current v1: CodeFileService containment and visible-editor routing were already covered; the missing proof was that agent/tool-originated `file.write` cannot mutate disk without a scoped R.5 grant.
- `EpistemosTests/ResourceRuntimeToolPathE2ETests.swift`: added `fileWriteWithoutGrantIsRejectedAndPreservesDisk`, which calls the real `executeToolCall` path for `file.write` without seeding a grant and asserts denial plus unchanged disk bytes.
- `docs/audits/RECURSIVE_CURRENT_APP_AUDIT_TODO_2026_05_09.md`: promoted `RCA10-P0-004` from partial to patched.
- No production CodeFileService, editor, Rust tool registry, vault, graph, or UI behavior changed.
...
### Pro/cloud provider access diagnostics - 2026-05-14

Changed:

- `Epistemos/Views/Settings/APIKeysHealthRow.swift`: Settings diagnostics now count stored OAuth/account sessions from `InferenceState.oauthCredential(for:)` as provider access, not only legacy API keys from `apiKey(for:)`.
- This matters for Pro cloud-agent readiness because the preferred v1 setup path is account-based for OpenAI/Codex CLI and Anthropic/Claude Code. A user with an imported account session should not see a misleading key-only `No provider keys stored` diagnosis.
- Empty state copy now says `No provider access stored`; connected rows can say `Account session saved`, `API key saved`, or `Account session + API key saved`.
- `Epistemos/Views/Settings/SettingsView.swift`: diagnostics comment updated from API-key visibility to account/API-key access visibility.
- No credential storage, Keychain mutation, provider routing, cloud request, agent tool loop, MAS gating, graph, or vault code changed.

Verification:

- Red before patch: `./scripts/xcodebuild_epistemos.sh -project Epistemos.xcodeproj -scheme Epistemos -destination 'platform=macOS' -derivedDataPath /tmp/EpistemosCloudAccessHealthRed -only-testing:EpistemosTests/CloudProviderAccessHealthRowTests test CODE_SIGNING_ALLOWED=NO -quiet` - FAIL. New guard caught missing `oauthCredential(for: provider)` and stale `No provider keys stored` copy.
- Green after patch: `./scripts/xcodebuild_epistemos.sh -project Epistemos.xcodeproj -scheme Epistemos -destination 'platform=macOS' -derivedDataPath /tmp/EpistemosCloudAccessHealthGreen -only-testing:EpistemosTests/CloudProviderAccessHealthRowTests test CODE_SIGNING_ALLOWED=NO -quiet` - PASS; xcresult `build/xcode-results/2026-05-14-161357-11501.xcresult`, `passedTests: 1`, `failedTests: 0`.

### MAS scratch note ask-bar no-runtime fallback - 2026-05-14

Changed:

- `V1-GATE-LIVE-MAS-001` follow-up: live MAS scratch-vault note ask could now be exercised without mutating the user's vault. In `com.epistemos.audit.vaultsoak.mas`, `Vault Soak Note 1` showed `Fast - Set Up Model`; entering `Rewrite this note in one shorter sentence.` cleared the field but produced no visible editor text or error. The underlying `NoteChatState` already set `error = UserFacingChatError.message(from:)`; the toolbar simply never surfaced that error.
- `Epistemos/Views/Notes/NoteDetailWorkspaceView.swift`: added `toolbarAskPlaceholder`, which shows the existing `noteChatState.error` copy in the ask field placeholder when the field is empty, otherwise falls back to `Ask this note`.
- `EpistemosTests/NoteEditorLayoutTests.swift`: added a source guard proving the note toolbar is wired to `toolbarAskPlaceholder` and to `noteChatState.error ?? "Ask this note"`.
- Screenshot evidence before the patch: `build/live-smoke-evidence/mas-noteask-disabled-scratch-2026-05-14.png`. The live interaction was in the disposable scratch MAS app only; no user vault note was mutated.
- This does not make no-runtime simple rewrite a full green release smoke. It converts a silent no-op into visible existing setup guidance. The full MAS note ask-bar simple rewrite gate still needs a safe scratch-vault run with a ready Apple Intelligence/local/cloud runtime.

Verification:

- Red before patch: `./scripts/xcodebuild_epistemos.sh -project Epistemos.xcodeproj -scheme Epistemos -destination 'platform=macOS' -derivedDataPath /tmp/EpistemosNoteAskErrorRedSuite -only-testing:EpistemosTests/NoteEditorLayoutTests test CODE_SIGNING_ALLOWED=NO -quiet` - FAIL after adding the guard; current source still hardcoded `placeholder: "Ask this note"`.
- Green after patch: `./scripts/xcodebuild_epistemos.sh -project Epistemos.xcodeproj -scheme Epistemos -destination 'platform=macOS' -derivedDataPath /tmp/EpistemosNoteAskErrorGreen -only-testing:EpistemosTests/NoteEditorLayoutTests test CODE_SIGNING_ALLOWED=NO -quiet` - PASS; xcresult `build/xcode-results/2026-05-14-165939-39944.xcresult`, `passedTests: 64`, `failedTests: 0`, `skippedTests: 1`.

## Current Verdict

Not release-ready. MAS build/scanner/live UI smoke are green for the isolated no-vault path, MAS Pro-only surfaces are hidden in diagnostics, the MAS and Pro scratch-vault import/Notes/schema paths now have isolated zero-runtime-issue evidence, and the first-run Platinum appearance/readable-font settings fix is green. Pro/direct diagnostics and Agent settings render with the expected Pro-only tool surfaces and approval posture. Local deterministic tool-loop, Pro local generation, local vault-tool routing, cloud routing contract checks, executable note/research tool parity, approval-to-R.5 grant bridging, AgentAuthority dispatch enforcement, CodeFileService/tool-write approval-loop proof, current-v1 child-process credential scrub proof, automated note ask-bar rewrite checks, OAuth callback loopback/forged-state proof, cloud provider access diagnostics, per-model local storage disclosure, requested real-glass sidebar/graph-note source guards, and the MAS scratch note ask-bar no-runtime visible-error fallback are green, but live Pro cloud-agent execution is blocked by missing provider account/API-key access in the isolated app. Remaining blockers: the scratch-vault graph has a protected first-open camera/framing bug where persisted nodes exist but are invisible until the user clicks Zoom to Fit, live MAS note ask-bar simple rewrite smoke remains incomplete in a safe scratch-vault/model-ready setup, first-run web approval live smoke is still pending because no live local/cloud tool turn can execute here, and the required five consecutive zero-new-blocker recursive passes have not been completed.
