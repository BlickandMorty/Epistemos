...
| V1-GATE-GRAPH-001 MAS scratch-vault graph loading stall | REOPENED / PROTECTED SURFACE BLOCKED | Graph runtime / MAS scratch vault | MAS blocker until diagnosed or explicitly deferred | Pro risk until checked in direct bundle | Isolated MAS audit bundle `com.epistemos.audit.vaultsoak.mas` with scratch vault `/Users/jojo/Downloads/Epistemos/build/vault-soak-scratch-vault`; graph view stayed on red spinner immediately after launch and after 35-second dwell; screenshots `build/live-smoke-evidence/mas-vault-soak-graph-spinner-2026-05-14.png` and `build/live-smoke-evidence/mas-vault-soak-graph-spinner-after35s-2026-05-14.png` | Reproduce with scratch vault, inspect non-rendering logs/data path first; any patch touching protected graph rendering/layout/camera/physics requires explicit approval | Documented evidence first; no graph code touched. Ask before patching protected graph files. | No, until approval for protected graph patch scope |
