...
4. **Send REAL text (0.23)** — live act inference; assert non-empty reply **and served-model ID == selected-model ID**
   (pass50 P1-a); owner's model; no HTTP requestFailed; no silent Codex/Qwen. **Build harness if missing.** Log
   prompt + ~80 chars + both model IDs. Work lane (0.29/1.7/W4): **REQUIRED** work harness — build if missing (P0-C).
5. **Screencapture** — build → open → `screencapture -x /tmp/epi_iter<N>_<surface>_YYYYMMDD-HHMM.png` → **Read PNG
...
14. **Full-plan witness (0.32, HARD GATE)** — append mandatory block to STRICT_RECERT_LOG: **highest attempted item ID**
    ...
    if highest attempted is before **1.1** unless all T0 are honest `[~]` AND you still attempted TIER 1+ same iteration.
...
Every iteration MUST attempt the full walk below. Skipping TIER 1–5 because act is broken is **FORBIDDEN** unless
TIER 0 items are honestly `[~]` with screencapture/send-text evidence — and even then you MUST attempt TIER 1+ same
...
| Item | Summary |
|------|---------|
| 2.1 | SUBSTRATE Phase 2 AnswerPacket |
| 2.2 | Helios salvage (7 items) |
| 2.3 | GUS salvage 1–18 |
| 2.4 | UNIFICATION orchestrator+TRINITY+router+chokepoint |
| 2.5 | EML honesty gate (GUS-2) |
| 2.6 | Eidos recall/rerank — NOT fake-green |
...
| 4.2 | System-prompts library + Epistemos Picks |
...
| 4.12 | Prose editor + MD-V2 coexist |
| 4.13 | Loved assets preserve |
