...
> Distills **6 research efforts** into ONE buildable spec: my 3 adversarial deep-research passes (`AGENT_FARM_RESEARCH_REPORT`)
> + 3 owner-commissioned reports (Gemini-1, Gemini-2, Claude). **2 more pending — this doc updates when they land.**
> Where ≥4 sources agree → CONVERGENT (build it). Where they conflict or one is wrong → CONTESTED/CORRECTED (read first).
...
- **Python bridge — two options, pick the proven one:** crossbeam-channel + async task (Gemini-1, Claude, my research =
  PROVEN/safe) vs **PyBevy/PyO3 in-process CPython** (Gemini-2 = powerful: zero-copy GPU, PEP-703 free-threading, but
  newer/riskier). **Start with channels; consider PyBevy only if you need zero-copy.**
...
## Status
- 6 sources distilled; **2 more reports pending** → fold in + re-rank confidence when they arrive.
- The convergence is the headline: **independent researchers (incl. adversarially-verified passes) landed on the same
  architecture.** The split, the Generative-Agents brain, EVE economy discipline, mine-don't-fork, and Bevy-as-foundation
  are now HIGH-confidence. The open risks are all *engineering* (Bevy churn, macOS fps, Python-bridge choice, MoA cost),
  not *design*.
