...
- **S.5 perf:** see Section 8 below for the 2026-04-25 audit + GraphPerformanceTests baseline + signpost coverage map + the two added signposts on Phase S verified-write paths. Status update from the 2026-04-25 S.5 evidence pass: the `perf_diagnostics` reliability gate has been refreshed (artifact `artifacts/reliability/20260425-053639/`, see §8.6); the four standalone perf suites have a refreshed cross-suite baseline (see §8.2b). The 2026-04-25 follow-up attempt to refresh the remaining five quality gates (`baseline`, `asan`, `tsan`, `ubsan`, `soak_repeat`) hit an xcodebuild test-runner connection hang on the first gate and did not produce evidence on any of them; see §8.7 for the exact failure shape. Remaining S.5 follow-ups: Instruments trace capture under typing+streaming load (still deferred -- needs a launched-app run); a retry pass on the five gates after diagnosing the test-runner-launch flake.
