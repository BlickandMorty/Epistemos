...
5. 🟡 **Phase R Rust module is PARTIALLY WIRED (as of 2026-04-23 commit `40bcd115`)**: `agent_core/src/resources/alias_registry.rs` (R.2) is now `#[uniffi::export]`'d with 2 helpers (`canonicalModelId`, `expandModelAliases`) and seeded with 11 model families (3+ variant forms each). `ResourceId` now has `uniffi::Enum` derive and crosses FFI. `ModelInvolvementSheet.expandModelIDsForFetch` uses the Rust resolver. **Still orphaned at the Swift boundary:** `ResourceService` (R.3), `AttachmentMode` (R.4), `PermissionService` (R.5 — Rust exists, Swift UI exists, wire absent), `verified_write()` (R.6 — Rust exists, Swift emission sites unmodified). Grep `"AliasRegistry\|ResourceId"` in `Epistemos/` now returns matches; the other types still return zero.
6. 🟡 **Key bugs status (post-commit `40bcd115`):** I-001 PARTIAL (read-side FIXED, write-edge deferred — see row below); I-002/003 still OPEN (6+ note lookup codepaths unchanged); I-004/5/6 still OPEN (snapshot-vs-live attachments unwired); I-007/008 still OPEN (7 unverified Swift write paths); I-009/010 still OPEN (no Swift PermissionService bridge).
