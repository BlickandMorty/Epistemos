...
5. 🟢 **Phase R Rust module is SUBSTANTIALLY WIRED (as of 2026-04-23 commits `40bcd115` / `6c5d5ecb` / `6a2c1de6`)**: R.2 (alias registry) + R.4 (attachment primitives) + R.5 (permission store) all have UniFFI exports and Swift bindings. `ResourceId`/`AttachmentMode`/`Capability`/`AttachedResource` cross FFI as typed enums/records. `PermissionGrantSummary` is a first-class Swift record. 17 UniFFI helpers across 3 bridge layers are live. **Still orphaned at the Swift boundary:** `ResourceService` (R.3 — the actual unified-gateway wiring for note read/write/create/delete), `verified_write()` (R.6 — Rust exists, Swift emission sites unmodified).
6. 🟡 **Key bugs status (post 6 commits this session):** I-001 PARTIAL (read-side FIXED), I-004/005/006 PARTIAL (FFI FIXED, Swift attachment-site migration deferred), I-009 PARTIAL (FFI + Settings UI FIXED, chat-handler auto-detection deferred), I-010 CONFIRMED-CLEAN, I-015 CONFIRMED-FIXED, I-017 CONFIRMED-CLEAN (partial), I-019 CONFIRMED-FIXED. **Still OPEN:** I-002/003 (6+ note lookup codepaths unchanged), I-007/008 (7 unverified Swift write paths), I-011/012/013 (model picker needs full redesign — blocked on UX mock).
