...
> truth is: Goose WebView owns models/providers/settings and every route inside the clean native frame.
>
...
**Last owner update:** 2026-06-30 — ✅ **PLAN-1 CANON RE-LOCKED.** After the auditor withdrawal pass below,
the owner approved the Goose mockup/spec: Claude-like clean flat UI + slight pixel twist, theme-aware, no blue
focus ring, no native nav rail, no native Models route. Invariants #3 and #11 now reflect that later owner-approved
state. The older auditor paragraphs below are retained as history only.
<br>**Superseded auditor pass:** 2026-06-30 (re-fire) — verified the flat-pixel-art WITHDRAWAL at that moment. Later
owner direction replaced it with the approved clean-flat + slight-pixel Goose spec.
<br>**Prior pass (loop-stop):** 2026-06-30 — owner stopped the old auditor loop after an earlier revert. Later owner
direction replaced that revert for Goose with the approved clean-flat + slight-pixel spec while keeping total
theme-awareness.
...
|---|---|---|---|---|
| 1 | **NO native chat** (Option 1) — chat stays WebView, reskinned; native = bare frame + native permission/elicitation pop-ups only | `grep -rniE "useNativeChatPath *= *true\|build native (chat\|transcript)\|chat-primary flip" docs/ \| grep -viE "DELETED\|SUPERSEDED\|HISTORICAL\|NOT\|no native\|do not\|ignore\|audit\|flag\|reskinned"` | empty (all residuals now HISTORICAL-marked or are correct "NO …" negations) | ✅ |
...
| 3 | **NO native Models route** — Goose web UI owns models/providers/settings and every route | `grep -rniE "Models picker = the ONE native route\|GooseNativeModelsView\\b\|EPISTEMOS_GOOSE_NATIVE_ROUTES\|epistemos\\.goose\\.nativeRoutes" docs/prompts docs/research docs/handoffs \| grep -viE "HISTORICAL\|SUPERSEDED\|OWNER OVERRIDE\|deleted\|retired\|no native Models"` | empty | ✅ 2026-06-30 owner override |
...
| 11 | **Goose flat/minimal + slight pixel twist is ACTIVE; total theme-awareness stays mandatory** | `grep -liE "Claude.*flat|slight pixel|TOTAL THEME-AWARENESS|theme-aware" docs/prompts/PROMPT_PLAN_1_GOOSE.md docs/research/EPISTEMOS_NATIVENESS_DOCTRINE_2026_06_29.md docs/research/GOOSE_NATIVE_WEB_RESKIN_2026_06_29.md docs/research/GOOSE_FLAT_PIXEL_RESKIN_SPEC_2026_06_30.md` | all 4 files listed | ✅ 2026-06-30 owner-approved mockup/spec |
...
- **PROMPT_PLAN_4_ICONS** (theme-canonical monochrome iconography; upgrades Plan-3's brand-logo spine, does not fork it).
  ...
  GRAPH DO-NOT-TOUCH · no fork/restructure of the shared logo spine (retheme-not-replace) · no runtime npm (MAS).
...
- **2026-06-30 — ✅ UPDATED (pixel-art design entry RESOLVED).** Owner approved the Claude-like clean flat
  Goose look with a slight pixel twist: borderless flat surfaces, no blue focus ring, pixel only on
  headings/labels/companions, and total theme-awareness. The radius-11 "open detail" is RESOLVED
  (radius 11 stands where the design system requires it). Current direction is now guarded by invariant #11.
  Remaining item is a BUILD bug, NOT a design decision:
