---
id: 4417EED6-5C21-4AE5-80D8-ACB4B4EAD0AB
title: Plan 3 — browser-use Pro vendor codepack (signed Pro payload, Pass 7)
---
...
`build/browser-use-pro/.venv/bin/python agent_core/vendor/browser-use/web-ui/webui.py --ip 127.0.0.1 --port <ephemeral> --theme Ocean`, probes the Gradio root document over loopback with a 5-600 second timeout bound, writes non-secret
...
notarization note, and second-precision UTC stamp. It rejects symlinked signature manifests and symlink components before parsing, and rejects symlinked package results so build-time
...
symlink-path rejected, read through no-follow descriptors, and capped before JSON decode. the manifest file itself is regular-file checked and capped at 1 MiB before JSON decode;
package results are capped at 64 KiB before JSON decode; manifest-declared
...
Additional verified gate evidence: artifact symlinks must resolve inside the vendor root; file-vs-directory mismatches
and symlink escapes are rejected before ready; manifest symlinks and oversized manifest files are rejected before decode;
uses ad-hoc signed `BrowserUsePro.bundle` fixtures for ready-path launch planning; lets the signed-payload gate reject
symlink escapes before runtime launch planning; rejecting symlinked env directories/files and symlinked parent components;
removes that exact generated `.env` again through a no-follow descriptor/`fstat` exact-content check.
...
existing `agent-browser --json <command>` shape. It maps `open/snapshot/click/fill/scroll/back/press/close/eval/ screenshot` to browser-use's `skill_cli` daemon, and `task` to a bounded browser-use `Agent.run(max_steps:)` subordinate
...
`--clear`. The console/errors compatibility stubs avoid browser-use runtime import and keeps console/errors compatibility stubs runtime. Rust bounds refs to short safe tokens before adapter execution. Command arguments after `--json <command>` are preserved even when they begin with `--`. The adapter
...
text fields, image `src` URLs are sanitized before the image text cap, and preserves page/adapter truncation flags;
...
schemes, then cap long URL/path strings before returning tool output. Adapter JSON error responses redact common
secret assignments after the error input is pre-bound before sanitizer regex work, then redact token/api-key aliases, OAuth-style
...
path is missing or non-executable. The bridge keeps the existing `browser_*` tool names behind `#[cfg(feature = "pro-build")]`. In both cases:
