...
> 🟡 **PARTIAL-SUPERSEDE 2026-07-02 (OpenChamber pivot).** The "lost features" inventory is durable + useful for the OpenChamber "nothing-lost" FEATURE LEDGER. Re-anchor surface/engine framing: Current surfaces = Experimental/1Code + MAS/June; OpenChamber/ProAgent are deletion targets; goose = one engine (not Goose-only). arXiv/Obscura kept dedicated. Canon: memory `project_ui_base_pivot_openchamber_2026_07_02`.
