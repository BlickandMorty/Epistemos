...
- Agent browser tools: `agent_core/src/tools/browser.rs` shells out to the bundled browser-use adapter when
  ...
  directory; it also rejects multiple screenshot output paths before runtime import. `browser_vision` also rejects
  screenshot paths that resolve outside the private screenshot directory before handing the image to any external vision
  provider. The
