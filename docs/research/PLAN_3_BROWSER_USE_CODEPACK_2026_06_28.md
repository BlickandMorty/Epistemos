...
- Agent browser tools: `agent_core/src/tools/browser.rs` shells out to the bundled browser-use adapter when
  ...
  text fields, and preserves page/adapter truncation flags; `browser_snapshot` caps snapshot text and derives refs only
  from that bounded text before returning capped refs;
