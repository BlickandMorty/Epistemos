...
cannot escape the vendor root; file artifacts must be files and directory artifacts must be directories. Launch remains
user-initiated and separate from the native WKWebView Browser.
...
- Gate artifact test: an armed manifest that claims staged packaging but lacks the declared runtime artifacts remains
  ...
  rejected before disk checks; file-vs-directory mismatches are rejected before ready.
