...
> 🟡 **PARTIAL-SUPERSEDE 2026-07-02 (OpenChamber pivot).** The editor DESIGN here (lens model Prose/Source/Note, MarkEdit engine, data-loss guardrails, note-context plumbing) is durable RESEARCH — but per owner 2026-07-02: editors = SIMPLIFY/POLISH, NOT build a Tolaria rival (OpenChamber's own editor carries agent-doc work; the 3-lens model is likely over-built — consolidation on the table). STALE: any "Goose note-context feeds Plan-1's Goose WebView/reskin under Option 1" / embedded Epdoc minichat framing — current surfaces are Experimental/1Code + MAS/June; OpenChamber/ProAgent are deletion targets; the Epdoc minichat is DEFERRED (agent works notes via shared files + a native companion mascot + note-edit provenance). Canon: memory `project_ui_base_pivot_openchamber_2026_07_02` + `project_product_shape_agent_center_2026_07_02`.
