---
id: 6BF8D23C-1D91-4261-AD16-ACE501F8465D
title: Plan 3 — Voice codepack (shipped code, Pass 8)
---
...
- **Kokoro-only TTS is live when a checked local CoreML package is installed:** `EpistemosSpeechSynthesizer.speak()`  
refuses playback while `KokoroVoiceGateStatus.isReady == false`; when the checked `mattmireles/kokoro-coreml`  
package is present and the native runtime is linked, it renders through native `KokoroPipeline` + `AVAudioEngine` playback. It does not  
...
- **Kokoro gate is honest:** `KokoroVoiceGateStatus` exists as the package/runtime gate. MAS and Pro use the same checked  
local package path; the gate requires `EPISTEMOS_KOKORO_VOICE_PRO_V0=1`, and missing `KokoroRuntimeManifest.json` / segmented  
...
local absolute model paths. A Voice settings section now shows the `TTS unavailable` / `Kokoro neural voice` runtime affordance and  
...
are committed. The download/install path is explicit opt-in and checked before it can enable playback. A higher-quality phonemizer remains future polish; the live path is  
an honest raw-vocabulary Kokoro path.
- **Local Kokoro package install/removal is real and playback-enabling:** `KokoroVoicePackageInstaller` lets users choose a  
...
settings row reports `packageReady`, and a failed replacement install rolls back to the previous package instead of deleting the backup; the same settings row now displays the gate's manifest-derived package  
evidence and can remove the installed local package, returning the gate to missing-model status. There is still no  
committed Kokoro model asset, Python, subprocess, hidden fallback, or unchecked Kokoro row.
- **Kokoro package quality is explicit and switchable:** the downloader surfaces the upstream Starter, Standard, and  
Highest Quality manifest paths. Starter uses `sdk/starter/KokoroRuntimeManifest.json`, Standard uses the repo-root  
`KokoroRuntimeManifest.json`, and Highest Quality uses `sdk/full/KokoroRuntimeManifest.json`. Current upstream Starter  
and Standard manifests declare the same seven-voice, 15-second-decoder package (about 0.48 GB); Highest Quality declares  
the full voice/bucket package (about 1 GB). Settings keeps the picker visible after install so a user can replace the  
installed package with another checked tier, and first-run onboarding offers the same optional Kokoro read-aloud setup.
- **Voice live smoke covers Kokoro gate, settings presentation, and checked package install/removal:** the bounded  
...
## Kokoro lane `[NATIVE COREML PLAYBACK WIRED]`

Kokoro-82M is checked-package gated until packaging and model-download gates are proven:
...
- `[DONE]` Add a Voice settings status/runtime affordance that says "TTS unavailable" until the checked package
gate is ready, then selects "Kokoro neural voice"; there is no Apple AVSpeech fallback lane.
- `[DONE]` Add a local checked-package installer/remover so a prepared package can reach `packageReady` and be
cleared again without adding a hidden fallback.
...
`AVAudioEngine` playback for checked local packages.
...
- The Kokoro runtime row must continue saying "TTS unavailable" until a checked package and linked native Kokoro runtime
exist.
...
- `Epistemos/Views/Settings/VoiceSettingsDetailView.swift` — composes visible voice preferences with the Kokoro
status/runtime affordance.
- `Epistemos/VoicePro/KokoroVoiceProSettingsSection.swift` — `TTS unavailable` / `Kokoro neural voice` row backed
by the gate status, with theme-derived badge tints, manifest-derived package evidence, checked Starter/Standard/Highest
Quality downloader controls that remain available after install, and shared native capsule install/remove/refresh chrome.
- `Epistemos/VoicePro/KokoroVoicePackageInstaller.swift` — local checked-package installer/remover with symlink
descendant rejection, staged copy, failed-finalization rollback, gate-backed removal, and bounded status diagnostics.
- `Epistemos/VoicePro/KokoroModelDownloadService.swift` — explicit opt-in checked-package downloader using URLSession,
CryptoKit, and the same installer/gate, with the three upstream Kokoro manifest paths represented honestly; no subprocess
or executable code download.
...
- `scripts/voice-live-smoke.swift` — bounded operator smoke for transcript/status helpers plus the Kokoro gate,
...
Kokoro status/install/remove UI, and no Kokoro/MAS subprocess leakage.
...
- Unit/source tests prove `speak()` does not call `AVSpeechSynthesizer.speak`; checked local packages use the native Kokoro
...
7. [DONE] Add the Kokoro gate as status-only.
8. [DONE] Add the Kokoro settings status/runtime affordance.
