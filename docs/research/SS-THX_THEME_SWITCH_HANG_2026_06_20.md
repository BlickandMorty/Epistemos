...
1. **[S, highest impact] CACHE the custom-theme resolve** — memoize `AppCustomTheme.resolved(isDark:)` keyed on a revision
   ...
   every `theme.resolved.*` re-reading UserDefaults. Target: one resolve per flip, not thousands → hang gone. (`EpistemosTheme
   .swift:306-311,1452-1568`.) **This is also the prerequisite for SS-TC.**
