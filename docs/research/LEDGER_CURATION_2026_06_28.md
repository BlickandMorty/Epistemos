...
> 🟡 **PARTIAL-SUPERSEDE 2026-07-02 (OpenChamber pivot).** Curation KEEP/MERGE/DEFER/CUT calls largely stand, but any "reskin Goose / Goose-as-surface / Option 1 / Goose-only" item is re-scoped: Current surfaces = Experimental/1Code + MAS/June; OpenChamber/ProAgent are deletion targets; goose = one engine. arXiv/Obscura kept dedicated (Obscura automation-vs-browser = open owner flag). Canon: memory `project_ui_base_pivot_openchamber_2026_07_02`.
