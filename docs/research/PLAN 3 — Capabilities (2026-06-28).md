...
scale before generation. The codepacks and first implementations now exist as shared Plan 3 surfaces. QuickLook preview titles keep
ellipsis inside configured caps. **Still not Plan 3-owned:** PDFKit `PDFView` viewer and PencilKit annotations; the PDF viewer
remains Plan 2.
...
- **Voice — STT SHIPPED; TTS KOKORO-ONLY GATED (Pass 8):** live Apple STT facade through `LiveVoiceInputService`,
  ...
  installer/remover; no Apple AVSpeech fallback, committed model asset, Python, subprocess, hidden fallback, or unchecked Kokoro row enters the App Store path.
