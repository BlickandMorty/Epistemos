...
- **arXiv pull — SHIPPED (Pass 6):** search arXiv, parse Atom metadata, download the PDF, convert through the local
  ...
  host/path/query; search query length and network-fed SwiftUI display strings are bounded before trimming; abstract text
...
- **Whole-app brand-logo coverage — SHIPPED:** the non-model `IntegrationBrand` registry and
  ...
  Meeting, settings sidebar marks for branded Plan 3 rows, and every Plan 3 landing feature button without runtime logo downloads or official-logo claims. Classifier input for arbitrary MCP/skill/connector names is raw-capped and
