...
> 🟡 **PARTIAL-SUPERSEDE 2026-07-02 (OpenChamber pivot).** The Plan-3 capability SET (PDF→MD, provenance, extensibility, browser-use Pro, arXiv, STT/voice, brand logos) is largely still valid. STALE: "Plan 1 = Goose surface / reskin Goose / AI is Goose-only" framing below → Current surfaces = Experimental/1Code + MAS/June; OpenChamber/ProAgent are deletion targets; goose = one engine. arXiv + Obscura KEPT dedicated + as capabilities (Obscura automation-engine-vs-browser = open owner flag, do not resolve). Live Plan 3 prompt = `docs/prompts/PROMPT_PLAN_3_CAPABILITIES.md` (override-headed). Canon: memory `project_ui_base_pivot_openchamber_2026_07_02` + `project_product_shape_agent_center_2026_07_02`.
