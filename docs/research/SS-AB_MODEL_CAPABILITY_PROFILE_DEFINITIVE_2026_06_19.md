...
ONE source of truth, **data-driven**, both model universes (MLX `LocalTextModelID` + GGUF `GemmaQATRuntime
Candidate` + cloud `CloudModelProvider`) resolve to it. Best metric chosen per dimension (provenance in []):
...
- **Pro:** the GGUF/llama.cpp lane (`#[cfg(feature="pro-build")]`, `--chat-template-file` + `-DLLAMA_LLGUIDANCE=
  ON`); ternary/BitNet (separate `bitnet.cpp` runtime, research).
