...
- **The loopback-server pattern is proven** — `WorkSPAServer` (an in-app `NWListener` loopback) already serves the Goose WebUI today. ✅ *(2026-07-03: proven FALLBACK only — the canonical MAS surface is native SwiftUI calling `agent_core` directly over UniFFI, no loopback; see `docs/prompts/PROMPT_PLAN_1_MAS_JUNE.md`.)*
...
2. **In-process transport.** ⚠️ **SUPERSEDED 2026-07-03:** the canonical MAS Surface B is **native SwiftUI (June-style)** calling `agent_core` directly over UniFFI (`runAgentSession` + `AgentEventDelegate`) — **no ACP hop, no loopback listener, no `network.server` entitlement**. Canon: `docs/prompts/PROMPT_PLAN_1_MAS_JUNE.md` §1/§3. The original step below applies ONLY if a web client for the agent is ever revisited: ~~Stand up an **in-app loopback ACP server** (reuse the `WorkSPAServer` `NWListener` pattern — `network.server` entitlement is already declared) that bridges WebUI ACP ↔ `agent_core` FFI.~~ *(Transport research §6 concluded: prefer `WKScriptMessageHandler` over loopback for any web path.)*
