...
- **Scaffolding (landed):**
  ...
  - **`tool_authz arm-by-arm expansion` (this session)** — `infer_tool_authz_target` now resolves three filesystem-targeting tools: `write_file` (`File { absolute_path }`, Write, `~/` expansion), `patch` (same shape, Write), and `trajectory_export` (File + Write when `output_path` is set, None for inline returns). A shared `file_target_from_path` helper centralises the trim + home-expand logic. 9 new arm tests (happy path + home-expand + missing-path + empty-path for the three arms, plus inline-omission for trajectory). One additional test (`non_resourceable_mutating_tools_return_none`) locks down the 20 mutating tools that explicitly do NOT have a ResourceId mapping today (shell passthroughs, messaging / Apple-app AppleScript tools, browser-* click/type/press, `interact`, `memory`, `skill_manage`, `tool_manage`, `cronjob`, `ssm_resume`, `nightbrain_trigger`) — so Step-2 default-enforcement-on doesn't accidentally block them.
- **Why still PARTIAL (until Step 2 lands):** (a) default mode is ADVISORY — gate fires + emits structured telemetry (`tracing::info!` with tool, capability, granted, active_grants, enforce) but does not block. Set `EPISTEMOS_R5_ENFORCE=1` to exercise the deny path; tests verify both branches. (b) in-memory store still disappears on app quit (persistence follow-up). Arm-by-arm coverage is complete for every tool that has a ResourceId mapping today; tools without one fall through to `None` with a test that locks that contract in place.
- **Remaining work:** (1) flip enforcement default from advisory to on (Step 2) — flips I-009 to FIXED; (2) migrate the permission store to on-disk persistence at a container-safe path; (3) backfill the parser hook + tool-check on MiniChat / Landing composer paths.
- **Verification today:** 38 Phase R.5 tests green (`PhaseRPermissionBridgeTests.swift` 9 + `PhaseR5ChatGrantWiringTests.swift` 7 + `tool_authz::tests` 17 + `tier_tests::r5_gate_*` 4, plus `non_resourceable_mutating_tools_return_none` single-pass covering 20 catch-all tools). The enforcement deny path is still proven by `r5_gate_denies_vault_write_when_grants_exist_but_not_for_this_resource`.
