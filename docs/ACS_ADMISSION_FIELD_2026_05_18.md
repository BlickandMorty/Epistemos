...
- `ACSPolicy` is a closed-schema, request-scoped, capability-aware object identified by a canonical policy ID; risk thresholds and operation-specific threshold overrides are closed-schema, required `VaultPath` paths must be trim-stable, while named required capabilities, `VaultPath` verbs, and `NetworkHost` hosts use the same canonical ASCII token alphabet.
