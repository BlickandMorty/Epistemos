...
| **Falsifier** | `F-AgentRuntime-HermesPromptParity` (**PASS** — verified iter-73): `EpistemosTests/HermesPromptFormatGuardTests.swift:57-60` has explicit `#expect(source.contains("<tools>"))` + `#expect(source.contains("<tool_call>"))` assertions. The test guards the Hermes-3 prompt format XML wrappers + tool-call XML format on the canonical `HermesPromptBuilder` source. Per `mod.rs:10-11` doc comment, the Hermes-3 prompt grammar is preserved because the local model (Nous Research spec) speaks that format — distinct from the purged subprocess. `F-AgentRuntime-SkillsRouteConsolidation` (NOT IMPLEMENTED): grep gate that asserts new agent-runtime callers go through `agent_runtime::skills::*` not the legacy `crate::skill_router::*` directly. |
...
| **Falsifier** | `F-CognitiveDag-SchemaVariantCount` (NOT IMPLEMENTED): the match-exhaustive variant-count test. `F-CognitiveDag-MerkleRootParity` (**PASS at unit-test layer**, NOT YET CI-GATED — verified iter-74): `agent_core/src/provenance/replay.rs:805` `verify_replay_catches_tampered_dag_merkle_root` test exists and asserts the merkle parity catches tampering; `verify_replay_catches_outer_integrity_tamper_first` (line 831) covers integrity-priority ordering. CI gate to make this run on every PR is the remaining gap (cross-row with [[Test substrate]] §23 — `F-TestSuite-NetGreen` is NOT IMPLEMENTED, so no test in this ledger is fully CI-protected). |
...
| **Falsifier** | `F-AnswerPacket-PerMessageEmission` (W-14 PARTIAL — substrate complete; persistence count gate pending). `F-AnswerPacket-FakeCitationRejection` (NOT IMPLEMENTED until T10 Eidos V0 lands — gated). `F-AnswerPacket-ClaimKindSchemaParity` (**PASS** — verified iter-77): `EpistemosTests/AnswerPacketCodableTests.swift:5` `@Suite("AnswerPacket Codable — V6.2 schema round-trip + backward-compat contracts")` with `@Test("AnswerPacket round-trips through JSON with answerPacketId-aware fields")` at line 10. `AnswerPacketEmitterTests.swift:298` adds `@Test("AnswerPacket Codable round-trips with interruptBucket field")`. **`HELIOSInvariantSourceGuardTests.swift:238-255`** `w2ClaimKindFiveArmEnumPresent` is the strongest invariant: `#expect(source.contains("pub enum ClaimKind"))` (line 244) plus loops over canonical arms asserting each is present — Rust enum-shape regression-protected from a Swift test by reading the Rust source file directly. Plus `HELIOSInvariantSourceGuardTests.swift:219` pins `pub ui_label: VrmLabel` field. |
...
| **Falsifier** | `F-Security-SubprocessHardeningGate` (NOT IMPLEMENTED): `cargo clippy --workspace` *forbidden* per disk pressure rules; alternative: a Rust integration test using `syn` to walk the crate AST and assert every `Command::new(...)` is followed within the same function by a `harden_cli_subprocess*` call. `F-Security-AllowlistDenylistDisjoint` (**PASS** — verified iter-74): `agent_core/src/security.rs:1116-1124` `#[test] fn allowlist_and_denylist_are_disjoint()` asserts `for &allowed in SUBPROCESS_ALLOWLIST: !SUBPROCESS_DENYLIST.contains(&allowed)` — defense-in-depth invariant. `F-Security-LDPreloadBlocked` (**PASS** — verified iter-74): `agent_core/src/security.rs:1126` `#[test] fn denylist_contains_doctrine_named_vectors` enforces LD_PRELOAD + DYLD_INSERT_LIBRARIES + DYLD_LIBRARY_PATH + MallocStackLogging are in the denylist; doctrine drift loudly fails. 16 `#[test]` markers in `security.rs` cover the full subprocess-hardening surface. |
...
| **Falsifier** | `F-OverseerAuditState-RealConsumer` (NOT IMPLEMENTED): grep gate asserting ≥ 1 production view reads `@Environment(OverseerAuditState.self)`. `F-GhostBrainCoauthor-NoCloudPath` (NOT IMPLEMENTED): structural check that `GhostBrainCoauthor` does not invoke cloud-provider FFI (parallel to `F-ComputerUse-NoCloudInferencePath` from iter-40). `F-VaultChatMutator-NoDuplicateAPI` (NOT IMPLEMENTED): grep gate that chat-vault mutations go through one canonical path, not split between `VaultSyncService` and `VaultChatMutator`. |
...
| **2 confirmed instances** (iter-79 correction: was 3; AppCoordinator/AppBootstrap pair removed after audit) | (1) **VaultSyncService vs VaultChatMutator** (iter-8 + iter-63): both mutate vault state. `VaultSyncService.swift` (4257 lines, `@MainActor @Observable final class` at line 274) is the canonical vault read+write surface; `VaultChatMutator.swift` (809 lines) is a separate mutator specifically for chat history. The split is plausible (chat-history mutations have different invariants than note mutations) but un-documented — verify chat-vault writes always go through `VaultChatMutator` and not via `VaultSyncService`'s broader API. (2) **AgentRuntimeRegistry (Swift, iter-55) vs `agent_core::agent_runtime` (Rust, iter-15)**: same conceptual name, distinct purpose. Swift `AgentRuntimeRegistry` is a 116-line local registry; Rust `agent_core::agent_runtime` is the renamed-from-hermes prompt-format / function-call / skills consolidation module. The name-overlap is doctrine-confusing without explicit documentation. CLAUDE.md FILE MAP should include a `## Naming Distinction` block. (Removed: **AppCoordinator vs AppBootstrap** — iter-79 audit showed AppCoordinator is a user-action orchestrator that *consumes* AppBootstrap-instantiated services, not a competing composition root.) |
...
| **Falsifier (meta)** | `F-DualAPI-CanonicalPathDocumented` (NOT IMPLEMENTED): CI check that, for each entry in a maintained `docs/canonical_path_registry.yaml`, the primary surface receives all new feature work (touched-files-in-PR analysis). `F-NamingCollision-DocumentedDistinction` (NOT IMPLEMENTED): grep gate that names colliding between Swift and Rust (e.g. `AgentRuntime`) appear in CLAUDE.md FILE MAP with an explicit `## Naming Distinction` block. |
...
| **Lane** | `R0` |
| **Ledger statistics (refreshed iter-81)** | **1144 lines** (was 1020 @ iter-65); **60 subsystem rows** (was 56); **63 falsifier fields**. Audit-pattern outcomes through iter-80 (`grep -c` on the doc): **19 PASS** markers (`(**PASS`), **16 PARTIAL** markers, **4 WOULD-PASS-TODAY** markers, **108 NOT IMPLEMENTED** markers (total — counts every falsifier; each row often has 2–4). 2 meta-pattern rows (§20 god-state, §21 dual-API). **3 W-rows authored** in CROSS_TERMINAL_WIRING_BACKLOG §12B: W-46 (CLAUDE.md macaroons-orphan doc drift), W-47 (MutationEnvelope naming collision + canonical alias table missing), **W-48 (P1 security gap — `omega-mcp/src/pty.rs` env-leak through `unistd::fork()`+`execvp()`)**. |
...
| **Falsifier (self-referential)** | `F-Ledger-EveryRowHasFalsifier` (NOT IMPLEMENTED): doc lint that asserts every `### Subsystem` header in the ledger is followed by a `| **Falsifier** |` field. |
...
| **Falsifier** | `F-TiptapBundle-HashGateSkipsWork` (NOT IMPLEMENTED): test that re-running `build-tiptap-bundle.sh` with no `package-lock.json` change skips npm install + bundle. `F-PerfBudgets-CIWired` (NOT IMPLEMENTED): verify `scripts/check-perf-budgets.sh` runs in CI and gates merges. `F-HeliosInvariants-CIWired` (NOT IMPLEMENTED): same for `check-helios-invariants.sh`. `F-ScriptsCacheNotInRepo` (NOT IMPLEMENTED): grep gate ensuring `scripts/__pycache__/` is in `.gitignore` and not committed. |
...
| **Lane** | `R0` |
| **Total W-rows referenced** | 27 distinct W-rows referenced across the ledger (out of 46 total in `docs/audits/CROSS_TERMINAL_WIRING_BACKLOG_2026_05_17.md`). Gap of 19 W-rows that this ledger does NOT yet cross-link — most are Phase ε / ζ / η / θ / ι items waiting on T-branch merges. |
| **Top-cited W-rows (high cross-link density indicates highest leverage)** | `W-14` (AnswerPacket runtime emission — **16** references): touches scope_rex::answer_packet, ChatCoordinator, Settings UI Diagnostics, §14 build-next list. `W-32` (Experimental Features panel — **10** refs): touches SearchIndexService feature-gate, ConfidenceRouter, Settings UI, §14. `W-29` (unified Substrate Health Panel — **10** refs): touches Settings UI Diagnostics, EML observatory (W-07), UAS-ACS substrate health (W-10), §14. `W-12` (per-model HONEST/EXPERIMENTAL/OFF badge — **10** refs): touches LocalAgentLoop, ConfidenceRouter, LocalModelManager, §14, Settings UI. `W-11` (ActiveConstellationRow live binding — **10** refs): touches Settings UI, LocalModelManager, §14, build-next. |
...
| **Uncovered W-rows (19)** | W-02, W-05, W-06, W-09, W-10, W-16, W-24, W-28, W-30, W-34-W-39 (biometric lock — gated on T8), W-40, W-43, W-44, W-45. Most are Phase η (biometric, W-34–W-39) or Phase ι (Research-tier capability ceiling, W-40+) — out of scope for T09 until corresponding T-branches merge. |
| **Missing proof** | (a) The 27/46 coverage rate suggests T09 is healthy on Phase ε/ζ subjects (substrate-to-product wiring) but light on Phase η (biometric) and Phase ι (research-tier) — that's correct per the prompt deck scope; (b) No automated check ensures this reverse index stays in sync with the W-rows actually cited in the doc — future ticks should regenerate it. |
| **Next action** | Future tick: regenerate this index on every batch-verify pass (i.e. when the ledger row count changes ≥ 5). |
| **Falsifier** | `F-WRowIndex-Reciprocal` (NOT IMPLEMENTED): doc check that for every `W-NN` cited in this ledger, the corresponding W-row in CROSS_TERMINAL_WIRING_BACKLOG.md exists. Catches typos like `W-99` or stale W-row deletions. |
...
| **Falsifier** | `F-MemoryTier-CanonParity` (NOT IMPLEMENTED): doc gate asserting `MemoryTier` variants match the prompt deck §"Preserve explicitly" list, OR that the gap is explicitly documented (e.g. "L4/L5/L7 are Research-tier and intentionally unimplemented in MAS"). `F-MemoryTier-SelfEvolvingNightBrainMutation` (NOT IMPLEMENTED): integration test that a NightBrain run promotes a `Cool` edge to `SelfEvolving`. `F-MemoryTier-OrderingDoctrine` (**PASS confirmed iter-77** — `agent_core/src/cognitive_dag/edge.rs:446` `#[test] fn memory_tier_ordering_reflects_doctrine()` asserts `Hot < Warm < Cool < Cold < SelfEvolving` per "Doctrine §2.8: lower numeric = hotter"). |
...
| **Falsifier** | `F-MutationEnvelope-NoDualDeclaration` (NOT IMPLEMENTED): assert exactly one `pub struct MutationEnvelope` in `agent_core/src/` and one in `graph-engine/src/` OR explicit documentation of the dual-API. `F-CanonicalEventTypes-Completeness` (NOT IMPLEMENTED): doc gate asserting every type named in the substrate handoff "current product spine" (`TypedArtifact → MutationEnvelope → RunEventLog / AgentEvent / GraphEvent → UI projection`) has a corresponding `pub struct` or documented alternative. |
