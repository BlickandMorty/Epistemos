...
| **Missing proof** | (a) **Dual MutationEnvelope declarations** at `agent_core/src/mutations/envelope.rs:40` AND `graph-engine/src/knowledge_core/store.rs:57` — this is a §21 dual-API instance candidate; verify the two structs are different concepts (graph-mutation envelope vs. general mutation envelope) or consolidate. (b) `RunEventLog` is referenced 7 times in CROSS_TERMINAL_WIRING_BACKLOG_2026_05_17.md (e.g. W-02 "every agent trace tagged with `UasKind::AgentTrace`") but not located via grep — may be a Swift-side type or a typed stream; deeper grep needed. (c) `TypedArtifact` is referenced in canon "current product spine: `TypedArtifact → MutationEnvelope → RunEventLog / AgentEvent / GraphEvent → UI projection`" — verify it's a Rust or Swift type and classify. |
