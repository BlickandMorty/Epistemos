...
| **3 confirmed instances** | (1) **VaultSyncService vs VaultChatMutator** (iter-8 + iter-63): both mutate vault state. `VaultSyncService.swift` (4257 lines, `@MainActor @Observable final class` at line 274) is the canonical vault read+write surface; `VaultChatMutator.swift` (809 lines) is a separate mutator specifically for chat history. The split is plausible (chat-history mutations have different invariants than note mutations) but un-documented — verify chat-vault writes always go through `VaultChatMutator` and not via `VaultSyncService`'s broader API. (2) **AppCoordinator vs AppBootstrap** (iter-48): both wire app-lifecycle services. AppBootstrap is the canonical composition root per AGENTS.md §"Patterns to Follow — `withAppEnvironment(bootstrap)`". AppCoordinator (376 lines) overlaps; if both wire the same service the second wiring silently wins or breaks. (3) **AgentRuntimeRegistry (Swift, iter-55) vs `agent_core::agent_runtime` (Rust, iter-15)**: same conceptual name, distinct purpose. Swift `AgentRuntimeRegistry` is a 116-line local registry; Rust `agent_core::agent_runtime` is the renamed-from-hermes prompt-format / function-call / skills consolidation module. The name-overlap is doctrine-confusing without explicit documentation. CLAUDE.md FILE MAP should include a `## Naming Distinction` block. |
