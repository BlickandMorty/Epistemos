...
| **Falsifier** | `F-MemoryTier-CanonParity` (NOT IMPLEMENTED): doc gate asserting `MemoryTier` variants match the prompt deck §"Preserve explicitly" list, OR that the gap is explicitly documented (e.g. "L4/L5/L7 are Research-tier and intentionally unimplemented in MAS"). `F-MemoryTier-SelfEvolvingNightBrainMutation` (NOT IMPLEMENTED): integration test that a NightBrain run promotes a `Cool` edge to `SelfEvolving`. `F-MemoryTier-OrderingDoctrine` (PASS — `edge.rs:446` test asserts canonical ordering). |
