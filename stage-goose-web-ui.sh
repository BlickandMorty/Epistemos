...
if [ "${EPISTEMOS_GOOSE_UI_VALIDATE_ONLY:-0}" = "1" ]; then
    ...
    grep -q "border-border-danger focus:ring-text-danger/20" "$WORK_ROOT/ui/desktop/src/components/ParameterInputModal.tsx"
    ...
    grep -q "border-border-danger focus-visible:ring-text-danger/20" "$WORK_ROOT/ui/desktop/src/components/ui/JsonSchemaForm.tsx"
    grep -q "isUrgent ? 'text-text-danger' : 'text-text-secondary'" "$WORK_ROOT/ui/desktop/src/components/ElicitationRequest.tsx"
    grep -q "border-\\[var(--epistemos-accent)\\] ring-\\[3px\\]" "$WORK_ROOT/ui/desktop/src/components/common/InlineEditText.tsx"
