...
struct MCPServersView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if registry.servers.isEmpty {
                VStack(spacing: 12) {
                    ...
                    Text("No MCP servers configured")
                        ...
                        .foregroundStyle(.secondary)
                    Text("Add servers to expose tools to Epistemos")
                        .font(AgentSkin.mono(11))
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("MCP (Model Context Protocol) servers provide tools to Epistemos")
                    .font(AgentSkin.mono(11))
                    .foregroundStyle(.secondary)
    ...
    private func serverRow(_ server: MCPServerConfig) -> some View {
        ...
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(spacing: 3) {
                    ...
                    case .connected:
                        ...
                        Text("Connected").font(AgentSkin.pixel(10)).foregroundStyle(.green)
                    case .connecting:
                        ProgressView().controlSize(.mini)
                        Text("Connecting...").font(AgentSkin.pixel(10)).foregroundStyle(.secondary)
                    case .disconnected:
                        Circle().fill(.secondary).frame(width: 6, height: 6)
                        Text("Disconnected").font(AgentSkin.pixel(10)).foregroundStyle(.secondary)
                    case .error(let message):
                        Circle().fill(.red).frame(width: 6, height: 6)
                        Text(message).font(AgentSkin.mono(10)).foregroundStyle(.red).lineLimit(1)
                    }
                }
            }
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 6) {
                    Text(server.name).font(AgentSkin.mono(12, .medium))
                    if server.autoStart {
                        Text("auto").font(AgentSkin.pixel(9)).foregroundStyle(.white)
                            .padding(.horizontal, 4).padding(.vertical, 1)
                            .background(.blue).clipShape(Capsule())
                    }
                }
                Text(server.displayAddress).font(AgentSkin.mono(11)).foregroundStyle(.secondary)
...
struct MCPServerEditView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Name").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
                ...
                if useHTTP {
                    // HTTP fields
                    VStack(alignment: .leading, spacing: 4) {
                        Text("URL").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
                        TextField("https://example.com/mcp", text: $urlText).textFieldStyle(.roundedBorder)
                    }
                    VStack(alignment: .leading, spacing: 4) {
                        Text("SSE Endpoint (optional)").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
                        TextField("/sse", text: $sseEndpointText).textFieldStyle(.roundedBorder)
                    }
                    VStack(alignment: .leading, spacing: 4) {
                        Text("HTTP Endpoint (optional)").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
                        TextField("/message", text: $httpEndpointText).textFieldStyle(.roundedBorder)
                    }
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Headers (Name: Value, one per line)").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
                    ...
                    // Stdio fields
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Command").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
                        TextField("/usr/local/bin/my-mcp-server", text: $command).textFieldStyle(.roundedBorder)
                    }
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Arguments (one per line)").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Environment Variables (KEY=value, one per line)").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("JSON").font(AgentSkin.pixel(11)).foregroundStyle(.secondary)
...
private struct PlainTextEditor: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        textView.font = NSFont(name: "JetBrainsMono-Regular", size: NSFont.smallSystemFontSize) ?? NSFont.monospacedSystemFont(ofSize: NSFont.smallSystemFontSize, weight: .regular)
