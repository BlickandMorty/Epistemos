...
extension AgentViewModel {
    ...
    func unregisterDaemon() {
        ...
        daemonPingOK = false
        appendLog("⚙️ Privileged helper unregistered.")
    }

    func unregisterAgent() {
        userService.shutdownAgent()
        userPingOK = false
        appendLog("⚙️ User helper unregistered.")
    }

    func testConnection() {
        appendLog("🔌 Testing connections...")
        Task {
            // Epistemos: only ping/mend the SMAppService helper if its plists are bundled (they're not — the root daemon
            // is MAS-fatal/unsafe). Otherwise report ONE honest state, not the mend/register spam.
