...
final class AgentViewModel {
    ...
    var thinkingExpanded: Bool = UserDefaults.standard.object(forKey: "thinkingExpanded") as? Bool ?? true {
    ...
    var toolStepsExpanded: Bool = UserDefaults.standard.object(forKey: "toolStepsExpanded") as? Bool ?? true {
    ...
    var userEnabled: Bool = UserDefaults.standard.object(forKey: "agentUserEnabled") as? Bool ?? true {
        didSet {
            ...
            if !userEnabled {
                ...
                userPingOK = false
                appendLog("⚙️ User service shut down. Re-enable: Connect → Register.")
            }
            syncServicesGroup()
        }
    }
    var rootEnabled: Bool = UserDefaults.standard.object(forKey: "agentRootEnabled") as? Bool ?? true {
        didSet {
            ...
            if !rootEnabled {
                ...
                daemonPingOK = false
                appendLog("⚙️ Privileged helper shut down. Re-enable: Connect → Register.")
            }
            syncServicesGroup()
        }
    }
    ...
    var epistemosHostContextSummary: String = ""
    ...
    // Epistemos: flat OpenCode-minimal by default — CRT/scanline overlay OFF unless explicitly opted in
    // (toggle preserved → no capability lost; just no retro phosphor chrome out of the box).
    var scanLinesEnabled: Bool = UserDefaults.standard.object(forKey: "scanLinesEnabled") as? Bool ?? false {
