...
extension AgentViewModel {
    ...
    nonisolated static func newTaskPrefix(
        projectFolder: String,
        prompt: String = "",
        hostContextSummary: String = ""
    ) -> String {
        let folderPrefix = projectFolder.isEmpty ? "" : "[project folder: \(projectFolder)] "
        let trimmedHostContext = hostContextSummary.trimmingCharacters(in: .whitespacesAndNewlines)
        let hostContextPrefix = trimmedHostContext.isEmpty ? "" : "[Epistemos context: \(trimmedHostContext)] "
        ...
        let taskHeader = isQuestion
            ...
            """
        return taskHeader + folderPrefix + hostContextPrefix + configPrefix
    }
