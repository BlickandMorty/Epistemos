...
extension AgentViewModel {
    ...
    private func sendAgentAck() {
        ...
        Task {
            ...
            if result.status == 0 {
                appendLog("Epistemos ack sent to \(handle)")
            } else {
                appendLog("Epistemos ack failed: \(result.output.prefix(100))")
            }
            flushLog()
        }
    }

    /// Send a reply via iMessage to the handle that triggered the task.
    func sendAgentReply(_ summary: String) {
        guard let handle = agentReplyHandle else { return }
        agentReplyHandle = nil

        // Strip leading wake words from outgoing replies so the receiving Mac
        // doesn't loop on its own command.
        ...
        Task {
            ...
            if result.status == 0 {
                appendLog("Epistemos reply sent to \(handle)")
            } else {
                appendLog("Epistemos reply failed: \(result.output.prefix(100))")
            }
            flushLog()
        }
    }
    ...
    // MARK: - Messages Wake Prefix

    /// True iff `text` starts with "epistemos" or the legacy "agent" alias as
    /// a complete leading word. Followed by end-of-string, "!", or whitespace.
    nonisolated static func hasAgentPrefix(_ text: String) -> Bool {
        wakePrefixEnd(in: text) != nil
    }

    private nonisolated static func wakePrefixEnd(in text: String) -> String.Index? {
        let lower = text.lowercased()
        for prefix in ["epistemos", "agent"] where lower.hasPrefix(prefix) {
            let after = lower.index(lower.startIndex, offsetBy: prefix.count)
            guard after < lower.endIndex else { return text.index(text.startIndex, offsetBy: prefix.count) }
    ...
    /// Strip the leading wake prefix and any following whitespace. Returns
    /// `text` unchanged if no prefix is present.
    nonisolated static func stripAgentPrefix(from text: String) -> String {
        guard var idx = wakePrefixEnd(in: text) else { return text }
    ...
    func sendMessagesTabReply(_ summary: String, handle: String) {
        // Strip leading wake words so the receiving Mac doesn't loop on its
        // own command.
    ...
    /// Poll for new incoming messages; log from enabled handles, act on wake prefix.
