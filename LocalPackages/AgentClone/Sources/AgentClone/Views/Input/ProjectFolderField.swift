...
struct ProjectFolderField: View {
    ...
    private var showFolderControls: Bool {
        isHovering || isFieldFocused || showTree || showRecentFolders
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if showRecentFolders && !recentFolders.isEmpty {
                ...
                .overlay(RoundedRectangle(cornerRadius: AgentSkin.radius).stroke(AgentSkin.border, lineWidth: 1))
                // Epistemos: flat — no drop shadow (theme hairline delineates the dropdown)
                .padding(.top, 4)
