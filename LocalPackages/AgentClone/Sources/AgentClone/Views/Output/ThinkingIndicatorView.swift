...
struct ThinkingIndicatorView: View {
    ...
    private var reasoningText: String {
        if let tab {
            return tab.reasoningOutput
        }
        return viewModel.reasoningOutput
    }
    ...
    var body: some View {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 8) {
                        ...
                        HStack(spacing: 3) {
                            ...
                            Text("LLM Output").font(AgentSkin.pixel(12))
                        }
                        .foregroundStyle(.secondary)
                        ...
                        HStack(spacing: 3) {
                            ...
                            Text(modelName).font(AgentSkin.mono(11)).lineLimit(1)
                        }
                        .foregroundStyle(.secondary)

                        HStack(spacing: 2) {
                            Text("↑").font(AgentSkin.mono(11)).foregroundStyle(.blue)
                            Text(Self.fmtTokens(inputTokens)).font(AgentSkin.mono(11)).foregroundStyle(.secondary)
                            Text("↓").font(AgentSkin.mono(11)).foregroundStyle(.green)
                            Text(Self.fmtTokens(outputTokens)).font(AgentSkin.mono(11)).foregroundStyle(.secondary)
                        }
                    ...
                    if showStreamText {
                        if !reasoningText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                            ReasoningOutputBox(text: reasoningText)
                        }
        ...
        // Flat dark surface instead of glossy material chrome.
        .background(colorScheme == .dark ? AgentSkin.surface.opacity(0.96) : Color.white.opacity(0.97))
        .onAppear {
...
/// Resizable reasoning box in the native chat activity pane.
private struct ReasoningOutputBox: View {
    let text: String
...
private struct LLMOutputBox: View {
    ...
    // Epistemos reskin: flat native activity panel, not a terminal emulator.
    private var termBg: Color {
        colorScheme == .dark
            ? AgentSkin.surface.opacity(0.92)
    ...
    var body: some View {
                ...
                if !displayText.isEmpty {
                    ...
                    .overlay {
                        if showScanlines {
                            ...
                            ScanlineOverlay(spacing: 4, color: Color(white: 0.5), opacity: 0.08, blurRadius: 0.25)
                        }
                    }
                    ...
                    VStack(spacing: 0) {
                        HStack(spacing: 0) {
                            Text(isStreaming ? "Waiting for model output..." : "No model output yet.")
                                .font(AgentSkin.mono(13))
        ...
        .background(termBg)
        .overlay(Rectangle().stroke(termBorder.opacity(0.55), lineWidth: 1))
    }
}
