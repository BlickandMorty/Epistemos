...
extension ActivityLogView.Coordinator {
    ...
    nonisolated func renderMarkdownTable(_ lines: [String]) -> NSAttributedString? {
        ...
        let boldBase = NSFont(name: "JetBrainsMono-Regular", size: font.pointSize) ?? NSFont.monospacedSystemFont(ofSize: font.pointSize, weight: .bold)
        let boldFont = NSFontManager.shared.convert(boldBase, toHaveTrait: .boldFontMask)
    ...
    nonisolated func renderMarkdownLine(_ line: String) -> NSAttributedString {
        ...
        if let match = MarkdownPatterns.headerPattern?.firstMatch(in: line, range: fullRange) {
            ...
            let headerBase = NSFont(name: "JetBrainsMono-Regular", size: size) ?? NSFont.monospacedSystemFont(ofSize: size, weight: .bold)
            let headerFont = NSFontManager.shared.convert(headerBase, toHaveTrait: .boldFontMask)
        ...
        if let match = MarkdownPatterns.blockquotePattern?.firstMatch(in: line, range: fullRange) {
            ...
            mutableRendered.addAttribute(.foregroundColor, value: AgentSkin.nsTextDim, range: rRange)
