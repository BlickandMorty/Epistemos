...
private struct TabItem: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 4)
        .foregroundStyle(isSelected ? tint : AgentSkin.textDim)
        .background(
            RoundedRectangle(cornerRadius: AgentSkin.radius)
                .fill(
                    isSelected
                        ? tint.opacity(0.18)
                        : isHovering ? tint.opacity(0.08) : AgentSkin.surface
                )
        )
        .overlay(
            RoundedRectangle(cornerRadius: AgentSkin.radius)
                .strokeBorder(isSelected ? tint.opacity(0.5) : AgentSkin.border, lineWidth: 1)
        )
        .contentShape(RoundedRectangle(cornerRadius: AgentSkin.radius))
