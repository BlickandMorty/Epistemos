    ...
    private static func checkDangerousRm(tokens: [String]) -> Verdict? {
        ...
        while i < tokens.count {
            ...
            if t == "--recursive" || t == "--Recursive" { hasR = true }
                ...
                return .block(
                    reason: "Refused: `rm --no-preserve-root` is only used to bypass macOS/GNU's safeguard against deleting `/`. This command is permanently disabled in Epistemos.",
                    rule: "rm.no-preserve-root"
                )
            }
            else if t.hasPrefix("--") {
        ...
        for target in positionals {
            if let reason = dangerousRmTargetReason(target) {
                return .block(
                    reason: "Refused: `rm -rf \(target)` — \(reason). Epistemos blocks this pattern locally before it reaches any shell. Narrow the path to a specific subdirectory you actually want to delete.",
