...
enum SafeSMAppServiceDaemon {
    ...
    static func registerDaemon() -> (success: Bool, message: String) {
        ...
        guard daemonPlistExists() else {
            return (false, "Privileged helper plist not found in app bundle. Rebuild and reinstall Epistemos.")
        }

        guard let service = createDaemon() else {
            return (false, "Privileged helper unavailable. Reinstall Epistemos.")
        }
        ...
        do {
            ...
            return (true, "Epistemos privileged helper registered. (was: \(statusName))")
        } catch {
            // Check if already enabled after attempted registration
            if service.status == .enabled {
                return (true, "Epistemos privileged helper is active.")
            }
            if service.status == .requiresApproval {
                SMAppService.openSystemSettingsLoginItems()
                NSWorkspace.shared.open(URL(string: "x-apple.systempreferences:com.apple.LoginItems-Settings.extension")!)
                return (false, "Please approve Epistemos in System Settings > Login Items.")
            }
            ...
            if status == .enabled {
                ...
                do {
                    try service.register()
                    return (true, "Epistemos privileged helper updated.")
                } catch {
...
final class HelperService {
    ...
    func execute(command: String, workingDirectory: String = "") async -> (status: Int32, output: String) {
        ...
        if !verdict.allowed {
            ...
            return (-1, verdict.reason ?? "Refused: command blocked by Epistemos shell safety guardrail.")
        }
        if !helperReady {
            let msg = restartDaemon()
            if !helperReady {
                return (-1, "Error: Epistemos privileged helper is not running — \(msg). Check System Settings > Login Items.")
            }
        }
