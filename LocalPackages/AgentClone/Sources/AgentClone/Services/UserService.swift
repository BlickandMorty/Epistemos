...
enum SafeSMAppService {
    ...
    static func registerUserAgent() -> (success: Bool, message: String) {
        ...
        guard userAgentPlistExists() else {
            return (false, "User helper plist not found in app bundle. Rebuild and reinstall Epistemos.")
        }

        guard let service = createUserAgent() else {
            return (false, "User helper unavailable. Reinstall Epistemos.")
        }
        ...
        do {
            ...
            return (true, "Epistemos user helper registered. (was: \(statusName))")
        } catch {
            // Check if already enabled after attempted registration
            if service.status == .enabled {
                return (true, "Epistemos user helper is active.")
            }
            ...
            if status == .enabled {
                ...
                do {
                    try service.register()
                    return (true, "Epistemos user helper updated.")
                } catch {
...
final class UserService {
    ...
    nonisolated static func userAgentPlistExists() -> Bool {
        SafeSMAppService.userAgentPlistExists()
    }
    ...
    func execute(command rawCommand: String, workingDirectory: String = "") async -> (status: Int32, output: String) {
        ...
        if !userReady {
            ...
            if !userReady {
                return (-1, "Error: Epistemos user helper is not running — \(msg). Check System Settings > Login Items.")
            }
        }
