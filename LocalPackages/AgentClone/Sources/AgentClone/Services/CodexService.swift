...
final class CodexService {
    ...
    func sendStreaming(
        ...
        onReasoning: @MainActor @Sendable @escaping (String) -> Void = { _ in }
    ) async throws
    ...
    nonisolated private static func streamRequest(
        ...
        url: URL,
        onDelta: @MainActor @Sendable @escaping (String) -> Void,
        onReasoning: @MainActor @Sendable @escaping (String) -> Void
    ) async throws -> (content: [[String: Any]], stopReason: String, inputTokens: Int, outputTokens: Int) {
        do {
            return try await streamRequestAttempt(
                bodyData: bodyData,
                auth: auth,
                url: url,
                onDelta: onDelta,
    ...
    nonisolated private static func streamRequestAttempt(
        ...
        for try await rawLine in bytes.lines {
            ...
            case "response.reasoning_summary_text.delta", "response.reasoning_text.delta":
                // Reasoning streams to the thinking HUD, not the assistant
                // answer stream. The final history still receives a separate
                // `thinking` block below.
            ...
            case "response.web_search_call.in_progress":
                await MainActor.run { onReasoning("\nweb_search...") }

            case "response.web_search_call.completed":
                await MainActor.run { onReasoning(" done\n") }
