...
public class ChatSessionTests: XCTestCase {
    ...
    func testInjectKVCacheRejectsIncompatibleShape() async throws {
        let session = ChatSession(model())

        let accepted = await session.injectKVCache([KVCacheSimple()])

        XCTAssertFalse(accepted)
        let cache = await session.extractKVCache()
        XCTAssertNil(cache)
    }
