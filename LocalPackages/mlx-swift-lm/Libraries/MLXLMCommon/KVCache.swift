...
public class CacheList: BaseKVCache {
    ...
    public subscript(index: Int) -> KVCache {
        precondition(index >= 0 && index < caches.count, "KV cache index out of bounds")
        return caches[index]
    }
...
public func savePromptCache(
    ...
    if cache.contains(where: { $0 is CacheList }) {
        throw KVCacheError(message: "CacheList prompt-cache serialization is not supported")
    }
...
public func loadPromptCache(
    ...
    for i in 0 ..< cacheData.count {
        ...
        case "CacheList":
            throw KVCacheError(message: "CacheList prompt-cache deserialization is not supported")
        default:
