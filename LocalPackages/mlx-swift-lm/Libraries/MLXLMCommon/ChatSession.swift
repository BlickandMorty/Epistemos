...
public final class ChatSession {
    ...
    // MARK: - Cache Persistence (Epistemos patch)

    /// Extract the current KVCache array for disk persistence.
    /// Returns nil if no generation has occurred yet or the session was cleared.
    public func extractKVCache() async -> [KVCache]? {
        let snapshot = await cache.read { c -> SendableBox<[KVCache]?> in
            guard case .kvcache(let array) = c else {
                return SendableBox(nil)
            }
            return SendableBox(array)
        }
        return snapshot.consume()
    }

    /// Replace the internal cache with a pre-built KVCache array.
    /// Use this to restore a previously saved session state, bypassing
    /// conversation replay for SSM models with persistent hidden state.
    @discardableResult
    public func injectKVCache(_ kvCache: [KVCache]) async -> Bool {
        let expectedCacheShape = await model.perform { context in
            SendableBox(
                context.model.newCache(parameters: nil).map { Self.cacheShapeDescription(for: $0) })
        }.consume()
        let injectedCacheShape = kvCache.map { Self.cacheShapeDescription(for: $0) }
        guard injectedCacheShape == expectedCacheShape else {
            return false
        }
    ...
    private static func cacheShapeDescription(for cache: KVCache) -> String {
        if let cacheList = cache as? CacheList {
            let subcaches = (0 ..< cacheList.subcacheCount).map {
                cacheShapeDescription(for: cacheList[$0])
            }
            return "CacheList<\(subcaches.joined(separator: ","))>"
        }
