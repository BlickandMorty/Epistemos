...
"${CARGO[@]}" run --manifest-path agent_core/Cargo.toml --bin falsify_residency_patternboost_no_hidden_authority
status=$?

"${CARGO[@]}" run --manifest-path agent_core/Cargo.toml --bin falsifier_validator -- \
  artifacts/falsifiers/residency_patternboost_no_hidden_authority/result.json
validator_status=$?
